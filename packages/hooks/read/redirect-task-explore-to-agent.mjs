 ...
 */
import {
  readHookInput, getProjectId, appendTelemetry, emitAgentLaunch,
...
async function main() {
  ...
  // NOT a launch — nothing to record. Must stay the first exit and must never emit.
  if (toolName !== "Task") process.exit(0);

  // backlog.fix.agent-launch-telemetry-ledger — HOISTED above the guardrails-off exit.
  // Previously this read sat below it, so the guardrails-off branch had no subagent_type or
  // description in scope and could only have recorded a useless null-shaped launch. The hoist is
  // behavior-preserving: a side-effect-free property read on an already-parsed object, and nothing
  // between the two points reads toolInput (isGuardrailsOff() consults only process.env).
  const toolInput = hookData.tool_input || {};
  const launch = {
    subagentType: toolInput.subagent_type || "unknown",
    description: toolInput.description || extractQuery(toolInput),
  };

  // Every ALLOW path records the launch before exiting. These three branches are the ONLY ways an
  // agent starts without a redirect, so they are the complete ledger — anything not written here is
  // a launch nobody can reconcile against later.
  if (isGuardrailsOff()) {
    emitAgentLaunch({ ...launch, allowReason: "guardrails_off" });
    process.exit(0);
  }

  // Allow Governor launches through — they ARE governance
  if (toolInput.resume) {
    emitAgentLaunch({ ...launch, allowReason: "resume" });
    process.exit(0);
  }
  if (isGovernorLaunch(toolInput)) {
    emitAgentLaunch({ ...launch, allowReason: "governor_launch" });
    process.exit(0);
  }

  const projectId = getProjectId();
  const subagentType = launch.subagentType;
