...
import { isKeyless, isKeylessNotesTarget } from "../system/hook-output.mjs";
...
async function main() {
  ...
  // Keyless mode (INV-4): permit a raw Write to the keyless-notes.* namespace — the ONE keyless
  // write target. Gated by the SHARED realpath-anchored isKeylessNotesTarget from hook-output.mjs
  // (NOT this hook's lexical `relativePath`, which has no ../ handling) so there is exactly one
  // keyless-notes path decision across all write hooks (no drift). Fail-closed on the predicate.
  if (isKeyless() && isKeylessNotesTarget(filePath)) process.exit(0);
