...
import {
  ...
  buildRedirectOutput, denyWithRedirect, isGuardrailsOff, isKeyless, isKeylessNotesTarget,
...
async function main() {
  ...
  // Keyless mode (INV-4, best-effort): allow dendron_create_note into the keyless-notes.* namespace,
  // using the SHARED bare-name predicate (which rejects any separator/traversal). Best-effort — the
  // server-side dendron_create_note handler may still route to the keyed dendron agent; the raw-Write
  // path (enforce-dendron-note-creation + redirect-edit-to-governor) is the reliable keyless-notes
  // mechanism. Exact tool match: no other dendron tool opens keyless.
  if (
    isKeyless() &&
    toolName === "mcp__rks__dendron_create_note" &&
    isKeylessNotesTarget(toolInput.filename)
  ) process.exit(0);
