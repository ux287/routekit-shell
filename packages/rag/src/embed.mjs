...
import { getProjectContext, RESERVED_NAMESPACES } from './utils.mjs';
...
// `projectSlug` was the third parameter and is gone: it was never referenced in the body,
// and namespace eligibility is no longer per-project — a project expresses its own policy by
// what it names its notes, not by a slug this function inspects.
function getShouldEmbed(relativePath, frontmatter) {
  // OPT-OUT only. The opt-IN (`rag: true`) was removed with the allow-list: for a reserved
  // namespace it is unreachable by construction, because selection drops the file and its
  // frontmatter is never read; for every other namespace it was dead code, because the
  // default below already admits.
  //
  // It is NOT outcome-neutral in one case, and that case has nothing to do with reserved
  // namespaces: a note carrying BOTH `rag: true` and `private: true` used to embed, because
  // the opt-in was tested first and short-circuited. It now skips. `private` winning over a
  // stale opt-in is the intended reading; the witness for it is in
  // tests/unit/rag/reserved-namespace-set.test.mjs.
  if (frontmatter.rag === false || frontmatter.private === true) return false;

  // DENY-LIST. The three positive arms this replaces (design./docs./how-to.) were already
  // dead — the default below admits them — so removing them changes nothing.
  if (RESERVED_NAMESPACES.some((ns) => relativePath.startsWith(ns))) return false;

  // Everything else embeds, including namespaces this code has never heard of.
  return true;
}
...
async function processNote(filePath, { vaultPath, projectSlug }) {
  try {
    ...
    if (result.skipped) {
      ...
      return { rows: [], reason: result.reason };
    }
    console.log(`✂️  Created ${result.rows.length} structure-aware chunks`);
    return { rows: result.rows, reason: null };
  } catch (error) {
 ...
 */
export function computeManifestUpdate({
  priorHashes = {},
  newHashes = {},
  visitedScope = null,
  unindexed = null,
}) {
  // backlog.fix.rag-embed-manifest-records-unindexed-content.
  //
  // THREE dispositions, not two. "Absent from newHashes" used to mean exactly one thing — gone from
  // disk — and that conflation is the defect. A file that is present but produced no rows this run
  // (excluded, empty, or an exception caught inside processNote) is absent from newHashes too, and
  // both of the old outcomes are wrong for it:
  //
  //   recording its hash   → the next run's skip check reads the manifest as "indexed" and skips it
  //                          forever. The manifest records SEEN; the skip check reads INDEXED.
  //   listing it as stale  → deleteByPaths removes the rows a PREVIOUS successful run wrote for it,
  //                          so a transient throw silently deletes real index content.
  //
  // `unindexed` is the third disposition: visited, not indexed, still on disk. Its prior hash is
  // DROPPED so the next run retries it, and it is kept OUT of `stale` so its existing rows survive.
  const held = unindexed instanceof Set ? unindexed : new Set(unindexed ?? []);

  if (!visitedScope) {
    // Full walk: newHashes is authoritative. Anything in the prior manifest the walk did not
    // re-hash is gone from disk — UNLESS the walk visited it and declined to index it.
    const hashes = { ...newHashes };
    for (const p of held) delete hashes[p];
    return {
      hashes,
      stale: Object.keys(priorHashes).filter((p) => !newHashes[p] && !held.has(p)),
    };
  }
  ...
  const stale = Object.keys(priorHashes).filter(
    (p) => scope.has(p) && !newHashes[p] && !held.has(p),
  );
...
async function embedNotes(
  ...
  try {
    ...
    // DERIVED from the same reserved set the predicate uses, so selection and the predicate
    // cannot drift. Applied as globby `ignore` on ALL THREE note-selection calls below —
    // including the incremental one, which never consults the glob at all, and is the route
    // a governor-triggered embed actually takes.
    //
    // The dot-terminated form is deliberate and it NARROWS what was ignored before: the old
    // list carried dotless `drafts*` and `z_archive*`, so a file named `draftsFoo.md` was
    // silently ignored. Under a namespace model that file is not in the `drafts.` namespace
    // and now embeds.
    //
    // The two DIRECTORY forms stay as literals: a name-prefix set cannot express a path.
    const archiveIgnorePatterns = [
      ...RESERVED_NAMESPACES.map((ns) => `${ns}*`),
      // DIRECTORY forms, deliberately literal and NOT derived — a name-prefix set cannot
      // express a path. They shadow two members of RESERVED_NAMESPACES by name; if either
      // member is ever renamed, these do not follow and must be changed here too.
      "z_archive/**",
      "drafts/**",
    ];
    ...
    // PER-FILE OUTCOMES. Every entry is written by the code path that actually handled that
    // file — never inferred from the input list, and never reconstructed from a counter.
    // Declared here rather than beside the counters below, because the not-selected
    // disposition is recorded before those exist.
    const fileOutcomes = [];
    const recordOutcome = (file, embedded, reason) => {
      fileOutcomes.push({ file, embedded, reason: reason ?? null });
    };
    ...
    if (incrementalPaths) {
      ...
      const selected = new Set(noteFiles.map((abs) => relative(currentContext.projectRoot, abs)));
      for (const abs of incrementalPaths) {
        const rel = relative(currentContext.projectRoot, abs);
        if (!selected.has(rel)) recordOutcome(rel, false, 'not-selected');
      }
    } else {
    ...
    const newHashes = {};
    // Visited, still on disk, produced no rows. Neither hashed nor stale — see computeManifestUpdate.
    ...
    for (const noteFile of noteFiles) {
      ...
      if (manifest.hashes[relPath] === contentHash) {
        ...
        recordOutcome(relPath, false, 'unchanged');
        continue;
      }

      // The hash is recorded AFTER processNote, and only when rows were actually produced.
      // Recording it here — before the call — is what let a note that threw, or that returned
      // 'excluded'/'empty', enter the manifest as though it had been indexed, after which every
      // later run skipped it as unchanged and its content was never in the store. Permanent:
      // `reset` drops the LanceDB table but never clears the manifest, so only a content edit or a
      // hand-deleted manifest recovered it. See computeManifestUpdate.
      const noteResult = await processNote(noteFile, { vaultPath, projectSlug: currentContext.projectSlug });
      const embeddings = noteResult.rows;
      if (embeddings.length) {
        newHashes[relPath] = contentHash;
        embeddedNotes++;
        chunkTotal += embeddings.length;
        recordOutcome(relPath, true, null);
      } else {
        // Visited, present on disk, not indexed. NOT stale — deleting here would remove rows an
        // earlier successful run wrote. NOT hashed — the next run must try it again.
        unindexedPaths.add(relPath);
        skippedNotes++;
        // The VERBATIM reason processNote observed, not a re-derivation.
        recordOutcome(relPath, false, noteResult.reason ?? 'no-rows');
      }
    ...
    for (const codeFile of codeFiles) {
      ...
      // KNOWN, UNFIXED — the SAME defect the note loop above just had, left in place deliberately
      // rather than overlooked. The hash is committed before processCodeFile runs, so a code file
      // that yields no chunks enters the manifest as indexed and is skipped forever after.
      // backlog.fix.rag-embed-manifest-records-unindexed-content is scoped to notes by every one of
      // its acceptance criteria, and this loop also makes no recordOutcome call at all, so it has no
      // per-file outcome to assert against. Both belong to the follow-up that covers code files.
      newHashes[relPath] = contentHash;
    ...
    if (allEmbeddings.length === 0 && !shouldRebuild) {
      ...
      return {
        ...
        // Whole-corpus row count, observed via countRows() above. Named apart from the
        // per-run work below so no field named for a run result holds a corpus total.
        corpusRowTotal: existingCount,
        processedNotes: processedCount,
        // Present on EVERY success-shaped return, with the value this run observed. These
        // three previously existed only on the full return, so the boundary was told to
        // project fields the producer did not carry here.
        embeddedNotes,
        skippedNotes,
        skippedCodeFiles,
        fileOutcomes,
    ...
    if (!refreshedTableNames.includes("embeddings")) {
      if (allEmbeddings.length === 0) {
        ...
        return {
          ok: true,
          indexed: 0,
          // The table is ABSENT — observed at refreshedTableNames above, not inferred from
          // this run having written nothing. That observation is what licenses the zero.
          corpusRowTotal: 0,
          mode: scopeMode,
          reset,
          processedNotes: processedCount,
    ...
    return {
      ok: true,
      indexed: finalCount,
      // Same observation as `indexed`, under a name that says what it counts. `indexed`
      // keeps its meaning because tools.mjs treats indexed === 0 as the zero-work predicate.
      corpusRowTotal: finalCount,
      fileOutcomes,
      addedEmbeddings: chunksAdded,
      removedEmbeddings,
      mode: scopeMode,
