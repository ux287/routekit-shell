 ...
 * Shared between packages/rag/src/query.mjs (CLI) and the research agent (MCP server).
...
// Canonical source roots that carry the retrieval boost. Broadened for Stage 3: the RAG pipeline
// relocated from scripts/rag/ INTO packages/rag/src/, so that root must be boosted too (else the
// moved canonical code would be silently under-ranked). packages/cli/src/ included for symmetry.
const CANONICAL_SOURCE_PREFIXES = ['packages/mcp-rks/src/', 'packages/rag/src/', 'packages/cli/src/'];

export function getCanonicalPathBoost(filePath) {
  if (!filePath) return 1.0;
  if (CANONICAL_SOURCE_PREFIXES.some(prefix => filePath.startsWith(prefix))) return NAMESPACE_BOOST.canonical_source;
  return 1.0;
}
