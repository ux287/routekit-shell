...
export async function handleProjectCommand({ sub, kv, SHELL_ROOT, args = [] } = {}, deps = {}) {
  ...
  if (sub === "upgrade") {
    ...
    if (all) {
      ...
      let succeeded = 0;
      let skipped = 0;
      // backlog.fix.upgrade-all-counts-non-upgrades-as-upgraded — non-failure outcome classes,
      // counted separately so a zero headline can be EXPLAINED rather than reported bare.
      let alreadyCurrent = 0;
      let refusedDowngrade = 0;
      let gatedMajor = 0;
      let previewed = 0;
      const failures = [];
      for (const record of projects) {
        ...
        try {
          ...
          if (report.ok) {
            ...
            // backlog.fix.upgrade-all-counts-non-upgrades-as-upgraded — the headline count is an
            // OBSERVATION of mutation, not an absence of failure. upgrade.mjs sets stampAdvanced
            // true only BELOW every early return (already-current, refused downgrade, gated major,
            // dry-run), so it is the only field that witnesses a child actually being upgraded.
            if (report.stampAdvanced === true) succeeded += 1;
            else if (report.dryRun) previewed += 1;
            else if (report.gated) gatedMajor += 1;
            else if (report.boundary === "downgrade") refusedDowngrade += 1;
      ...
      // The shell's own record is not a child, so it is not part of the denominator.
      const total = projects.length - skipped;
      // backlog.fix.upgrade-all-counts-non-upgrades-as-upgraded — account for the children that
      // were NOT upgraded. Keyed on the measured class counts, never on `succeeded === 0`, which
      // is also true of a batch in which every child FAILED.
      const accounted = [];
      if (previewed > 0) accounted.push(`${previewed} previewed, nothing written (dry-run)`);
      if (alreadyCurrent > 0) accounted.push(`${alreadyCurrent} already current`);
      if (refusedDowngrade > 0) accounted.push(`${refusedDowngrade} older than the shell, downgrade refused`);
      if (gatedMajor > 0) accounted.push(`${gatedMajor} gated at a major boundary`);
      const because = accounted.length > 0 ? ` — ${accounted.join(", ")}` : "";
      if (failures.length === 0) {
        console.log(`Upgraded ${succeeded}/${total} children${_fromRelease ? ` from ${contentRoot}` : ""}${because}.`);
        processExit(0);
      } else {
        console.log(`Upgraded ${succeeded}/${total} children${because}; ${failures.length} failed.`);
        processExit(1);
      }
      return;
    }
