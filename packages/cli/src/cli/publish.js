...
export async function handlePublishCommand({ kv = {}, args = [], SHELL_ROOT } = {}, deps = {}) {
  ...
  try {
    ...
    // A DISARMED remote is not a failure. `.routekit/publish-profiles.yaml` sets
    // remotes.<name>.enabled: false, and publish() suppresses the force-push at remote
    // resolution — which is BEFORE its dry-run block, so a suppressed --dry-run returns the
    // suppressed result rather than a preview. Discriminate it AHEAD of the generic ok-check
    // below (which would print "publish failed: unknown error", since a suppressed result
    // carries `reason`, not `error`) and AHEAD of the dry-run branch (which would otherwise
    // report it as a successful preview). Exit 3 keeps suppression distinct from both
    // success (0) and failure (1).
    if (result && result.suppressed === true) {
      errorLog(
        `publish SKIPPED — ${result.reason || `remote '${remote}' is disarmed (enabled: false)`}`,
      );
      errorLog(
        `No push was attempted${dryRun ? "; no dry-run preview was produced" : ""}. ` +
          `Set remotes.${remote}.enabled: true in .routekit/publish-profiles.yaml to re-arm.`,
      );
      return processExit(3);
    }
