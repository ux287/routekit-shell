...
/**
 * backlog.feat.project-adopt-verb — the resolved `projects/index.jsonl` path.
 *
 * ADDITIVE BY CONSTRUCTION: a new export, so no existing return shape moves. Callers that need to
 * TELL the operator which registry they wrote must read it from here rather than recomputing
 * `path.join(baseDir, REGISTRY_FILE)` themselves — a recomputation can silently disagree with the
 * `opts.registryPath` override the write actually used, which is exactly the class of mismatch that
 * made a registry row land in an unexpected shell with nothing in the output saying so.
 *
 * Deliberately NOT folded into upsertProject's return: it returns `record`, and two of its five
 * callers live under packages/mcp-rks/src/, so widening that shape would cross a package boundary.
 */
export function resolveRegistryPath(baseDir = process.cwd(), opts = {}) {
  return opts.registryPath || path.join(baseDir, REGISTRY_FILE);
}

export function appendProject(record, baseDir = process.cwd(), opts = {}) {
  const filePath = resolveRegistryPath(baseDir, opts);
