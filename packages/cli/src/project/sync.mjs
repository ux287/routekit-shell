...
import { readShellIdentity, substituteSourceIdentity } from "./source-identity.mjs";
...
/**
 * Write the child's own `.routekit/skills-manifest.json` from the skills it ACTUALLY received.
 *
 * backlog.fix.skills-manifest-never-distributed-to-children. Nothing wrote this file into a child,
 * so `loadSkillsManifest(projectRoot)` in preflight always hit ENOENT, and its `manifest_missing`
 * branch sets `skillsPassed = true` WITHOUT STATTING A SINGLE SKILL DIRECTORY. `core_skills` was a
 * permanent no-op green in every child — a health oracle certifying a corpse, which is the exact
 * failure the skills check was added to end, reproduced one layer out.
 *
 * DERIVED, never restated. `skillNames` is the set that survived the `continue` in the caller's copy
 * loop — the names the child genuinely got. Copying the SHELL's manifest instead would let the child
 * advertise a skill it never received: on a published-mirror clone `whitepaper` is stripped, so the
 * child would claim it, preflight would stat it, and a clean tree would read as a deficit. That is
 * `backlog.fix.preflight-core-skills-not-distributed`'s false RED, reintroduced one layer out.
 *
 * Deriving it at CHECK time from the child's own disk would be worse still: expected would equal
 * actual by construction, so a wipe would delete the skills and the evidence together, and the check
 * could never fail. This file is a RECORD OF AN OBSERVATION, taken at distribution time and outliving
 * whatever happens to the directories afterwards. That is what lets it witness a wipe.
 *
 * `shellOnly` is `[]` because a shell-only skill is never copied — the caller's loop skips it before
 * reaching here. `notPublished` is deliberately OMITTED: both readers default it to `[]`, so every
 * absent skill classifies as genuinely missing. Fails closed.
 *
 * An EMPTY copy set writes NOTHING. `loadSkillsManifest` rejects an empty `skills` array as
 * `manifest_malformed`, which today degrades to the same green as a missing file but becomes an
 * unconditional RED once backlog.fix.preflight-core-skills-greens-unreadable-manifest ships. Emitting
 * `{"skills": []}` would therefore be harmless now and fatal later — a landmine set by this story for
 * that one. Absent is the honest state for a child that received no skills.
 *
 * @param {string} projectRoot Child project root.
 * @param {string[]} skillNames Skill directory names actually copied into the child.
 * @returns {string|null} The path written, or null when nothing was written.
 */
export function writeChildSkillsManifest(projectRoot, skillNames) {
  const skills = [...new Set(skillNames)].sort();
  if (skills.length === 0) return null;
  const dir = path.join(projectRoot, ".routekit");
  ensureDir(dir);
  const manifestPath = path.join(dir, "skills-manifest.json");
  const body = {
    $comment:
      "Inventory of the skills THIS CHILD received, recorded at distribution time by rks. Derived " +
      "from the skills actually copied — not restated from the shell's manifest — so it cannot " +
      "advertise a skill the child does not have. Rewritten on every bootstrap and sync. Do not " +
      "hand-edit: a hand-authored list is a second list free to drift, which is what this file exists " +
      "to prevent.",
    version: 1,
    skills,
    shellOnly: [],
...
export function syncProject({ projectRoot, projectId, shellRoot, refreshStamp = true }) {
  ...
  }
  {
    ...
    // MIRROR-CLONE FALLBACK. On a published-mirror clone the sentinel is already spent, so the
    // sentinel-only replacement below matches nothing and a child that bootstrapped correctly has
    // its skills reverted to source-project names by the next sync. Same recovery bootstrap uses,
    ...
    // The observed copy set. Appended AFTER the copy, so a skill that threw on the way in is not
    // recorded as delivered — the manifest states what happened, not what was attempted.
    const copiedSkills = [];
    for (const entry of fs.readdirSync(srcSkills, { withFileTypes: true })) {
      ...
      // Substitutes THE SENTINEL ONLY. The surviving placeholder token is deliberately not
      // named anywhere in this file and must never be substituted here. The projectId-equality
      // exemption is DELETED: it would exempt the public mirror (whose registered id IS
      // "routekit-shell") and ship it an unsubstituted sentinel. Self-protection is the
      // same-directory refusals above.
      if (projectId) {
        // Shallow by design — one level, skill dir to *.md — matching this loop's existing shape.
        for (const f of fs.readdirSync(destSkill).filter(n => n.endsWith(".md"))) {
          const fp = path.join(destSkill, f);
          const content = fs.readFileSync(fp, "utf8");
          skillFilesVisited += 1;
          let replaced = content.replace(/__RKS_SOURCE_PROJECT__/g, projectId);
          if (replaced !== content) anySentinelSubstituted = true;
          if (shellIdentity.id && shellIdentity.id !== projectId) {
            const afterIdentity = substituteSourceIdentity(replaced, shellIdentity.id, projectId);
            if (afterIdentity !== replaced) anyIdentitySubstituted = true;
            replaced = afterIdentity;
          }
    ...
    // Rewritten on EVERY sync, not only when the set changes. A sync that adds or removes a skill
    // must leave the manifest agreeing with the directories it just wrote; a write-once manifest
    // would go stale in exactly the case the check exists to catch.
    writeChildSkillsManifest(projectRoot, copiedSkills);
    // A sync that substituted NOTHING while there was something to substitute is the silent no-op
    // this story removes. Reporting success on a tree it did not change is a status not sourced
    // from an observation of that tree.
    ...
    if (
      ...
      console.warn(
        `[rks] sync: nothing was substituted in the skills delivered to "${projectId}". ` +
          `Looked for ${sought} read from ${shellIdentity.idFile}, and for the source-project ` +
          `sentinel; neither appeared in ${srcSkills}. The child's skills may still name the ` +
          `source project.`,
      );
    }
  }
