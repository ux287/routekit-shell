...
// backlog.fix.mirror-clone-bootstrap-sentinel-prestripped: on a PUBLISHED MIRROR the sentinel is
// already spent. publish.mjs:237 resolves __RKS_SOURCE_PROJECT__ to the literal public id across the
// delivered skills tree, so the sentinel replacement below matches nothing on a clone and the
// cloner's child keeps skills naming the SOURCE project. Recover the source identity from the shell
// checkout's own .rks/project.json — publish rewrites that same `id` from the SAME `to` value at
// :194, and .gitignore:20 re-includes the file, so every clone carries it — and substitute it too.
function readShellIdentity(shellRoot) {
...
function substituteSourceIdentity(content, sourceId, projectId) {
...
export function ensureGovernorArtifacts({ projectRoot, projectId, shellRoot }) {
  ...
  if (projectId && skillFilesVisited > 0 && !anySentinelSubstituted && !anyIdentitySubstituted) {
