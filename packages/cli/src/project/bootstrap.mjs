...
import { sameDirectory, SelfSyncRefusedError, writeChildSkillsManifest } from "./sync.mjs";
import {
  readShellIdentity,
  substituteSourceIdentity,
...
// backlog.fix.mirror-clone-sync-reverts-child-skill-identity: readShellIdentity,
// escapeForRegExp and substituteSourceIdentity moved to ./source-identity.mjs so sync.mjs reaches
// the SAME implementation. Two copies of one substitution rule is the defect that story closes.

// Exported for backlog.fix.skill-distribution-invariants-harness: the distribution-invariants
// harness drives THIS loop rather than reimplementing it. A test that reimplemented the
// copy+substitute would go vacuous the moment this diverged, which is the defect class the
// harness exists to eliminate. Export only — no behaviour change, and no caller is added.
export function ensureGovernorArtifacts({ projectRoot, projectId, shellRoot }) {
  ...
  const shellIdentity = readShellIdentity(shellRoot);
  let anySentinelSubstituted = false;
  let anyIdentitySubstituted = false;
  // Counts .md files actually inspected. The AC (f) warning must fire only when there WAS something
  // to substitute and nothing was — an empty or markdown-free skills tree is a legitimate state
  // (stripped-down fixtures do it), and warning that "the child's skills may still name the source
  // project" when there are no skills at all would be a status not sourced from an observation:
  // precisely the defect class this story removes, pointed the other way.
  let skillFilesVisited = 0;
  // The observed copy set — see writeChildSkillsManifest in sync.mjs. Appended AFTER the copy, so a
  // skill that threw on the way in is not recorded as delivered.
  const copiedSkills = [];
  if (fs.existsSync(srcSkills)) {
    ...
    for (const entry of fs.readdirSync(srcSkills, { withFileTypes: true })) {
      ...
      // Substitutes THE SENTINEL ONLY. __PROJECT_ID__ is the SURVIVING placeholder and must
      // never be replaced here — widening this to __PROJECT_ID__ destroys the placeholder in
      // every child. The projectId-equality exemption is DELETED: the public mirror's
      // registered id is literally "routekit-shell", so an equality guard would exempt a real
      // target and ship it an unsubstituted sentinel. Self-protection is the same-directory
      // refusal above, which throws before the copy loop.
      if (projectId) {
        // NOTE: this walk is ONE LEVEL DEEP — skill dir → *.md. publish.mjs walks recursively.
        // The asymmetry is a deferred follow-up recorded in this story; if a skill ever nests
        // markdown in a subdirectory, substitution silently misses it.
        for (const f of fs.readdirSync(destSkill).filter(n => n.endsWith(".md"))) {
          skillFilesVisited += 1;
          const fp = path.join(destSkill, f);
          const content = fs.readFileSync(fp, "utf8");
          let updated = content.replace(/__RKS_SOURCE_PROJECT__/g, projectId);
          if (updated !== content) anySentinelSubstituted = true;
          // MIRROR-CLONE PATH. The sentinel is gone; the delivered skills name the source project
          // literally. Substitute that recorded identity in the same value positions.
          if (shellIdentity.id && shellIdentity.id !== projectId) {
            const afterIdentity = substituteSourceIdentity(updated, shellIdentity.id, projectId);
            if (afterIdentity !== updated) anyIdentitySubstituted = true;
            updated = afterIdentity;
          }
    ...
    writeChildSkillsManifest(projectRoot, copiedSkills);
  }

  // AC (f) — a failed identity recovery WARNS; it does not silently no-op. Returning success from a
  // skills tree this function did not change is the same "status not sourced from an observation"
  // defect the mirror-clone fix removes. Fires when NOTHING was substituted by either path.
  // shellIdentity.id === projectId is the LEGITIMATE no-op: the delivered tree already names
  // this child, because this child IS the source project. Warning there would be the same
  // false status, pointed the other way.
  if (
    projectId &&
    skillFilesVisited > 0 &&
    shellIdentity.id !== projectId &&
    !anySentinelSubstituted &&
    !anyIdentitySubstituted
  ) {
    const sought = shellIdentity.id
      ? `source identity "${shellIdentity.id}"`
      : "a source identity (absent or unreadable)";
    console.warn(
      `[rks] bootstrap: nothing was substituted in the skills delivered to "${projectId}". ` +
        `Looked for ${sought} read from ${shellIdentity.idFile}, and for the ` +
        `__RKS_SOURCE_PROJECT__ sentinel; neither appeared in ${srcSkills}. ` +
        `The child's skills may still name the source project.`,
    );
  }
...
export function loadHookManifest(shellRoot) {
...
// ── The canonical hook REGISTRATION block ────────────────────────────────────
// backlog.fix.child-hook-registration-repair-and-audit
//
// Default egress posture written into every project's .rks/project.json.
//
// WHY THIS EXISTS. `rks_fetch_raw` is default-deny: with no `fetchRaw` block it
// refuses EVERY host. Bootstrap never wrote one, so every project ever created was
// born unable to fetch anything — and the refusal was silent, surfacing only as a
// thinner research answer. External research was quietly degraded to search
// snippets for the entire life of the tool.
//
// Posture is deliberately `allowlist`, not `open`. An earlier proposal
// (notes/research.2026.07.15.fetch-raw-default-egress-redesign.md) argued for
// flipping the global default to `open`, but that was driven by denials being
// invisible. Denials are loud now, so the pressure to widen the default is gone
// and a curated read-only list is strictly safer. This is a network egress
// default that propagates to every child project — keep it conservative.
//
// Hosts are matched EXACTLY for bare names, so subdomains must be enumerated:
// `github.com` does not cover `docs.github.com`. Redirect hops are re-validated
// against the same list, which is why objects.githubusercontent.com is present —
// raw.githubusercontent.com redirects there.
export const DEFAULT_FETCH_RAW = {
  mode: "allowlist",
  allowedHosts: [
    "code.claude.com",
    "docs.claude.com",
...
/**
 * Backfill the fetchRaw egress defaults into a project config, if and only if the
 * project has not already made its own choice.
 *
 * Pure and exported ON PURPOSE. The bootstrap config write has no in-process test
 * coverage — every case in tests/project-bootstrap.test.mjs is `.skip()`d — so a
 * decision buried inside attachProject would ship untested. Isolating it from all
 * I/O is what makes it assertable.
 *
 * NEVER clobbers: a project that has set `fetchRaw` owns it, including an
 * intentionally empty allowlist. Absence is the only trigger.
 *
 * @param {object} config Parsed .rks/project.json contents (possibly empty).
 * @returns {object} The config, with fetchRaw defaults added when it had none.
 */
export function withFetchRawDefaults(config) {
  const cfg = config && typeof config === "object" ? config : {};
  if (cfg.fetchRaw && typeof cfg.fetchRaw === "object") return cfg;
  return {
    ...cfg,
    fetchRaw: {
      mode: DEFAULT_FETCH_RAW.mode,
      allowedHosts: [...DEFAULT_FETCH_RAW.allowedHosts],
    },
  };
}

// THE CANONICAL LIST of MCP agent tools that register the provenance hook.
//
// Each tool gets its OWN PostToolUse matcher, never a pipe-alternated list.
// Claude Code issue #33585: an `a|b|c` matcher binds only its FIRST alternative.
// The previous 12-way alternation therefore fired only for rks_agent_run — which
// the governor chain-state machine effectively never permits — so provenance
// minting was dead for every agent tool, in this shell and in every child project
// bootstrapped from it. Do NOT re-collapse these into a single matcher string.
//
// This array is CANONICAL. Two static JSON files carry the same 12 names and
// cannot import from JS: .claude/settings.json and
// templates/base/.claude/settings.json. Adding or removing a name here REQUIRES
// the matching edit in both of those files. They are pinned to this list by exact
// set equality in tests/unit/posttooluse-mcp-matcher-registration.test.mjs, which
// fails and names the out-of-sync site until they match. Do not add a name here
// and "fix the test later" — the test is the only thing keeping them aligned.
export const AGENT_PROVENANCE_TOOLS = [
...
// buildHookRegistration is the ONLY producer of the PreToolUse/PostToolUse shape
// a child's .claude/settings.json carries. Both the fresh-attach generator
// (ensureClaudeSettings, below) and the repair writer (ensureHookRegistration)
// consume it, so fresh-attach output and repair output cannot diverge. It used
// to be an inline literal inside ensureClaudeSettings, reachable only from
// `attach` — that inaccessibility is why no update path could ever repair a
// child whose hooks were never registered.
//
// Emits TIERED hook paths sourced from the manifest. If the manifest is
// unavailable (synthetic/older shellRoot) it falls back to a flat path so attach
// does not crash — real shells always ship the manifest, and the child-settings
// regression guard enforces tiered paths for real builds.
//
// That flat degrade is EXPECTED to fail doctor Check 6 condition (d) in a real
// child, because a real child carries the tiered payload on disk and a flat
// .routekit/hooks/<name>.mjs does not resolve there. The asymmetry is
// deliberate: bootstrap degrades so `attach` does not hard-fail on a partially
// built shell; doctor fails CLOSED so a degraded child is never certified
// healthy. Do not "reconcile" the two policies by softening either one.
export function buildHookRegistration(manifest) {
  const hookCmd = (name) => {
  ...
  return {
    ...
    PostToolUse: [
      ...AGENT_PROVENANCE_TOOLS.map((tool) => ({
        matcher: tool,
        hooks: [
...
/**
 * Merge the canonical hook registration into a child's settings.json.
 * backlog.fix.child-hook-registration-repair-and-audit
 *
 * `hooks` is the ONLY key rks authors in that file — everything else in it
 * (mcpServers, permissions including user-added allow/deny, env, and any key rks
 * does not know about) is user content and survives untouched, in place.
 *
 * A hand-broken (unparseable) settings.json is REFUSED, never overwritten —
 * mirroring migrateChildSettingsHookPaths. A loud failure beats a silent clobber
 * of a file we cannot read.
 *
 * An ABSENT settings.json is created carrying the registration alone. That is
 * the doctor/upgrade repair shape: attach (ensureClaudeSettings) is what emits
 * the full file with env + permissions; a fixer's job is only to make the child
 * governed, never to invent user content.
 *
 * @returns {{changed: boolean, reason: string}} reason ∈
 *   created | registered | unchanged | unparseable
 */
export function ensureHookRegistration({ settingsPath, manifest }) {
  const canonical = buildHookRegistration(manifest);

  if (!fs.existsSync(settingsPath)) {
    writeJSONWithBackup(settingsPath, { hooks: canonical });
  ...
  // Idempotent: an already-canonical block is left byte-identical — no rewrite,
  // no backup churn, no reordering.
  if (JSON.stringify(settings.hooks) === JSON.stringify(canonical)) {
    return { changed: false, reason: "unchanged" };
  }

  // Assigning in place preserves the existing key ORDER and appends `hooks` only
  // when it was absent, so the top-level key-set difference is exactly {hooks}.
  settings.hooks = canonical;
...
export function ensureClaudeSettings({ projectRoot, shellRoot }) {
  ...
  // then ENSURE the canonical registration is actually present.
  //
  // backlog.fix.child-hook-registration-repair-and-audit — this branch used to
  // return right after the migration. migrateChildSettingsHookPaths only
  // rewrites command strings INSIDE an existing hooks block, so a child whose
  // settings.json had no hooks key at all got nothing written and stayed
  // permanently ungoverned, while preflight/upgrade/doctor all reported green.
  // "Never clobber the user's settings" is not the same requirement as "never
  // write anything"; ensureHookRegistration honours the first without the second.
  if (fs.existsSync(settingsPath)) {
    if (manifest) migrateChildSettingsHookPaths({ settingsPath, manifest });
    ensureHookRegistration({ settingsPath, manifest });
    return settingsPath;
  }

  // Fresh child: the full scaffold — env + permissions + the SAME canonical
  // registration the repair path writes.
  const settings = {
    env: {
      RKS_GUARDRAILS: "on"
    },
    ...
    hooks: buildHookRegistration(manifest)
  };
...
export async function attachProject({
  ...
  try {
    // NOTE: readJSON swallows ENOENT and returns its fallback, so this branch runs
    // for BRAND-NEW projects too — the catch below is effectively dead for the
    // fresh case. `routekit project init` also writes .rks/project.json (via
    // init-stack.js) before attachProject ever runs. This is therefore the ONLY
    // path that both births new children and repairs dormant ones, which is why
    // the fetchRaw backfill must live here. Wiring only the catch would fix nobody.
    const rksJson = withFetchRawDefaults(readJSON(rksProjectJsonPath, {}));
    ...
    writeJSONWithBackup(rksProjectJsonPath, withFetchRawDefaults({
      id: projectId,
