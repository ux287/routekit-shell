...
function usage() {
  console.log(`routekit commands:
      ...
      project add-existing  --id=<id> --path=<abs path> [--stack=<stackId>]   # registry upsert only
      project adopt         --id=<id> --path=<abs path> [--stack=<stackId>] [--force-repin] [--from-release=<path>]
                            # register + upgrade + tell you to restart the child's MCP server
      project upgrade       --id=<id> | --all [--from-release=<path>] [--dry-run] [--no-backup]
      project list
      project info          --id=<id>
...
if (cmd === "doctor") {
  ...
  try {
    ...
    // Check 6 — backlog.fix.child-hook-registration-repair-and-audit. Without this line
    // the finding lives only in the returned object, and the operator sees a green doctor
    // over a child whose entire guardrail layer is inert.
    for (const c of f.childHookRegistration || []) {
      if (!c.ok) {
        const detail = c.detail && c.detail.length ? ` (${c.detail.join(", ")})` : "";
        console.log(`  child ${c.id}: hook registration: ${c.reason}${detail}`);
      }
    }
