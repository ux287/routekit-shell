...
/**
 * Is this hunk an ANCHOR rather than a change?
 *
 * An identity hunk (SEARCH side byte-equal to REPLACE side) is a LEGAL and
 * deliberate construct: `.rks/prompts/governor-po.md` sanctions it as a pure
 * location anchor, and refine's add_search_pattern emits it mechanically
 * (server/refine.mjs:1801 puts the same `anchor` on both sides). It is NOT an
 * error. It simply specifies no change, so it must not be COUNTED as one.
 *
 * Normalization: per-line trailing spaces/tabs and trailing newlines are ignored.
 *
 * DELIBERATELY NOT THE SAME PREDICATE as the guard at llm/reviewer.mjs:247-252.
 * That guard uses String.prototype.trimEnd(), which strips only the end of the
 * WHOLE string; the normalizer below carries /gm, so $ matches EVERY line end.
 * The two are INCOMPARABLE, not aligned. They diverge in BOTH directions:
 *   - wider here:    "a  \nb" vs "a\nb" is identity here, a real edit to the reviewer
 *   - narrower here: at the TRAILING position, over the code points trimEnd() strips
 *                    and norm() does not. trimEnd() strips every Unicode WhiteSpace
 *                    and LineTerminator code point (including CR, FF, VT, NBSP,
 *                    ZWNBSP, U+2028, U+2029, the Zs class); norm() strips per-line
 *                    trailing spaces and tabs, and trailing newlines. The remainder
 *                    of that set is the
 *                    divergence, and its canonical member is a carriage return:
 *                    "x\r\n" vs "x\n" both trimEnd() to "x", so the reviewer skips
 *                    the pair as identity, while norm() keeps the carriage return and
 *                    this returns false (fails safe toward "real change")
 *   - NOT a divergence: interior CRLF. "a\r\nb" vs "a\nb" has no trailing whitespace
 *                    on either side, so trimEnd() changes nothing and BOTH predicates
 *                    report a real edit. The narrowness above belongs to the TRAILING
 *                    position alone.
 * Pointing reviewer.mjs at this function is therefore a BEHAVIOUR CHANGE, not a
 * refactor: it MOVES the reviewer's skip set in BOTH directions — suppressing pairs
 * it currently emits ("a  \nb" vs "a\nb") and emitting pairs it currently skips
 * ("x\r\n" vs "x\n"). See backlog.fix.identity-hunk-passes-as-change-spec follow-up 1.
 *
 * A comment-only difference is NOT identity — it lands real bytes, and
 * classifying it as identity would need a tokenizer.
 *
 * @param {string} search
 * @param {string} replace
 * @returns {boolean} true when the hunk specifies no change
 */
export function isIdentityHunk(search, replace) {
  // Non-string input is never identity. Without this guard `String(s ?? "")` coerces,
  // so isIdentityHunk(42, "42") would return true — and a caller handing this a
  // non-string has a bug the fail-safe direction is to surface as "real change",
  // not to suppress. The story's testRequirement is explicit that both sides must
  // be strings; the anchor payload omitted the check.
  if (typeof search !== "string" || typeof replace !== "string") return false;
  const norm = (s) => s.replace(/[ \t]+$/gm, "").replace(/\n+$/, "");
  return norm(search) === norm(replace);
}
...
/**
 * THE single presence counter. Exact bytes only.
 *
 * backlog.fix.search-pattern-presence-not-reconciled-across-validators: this used to be
 * duplicated as a module-private `countOccurrences` in llm/reviewer.mjs. Two implementations
 * that merely agree today are exactly how this defect class starts, so presence is now ONE
 * derivation reached from both call sites — the same shape AMBIGUITY_POLICY already gives the
 * ambiguity verdict. Exact-only is deliberate: server/step-apply.mjs applies with a bare
 * content.indexOf(search), so a normalised match is a promise the applier cannot keep.
 */
export function countPatternOccurrences(content, pattern) {
...
/**
 * Presence WITH tier attribution, so a failure is diagnosable from the payload.
 *
 * `tiers` records what each tier actually OBSERVED rather than only the outcome. A pattern
 * that fails exact while matching normalised is a whitespace defect, not an absent pattern,
 * and reporting it as a bare `pattern_not_found` is what made this class undiagnosable.
 */
export function patternPresenceInContent(content, pattern) {
  const occurrences = countPatternOccurrences(content, pattern);
  const tiers = [{ tier: "exact", observed: occurrences }];
  if (occurrences > 0) return { found: true, occurrences, tiers };
  const normalizedMatch = normalizeWhitespace(content).includes(normalizeWhitespace(pattern));
  tiers.push({ tier: "normalized", observed: normalizedMatch ? "match" : "no-match" });
  return { found: false, occurrences: 0, whitespaceMismatch: normalizedMatch, tiers };
}
