...
/**
 * Field to permitted-value vocabulary, enforced at the entry of BOTH dendron writers.
 *
 * backlog.fix.field-vocabulary-guard-parity: the phase check used to live inline inside
 * `updateField` only, so the array-valued router path through `updateFieldDirect` —
 * reached from server.mjs and agents/dendron.mjs, both of which dispatch on
 * `Array.isArray(value)` — met no guard at all, and `phase: ["not-a-real-phase"]` was
 * writable. One registry plus one helper called from both entry points gives the rule
 * exactly one home.
 *
 * Seeded with `phase` alone. A field absent from this map is untouched.
 */
export const FIELD_VOCABULARIES = {
  phase: VALID_PHASES,
};

// Fields that must always be stored as YAML arrays
export const ARRAY_FIELDS = new Set(["targetFiles", "dependsOn", "testFiles", "testRequirements", "baselineRedTests"]);
...
/**
 * Refuse an out-of-vocabulary field write before the note is read or touched, so a
 * refused call leaves the file byte-identical.
 *
 * Takes no `options`, deliberately. `assertNotReservedArchField` carries an
 * `internalWriter` escape hatch; a vocabulary violation has no legitimate internal
 * caller, so the signature simply offers nowhere for a bypass to be threaded.
 *
 * `Object.hasOwn` rather than a truthiness test: a bare `FIELD_VOCABULARIES[field]`
 * lookup resolves inherited keys such as `constructor` to a truthy non-array.
 */
function assertValidFieldVocabulary(field, value) {
  if (!Object.hasOwn(FIELD_VOCABULARIES, field)) return;
  const vocabulary = FIELD_VOCABULARIES[field];
  if (!vocabulary.includes(value)) {
    throw new Error(`Invalid ${field} '${value}'. Valid ${field}s: ${vocabulary.join(", ")}`);
  }
}
...
export function findMatchingSchema(notesDir, filename) {
  try {
    // backlog.fix.schema-discovery-dendron-standard-filenames: accept BOTH filename
    // conventions. `<name>.schema.yml` is the legacy form child-project scaffolds still
    // ship (templates/app.web.react.spa/skeleton/notes/root.schema.yml), and
    // `schema.<name>.yml` is the Dendron-standard form this vault migrated to and that
    // dendron.yml registers by path. Matching only the legacy form meant
    // notes/schema.backlog.yml — the sole file whose id is `backlog` — was never a
    // candidate, so no backlog note ever received its template frontmatter.
    const candidates = (fs.readdirSync(notesDir) || []).filter(
      (f) => f.endsWith(".schema.yml") || (f.startsWith("schema.") && f.endsWith(".yml")),
    );
...
export function updateFieldDirect(notesDir, filename, field, value, options = {}) {
  ...
  assertValidFieldVocabulary(field, value);
