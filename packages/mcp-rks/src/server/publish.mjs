...
// The substitution sentinel carried by the delivered .claude/skills tree. It is NOT a
// projectId and never equals `from` or `to`; it is a distinct token from the surviving
// placeholder that children resolve at CLAUDE.md-read time.
const SOURCE_PROJECT_SENTINEL = "__RKS_SOURCE_PROJECT__";
...
export function normalizeExportIdentity(tmpDir, from, to, { log = () => {} } = {}) {
  ...
  // 7. The delivered .claude/skills tree — the SUBSTITUTION SENTINEL, not `from`.
  //    The exported skills carry `__RKS_SOURCE_PROJECT__` in projectId-VALUE position, which
  //    bootstrap/sync/vendor resolve for a child. The mirror is delivered ready-to-run, so the
  //    sentinel must resolve to the mirror's own id here too. Still TARGETED, never global:
  //    scoped to this one tree and to this one token, so package-lock integrity hashes,
  //    `<org>/<repo>` URLs and the `tests/**` fixtures are untouched.
  //
  //    ORDERING, MANDATORY: this sits AFTER the `from === to` short-circuit above. The
  //    ACCEPTED CONSEQUENCE is that a `from === to` export leaves sentinels unsubstituted —
  //    which cannot arise for the real rks-public profile (routekit-shell-core → routekit-shell).
  //    The SURVIVING placeholder token is a DIFFERENT token and is deliberately not touched.
  ...
  if (fs.existsSync(skillsRoot)) {
    const walk = (dir) => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        const before = fs.readFileSync(abs, "utf-8");
        const after = before.split(SOURCE_PROJECT_SENTINEL).join(to);
        if (after !== before) {
          fs.writeFileSync(abs, after);
          changed.push(path.relative(tmpDir, abs).split(path.sep).join("/"));
        }
      }
    };
    walk(skillsRoot);
...
/**
 * The complete set of keys a `remotes.<name>` entry may carry. A typo'd sibling key
 * (`enabeld: false`) cannot be caught by value checking, so the RESOLVED entry is validated
 * against this set and an unknown key BLOCKS the publish.
 */
export const KNOWN_REMOTE_KEYS = ["url", "profile", "branch", "enabled"];

/** Render an offending value for an error message without losing its type. */
...
/**
 * Validate the RESOLVED remote entry — the one this publish is actually about to push to.
 *
 * Deliberately NOT called from `loadPublishProfiles`: load-time validation would validate
 * every entry in the file, so one stale unrelated entry would brick publishing, and it would
 * change `listRemotes` / `rks_publish_profiles` behaviour. (ARCH D2.)
 *
 * Returns `null` when the publish may proceed. Otherwise returns a publish-result object
 * whose `ok` is NOT truthy, discriminated as:
 *   - SKIP  → `{ ok: false, suppressed: true, reason }`  (operator disarmed the remote)
 *   - BLOCK → `{ ok: false, blocked: true, error }`      (ambiguous/unknown config — fail closed)
 */
export function validateRemoteEntry(name, entry) {
  if (!entry || typeof entry !== "object" || Array.isArray(entry)) return null;

  const unknownKeys = Object.keys(entry).filter((k) => !KNOWN_REMOTE_KEYS.includes(k));
  if (unknownKeys.length > 0) {
    return {
      ...
      remote: name,
      error:
        `Remote "${name}" in .routekit/publish-profiles.yaml carries unknown key(s): ` +
        `${unknownKeys.map((k) => `"${k}"`).join(", ")}. Known keys are: ` +
        `${KNOWN_REMOTE_KEYS.join(", ")}. Publish BLOCKED (fail closed — a typo'd suppression ` +
        `key must never be read as permission).`,
    };
  }

  if (!Object.prototype.hasOwnProperty.call(entry, "enabled")) return null; // absent → proceed
  if (entry.enabled === true) return null; // literal boolean true → proceed
  ...
  if (entry.enabled === false) {
    return {
      ...
      remote: name,
      reason:
        `Remote "${name}" is disarmed: .routekit/publish-profiles.yaml sets ` +
        `remotes.${name}.enabled: false. Publish SKIPPED — no push was attempted.`,
    };
  }
  ...
  return {
    ...
    remote: name,
    error:
      `Remote "${name}" has a malformed "enabled" value: ${describeValue(entry.enabled)} ` +
      `(type ${Array.isArray(entry.enabled) ? "array" : typeof entry.enabled}). It must be a ` +
      `literal boolean true or false. Publish BLOCKED (fail closed — a malformed suppression ` +
      `flag is never read as permission).`,
  };
}
...
export async function publish(projectRoot, options = {}) {
  ...
  // Suppression / fail-closed validation of the RESOLVED remote entry. This sits BEFORE the
  // `if (remoteConfig?.url)` branch on purpose: an entry with `enabled: false` and no `url`
  // must not fall through to the `else if (!remoteExists(...))` fallback and force-push to a
  // locally-configured git remote of the same name (ARCH D1). It also sits before the dry-run
  // block, so a disarmed remote returns the suppressed result rather than a preview (ARCH D4),
  // and before any temp-export work — no snapshot is built for a push that will not happen.
  const remoteGuard = validateRemoteEntry(remote, remoteConfig);
  if (remoteGuard) {
    if (remoteGuard.suppressed) {
      collector.emit("publish.skipped", options.projectId || "unknown", {
        remote,
        profile: profileName,
        branch,
        reason: remoteGuard.reason,
      });
      console.log(`[publish] ${remoteGuard.reason}`);
    } else {
      collector.emit("publish.failed", options.projectId || "unknown", {
        error: remoteGuard.error,
      });
    }
    return { ...remoteGuard, profile: profileName, branch };
  }
...
export function listRemotes(projectRoot) {
  ...
  return Object.entries(config.remotes || {}).map(([name, remote]) => ({
    ...
    // Honest reporting (ARCH D3): a disarmed mirror must not be displayed as if it were armed.
    // Projected raw — `undefined` when absent, so an armed remote reads exactly as before, and
    // a malformed value is visible rather than coerced. listRemotes does NOT validate.
