...
export async function runRelease({ projectRoot, version = "patch", changelog = null, projectId = "unknown", projectRecord = null, projectJson = null }) {
  ...
  try {
    ...
    if (syncAhead > 0 && syncBehind === 0) {
      return { ok: false, error: `Local ${branchConfig.integration} has unpushed commits. Push ${branchConfig.integration} first.` };
    }
    ...
    const notes = changelog || "Release " + newVersion;
    if (stagingHeadSubject !== `chore(release): ${newVersion}`) {
      ...
      spawnSync("git", ["add", "package.json", "CHANGELOG.md"], { cwd: projectRoot });
