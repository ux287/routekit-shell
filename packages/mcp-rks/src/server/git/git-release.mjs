...
/**
 * Is this commit subject the bump commit a prior release run wrote?
 *
 * EXACT `MAJOR.MINOR.PATCH`, and nothing else. The previous test was `(.+)`, which
 * admitted every `chore(release): ` subject — including the rescue commits written by hand
 * after earlier releases left transitions uncommitted, whose verbatim subjects are
 * `chore(release): record v0.52.0 story phase transitions left uncommitted by rks_release`
 * and `chore(release): chore(release): record v0.52.1 story phase transition left
 * uncommitted by rks_release`. With one of those at the integration HEAD, the captured
 * TEXT became `newVersion`: the bump would be skipped, `git tag -a v<that text>` would fail
 * because a tag name cannot contain spaces or colons, and the guarded rollback would then
 * `git reset --hard HEAD~1` and delete the rescue commit.
 *
 * @returns {string|null} the version, or null when this is not a bump commit.
 */
export function isReleaseBumpSubject(subject) {
  if (typeof subject !== "string") return null;
  const match = subject.match(/^chore\(release\): (0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/);
  return match ? `${match[1]}.${match[2]}.${match[3]}` : null;
}

/**
 * Commit the transitioned notes on the integration branch, on a path where they cannot
 * join the bump commit (the resume paths, where that commit already exists).
 *
 * The subject must NOT be readable as a bump by isReleaseBumpSubject — a release commit
 * that a later run mistakes for an interrupted bump is the defect AC9 exists to close.
 *
 * @returns {{ok: boolean, error?: string}}
 */
function commitTransitions(projectRoot, storyIds, newVersion) {
  const paths = storyIds.map((id) => path.join("notes", `${id}.md`));
  const add = spawnSync("git", ["add", ...paths], { cwd: projectRoot, encoding: "utf8" });
  if (add.status !== 0) return { ok: false, error: (add.stderr || add.stdout || "git add failed").trim() };
  const message = `chore(story): mark ${storyIds.length} story note(s) released in ${newVersion}`;
  const commit = spawnSync("git", ["commit", "-m", message, "-m", "[skip ci]"], { cwd: projectRoot, encoding: "utf8" });
  if (commit.status !== 0) return { ok: false, error: (commit.stderr || commit.stdout || "git commit failed").trim() };
  return { ok: true };
}

/**
 * Does this dirty working tree hold ONLY stranded release transitions?
 *
 * A release that transitioned notes without committing them leaves exactly this residue:
 * modified `notes/backlog.*.md` files whose only change is the phase moving to `released`
 * plus a `releasedIn` stamp. The bare "Working tree not clean" refusal could not tell that
 * apart from ordinary uncommitted work, so the operator was told nothing about the state
 * the previous release itself created.
 *
 * REFUSES, never repairs. Auto-committing here would sweep whatever else sits in the tree
 * into a release commit, so any path that does not match — one unrelated file is enough —
 * falls through to the generic refusal.
 *
 * `diffFor` must report changes against HEAD, not just unstaged ones: a transition that was
 * already `git add`ed shows NOTHING in a worktree-only diff, and an empty diff is refused.
 *
 * @returns {{stranded: true, notes: string[]}|null}
 */
export function classifyDirtyTree(porcelain, diffFor) {
  const lines = String(porcelain || "").split("\n").filter(Boolean);
  if (lines.length === 0) return null;
  const notes = [];
  // The exact porcelain XY codes a modification produces: staged, unstaged, or both.
  // Anything else — an add, a delete, a rename, an untracked file — is not transition
  // residue, and `MM` is listed explicitly rather than admitted by a loose prefix test.
  const MODIFIED = new Set(["M ", " M", "MM"]);
  for (const line of lines) {
    const filePath = line.slice(3).trim();
    if (!/^notes\/backlog\..*\.md$/.test(filePath)) return null;
    if (!MODIFIED.has(line.slice(0, 2))) return null;
    const diff = String(diffFor(filePath) || "");
    const changed = diff
      .split("\n")
      .filter((l) => (l.startsWith("+") || l.startsWith("-")) && !/^(\+\+\+|---)/.test(l));
    if (changed.length === 0) return null;
    const onlyTransition = changed.every((l) =>
      /^[+-]phase:\s*["']?(integrated|released)["']?\s*$/.test(l) || /^[+-]releasedIn:/.test(l),
    );
    if (!onlyTransition) return null;
    notes.push(filePath);
  }
  return notes.length > 0 ? { stranded: true, notes } : null;
}
...
export async function runRelease({ projectRoot, version = "patch", changelog = null, projectId = "unknown", projectRecord = null, projectJson = null, enforcementOverride = null }) {
  ...
  try {
    ...
    if (status.stdout.trim()) {
      // NAME the state a previous release left behind, rather than reporting it as ordinary
      // uncommitted work. Refuses either way — this never commits anything.
      const stranded = classifyDirtyTree(status.stdout, (file) =>
        (spawnSync("git", ["diff", "HEAD", "--unified=0", "--", file], { cwd: projectRoot, encoding: "utf8" }).stdout || ""),
      );
      if (stranded) {
        const error =
          `${stranded.notes.length} story note(s) carry an uncommitted release transition left by a ` +
          `previous rks_release run. Commit them on ${branchConfig.integration} (they are the ` +
          `phase: released + releasedIn changes), then retry rks_release.`;
        collector.emit("release.failed", projectId, { version, durationMs: Date.now() - releaseStartMs, error, reason: "stranded_release_transitions" });
        return { ok: false, reason: "stranded_release_transitions", error, strandedNotes: stranded.notes };
      }
    ...
    // Declared out here because the fresh path transitions BEFORE the bump commit and the
    // resume fall-through transitions into its own commit; the success return reads whichever
    // ran. `transitionsInTag` records, on the response, whether the transitions are reachable
    // from the tag — emitted on BOTH paths, because an absent key reads as falsy and would be
    // indistinguishable from the resume value.
    let freshReleasedStories = [];
    let transitionsInTag = false;
    let transitionCommitWarning = null;
    let newVersion;
    ...
    const resumeMatch = isReleaseBumpSubject(resumeHeadSubject);
    if (resumeMatch) {
      resuming = true;
      newVersion = resumeMatch;
      ...
      if (resumeRemoteMain === resumeStagingHead && resumeTagOnOrigin) {
        ...
        // Resume Row 1 — the release is already on origin, so these transitions land AFTER
        // the tag and are not tag-reachable. They still get committed and pushed here; the
        // run used to return ok:true with the notes dirty in the worktree.
        let resumedStories = await transitionIntegratedStories(projectRoot, newVersion, projectId);
        let resumeWarning = null;
        if (resumedStories.length > 0) {
          const committed = commitTransitions(projectRoot, resumedStories, newVersion);
          if (committed.ok) {
            const push = spawnSync("git", ["push", "origin", branchConfig.integration], { cwd: projectRoot, encoding: "utf8" });
            if (push.status !== 0) {
              resumeWarning = `Story phase transitions were committed but ${branchConfig.integration} could not be pushed: ${(push.stderr || push.stdout || "").trim()}`;
            }
          } else {
            // NEVER report ids that live only in the worktree as released.
            resumeWarning =
              `Story phase transitions for ${resumedStories.length} note(s) could not be committed: ` +
              `${committed.error}. They remain uncommitted in the working tree.`;
            resumedStories = [];
          }
        }
        return {
          ...
          tag: `v${newVersion}`,
          resumed: true,
          releasedStories: resumedStories,
          transitionsInTag: false,
    ...
    if (stagingHeadSubject !== `chore(release): ${newVersion}`) {
      ...
      // THE TRANSITIONS BELONG IN THIS COMMIT. They used to run after the tag and the push
      // and were never staged at all, so every release left the notes dirty in the worktree
      // and `git show v<version>:notes/<id>.md` showed the pre-release phase. Running them
      // here — after the pre-mutation gate, before this commit — makes them tag-reachable
      // and makes the rollbacks below (`reset --hard`) undo them along with the bump.
      freshReleasedStories = await transitionIntegratedStories(projectRoot, newVersion, projectId);
      if (freshReleasedStories.length > 0) {
        const addNotes = spawnSync("git", ["add", ...freshReleasedStories.map((id) => path.join("notes", `${id}.md`))], { cwd: projectRoot, encoding: "utf8" });
        // Fail CLOSED: an unchecked add is how the transitions went missing from the commit
        // in the first place. A release that cannot stage them must not claim them.
        if (addNotes.status !== 0) {
          return { ok: false, error: `Failed to stage story phase transitions: ${(addNotes.stderr || addNotes.stdout || "").trim()}` };
        }
      }
      ...
      transitionsInTag = true;
    } else {
      // Resume fall-through: the bump commit already exists, so the transitions cannot join
      // it. They get their own commit here. It reaches origin through the integration-branch
      // push this same run performs after the tag push (the `git push origin
      // branchConfig.integration` below) — this branch deliberately does not push twice, and
      // that later push is load-bearing for it. Resume Row 1 returns before reaching that
      // step, which is why it pushes for itself. The subject deliberately does NOT match
      // isReleaseBumpSubject — a later run must not read this commit as an interrupted bump.
      freshReleasedStories = await transitionIntegratedStories(projectRoot, newVersion, projectId);
      if (freshReleasedStories.length > 0) {
        const committed = commitTransitions(projectRoot, freshReleasedStories, newVersion);
        // MEASURED, not assumed. This commit lands on the integration branch BEFORE the
        // ff-merge and the tag below, so it IS inside the tagged history — unlike Resume
        // Row 1, which returns before those steps and therefore reports false. The story
        // text predicted false for both resume paths; the fall-through contradicts it, and
        // the observation wins.
        if (committed.ok) transitionsInTag = true;
        if (!committed.ok) {
          transitionCommitWarning =
            `Story phase transitions for ${freshReleasedStories.length} note(s) could not be committed: ` +
            `${committed.error}. They remain uncommitted in the working tree.`;
          freshReleasedStories = [];
        }
      }
    }
    ...
    // The transitions already ran — into the bump commit on the fresh path, into their own
    // commit on the resume fall-through. Running them here is what left every release with a
    // dirty tree and a tag whose notes still read `integrated`.
    const releasedStories = freshReleasedStories;

    const result = { ok: true, version: newVersion, tag: `v${newVersion}`, releasedStories, transitionsInTag, publishResult, metadata: { timestamp } };
    ...
    // Appended, never assigned: an earlier warning in this chain must not be dropped.
    if (transitionCommitWarning) result.warning = result.warning ? `${result.warning}\n${transitionCommitWarning}` : transitionCommitWarning;
