    ...
    if (planStatus === "note_only" || planStatus === "quality_failed") {
      ...
      if (res.skipped) {
        ...
        try {
          ...
          spawnSync("git", ["commit", "-m", `docs(backlog): mark ${normalizedProblem} as ${res.to}`], { cwd: projectRoot, encoding: "utf8" });
        } catch { }
      }
      phaseWrite = { attempted: res.attempted, ok: true, from: res.from ?? null, to: res.to, verified: !!res.verified, skipped: res.skipped ?? null };
    } else {
