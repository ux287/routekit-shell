...
import { normalizeCheckRollup, allChecksPassed } from "../shared/check-rollup.mjs";
// Verbatim re-export, never a wrapper: the structural pin asserts every consumer reaches the
// SAME function object, and a wrapper would break that identity while looking correct.
export { normalizeCheckRollup, allChecksPassed } from "../shared/check-rollup.mjs";
...
export function ghPrView({ projectRoot, prNumber }) {
  ...
  try {
    const raw = JSON.parse(result.stdout);
    const checks = normalizeCheckRollup(raw.statusCheckRollup);
    return {
      ok: true,
      pr: {
        number: raw.number,
        ...
        checks,
        allChecksPassed: allChecksPassed(checks),
        body: raw.body,
