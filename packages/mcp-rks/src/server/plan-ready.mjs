...
import { AMBIGUITY_POLICY, extractExplicitEdits, resolveTargetPath } from "../llm/reviewer.mjs";
...
export async function runPlanReadyTool({ projectId, problemId, projectRoot }) {
      ...
      if (hasDirective) {
        ...
        if (!isSynthesizedBody(blockKey === undefined ? undefined : createFileBlocks.get(blockKey), target)) {
          issues.push({
            ...
            suggestion: `Add a fenced code block containing real file content immediately after "// CREATE FILE: ${target}" (same "###" section), or under a "### ${target}" heading. A directive alone leaves the planner to author the file from prose, which fails with failureClass "structural" and cannot be refined.`
          });
        }
  ...
  for (const [blockIndex, edit] of extractExplicitEdits(body, projectRoot, allTargets).entries()) {
  ...
  for (const target of allTargets) {
    ...
    //
    // BOTH heading forms count. This check previously recognised only `### Target: <path>`,
    // which was also the form the reviewer's resolveTargetPath REJECTED — so plan_ready and
    // the planner disagreed about what a target heading looks like, and the form documented
    // in this file was the rejected one. With the label now stripped in resolveTargetPath,
    // `### Target: <path>` and `### <path>` are the same heading; this asks the shared
    // resolver rather than string-matching one spelling, so the two cannot drift apart again.
    const headingNamesTarget = [...body.matchAll(/^#{1,6}[ \t]+([^\n]+)$/gm)]
      .some((h) => resolveTargetPath(h[1].trim(), projectRoot, allTargets) === target);
