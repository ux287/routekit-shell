...
export async function gatherTargetContext({ projectRoot, projectId, normalizedProblem, normalizedTask, context, finalSlug, _enhancedContext }) {
  ...
  // backlog.fix.planner-prune-silent-and-provenance-split: pass the report out-param the
  // function already supports. refine.mjs has always passed one; this call site never did,
  // so the four loss counters were computed nowhere on the path that decides what the
  // planner actually sees. A prune that discards authored content must not be silent.
  const pruneReport = {};
  planningText = pruneRefineBlocks(planningText, { report: pruneReport });
  const pruneShedAnything =
    (pruneReport.linesShed || 0) > 0 ||
    (pruneReport.createDirectivesShed || 0) > 0 ||
    (pruneReport.fencesShed || 0) > 0 ||
    (pruneReport.anchorsShed || 0) > 0;
  if (pruneShedAnything) {
    console.error(
      `[rks.plan] project=${projectId} note prune shed content: ` +
        `linesShed=${pruneReport.linesShed || 0} ` +
        `createDirectivesShed=${pruneReport.createDirectivesShed || 0} ` +
        `fencesShed=${pruneReport.fencesShed || 0} ` +
        `anchorsShed=${pruneReport.anchorsShed || 0}`,
    );
  }
