...
export function detectCreateFileDirective(planningText, frontmatterTargets) {
  ...
  if (planningText) {
    ...
    // Heading form, line-anchored — reconciled with planner-utils by
    // backlog.fix.create-file-directive-grammar-not-reconciled.
    if (/^[^\S\n]*#{2,3}\s*CREATE\s+FILE:/im.test(planningText)) return true;
