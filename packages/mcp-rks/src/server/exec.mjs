...
async function runExecTool({ projectId, label, skipTests = false, autoCommit = false, dryRun = false }) {
  ...
  const unexpectedFiles = modifiedFiles.filter(f => {
    ...
    // Allow .rks/ internal files
    if (f.startsWith('.rks/')) return false;
    // Allow .routekit/ infrastructure files (hooks managed by guardrailsOff/On)
    if (f.startsWith('.routekit/')) return false;
    // Allow files generated by preCommands (e.g., package-lock.json from npm install)
    if (preCommandGeneratedFiles.has(f)) return false;
    // Allow implicit parent directories of expected files
    if (implicitDirs.has(f) || implicitDirs.has(f.replace(/\/$/, ''))) {
      console.error(`[rks.exec] Implicit directory allowed: ${f}`);
      return false;
    }
    return true;
  });
  ...
  try {
    ...
    for (const step of plan.steps) {
      ...
      if (_execContext) {
        ...
        const divergenceCheck = detectPerStepDivergence(projectRoot, expectedFilesThrough);
