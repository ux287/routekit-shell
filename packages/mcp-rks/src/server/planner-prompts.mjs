...
export function formatDependenciesForPrompt(pkgDeps) {
  ...
  const directive = "IMPORTANT: Only import packages listed above (or Node built-ins / relative paths). Do NOT import a package that is not declared here based on ecosystem convention — if a new package is genuinely required, emit an explicit dependency-add step (e.g. an npm install run_command or a package.json edit) in the same plan.";
  return [lines.join("\n"), directive];
}
