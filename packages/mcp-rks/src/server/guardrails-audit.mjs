...
// localMerge is NOT imported here any more: this rail's delivery is delegated to
// deliverFeatureBranch, which calls it. Importing it without a call site is the same dead
// reference AC 15 removes on the on-rail side.
...
/**
 * Bounds on the acCoverage payload, RE-EXPORTED from shared/ac-coverage.mjs.
 *
 * The caps, normalizeAcCoverage and its list helper moved there so the review
 * ship step built below and the review.complete telemetry emit in review.mjs
 * apply ONE cleaning rule. This module still must not import review.mjs
 * statically (see RECOGNIZED_REVIEW_CAUSES above), and review.mjs importing
 * this module would reverse the one-way dynamic dependency, so both import the
 * zero-import leaf. Re-exported because tests read the caps from this module.
 */
export { MAX_AC_COVERAGE_ENTRIES, MAX_AC_COVERAGE_ENTRY_CHARS } from "../shared/ac-coverage.mjs";
...
/**
 * Derive the phase-advance telemetry fields for an auto-ship emit from the
 * advance_phase entry the executed path RECORDED on shipSteps. It reads no gate
 * object and no topology flag, so every value is sourced from what this ship did.
 *
 * phaseAdvanceDecision is the gate's decision AS APPLIED on this ship.
 *   suppressed             the suppressed entry was recorded, so no reconcile ran
 *   permitted              a reconcile was attempted with no permit notice
 *   permitted_with_notice  a reconcile was attempted under a permit notice
 *   not_consulted          no entry was recorded. The 3-branch arm, and a ship
 *                          with no problemId, never read the gate's output.
 * phaseAdvanceResult is the OBSERVED outcome of the reconcile.
 *   not_attempted | reconcile_failed | reached_integrated |
 *   reconcile_ok_not_integrated
 * reached_integrated names the phase OBSERVED after the reconcile, read from
 * the recorded `to` and never inferred from `ok`. It is not proof this ship
 * moved the phase, since a note that was already integrated records the same.
 *
 * Pure and total. Never throws, never returns a key whose value is undefined,
 * and never reads the entry's remedy.
 */
export function buildPhaseAdvanceTelemetry(shipSteps) {
  const entry = Array.isArray(shipSteps)
    ? shipSteps.find((s) => s && s.step === "advance_phase")
    : undefined;
  if (!entry) {
    return { phaseAdvanceDecision: "not_consulted", phaseAdvanceResult: "not_attempted" };
  }
  if (entry.skipped === true) {
    return {
      phaseAdvanceDecision: "suppressed",
      phaseAdvanceResult: "not_attempted",
      ...(typeof entry.reason === "string" ? { phaseAdvanceSuppressionReason: entry.reason } : {}),
    };
  }
  const notice = typeof entry.notice === "string" && entry.notice.length > 0 ? entry.notice : null;
  let phaseAdvanceResult = "reconcile_failed";
  if (entry.ok === true) {
    phaseAdvanceResult = entry.to === "integrated" ? "reached_integrated" : "reconcile_ok_not_integrated";
  }
  return {
    phaseAdvanceDecision: notice ? "permitted_with_notice" : "permitted",
    phaseAdvanceResult,
    ...(notice ? { phaseAdvanceNotice: notice } : {}),
  };
}
...
/**
 * The remedy for the two COVERAGE causes, and the reason it is a different string.
 *
 * backlog.fix.phase-advance-suppression-terminal-and-remedy-inoperative. Both coverage
 * branches used to carry AC_REMEDY, and following it does nothing: `advanceOnUnassessedAC`
 * is read at rule 5, BELOW both of their returns, so setting the flag cannot reach them. The
 * gate was emitting an instruction for recovering from its own decision that could not work —
 * a status not sourced from an observation, pointed at the operator.
 *
 * Widening the flag to cover these was the other option and is wrong: the flag deletes the
 * signal, and rule 5's case is a genuinely different fact (no evidence produced at all, versus
 * evidence produced and outstanding). So the causes keep their decision and gain a remedy that
 * is true.
 *
 * This string is also what gives `rks_phase_repair` a consumer, on the exact code path that
 * produces the condition the tool repairs. `migrateImplementedToIntegrated` is the counter-
 * example already in the tree: an exported phase-repair whose only occurrence in `packages` is
 * its own export line.
 */
const REPAIR_REMEDY =
  "resolve the outstanding acceptance criteria and re-ship, or — if the implementation is " +
  "already merged — call rks_phase_repair with this storyId to advance the phase on the " +
  "evidence of the merged commit. rks_phase_repair refuses unless a commit carrying this " +
  "story's trailer is reachable from the integration branch.";
...
function getGitState(projectRoot) {
  try {
    ...
    const dirtyRaw = execSync("git status --porcelain", { cwd: projectRoot, encoding: "utf8" });
    const dirty = dirtyRaw.trim();
    // `dirtyPaths` is the porcelain path list, not just the boolean.
    //
    // backlog.fix.guardrails-abort-discards-pre-session-work: collapsing this to
    // `dirty.length > 0` and DISCARDING THE PATHS is the origin of that defect — abort
    // could tell THAT the tree was dirty at open but never WHICH files, so it had nothing
    // to spare and nothing to name in a refusal.
    //
    // Parsed from the UNTRIMMED output on purpose. Porcelain lines are `XY <path>`, and
    // `XY` may lead with a space (` M path`) — trimming the whole blob first eats that
    // space on the FIRST line only, so a fixed-width slice would silently drop a
    // character from exactly one path. A rename reads `R  old -> new`; the destination is
    // the path that exists on disk, so that is the one recorded.
    const dirtyPaths = dirtyRaw
      .split("\n")
      .filter((l) => l.length > 3)
      .map((l) => {
      ...
      .filter(Boolean);
    return { head, branch, dirty: dirty.length > 0, dirtyPaths };
  } catch (e) {
    // `dirtyPaths: null` — UNKNOWN, deliberately not `[]`. An empty array means "measured
    // and clean"; a failed measurement must not be readable as one.
    return { head: null, branch: null, dirty: false, dirtyPaths: null, error: e.message };
  }
}
...
export async function guardrailsOff(projectRoot, reason = "unspecified", scope = "all", problemId = null, projectId = "unknown") {
  ...
  const session = {
    ...
    // The PRE-SESSION DIRTY MANIFEST. Captured here, from `getGitState` above, BEFORE
    // this function performs any mutation of its own — before the .routekit/hooks ->
    // .routekit/hooks.bak relocation and before the scope-file write. Capturing it later
    // would record a non-empty manifest for a session that opened clean, because the hook
    // tiers are tracked, and every abort call site in the corpus would then refuse.
    //
    // EMPTY ARRAY and ABSENT are different answers and must stay distinguishable: `[]`
    // means measured-and-clean, an absent key means a session opened before this field
    // existed. guardrailsAbort keys its legacy branch on exactly that distinction.
    preExistingDirty: gitState.dirtyPaths ?? [],
  ...
  const scopeData = {
    ...
    reason,
    preExistingDirty: session.preExistingDirty,
  };
 ...
 * Throws away working-tree changes by `git reset --hard` to the session's headCommit plus
 * `git clean -fd` for new untracked files, and restores the guardrails hooks, with NO
 * commit, branch, merge, or push.
 *
 * "Since the session started" is NOT enforced by the reset — the reset targets a COMMIT,
 * so it takes anything uncommitted, including work that predated the session. That scope
 * is enforced instead by the PRE-SESSION DIRTY MANIFEST recorded at session open and by
 * the refusal below: if the tree was already dirty when the session opened, this returns
 * `ok: false` and discards nothing unless the caller passes `discardPreExisting: true`.
 * See backlog.fix.guardrails-abort-discards-pre-session-work.
 *
...
export async function guardrailsAbort(projectRoot, options = {}, projectId = "unknown") {
  ...
  // ── PRE-SESSION WORK GATE ──────────────────────────────────────────────────────────
  // backlog.fix.guardrails-abort-discards-pre-session-work
  //
  // The reset below targets the session's START COMMIT, so it destroys uncommitted work
  // that PREDATES the session and was never this session's to discard. That is not
  // hypothetical: an abort on 2026-09-05 took an uncommitted archive rename and a
  // never-committed research note with it, and neither was recoverable.
  //
  // A path-scoped reset cannot fix this. The overlap case — a path dirty at open AND
  // modified during the session — has no correct narrow answer: resetting it destroys the
  // pre-session work, skipping it leaves session work behind, so the abort is not an
  // abort. Refusal plus an explicit opt-in is the only shape that stays honest.
  //
  // ORDERING MATTERS TWICE. This sits AFTER the orphan early-return, so a session with no
  // headCommit keeps its warn-and-restore behaviour. And it sits BEFORE the hook
  // relocation below, so a refusal leaves the hooks EXACTLY as found — a half-abort that
  // restores hooks and then declines the discard leaves the caller in a state neither
  // rks_guardrails_on nor a retried abort describes.
  //
  // The refusal returns WITHOUT clearing guard state, so the session stays ACTIVE and an
  // immediate retry with discardPreExisting: true succeeds without re-opening it.
  const preExisting = activeSession.preExistingDirty;
  const manifestAbsent = preExisting === undefined || preExisting === null;

  if (options?.discardPreExisting !== true) {
    if (manifestAbsent) {
      // LEGACY session, opened before the manifest existed. Refuse only if the record
      // ALSO cannot establish the tree was clean — a CONJUNCTION, not an unconditional
      // gate. `dirtyAtStart` is truthful on legacy records, so refusing on every one of
      // them would WEDGE the session: guardrails are already off, it cannot be reopened,
      // and the only escape would be an opt-in to destroying nothing.
      if (activeSession.dirtyAtStart !== false) {
        return {
          ok: false,
          aborted: false,
          reason: "pre_session_state_unknown",
          sessionId: activeSession.sessionId,
          // NULL, not []. Reporting an empty list here would present an absence of
          // measurement as an observation of cleanliness.
          preExistingDirty: null,
          error:
            "This session predates the pre-session dirty manifest, and its record does not establish that the tree was clean when it opened. The specific paths are UNKNOWN. Refusing to discard: the reset targets the session's start commit and could destroy work that was never this session's.",
          hint: "Inspect the working tree yourself (git status), commit or stash anything worth keeping, then re-run with discardPreExisting: true.",
        };
      }
      // dirtyAtStart === false — provably clean at open. Nothing pre-session to protect,
      // so fall through to the unchanged discard.
    } else if (preExisting.length > 0) {
      return {
        ok: false,
        aborted: false,
        reason: "pre_session_work_present",
        sessionId: activeSession.sessionId,
        preExistingDirty: preExisting,
        error: `Refusing to discard: ${preExisting.length} path(s) were already modified when this session opened and are not this session's to throw away.`,
        hint: "Commit or stash those paths, then retry — or re-run with discardPreExisting: true to destroy them deliberately.",
      };
    }
  }
  ...
  return {
    ...
    ...(changeCountError ? { changeCountError } : {}),
    // Named only when the caller EXPLICITLY opted in to destroying work that was not this
    // session's. Absent on the ordinary path, so its presence is a receipt for a
    // deliberate act rather than a field that always reads empty.
    ...(options?.discardPreExisting === true && !manifestAbsent && preExisting.length > 0
      ? { preExistingDiscarded: preExisting }
      : {}),
    hooksRestored: true,
    scopeFileRemoved,
...
/**
 * Build the off-rail `"cost-report"` ship step.
 *
 * Off-rail emitted NO cost report at all before this. That was a REGRESSION, not an
 * omission: `backlog.z_implemented.feat.off-rail-pr-body-parity` shipped cost reporting
 * into this file, and `guardrails-on-no-feature-branch-push` removed the off-rail PR six
 * days later — the report went with it, because the PR body was the only vehicle. The
 * vehicle is now a ship step on `shipSteps`, which the caller already reads.
 *
 * ACCURACY, not mere presence. Three ways a cost step can lie, each refused here:
 *   1. EMPTY DRESSED AS REAL — `generateCostReport` returns `{ ok: true, noData: true }`
 *      with NO `markdown` key when there are no token events. That yields a skipped step,
 *      never a report.
 *   2. WRONG SCOPE PRESENTED AS THIS STORY — cost-report.mjs's reader accepts EVERY event
 *      in the file when `storyId` is falsy, while still labelling the result
 *      `scope: 'story'`. A falsy story id therefore yields a skipped step, never a report,
 *      and the step carries the id the report was generated FOR.
 *   3. RE-RENDERED OR INVENTED — the step carries the generator's own `markdown`,
 *      `rawCost` and `efficientCost` by identity, not a re-render or a summary.
 *
 * NEVER FAILS A SHIP. `ok: true` only when a real report was observed; otherwise an
 * `ok`-less `skipped` step with a reason. NEVER `ok: false` — `collectFailedShipSteps`
 * promotes an `ok: false` step into failedShipSteps, and a missing cost figure is not a
 * ship failure.
 *
 * The payload must not contain `hook`, `sync` or `drift` in any case:
 * tests/integration/off-rail-scoped-staging.test.mjs serializes every shipSteps entry and
 * asserts against that pattern.
 */
export async function buildOffRailCostStep(projectRoot, storyId, projectJson) {
  const includeCostReport = projectJson?.prBodyIncludeCostReport !== false;
  if (!includeCostReport) {
    return { step: "cost-report", skipped: true, reason: "disabled_by_config" };
  }
  // Discriminator 2: a falsy story id would silently widen the report to every event in
  // the file while still labelling itself a story report. Refuse rather than fabricate.
  if (!storyId) {
    return { step: "cost-report", skipped: true, reason: "no_story_id" };
  }
  try {
    const { generateCostReport } = await import("@routekit/telemetry/cost-report");
    const report = generateCostReport(projectRoot, {
      scope: "story",
      storyId,
      format: "markdown",
    });
    // Discriminator 1: noData carries no `markdown` key at all.
    if (!report || report.noData || !report.markdown) {
      return { step: "cost-report", skipped: true, reason: "no_token_events", storyId };
    }
    // Discriminator 3: the generator's own output, carried by identity.
    return {
      step: "cost-report",
      ok: true,
      storyId,
      markdown: report.markdown,
      rawCost: report.rawCost,
      efficientCost: report.efficientCost,
    };
  } catch (e) {
    // Best-effort — a cost report must never block or fail a ship.
    return { step: "cost-report", skipped: true, reason: "generator_failed" };
  }
}
...
export async function guardrailsOn(projectRoot, options = {}, projectId = "unknown") {
  ...
  if (touchedTotal > 0 && !options.skipAutoShip) {
    ...
    const isThreeBranch = isThreeBranchTopology(branchConfig);
    ...
    try {
      ...
      if (isThreeBranch) {
        ...
        //
        // Delivery is delegated to the SHARED implementation. `target` is REQUIRED and has
        // no default: we pass `gitState.branch`, the branch the operator was on when
        // guardrails went off, which is what this rail has always merged into. Passing
        // `branchConfig.working` here would silently retarget an escape hatch that can be
        // started from any branch.
        const delivery = deliverFeatureBranch({
          projectRoot,
          featureBranch: branchName,
          branchConfig,
          target: gitState.branch,
        });
        const lmRecord = delivery.steps.find((s) => s.kind === "local_merge");
        // RE-RAISE (Decision 3, option a). The off-rail failure observable is preserved
        // byte-for-byte: same message shape, same `try`, so response.failedStage stays
        // "integrate" (set just above), response.shipError/message are unchanged and
        // shipOutcome stays "failed". Mapping this to an ok:false step instead would
        // reclassify a merge that never merged as "shipped_with_failures".
        if (!delivery.ok) {
          throw new Error(`Local merge failed: ${delivery.error}`);
        }
        // NOTE: this arm's step name is SNAKE `local_merge` — the 2-branch arm below uses
        // KEBAB `local-merge`. Off-rail's vocabulary is mixed, not uniformly kebab, and
        // tests/unit/guardrails-on-three-branch.spec.mjs:143/:145 pin this spelling.
        shipSteps.push({
          step: "local_merge",
          ok: true,
          from: branchName,
          to: gitState.branch,
          warning: lmRecord?.warning,
        });
        ...
        shipSteps.push(await buildOffRailCostStep(projectRoot, activeSession.problemId, projectJsonForBranchConfig));
        ...
        try {
          ...
          collector.emit("guardrails.auto_shipped", projectId, {
            ...
            ...buildPhaseAdvanceTelemetry(shipSteps),
        ...
        // 2-branch: local merge into the working branch, delete feature branch,
        // push the working branch directly — no remote feature branch, one CI run.
        //
        // Delivery is delegated to the SHARED implementation. `target` is REQUIRED and has
        // no default: we pass `gitState.branch` for the same reason as the 3-branch arm.
        // The `git checkout` this block used to perform before deleting is gone — it was
        // redundant, because local-merge.mjs:19 already checks out the target, and the
        // delete now happens inside the shared module immediately after that checkout.
        const delivery = deliverFeatureBranch({
          projectRoot,
          featureBranch: branchName,
          branchConfig,
        ...
        // Map the shared module's NEUTRAL record kinds into THIS rail's own vocabulary.
        // Off-rail's vocabulary is mixed by history — kebab here, snake in the 3-branch
        // arm above — so the mapping is per-arm and neither spelling may drift.
        //
        // RE-RAISE (Decision 3, option a). Delivery failures are reported by THROWING, as
        // they always were: same two message shapes, inside the same `try`, after
        // `shipStage = "integrate"`. So response.failedStage, response.shipError,
        // response.message and shipOutcome "failed" are all preserved byte-for-byte.
        // Mapping these to ok:false steps instead would reclassify a merge that never
        // merged as "shipped_with_failures" — merged and pushed, but with a failed step —
        // which is false.
        ...
        if (deleteRecord) {
          // `ok` is DERIVED from the shared module's observed record, never asserted as a
          // literal — same discipline as the on-rail mapping.
          shipSteps.push({ step: "delete-branch", ok: deleteRecord.ok === true, branch: branchName, ...(deleteRecord.error ? { error: deleteRecord.error } : {}) });
        }

        // A failed DELETE does NOT abort the push and does NOT throw — the shared module
        // pushes regardless, exactly as this block's fall-through did. Only a failed PUSH
        // re-raises.
        const pushRecord = delivery.steps.find((s) => s.kind === "push_working");
        if (!pushRecord || pushRecord.ok === false) {
          throw new Error(`Push failed: ${pushRecord?.error ?? delivery.error}`);
        }
        ...
        shipSteps.push({
          ...
          // backlog.fix.cycle-complete-discards-uncommitted-changes: on success, the tracked paths
          // observed dirty before AND after the sync — the post-condition backing
          // `unstagedOutOfScope` above. Always an array.
          ...(cycleResult.ok ? { preservedPaths: cycleResult.preservedPaths ?? [] } : {}),
          ...
          ...(cycleResult.ok ? {} : {
            ...
            dirtyPaths: cycleResult.dirtyPaths ?? [],
          }),
        });
