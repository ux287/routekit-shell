...
export function extractCreateFileBlocks(markdown) {
  ...
  // Comment form (canonical) and heading form. The heading arm is LINE-ANCHORED for the
  // same reason as extractCreateFileDirectives — and it matters more here, because this
  // extractor produces file CONTENT, not just a file name. The ### section split above
  // already stops a ##-level directive reaching a fence that lives after a ### heading.
  const createFilePatterns = [
    /\/\/\s*CREATE\s+FILE:\s*(\S+)[\s\S]*?```(?:\w*)\n([\s\S]*?)```/gi,
    /^[^\S\n]*#{2,3}\s*CREATE\s+FILE:\s*(\S+)[\s\S]*?```(?:\w*)\n([\s\S]*?)```/gim,
  ];
