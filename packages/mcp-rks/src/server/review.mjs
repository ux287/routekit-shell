...
import { normalizeAcCoverage, coerceAcCoverageEntry } from '../shared/ac-coverage.mjs';
 ...
 * criteria. This object reaches three sinks, and ALL THREE now apply
 * normalizeAcCoverage from shared/ac-coverage.mjs AFTER this function: the
 * rks_guardrails_on response through buildOffRailReviewStep, the review.complete
 * telemetry emit of a review whose reviewer ran, and the story-ship payloads,
 * which carry the whole redacted review and normalize it at the same boundary
 * they are redacted at. Reuses scrubSecretLiterals with
 * DEFAULT_SECURITY_PATTERNS so "secret-shaped" keeps ONE definition in this
 * module.
 *
 * Each entry of the three lists is coerced to a string by
 * coerceAcCoverageEntry, the normalizer's own rule, BEFORE it is scrubbed, so a
 * nested array cannot carry a credential past the scrubber and into the
 * normalizer's coercion. An entry that coercion throws on is kept unchanged,
 * so the throw never reaches the catch below, and normalizeAcCoverage, which
 * caps a list before it coerces, stays the only decider of whether coverage
 * survives. Keys are not allowlisted and lengths are not bounded
 * here. That is normalizeAcCoverage's job, and scrubbing first keeps a
 * truncation from cutting a credential mid-literal. Never mutates its input,
 * and never throws on a malformed one.
 */
function redactAcCoverage(acCoverage) {
  ...
  const clean = (value) =>
    Array.isArray(value)
      ? value.map((entry) => {
          // Per-entry guard, DECISION-PRESERVING. An entry String() cannot
          // coerce is kept as it is, so normalizeAcCoverage, which caps the list
          // before it coerces, stays the only decider of whether coverage
          // survives. Do not make the coercion total instead, which flips the
          // phase-advance decision the other way.
          let coerced;
          try {
            coerced = coerceAcCoverageEntry(entry);
          } catch {
            return entry;
          }
          return scrubSecretLiterals(coerced, policy);
        })
      : value;
  try {
    return {
...
export async function runReview({ projectId, problemId, branch, targetBranch = 'staging' }) {
  ...
  try {
    ...
    // The ONE bound acCoverage, read unchanged by the returned review.
    // llmResult.acCoverage carries the model's own assessed claim and must never
    // reach telemetry unbound. The emit carries the shape the gate reads from
    // the returned review — redacted, then normalized by the shared rule — so
    // the value on disk is allowlisted and bounded exactly as the gate reads it.
    const boundAcCoverage = bindAcAssessment(llmResult.acCoverage, acSection.found, partialDiff);
    const emittedAcCoverage = normalizeAcCoverage(redactAcCoverage(boundAcCoverage));
    ...
    // The ONE findings array every consumer reads, computed once. The emit used to record
    // allFindings while the returned review carried the softened array, so on a partial diff
    // telemetry showed an ac_coverage finding at `block`, and a blockerCount counting it,
    // that no consumer ever saw — the phase-advance gate reads the softened one. Same
    // direction as the acCoverage fix on this emit: record the value the decision used.
    // findingCount is unaffected, the array's length is the same; only severities move.
    const reportedFindings = softenUnassessableAcFindings(allFindings, partialDiff);

    collector.emit('review.complete', projectId, {
      problemId,
      verdict: finalVerdict,
      findingCount: reportedFindings.length,
      blockerCount: reportedFindings.filter(f => f.severity === 'block').length,
      warningCount: reportedFindings.filter(f => f.severity === 'warn').length,
      findings: redactFindings(reportedFindings),
      ...(emittedAcCoverage ? { acCoverage: emittedAcCoverage } : {}),
    });
