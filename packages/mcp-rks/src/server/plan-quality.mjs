...
function collectDependencyAdditions(plan) {
  const added = new Set();
  let pkgJsonContent = '';
...
export async function reviewPlan({ projectRoot, plan, problemContent, llmCall, storyMeta, targetFiles, checkExecutableSteps = false }) {
  ...
  return {
    ...
    suggestions: errors.length > 0 ? [
      { type: 'decompose', detail: 'Break into smaller stories with single-file scope' },
      { type: 'use_search_replace', detail: 'Specify search_replace patterns in requirements' }
    ] : [],
