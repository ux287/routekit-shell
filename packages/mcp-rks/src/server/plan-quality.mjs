...
import { readPackageDependencies, readPackageDependenciesForFile } from './planner-prompts.mjs';
...
/**
 * True when `spec` could syntactically BE a module specifier.
 *
 * backlog.fix.import-specifier-extraction-overcaptures. The extractor scans raw
 * step content, including prose in `create_file` steps, so a match can capture a
 * quoted string that is not a specifier at all — a field report from a child
 * project had `import_not_declared` raised against the two-character package
 * `", "`, which blocked its story with `failureClass: output_invalid`.
 *
 * This NARROWS the check; it must never blind it. An over-strict predicate is the
 * worse failure: `reviewPlan` would report `ok` and a genuinely undeclared import
 * would surface at exec instead. So it rejects only what could not be a specifier,
 * and does not police package naming.
 */
function isValidModuleSpecifier(spec) {
  const s = String(spec || '');
  if (!s) return false;

  // 1. Whitespace or a comma. This is the clause that kills the `, ` capture.
  if (/[\s,]/.test(s)) return false;

  // 2. Relative, absolute and node:-prefixed forms pass through untouched —
  //    checkImportGrounding already decides these, and the guard must not take
  //    over that call site's skip decisions.
  if (s.startsWith('.') || s.startsWith('/') || s.startsWith('node:')) return true;

  // 3. The base segment must satisfy the npm name grammar. Dots (`chart.js`,
  //    `lodash.merge`) and uppercase (legacy names) are LEGAL and must not be
  //    rejected. Reuses basePackageName rather than re-implementing @scope/name.
  //
  //    A bare `@`-prefixed path alias (`@/components/Button`, base `@/components`)
  //    and a scope-only specifier (`@foo`) both fail here and are dropped. That is
  //    deliberate: neither is installable, so the finding's own prescribed remedy —
  //    declare it or emit a dependency-add step — cannot be performed, which makes
  //    it a false positive of the same family this guard closes. A protocol-prefixed
  //    specifier (`https://cdn.example/x.js`, base `https:`) is dropped for the same
  //    reason and with the same justification.
  const base = basePackageName(s);
  if (!/^(?:@[A-Za-z0-9\-~][A-Za-z0-9\-._~]*\/)?[A-Za-z0-9\-~][A-Za-z0-9\-._~]*$/.test(base)) return false;

  // 4. Any subpath is a `/`-delimited sequence of non-empty segments, and may
  //    carry a file extension: react-dom/client, some-pkg/dist/style.css.
  const subpath = s.slice(base.length);
  if (subpath && !/^(?:\/[^/]+)+$/.test(subpath)) return false;
...
function extractImportedSpecifiers(content) {
  ...
  const patterns = [
    /\bimport\s+(?:[\w*{}\s,]+\s+from\s+)?['"]([^'"]+)['"]/g, // import x from 'p' | import {a} from 'p' | import 'p'
  ...
  return [...specs].filter(isValidModuleSpecifier);
}
...
function checkImportGrounding(projectRoot, plan) {
  ...
  // The declared set depends on WHICH file the import appears in, so it cannot be
  // computed once for the whole plan.
  const declaredCache = new Map();
  const declaredFor = (target) => {
    const key = target || "";
    if (!declaredCache.has(key)) {
      const d = readPackageDependenciesForFile(projectRoot, target);
      declaredCache.set(key, d ? new Set(Object.keys(d)) : null);
    }
  ...
  for (const step of plan.steps || []) {
    const target = step.path || step.target;
    const declared = declaredFor(target);
    if (!declared) continue; // no manifest above this file → safe no-op
    const contents = [];
...
export async function reviewPlan({ projectRoot, plan, problemContent, llmCall, storyMeta, targetFiles, checkExecutableSteps = false }) {
  ...
  // Layer 0b (backlog.fix.plan-duplicate-create-file-steps): UNCONDITIONAL — outside the
  // declaredTargets block, so it also fires from the exec.mjs recheck, which passes no targetFiles.
  // Two create_file steps for one path means exec silently applies the later copy.
  {
    const createIdsByPath = new Map();
    for (const s of plan.steps || []) {
      if (!s || s.action !== 'create_file') continue;
      const raw = s.path || s.target;
      if (!raw) continue;
      const key = path.posix.normalize(String(raw).replace(/\\/g, '/'));
      if (!createIdsByPath.has(key)) createIdsByPath.set(key, []);
      createIdsByPath.get(key).push(s.id ?? null);
    }
    const duplicates = [...createIdsByPath].filter(([, ids]) => ids.length > 1).map(([p, ids]) => ({ path: p, stepIds: ids }));
    if (duplicates.length > 0) {
      issues.push({
        check: 'duplicate_create_file_path',
        severity: 'error',
        type: 'duplicate_create_file_path',
        message: `Plan has more than one create_file step for the same path: ${duplicates.map(d => `${d.path} (steps ${d.stepIds.join(', ')})`).join('; ')}. exec would apply the later copy over the earlier one — re-plan.`,
        duplicates,
        suggestion: 'replan',
      });
    }
 ...
 *
 * A plan could not be verified before it executed. `rks_exhaustive_search` answers "does the
 * tree contain this literal", but nothing reachable from chain state `planned` answered "what
 * does the plan SAY": rks_plan_review returned only { ok, errors, warnings, suggestions, hash }
 * — anchor text appeared solely as an 80-char patternPreview on a FAILING pattern, so a passing
 * review told you nothing about what would be written. rks_exec's dryRun was doubly unusable,
 * and dendron_read_note is not admitted in `planned`.
 *
 * These two shapers exist as exported functions rather than inline handler code on purpose:
 * server.mjs's only live harness is the subprocess MCP contract client and this project forbids
 * source-text assertions, so logic inlined at the handler would be untestable by any sanctioned
 * means.
 *
 * Field-layout note (decided at ARCH, from evidence already in the tree): `_governorState`
 * occurs at 8 sites in server.mjs and always means "about the governor session, not about the
 * artifact". So session metadata keeps the underscore prefix and plan identity/contents — which
 * describe the artifact — are plain. Identity is ONE object rather than three loose top-level
 * keys, which is exactly how two stories previously arrived at two names for one thing.
 */

/** Max steps disclosed before truncating. Bounds the response; the count is always exact. */
export const PLAN_DISCLOSURE_MAX_STEPS = 200;

/**
 * Identity of the plan a review describes.
 *
 * Emitted on EVERY branch that resolved a run dir, not gated on `ok` — a caller debugging a
 * FAILING review needs to know which plan failed at least as much as a passing one does.
 *
 * @param {object} args
 * @param {string|null} [args.planPath]
 * @param {string|null} [args.runDir]
 * @param {string|null} [args.slug]
 * @param {string|null} [args.problemId]
 * @param {string|null} [args.identifiedBy] - how the run dir was chosen ('label' | 'problemId' | 'latest')
 * @returns {{ planPath: string|null, runDir: string|null, slug: string|null, problemId: string|null, identifiedBy: string|null }}
 */
export function resolvedPlanIdentity({ planPath = null, runDir = null, slug = null, problemId = null, identifiedBy = null } = {}) {
  return { planPath, runDir, slug, problemId, identifiedBy };
}

/**
 * What the plan will actually do, per step.
 *
 * `search` is the load-bearing field: search_replace steps carry their payload on `s.edits`,
 * not on `s.content`, which is why every previous preview came back null for exactly the step
 * type a caller needs to verify. Both shapes are read here.
 *
 * @param {object} plan
 * @param {{ maxSteps?: number }} [opts]
 * @returns {{ stepCount: number, truncated: boolean, steps: Array<object> }}
 */
export function describePlanContents(plan, { maxSteps = PLAN_DISCLOSURE_MAX_STEPS } = {}) {
  const all = Array.isArray(plan?.steps) ? plan.steps : [];
  const shown = all.slice(0, maxSteps);
  const steps = shown.map((s, index) => {
    const target = s?.path || s?.target || null;
    // search_replace: payload lives on `edits`. Other actions may carry `content`.
    const edits = Array.isArray(s?.edits) ? s.edits : null;
    const search = edits
      ? edits.map((e) => (typeof e?.search === 'string' ? e.search : null)).filter((v) => v !== null)
    ...
    return {
      ...
      target,
      search,
      editCount: edits ? edits.length : search.length,
    };
  });
  ...
  return {
    ...
    truncated,
    // Enumerability requires the caller be told WHAT was withheld and why, not merely that
    // something was. A bare boolean leaves "did I see everything?" unanswerable without
    // knowing the cap, so the cap and the withheld count travel with the flag.
    maxSteps,
    omittedSteps: all.length - shown.length,
    truncationReason: truncated
      ? `Only the first ${maxSteps} of ${all.length} steps are listed; stepCount is exact.`
      : null,
    steps,
  };
}

/** Truncate for preview without assuming the input is already a string. */
...
/**
 * Shape one plan step for an exec dry-run preview.
 *
 * Extracted as a pure exported function so its test can assert BEHAVIOUR. The first version of
 * this coverage scanned exec.mjs source for the tokens `s.edits` and `s.content`, which pins
 * spelling rather than conduct and is the same brittleness this repo's ARCH prompt tells
 * stories to avoid — flagged in review, and correctly.
 *
 * Length is measured on the COERCED string: reading `.length` off the original value while
 * slicing the coercion appends the ellipsis against the wrong measurement for any non-string
 * content (a Buffer, a number).
 *
 * @param {object} step
 * @param {number} idx - zero-based index; the returned `step` field is 1-based
 * @returns {{ step: number, action: any, file: string|null, preview: string|null, edits: Array<{search: string|null, replace: string|null}>|null }}
 */
export function describeDryRunStep(step, idx) {
  const s = step || {};
  return {
    step: idx + 1,
    action: s.action,
    file: s.path || s.target || null,
    preview: previewText(s.content),
    // search_replace steps carry their payload here, NOT on `content` — which is why a
    // content-only preview returned null for exactly the step type a dry run exists to show.
    edits: Array.isArray(s.edits)
      ? s.edits.map((e) => ({ search: previewText(e?.search), replace: previewText(e?.replace) }))
      : null,
  };
}

/**
 * Whether a tool call should be treated as state-preserving.
 *
 * Exported as a pure predicate because the entry transition fires BEFORE the tool body
 * (advanceState is keyed on tool name and cannot see arguments), so a dry run moved the chain
 * to `executing` — strictly worse than `planned` — while doing no work at all.
 *
 * @param {string} toolName
 * @param {object} [args]
 * @returns {boolean}
 */
export function isStatePreservingCall(toolName, args = {}) {
  return toolName === 'rks_exec' && Boolean(args?.dryRun);
}
