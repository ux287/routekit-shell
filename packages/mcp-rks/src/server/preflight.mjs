...
import { loadSkillsManifest, findMissingSkills, partitionMissingSkills } from "../shared/skills-manifest.mjs";
...
/**
 * Keyless-RAG preflight messaging (pure, unit-drivable — no I/O).
 *
 * The RAG retrieval path is fully keyless (local ONNX embedder + LanceDB vector search); only
 * answer synthesis and governed workflows need an LLM credential. When the RAG index is
 * initialized but no key is present, preflight must NOT imply RAG is dead — retrieval still works.
 * Returns the message/hint to surface on the `api_key` check for that state; returns nulls
 * otherwise, so the caller falls back to the plain "set your key" hint.
 *
 * Extracted here (mirroring checkGitReadiness) because the inline rks_preflight handler in the
 * >1000-SLOC server.mjs is not unit-drivable (unit-tier-purity Rule B) and source-slice pinning is
 * banned; this is the behavioral seam the AC2 unit test drives.
 *
 * @param {{ ragInitialized: boolean, hasApiKey: boolean }} state
 * @returns {{ message: string|null, hint: string|null }}
 */
export function ragKeylessPreflightMessage({ ragInitialized, hasApiKey } = {}) {
  if (ragInitialized && !hasApiKey) {
    return {
      message: "RAG retrieval available (keyless)",
      hint:
        "RAG retrieval works without a key (local embedder + LanceDB). " +
        "Answer synthesis and governed workflows require ANTHROPIC_API_KEY.",
    };
  }
...
export function checkGitReadiness({ projectRoot, projectJson, timeoutMs = 15_000, diskVersionOverride } = {}) {
  ...
  if (!skillsManifest.ok) {
    ...
    // A skill excluded from THIS distribution is not a deficit. Only a genuinely absent skill reds.
    // See partitionMissingSkills: the discriminator is the companion path, so a real skill wipe —
    // which leaves the companion package standing — still classifies as genuinely missing.
    const { genuinelyMissing, notDistributed } = partitionMissingSkills(
      projectRoot,
      expected,
      skillsManifest.notPublished,
    );
    skillsPassed = genuinelyMissing.length === 0;
    // Count against what this distribution is actually expected to carry, so a clean public-mirror
    // clone reads as whole instead of "17/18". A deficit fraction is what sent a clean-machine UAT
    // hunting for a fault that did not exist.
    const distributedCount = expected.length - notDistributed.length;
    const presentCount = distributedCount - genuinelyMissing.length;
    const notDistributedNote = notDistributed.length
      ? ` (not in this distribution: ${notDistributed.join(", ")})`
      : "";
    skillsDetail = skillsPassed
      ? `${presentCount}/${distributedCount} present${notDistributedNote}`
      : `${presentCount}/${distributedCount} present — MISSING: ${genuinelyMissing.join(", ")}${notDistributedNote}`;
    if (!skillsPassed) {
      skillsHint =
        ...
        (notDistributed.length
          ? ` Separately, ${notDistributed.join(", ")} is not part of this distribution and cannot be restored — do not expect the command above to bring it back.`
          : "");
    } else if (notDistributed.length) {
      // Informational only. NO remediation, and deliberately NO `git checkout` instruction: the
      // path was never in this tree's HEAD, so that command silently does nothing and strands the
      // user with a red check they cannot clear. This is the first-run message a public-mirror
      // clone sees, and it must read as "expected", not as "broken".
      skillsHint =
        `${notDistributed.join(", ")} is not part of this distribution and is expected to be absent. ` +
        `Nothing is wrong and there is nothing to restore.`;
    }
  }
  checks.push({
    name: "core_skills",
