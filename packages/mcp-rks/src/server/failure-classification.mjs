...
function messageFor(failureClass, marker) {
  ...
  if (failureClass === "output_invalid") {
    return outputInvalidMessage(marker);
  }
  return "The plan worker crashed before producing a plan; see error. Re-run rks_plan.";
}

const WORKER_LOG_POINTER = "the detail was not carried on the result — see the worker log (workerLogPath)";

/**
 * output_invalid covers several planner verdicts; the message must describe the one that
 * actually happened. It used to be a fixed "no executable steps" string, which was false for
 * coverage_gap (the plan HAD steps; a target got none) and sent operators chasing the wrong cause.
 * A list the marker lacks is reported as unavailable, never invented.
 */
function outputInvalidMessage(marker = {}) {
  const list = (v) => (Array.isArray(v) ? v.filter(Boolean) : []);
  if (marker.reason === "note_only") {
  ...
  if (marker.reason === "coverage_gap") {
    const targets = list(marker.uncoveredTargets);
    const named = targets.length ? `: ${targets.join(", ")}` : ` (${WORKER_LOG_POINTER})`;
    return `The plan had executable steps, but ${targets.length || "some"} target file(s) got none${named}. Add an @@SEARCH/@@REPLACE anchor or an authorable block for each, then re-plan.`;
  }
  if (marker.reason === "dropped_work") {
    const steps = list(marker.droppedSteps).map((s) =>
      typeof s === "string" ? s : `${s.label || s.title || s.path || "<step>"}${s.reason ? ` (${s.reason})` : ""}`);
    const named = steps.length ? `: ${steps.join("; ")}` : ` (${WORKER_LOG_POINTER})`;
    return `Generated steps were dropped from the plan and would not run${named}. Refine the story so those steps apply cleanly, then re-plan.`;
  }
  const why = marker.reason ? ` (reason: ${marker.reason})` : "";
  return `The generated plan was rejected${why}. Refine the story (clearer targets or @@SEARCH/@@REPLACE blocks) and re-plan; see the worker log (workerLogPath) for details.`;
}

/**
 * The rks_plan_review response for a failed worker. Pure, so it is testable without the
 * handler. `classified` lets the caller pass the result of its own classifyMarkerFailure call.
 */
export function buildPlanReviewFailureResponse(marker = {}, elapsedSeconds, classified = classifyMarkerFailure(marker)) {
  const { failureClass, message } = classified;
  const response = {
    ok: false,
    status: marker.status || "failed",
    failureClass,
    elapsedSeconds,
    // Relayed or null — never a fabricated default that reads like a diagnosis.
    error: marker.error ?? null,
    message: `Plan worker failed (${failureClass}). ${message}`,
    workerLogPath: marker.workerLogPath ?? null,
  };
  for (const field of [
    "reason", "errors", "issues", "warnings", "hint", "workflow", "suggestions",
    "uncoveredTargets", "droppedSteps", "rejectionReasons", "noteSteps", "uncoveredCreateTargets", "refinable",
  ]) {
    if (marker[field] !== undefined && marker[field] !== null) response[field] = marker[field];
  }
  return response;
}
