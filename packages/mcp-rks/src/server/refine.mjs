...
import { splitNoteFrontmatter, joinNoteFrontmatter } from "../shared/frontmatter.mjs";
...
export async function runRefineTool({ projectRoot, problemId, trigger, context, testOutput, projectId }) {
  ...
  - "Edge case: [boundary condition]"`,
    ...
    // `analysis.createFiles` is a Set internally, and JSON.stringify(new Set([...])) is
    // `{}` — ALWAYS, with three create targets exactly as with none. Over the MCP boundary
    // that made an operator read "createFiles: {}" as evidence the extractor found nothing.
    // Serialise at the return site only; the four internal call sites keep the Set.
    const serialisableAnalysis = {
      ...analysis,
      createFiles: Array.from(analysis.createFiles || []),
    };
...
export async function runRefineApplyTool({ projectRoot, problemId, refinements, projectId = "unknown" }) {
  try {
    ...
    // Same regex and same slice as before — byte-identical output, now reached through the ONE
    // shared pair so the two write sites below cannot drift from it again. The read's SEMANTICS
    // are deliberately unchanged: the size-cap boundary fixtures pad against this exact shape.
    const split = splitNoteFrontmatter(storyContent);
    let frontmatter = split.frontmatter ?? "";
    let body = split.body;
    // Captured for evidence-bound reporting below — the "unchanged" claim is derived from
    // comparing this against the body actually written, never asserted.
    const bodyAtRead = body;
        ...
        if (snippetFile) {
          ...
          if (snippet) {
            ...
            if (projectedBodyLength > MAX_NOTE_BODY_BYTES) {
              ...
              // OBSERVED, not assumed. On a second identical round the durable marker is already
              // present, so the body is NOT changed — and this used to report "delivered ... out of
              // band" anyway, claiming an action it did not perform. The note growing by a byte per
              // apply hid it: tests/unit/refine-note-size-cap-search-gate.test.mjs AC-6 compares
              // round 1 against round 2 modulo the `updated:` stamp, and the growth byte made them
              // differ regardless, so its stall check never fired. Byte-stability removed that
              // second vacuity mechanism and exposed this. The context is still delivered out of
              // band either way; only the CLAIM is now sourced from whether the note changed.
              const markerAlreadyPresent = body.includes(marker);
              if (!markerAlreadyPresent) body = body.trim() + "\n\n" + marker + "\n";
              const oobMsg = markerAlreadyPresent
                ? "skipped — code context for " + snippetFile +
                  " was already delivered out of band; the note already carries the durable marker " +
                  "and this apply changed nothing"
  ...
  it('TODO: replace with test description', () => {
    ...
    expect(result).toBeDefined(); // TODO: replace with real assertion
      ...
      if (type === "decompose") {
        ...
        const decomposedContent = joinNoteFrontmatter(frontmatter.trim(), body);
    ...
    for (const refinement of refinements) {
      ...
      try {
        ...
        // The note-change clause is appended AFTER the write, from an observation. Do not
        // reinstate a literal here.
        applied.push({ type, result: `disk-fetched ${diskFile} (${content.length} bytes)`, diskFetch: true });
      } catch (e) {
    ...
    const updatedContent = joinNoteFrontmatter(frontmatter.trim(), body);
    // EVIDENCE-BOUND REPORTING. The disk-fetch result used to carry a hardcoded
    // "story note unchanged" clause — a claim about the note, sourced from nothing that looked at
    // the note. It was false on every apply, because a disk-fetch forces isNoop false and the
    // write below always runs. Derived here from an actual comparison instead.
    const noteBodyChanged = body !== bodyAtRead;
    for (const entry of applied) {
      if (entry && entry.diskFetch) {
        entry.noteBodyChanged = noteBodyChanged;
        entry.result += noteBodyChanged
          ? " — story note body CHANGED by this apply"
          : " — story note body unchanged";
      }
    }
