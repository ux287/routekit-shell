  ...
  - "Edge case: [boundary condition]"`,
    ...
    if (analysis.hasTargetFiles && analysis.createFiles.size > 0 && !parsedData.testStory) {
      ...
      for (const createFile of analysis.createFiles) {
        ...
        if (!isTest) continue;
        if (body.includes("### Test Exemplar:")) continue; // Already has one
    ...
    // Suggest decomposition for complex stories
    if (analysis.estimatedComplexity === "high" && !analysis.hasChildStories) {
      const decomposeReason = analysis.decomposeReasons?.length > 0
      ...
      suggestions.push({
        ...
        hint: "Break into smaller child stories with focused scope. Pass data.children[i] = {childSlug: kebab-label} for semantic IDs; auto-derived from AC text when omitted.",
      });
    }
  ...
  try {
    ...
    for (const refinement of refinements) {
      ...
      if (type === "add_test_exemplar") {
        ...
        if (body.includes("### Test Exemplar:")) {
          applied.push({ type, result: "skipped (already present)" });
          continue;
        }
