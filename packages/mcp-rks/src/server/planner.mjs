...
import { reviewPlan, computePlanHash, checkOpMatch } from "./plan-quality.mjs";
...
`;
  ...
  // create_file step (non-empty content) for an op:create target, it passes through unchanged
  // below (authorable creates still work). When it produced NO usable content for an uncovered
  // op:create target, route IMMEDIATELY to the loud structural failure — no loop, no stub.
  const llmCreatePaths = new Set(
    (llmActions || [])
      .filter(a => a?.action === "create_file" && a?.content && String(a.content).trim())
  ...
  let combinedSteps = rawCombined
    .map((s) => {
      if (!s) return null;
      // Match TODO/FIXME only when preceded by comment markers (// or /* or #) to avoid false positives on variable names like "todo"
      if (s?.content && /(\/\/|\/\*|#)\s*(TODO|FIXME)\b|placeholder text|replace this|integration logic/i.test(s.content)) {
        rejectionReasons.push({ path: s.path || "(none)", reason: "placeholder content" });
        return { ...s, action: "note" };
      }
      if (s?.content && typeof s.content === "string" && /^\s*\/\//.test(s.content.trim())) {
        // Skip "comment-only" check for file types that use // as valid comment syntax
        const ext = (s.path || "").split(".").pop()?.toLowerCase();
        const allowsSlashComments = ["js", "mjs", "cjs", "ts", "tsx", "jsx", "scss", "sass", "css", "less", "go", "java", "c", "cpp", "h", "hpp", "rs", "swift", "kt", "cs"].includes(ext);
        if (!allowsSlashComments) {
          rejectionReasons.push({ path: s.path || "(none)", reason: "comment-only content" });
          return { ...s, action: "note" };
        }
      }
