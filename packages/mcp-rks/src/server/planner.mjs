...
import { createTargetPathsMatch } from "../shared/create-target-path-match.mjs";
import { classifyMarkerFailure } from "./failure-classification.mjs";
import { getLastEmbedTime, ensureRagIndex, runRagEmbed } from "@routekit/rag";
...
import { validateSearchReplacePatterns, autoCorrectSearchPatterns, isIdentityHunk } from "../validation/search-replace.mjs";
...
export function buildStructuralFailure(uncoveredCreateTargets) {
  ...
  return {
    ...
    refinable: false,
    // ADDITIVE reword only. tests/unit/planner-note-step-degeneracy.test.mjs pins
    // /no usable content/i, /hand-author/i, /escalate to a stronger model/i and
    // /will not help/i, and pins that this message must NOT say "run rks_refine".
    // The added sentence names the concrete fix, which plan-ready now catches up front.
    message: `planner could not author create-only target(s) ${named} — the model returned no usable content. Add a fenced code block containing real file content immediately after the "// CREATE FILE:" directive for each target, then re-plan. Hand-author the file(s) or escalate to a stronger model; decompose/analyze will not help.`,
  };
}
...
function validateStep(step, allowedTargets = null, projectRoot = repoRoot) {
  ...
  if (step.action === "edit_file" || step.action === "create_file" || step.action === "delete_file") {
    ...
    if (allowedTargets && !allowedMatch) {
      // backlog.fix.planner-note-step-false-rejection: REJECT the step — do not disguise it as a
      // note. This used to return `{action:"note", title: step.title, ...}` with NO `path` key, so
      // a step that arrived without a title (the old About-page injection did) became a note with
      // neither path nor title. hasNoteSteps then killed the whole plan, and every diagnostic —
      // which renders `path || title` — named it as the empty string. A step outside the story's
      // targets is INVALID, not documentation: drop it and record WHY, with its identity intact so
      // it can always be named.
      return {
        _invalid: true,
...
/**
 * backlog.fix.planner-note-step-false-rejection: the single place that turns raw candidate steps
 * into a classified plan. Extracted from an inline map inside runPlanTool so it can be exercised
 * directly — the previous inline version was only reachable through a live LLM call, which is why
 * no test ever caught that it manufactured a nameless note step and threw away a valid plan.
 *
 * Returns BOTH halves deliberately:
 *   - `steps`      — everything that survived, notes included (diagnostics read this)
 *   - `noteSteps`  — the notes on their own, each ALWAYS nameable (see describeStep)
 *   - `executable` — notes stripped. THIS is what may be persisted and handed to exec.
 *
 * The split matters: exec (exec.mjs, the "safety net") throws a hard, unrefinable McpError on ANY
 * note step it finds in plan.json. So a note must never ride into the persisted plan — but it must
 * still be visible to the retry logic and the failure payload, or we are back to blaming the model
 * for steps it never wrote.
 */
export function classifySteps({ rawSteps, allowedTargets = null, projectRoot = repoRoot }) {
  const rejectionReasons = [];
  // backlog.fix.planner-dropped-step-diagnostics: the ONLY place the raw step and the reason it was
  // dropped are both in hand. Keyed on the RAW STEP OBJECT REFERENCE, so a later consumer can name a
  // drop without joining on `label`/`path` — two different steps can share a path (a kept one and a
  // dropped one), and a label-keyed join silently attributes the wrong reason to the wrong step.
  // Additive: it does not change the meaning of any existing field. See computeDroppedSteps.
  const rejectionByStep = new Map();
  const reject = (rawStep, descriptor) => {
    rejectionReasons.push(descriptor);
    rejectionByStep.set(rawStep, descriptor);
  };
  ...
  const steps = (rawSteps || [])
    .map((s) => {
      ...
      // real body?" — the same one the create-coverage gate uses, so the two can never disagree
      // again (that divergence is what dumped greenfield plans into an unwinnable loop). It is a
      // WHOLE-BODY test: a real 200-line component containing one `// TODO:` stays executable;
      // only a body that is ONLY placeholder/comment is downgraded.
      if (s.content !== undefined && s.content !== null && !isSynthesizedBody(s.content, s.path)) {
        reject(s, describeStep(s, "placeholder content"));
        return { ...s, action: "note" };
      }

      const validated = validateStep(s, allowedTargets, projectRoot);
      if (!validated) {
        reject(s, describeStep(s, "failed validation"));
        return null;
      }
      if (validated._invalid) {
        reject(s, describeStep({ ...s, ...validated }, validated._invalidReason || "validation failed"));
        return null;
      }
      if (validated._invalidReason) {
        // NOT a drop: the step survives and stays executable. Advisory only — deliberately NOT
        // recorded in rejectionByStep, or it would be named as dropped work it never was.
        rejectionReasons.push(describeStep({ ...s, ...validated }, validated._invalidReason));
      }
      return validated;
    })
    .filter(Boolean);

  const noteSteps = steps.filter((s) => s?.action === "note");
  const executable = steps.filter((s) => s?.action && s.action !== "note");
  ...
  return {
    ...
    rejectionReasons,
    rejectionByStep,
    hasExecutableSteps: executable.length > 0,
  };
}

/**
 * backlog.fix.planner-dropped-step-diagnostics: the dropped-step set — the work the LLM MEANT to be
 * executable that never made it into the plan.
 *
 *   { rawSteps the LLM authored as executable }  MINUS  { classified.executable }
 *
 * Two properties are load-bearing, and both were learned the hard way:
 *
 * 1. It is an IDENTITY-keyed set difference (object reference), never label- or path-keyed.
 *    `validateStep` returns the SAME object reference for every survivor, so identity is exact.
 *    A label-keyed difference computes {P} \ {P} = ∅ when a KEPT step and a DROPPED step share path
 *    P — it loses the drop entirely, which is worse than naming it wrong.
 *
 * 2. It NEVER gates on `rejectionReasons`. That array is a SUPERSET of the drops: runPlanTool pushes
 *    advisory "missing paired test/doc" descriptors into it for steps that were KEPT and are still
 *    executable — and it pushes them into the SAME array object classifySteps returned, mutating it
 *    in place. A guard keyed on it would fire on almost every ordinary plan, and a diagnostic sourced
 *    from it would scold the LLM for steps it never got wrong. Reasons are JOINED (by identity, via
 *    rejectionByStep) onto members already established as dropped — never the other way round.
 *
 * Pure, order-independent, and total over its inputs, so it can be witnessed directly.
 */
export function computeDroppedSteps({ rawSteps = [], classified = {} }) {
  const survived = new Set(classified.executable || []);
  const reasons = classified.rejectionByStep instanceof Map ? classified.rejectionByStep : new Map();
  return (rawSteps || [])
    .filter((s) => s && s.action && s.action !== "note")
    .filter((s) => !survived.has(s))
    .map((s) => reasons.get(s) || describeStep(s, "dropped from the executable plan"));
}

// backlog.fix.plan-duplicate-create-file-steps: story-block create steps (automatedSteps) and
// reviewer create_file_block steps (llmActions) can name the same path. exec applies every
// create_file in order, so the later, trimEnd()'d copy would silently win. One create per path.
function normalizeCreatePath(s) {
  const p = s?.path || s?.target;
  return p ? path.posix.normalize(String(p).replace(/\\/g, "/")) : null;
}

/** Pure: one { path, keptId, discardedIds } per normalized path carried by 2+ create_file steps. */
export function findDuplicateCreateFileSteps(steps) {
  const byPath = new Map();
  for (const s of steps || []) {
    if (!s || s.action !== "create_file") continue;
    const key = normalizeCreatePath(s);
    if (!key) continue;
    if (!byPath.has(key)) byPath.set(key, []);
    byPath.get(key).push(s.id ?? null);
  }
  const out = [];
  for (const [p, ids] of byPath) {
    if (ids.length > 1) out.push({ path: p, keptId: ids[0], discardedIds: ids.slice(1) });
  }
  return out;
}

/** Pure (apart from the log line): keep the FIRST create_file per normalized path; other steps untouched, in order. */
export function dedupeCreateFileSteps(steps) {
  const kept = new Map();
  const out = [];
  for (const s of steps || []) {
    const key = s && s.action === "create_file" ? normalizeCreatePath(s) : null;
    if (key && kept.has(key)) {
      console.error(`[rks.plan] discarded duplicate create_file step ${s.id} for ${key} (kept ${kept.get(key).id})`);
      continue;
    }
    if (key) kept.set(key, s);
    out.push(s);
  }
  return out;
}
...
/**
 * backlog.fix.planner-dropped-step-diagnostics: the plan guard, as ONE pure exported decision.
 *
 * It takes RAW inputs and computes `taskMode` and `hasTargetCoverageGap` ITSELF. That is deliberate
 * and it is the whole reason the mode gate is testable: if this function were handed a pre-gated
 * `droppedWork` boolean, a caller could OR the dropped-step arm in unconditionally and nothing here
 * — and therefore no test — would be able to see it.
 *
 * THE MODE GATE: the dropped-step arm is TASK-MODE-ONLY.
 *   - Story mode (the story declares editable targets): COVERAGE governs, and nothing else. A step
 *     outside the story's targets is hard-rejected, lands in the dropped set, and MUST NOT fire the
 *     guard — otherwise one stray out-of-target step discards an otherwise-perfect, fully-covered
 *     plan. That is exactly the v0.27.0 P0 (the injected About.tsx step), and this gate is the only
 *     thing keeping it dead.
 *   - Task mode (no targets, so coverage is not computable): DROPPED WORK governs. Without this arm
 *     a task-mode plan silently ships minus every step that was dropped, and says nothing about it.
 *
 * LLM-authored `note` steps are never dropped work — commentary is not lost work, and must not cost
 * the user a build.
 */
export function evaluatePlanGuard({ combinedSteps = [], enhancedEditableTargets = null, droppedSteps = [] }) {
  const steps = combinedSteps || [];
  const targets = enhancedEditableTargets || [];
  const dropped = droppedSteps || [];

  const hasExecutableSteps = steps.some((s) => s?.action && s.action !== "note");
  const executablePaths = steps
    .filter((s) => s?.action && s.action !== "note" && s.path)
    .map((s) => s.path);
  const uncoveredTargets = targets.filter(
    (t) => !executablePaths.some((p) => p === t.path || p?.endsWith(t.path) || t.path?.endsWith(p)),
  );
  const hasTargetCoverageGap = uncoveredTargets.length > 0;

  const taskMode = targets.length === 0;
  const hasDroppedWork = taskMode && dropped.length > 0;

  const fire = !hasExecutableSteps || hasTargetCoverageGap || hasDroppedWork;
  const reason = !hasExecutableSteps
    ? "note_only"
  ...
  return {
    ...
    taskMode,
    uncoveredTargets,
    droppedLabels: dropped.map((d) => d.label),
  };
}

/**
 * backlog.fix.planner-dropped-step-diagnostics: the retry prompt's guidance, extracted so it can be
 * witnessed. The original bug was invisible precisely because this text was built inline inside a
 * function no test could drive: it rendered "The following steps were converted to non-executable
 * notes: ." — naming nothing — and no test could see it.
 *
 * Every dropped step is named with action + path/label + title + reason. Never an empty list.
 */
export function buildRetryGuidance({ noteLabels = [], droppedSteps = [], uncoveredTargets = [] }) {
  const noteGuidance = noteLabels.length
    ? `\nThe following steps were converted to non-executable notes: ${noteLabels.join(", ")}. Rewrite them as executable create_file or search_replace actions with actual code content.`
    : "";
  const droppedGuidance = droppedSteps.length
    ? `\nThe following generated steps were DROPPED from the plan and will NOT run: ${droppedSteps
      .map((d) => {
        const named = d.title && d.title !== d.label ? `${d.label} ("${d.title}")` : d.label;
        return `${d.action || "unknown"} ${named} — ${d.reason}`;
      })
      .join("; ")}. Re-author them as valid executable steps, or leave them out deliberately.`
    : "";
  const coverageGuidance = uncoveredTargets.length
    ? `\nEnsure ALL target files have executable steps: ${uncoveredTargets.map((t) => t.path).join(", ")}`
    : "";
  return { noteGuidance, droppedGuidance, coverageGuidance, text: `${noteGuidance}${droppedGuidance}${coverageGuidance}` };
}

/**
 * backlog.fix.planner-dropped-step-diagnostics: the "No executable actions generated" description,
 * extracted so it can be witnessed.
 *
 * Renders each rejection by its `label`, NOT its `path`. `path` is `step?.path ?? null` — a step that
 * arrived without one used to render as the literal string "null: <reason>". `label` exists precisely
 * so that no step is ever unnameable (it falls back path → title → <action step>).
 */
export function buildNoExecutableDescription({ llmActions = undefined, llmDebug = null, rejectionReasons = [] }) {
  if (llmActions === null) {
    return `LLM call failed: ${llmDebug?.error || "no response received"}. ${
  ...
  if (rejectionReasons.length) {
    return `All actions failed validation. Rejections: ${rejectionReasons
      .map((r) => `${r.label}: ${r.reason}`)
      .join("; ")}${llmDebug?.rawPreview ? ` | LLM raw preview: ${llmDebug.rawPreview}` : ""}`;
  }
  return `All actions failed validation (paths/content). Review llm-output.json and snippets.${llmDebug?.rawPreview ? ` Raw: ${llmDebug.rawPreview}` : ""}`;
}

/**
 * backlog.fix.planner-dropped-step-diagnostics: the exhausted-retry payload, extracted so it can be
 * witnessed BEHAVIORALLY.
 *
 * It used to be built inline, and the only "test" of it was a readFileSync grep of this file's own
 * source asserting the literal text `noteSteps: combinedSteps.filter`. That grep asserted nothing
 * about behavior, broke on any refactor, and pinned a contorted expression in place — production
 * code was written the long way ROUND to keep the grep green. The field is now `noteSteps: noteLabels`
 * and this function is what the test drives.
 */
export function buildExhaustedPayload({
  combinedSteps = [],
  verdict,
  droppedSteps = [],
  rejectionReasons = [],
  ...
  const noteSteps = (combinedSteps || []).filter((s) => s?.action === "note");
  const noteLabels = noteSteps.map((s) => s.path || s.title || `<${s.action || "unknown"} step>`);

  // PART B (backlog.fix.planner-note-step-degeneracy): an uncovered op:create target means the plan
  // is STRUCTURALLY unauthorable — refine/decompose/analyze cannot help. Attach a loud,
  // machine-readable discriminator ADDITIVELY on top of the still-valid pinned fields.
  const createStepPaths = new Set(
    (combinedSteps || []).filter((s) => s?.action === "create_file").map((s) => s.path).filter(Boolean),
  );
  const uncoveredCreates = Array.from(frontmatterCreateFiles || [])
    .filter((p) => ![...createStepPaths].some((s) => createTargetPathsMatch(s, p)));
  const structural = uncoveredCreates.length > 0 ? buildStructuralFailure(uncoveredCreates) : null;
  ...
  return {
    payload: {
      ...
      // Every entry is nameable by construction — the `<action step>` fallback is what makes the old
      // empty-string render impossible.
      noteSteps: noteLabels,
      // The steps the LLM authored as executable that never reached the plan, each carrying
      // action + path + title + reason. When something was dropped, this is NEVER empty.
      droppedSteps,
      uncoveredTargets: (verdict?.uncoveredTargets || []).map((t) => t.path),
      rejectionReasons,
      ...(structural
        ? {
...
/**
 * A step descriptor that can ALWAYS be named. The bug this story fixes surfaced as a diagnostic
 * that rendered `path || title` over a step carrying neither, producing the empty string — so the
 * planner told the LLM "these steps became notes:" and then listed nothing, three times, while the
 * LLM's plan was perfect. Never emit an unnameable step again: fall back through path → title →
 * action, and always carry the reason.
 */
function describeStep(step, reason) {
  const label = step?.path || step?.title || (step?.action ? `<${step.action} step>` : "<unknown step>");
  return {
    label,
    path: step?.path ?? null,
...
async function runPlanTool({ projectId, task, problemId, label, autoEmbed = true, _noteOnlyRetryCount = 0, _enhancedContext = null }) {
  ...
  // backlog.fix.planner-note-step-false-rejection: the legacy "About page automation" that used to
  // live here is DELETED. It fired on /about page/i against the story PROSE and pushed a hardcoded
  // web-agency About.tsx ("About Our Studio", HeroSection/CTASection) into automatedSteps — a file
  // the story never asked for, with NO title. automatedSteps are merged ahead of llmActions below,
  // so it rode along on plans the LLM had authored perfectly. Its path was not in the story's
  // targets, so the allowed-targets gate downgraded it to a note carrying neither path nor title;
  // hasNoteSteps then discarded the entire plan, and the retry prompt — which renders
  // `path || title` — told the LLM it had produced notes and named NONE of them. That is how a
  // valid 7-step calculator plan died three times in a row, and why the failure looked like model
  // non-determinism: the trigger was whether the story's prose happened to contain "about page".
  ...
  const uncoveredCreatePaths = Array.from(frontmatterCreateFiles || [])
    .filter(p => ![...automatedStepPaths].some(s => createTargetPathsMatch(s, p)));
  ...
  if (editableTargetPaths && editableTargetPaths.length > 0) {
    // backlog.fix.identity-hunk-passes-as-change-spec: this loop was lifted VERBATIM
    // into the exported computeEarlyExitCoverage (beside shouldEarlyExitToSteps) so the
    // unit suite can EXECUTE it instead of re-implementing it. The rationale comment
    // above stays here, at the gate it explains.
    const { allCovered, steps: earlyExitSteps } = computeEarlyExitCoverage(problemNote?.content || planningText, editableTargetPaths);
  ...
  if (llmOrchResult.refinementRequired) {
    ...
    // Classified OUTSIDE the best-effort try: a throwing collector must not cost the
    // returned result its class. One derivation feeds both the emit and the return.
    const refineClass = classifyMarkerFailure({ status: "refinement_required", reason: failReason }).failureClass;
    try { getTelemetryCollector().emit("plan.failed", projectId, { problemId: normalizedProblem, slug: finalSlug, reason: failReason, failureClass: refineClass, durationMs: Date.now() - planStartMs }); } catch (e) { /* telemetry is best-effort */ }
  ...
  const uncoveredCreatesAfterLlm = Array.from(frontmatterCreateFiles || [])
    .filter(p => ![...automatedStepPaths].some(s => createTargetPathsMatch(s, p))
      && ![...llmCreatePaths].some(s => createTargetPathsMatch(s, p)));
  ...
  // One create_file per path (backlog.fix.plan-duplicate-create-file-steps). The DEDUPED list
  // feeds both classifySteps and computeDroppedSteps: a discarded duplicate is not dropped work.
  const collapsedDuplicateCreates = findDuplicateCreateFileSteps(rawCombined);
  const dedupedCombined = dedupeCreateFileSteps(rawCombined);
  const classified = classifySteps({
    rawSteps: dedupedCombined,
    allowedTargets: editableTargetPaths && editableTargetPaths.length ? resolvedTargets : null,
    projectRoot,
  });
  const rejectionReasons = classified.rejectionReasons;
  let combinedSteps = classified.steps;
  // backlog.fix.planner-dropped-step-diagnostics: the work the LLM authored as executable that never
  // survived classification. Computed HERE, from `classified`, by identity — NOT from
  // `rejectionReasons`, which the advisory loop below mutates IN PLACE (it is the same array object)
  // and which carries entries for steps that were KEPT.
  let droppedSteps = computeDroppedSteps({ rawSteps: dedupedCombined, classified });
  ...
  if (isHelloApply && llmStatus !== "error") {
    ...
    // The plan was replaced wholesale, so nothing the LLM authored was "dropped" — clearing this
    // keeps the task-mode guard from firing on steps that are no longer part of the plan at all.
    droppedSteps = [];
  }
  ...
  combinedSteps.forEach((s) => {
    if (isCodeEdit(s) && (!hasTest || !hasDoc)) {
      // Same nameable shape as classifySteps' describeStep — every rejection must identify itself.
      rejectionReasons.push({
        label: s.path || s.title || `<${s.action || "unknown"} step>`,
        path: s.path ?? null,
        title: s.title ?? null,
        action: s.action ?? null,
        reason: "missing paired test/doc",
      });
    }
  });

  const hasActionable = combinedSteps.some((s) => s.action && s.action !== "note");
  // backlog.fix.planner-note-step-false-rejection (ARCH D1): STRIP note steps from the plan that
  // gets persisted. `finalSteps` is what lands in plan.json (planner-persistence) and exec re-reads
  // it FROM DISK — and exec.mjs throws a hard, unrefinable McpError on any note step it finds. So a
  // note must never ride into the persisted plan.
  //
  // Note the asymmetry, and keep it: `combinedSteps` (notes included) still feeds the diagnostics
  // and the retry logic below, so we can still SAY what was rejected and why. Stripping the wrong
  // array — combinedSteps — would silence the diagnostics AND still ship the note to exec.
      ...
      : [
        {
          ...
          description: buildNoExecutableDescription({ llmActions, llmDebug, rejectionReasons }),
          order: 1,
        },
      ];
  ...
  const noteSteps = combinedSteps.filter(s => s?.action === "note");
  const hasNoteSteps = noteSteps.length > 0;
  // Every note is nameable now — `describeStep` falls back path → title → <action step>, so the
  // diagnostics below can never render the empty string that made this bug undebuggable.
  const noteLabels = noteSteps.map(s => s.path || s.title || `<${s.action || "unknown"} step>`);

  // backlog.fix.planner-dropped-step-diagnostics: ONE decision, computed by ONE pure exported
  // function. Both guard sites below are pure delegations to `verdict.fire` — neither may add a
  // disjunct of its own. The mode gate (dropped work counts in task mode only) lives INSIDE
  // evaluatePlanGuard, which is what makes it testable.
  const verdict = evaluatePlanGuard({ combinedSteps, enhancedEditableTargets, droppedSteps });
  const { hasTargetCoverageGap, uncoveredTargets } = verdict;
  ...
  if (hasNoteSteps) {
    console.error(`[rks.plan] note step(s) stripped from the executable plan: ${noteLabels.join(', ')}`);
  }
  if (droppedSteps.length) {
    console.error(`[rks.plan] step(s) dropped from the executable plan: ${droppedSteps.map(d => `${d.label} (${d.reason})`).join(', ')}`);
  }

  // backlog.fix.planner-note-step-false-rejection: a note step is NO LONGER fatal on its own.
  //
  // It used to be: `hasNoteSteps` alone discarded the entire plan, so ONE junk step could throw
  // away six perfect ones. That is what killed the calculator plan — a foreign About.tsx step the
  // planner itself injected turned into a note, and took the LLM's flawless work down with it.
  //
  // Notes are now STRIPPED from the persisted plan (see finalSteps), so exec never sees one.
  //
  // backlog.fix.planner-dropped-step-diagnostics: coverage alone was not enough either. It is
  // computable ONLY in story mode — a task-mode plan has no targets, so the coverage gate is
  // permanently false there and a task-mode plan shipped silently minus every step that was
  // dropped. `evaluatePlanGuard` asks both questions, each in the mode where it means something.
  if (verdict.fire && retryCount < MAX_NOTE_ONLY_RETRIES && !skipLlm) {
    ...
    try {
      ...
      collector.emit("plan.retry", projectId, {
        ...
        noteSteps: hasNoteSteps ? noteLabels : undefined,
        droppedSteps: droppedSteps.length ? droppedSteps : undefined,
    ...
    //
    // backlog.fix.planner-note-step-false-rejection: this prompt used to render
    // `combinedSteps.filter(note).map(s => s.path || s.title)` — and when the offending note had
    // NEITHER (the injected About step), it produced:
    //     "The following steps were converted to non-executable notes: ."
    // The planner scolded the LLM for steps it never wrote and named none of them, so no retry
    // could ever converge. Only mention notes when we can actually NAME them, and never emit an
    // empty list.
    //
    // backlog.fix.planner-dropped-step-diagnostics: and SAY WHAT WAS DROPPED. A step hard-rejected
    // as _invalid never becomes a note, so it used to vanish from this prompt entirely — the retry
    // asked the LLM to fix something it was never told about.
    const { text: retryGuidance } = buildRetryGuidance({ noteLabels, droppedSteps, uncoveredTargets });
...
Do NOT generate note steps. Produce actual code changes with search and replace patterns.${retryGuidance}
...
`;
  ...
  // After max retries, return refinement_required rather than shipping a plan that cannot succeed.
  //
  // backlog.fix.planner-note-step-false-rejection: the condition was `!hasExecutableSteps ||
  // hasNoteSteps` — it failed on the mere PRESENCE of a note while never checking whether the
  // story's targets were actually covered. Both halves were wrong. A note is no longer fatal (it is
  // stripped from the persisted plan), and coverage is the thing that actually matters: if a target
  // has no executable step, the file never gets written and the AC silently fails. So the guards
  // now agree — this mirrors the retry condition above.
  if (verdict.fire && retryCount >= MAX_NOTE_ONLY_RETRIES && !skipLlm) {
    const exhaustedReason = verdict.reason;
    console.error(`[rks.plan] LLM refused to generate clean plan (${exhaustedReason}) after ${MAX_NOTE_ONLY_RETRIES} retries`);

    // PART B (backlog.fix.planner-note-step-degeneracy): when an op:create target remains uncovered,
    // this is a STRUCTURAL failure — refine/decompose/analyze cannot help (proven deterministic).
    // buildExhaustedPayload attaches the LOUD, machine-readable discriminator
    // (failureClass:"structural") naming the uncovered create target(s), ADDITIVELY on top of the
    // still-valid fields (status/reason/noteSteps/rejectionReasons), so the outer refine→plan loop
    // can short-circuit.
    const { payload, structural } = buildExhaustedPayload({
      combinedSteps,
      verdict,
      droppedSteps,
      rejectionReasons,
    ...
    // SSOT: one class, computed once, carried on BOTH the marker and the emitted payload.
    // Previously `failureClass` was spread only when `structural` was set, so a NON-structural
    // retry-exhausted run emitted `payload.failureClass === undefined` and the Governor view
    // re-derived a class independently — two answers to one question. classifyMarkerFailure
    // returns the stamped class verbatim when present (structural) and otherwise derives it
    // from status/reason, so both paths agree by construction rather than by coincidence.
    const exhaustedClass = classifyMarkerFailure(payload).failureClass;
    ...
    try {
      ...
      collector.emit("plan.retry.exhausted", projectId, {
        attempts: retryCount,
        slug: finalSlug,
        failureClass: exhaustedClass,
        ...(structural ? { uncoveredCreateTargets: structural.uncoveredCreateTargets } : {}),
      });
    } catch (e) {
      console.error(`[rks.plan] telemetry emit failed: ${e?.message}`);
    }
    ...
    payload.failureClass = exhaustedClass;
    return payload;
  }
  ...
  const stillUncoveredCreates = Array.from(frontmatterCreateFiles || [])
    .filter(p => ![...finalCreateStepPaths].some(s => createTargetPathsMatch(s, p)));
  ...
  const plan = {
    ...
    steps: finalSteps,
    ...(collapsedDuplicateCreates.length ? { collapsedDuplicateCreates } : {}),
  ...
  // Stamped only for a status the classifier actually names. persistAndFinalize also fails
  // with phase_write_failed or phase_immutable_plan_rejected; the classifier's fallback arm
  // would call those a crash nobody observed, so they carry no class at all. Computed
  // OUTSIDE the best-effort try so a collector throw cannot skip the stamp.
  let persistClass = null;
  if (planResult.status === "quality_failed") {
    persistClass = classifyMarkerFailure(planResult).failureClass;
    planResult.failureClass = persistClass;
  }
  try {
    const collector = getTelemetryCollector();
    if (planResult.ok) {
      ...
      collector.emit("plan.failed", projectId, { problemId: normalizedProblem, slug: finalSlug, reason: planResult.status || "quality_failed", durationMs: Date.now() - planStartMs, ...(persistClass ? { failureClass: persistClass } : {}) });
    }
  } catch (e) { /* telemetry is best-effort */ }
  if (collapsedDuplicateCreates.length) planResult.collapsedDuplicateCreates = collapsedDuplicateCreates;
  return planResult;
}
 ...
 */
/**
 * Coverage pass for the deterministic LLM-bypassing early-exit.
 *
 * Lifted verbatim out of runPlanTool so tests can EXECUTE it rather than re-implement
 * it: the previous local mirror in tests/unit/planner-early-exit.test.mjs had already
 * drifted from this loop silently, and could not observe changes to it at all.
 *
 * Identity hunks are dropped BEFORE coverage is judged. An identity hunk is an ANCHOR,
 * not a change (see validation/search-replace.mjs isIdentityHunk). Counting one as
 * coverage made the early-exit bypass the LLM and emit a no-op plan classified
 * 'executable' — the exact opposite of the anchor's stated purpose ("anchor the
 * PLANNER to a location"). Dropping them routes an anchors-only target through the
 * zero-blocks branch below, which already falls through to the LLM.
 *
 * Semantics preserved exactly, including accumulate-then-break: on the uncovered path
 * the steps accumulated so far are RETURNED, not discarded. shouldEarlyExitToSteps is
 * what actually gates the bypass.
 *
 * @param {string} planningText
 * @param {string[]} editableTargetPaths
 * @returns {{ allCovered: boolean, steps: object[] }}
 */
export function computeEarlyExitCoverage(planningText, editableTargetPaths) {
  const steps = [];
  let allCovered = true;
  for (const targetPath of editableTargetPaths) {
    const blocks = extractSearchReplaceBlocks(planningText || '', targetPath)
      .filter((b) => !isIdentityHunk(b.search, b.replace));
    if (blocks.length === 0) {
      allCovered = false;
      console.error(`[planner] early-exit: no non-identity @@SEARCH blocks for ${targetPath} — falling through to LLM`);
      break;
    }
    steps.push({
