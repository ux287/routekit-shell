...
export function applyFidelity(result, fidelity = FIDELITY_LEVELS.L2_REDACTED) {
  ...
  // L3: Full text — but literal secret values are STILL scrubbed. Full-fidelity retrieval over an
  // owned corpus returns complete content so the caller can synthesize a grounded answer, but a
  // credential must never be echoed into the response/transcript even when the caller owns the
  // source. Full CONTENT fidelity, minus literal secret values (reuses the L2 REDACTION_PATTERN).
  return {
    id,
    path,
    score,
    source_class,
    fidelity: 'L3',
    text: typeof text === 'string' ? text.replace(REDACTION_PATTERN, '[REDACTED]') : text,
    ...rest
  };
}
