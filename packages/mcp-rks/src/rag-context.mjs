...
import { getSharedEmbeddingPipeline } from "@routekit/rag";
import { getRagPaths } from "../../cli/src/rag/config.mjs";
import { missingRequiredColumns, selectableProjection, tableFieldNames, toPlainList } from "@routekit/rag";
...
async function queryDbByType(dbPath, queryVector, type, limit = DEFAULT_LIMIT) {
  ...
  try {
    ...
    const filtered = rows.filter((row) => {
      // Decode before filtering: an undecoded Arrow Vector's `.includes` semantics are
      // driver-dependent, so code-vs-notes bucketing silently mis-sorted the planner's context.
      const hasCodeTag = toPlainList(row.tags).includes("code");
