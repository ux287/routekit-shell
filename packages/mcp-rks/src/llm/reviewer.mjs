 ...
 */
/**
 * The reconciled non-unique-pattern policy, surfaced by BOTH validators.
 *
 * Evidence: notes/research.2026.08.29.dual-search-validator-asymmetry.md.
 *
 * `plan-ready` used to WARN on a pattern matching more than once while the reviewer
 * FAILED on it, so a story could pass the documented readiness gate and then be
 * rejected by the planner for the same pattern. Direction chosen: BLOCK in both.
 * A REPLACE against an ambiguous pattern would edit an arbitrary one of N identical
 * sites, so leniency is the unsafe half of the disagreement.
 */
import {
...
export const AMBIGUITY_POLICY = "block";

/** Human-readable label per issue type. Four DISTINCT causes, not one. */
const FAILURE_CAUSE_LABELS = Object.freeze({
  target_unresolved: "target could not be attributed to a file",
  file_not_found: "target file not found on disk",
  pattern_ambiguous: "SEARCH pattern matches more than once",
  pattern_not_found: "SEARCH pattern not found in current code",
  // Emitted at the attribution step below. Their absence here is why the summary line read
  // the raw type name back at the operator ("1 missing_file") instead of a cause.
  missing_file: "SEARCH block could not be attributed to any file",
  target_ambiguous: "SEARCH block matches more than one declared target",
});

/** Count failures by their ACTUAL type. */
export function summariseEditFailures(issues = []) {
  const counts = {};
 ...
 *
 * The old template said "N of M SEARCH patterns not found in current code" for EVERY
 * failure mode — missing file, unresolved target, ambiguity, or genuine absence. Three
 * of those four are not "not found", and that mislabelling is why this class of failure
 * was undiagnosable: the error actively pointed away from the cause.
 */
export function describeEditFailures(issues = [], failedCount = 0, totalCount = 0) {
  const counts = summariseEditFailures(issues);
  const parts = Object.entries(counts).map(
    ([type, n]) => `${n} ${FAILURE_CAUSE_LABELS[type] || type}`,
  );
  const detail = parts.length ? parts.join("; ") : "cause not recorded";
  return `${failedCount} of ${totalCount} explicit edits failed validation — ${detail}`;
}

/**
 * Does this candidate even have the SHAPE of a path?
 *
 * The old guard was `/[/.]/` — "contains a slash or a period" — which accepts an
 * ordinary numbered markdown heading like `### 2. Wire the ledger row`, the single
 * most common heading style in this project's notes. Whitespace is the discriminator
 * that prose has and paths do not.
 */
export function looksLikePath(candidate) {
...
/**
 * Resolve a candidate to a real target, or to null.
 *
 * Shape alone is not enough — a well-formed path that names nothing is still not a
 * target. A candidate is accepted only if it ALSO either resolves under projectRoot
 * or appears in the declared target set.
 *
 * The declared-target branch is deliberately independent of `projectRoot`: the gate
 * must stay active when projectRoot is null, or any caller omitting it silently
 * disables the fix and leaves the defect reachable.
 */
export const TARGET_LABEL_PREFIX = /^\s*(?:File|Target)\s*:\s*/i;

export function resolveTargetPath(candidate, projectRoot = null, declaredTargets = []) {
  // Strip a leading `File:` / `Target:` label before shape-testing.
  //
  // This removes an INTERNAL INCONSISTENCY, it does not add a grammar. `atAtRegex`'s first
  // alternative already strips exactly this prefix off an adjacent LINE, so `Target: a/b.mjs`
  // binds there today. Its second alternative captures a HEADING raw, so the identical text
  // written as `### Target: a/b.mjs` reached here with the label attached and died in
  // `looksLikePath` on the embedded space. Same text, opposite outcomes, decided only by the
  // `###` marker. plan-ready.mjs documents the heading form, so the documented remedy was the
  // rejected one.
  //
  // Only the leading label is removed. What remains goes through the UNCHANGED looksLikePath
  // and declared-or-on-disk tests, so this cannot widen acceptance: `Target: 2. Wire the
  // ledger row` still resolves to null, because the remainder still carries a space.
  if (typeof candidate !== "string") return null;
  const stripped = candidate.replace(TARGET_LABEL_PREFIX, "");
  if (!looksLikePath(stripped)) return null;
  const v = stripped.trim();
  if (Array.isArray(declaredTargets) && declaredTargets.includes(v)) return v;
  if (projectRoot) {
    try {
      if (fs.existsSync(path.resolve(projectRoot, v))) return v;
    } catch { /* treat an unreadable path as unresolved */ }
  }
  return null;
}

export function extractExplicitEdits(storyContent, projectRoot = null, declaredTargets = []) {
  const edits = [];
  ...
  // backlog.fix.create-file-directive-grammar-not-reconciled: the comment form is the
  // canonical directive grammar and every other binder reads it, so the reviewer reads it
  // too. The path capture is (\S+), NOT rest-of-line: the comment form is precisely the
  // form authors append a description to (that is why plan-ready raises create_file_syntax),
  // and a rest-of-line capture would synthesise a create_file step writing a file named
  // "src/new-thing.mjs - the new module".
  const createFileCommentRegex = /\/\/\s*CREATE\s+FILE:\s*(\S+)[^\n]*\n\s*```[\w]*\n([\s\S]*?)```/gi;

  for (const rx of [createFileRegex, createFileCommentRegex]) {
  rx.lastIndex = 0;
  while ((match = rx.exec(storyContent)) !== null) {
  ...
  // backlog.fix.reviewer-search-block-heading-adjacency-attribution
  //
  // The heading above is an OPTIONAL PREFIX GROUP that must be CONTIGUOUS with the marker —
  // `\n\s*` tolerates blank lines but NOT prose. So a heading separated from its blocks by a
  // sentence of explanation never fires the group, and every block under it inherited the
  // sticky `currentFile` from the PREVIOUS heading. The rule in force was "immediately-adjacent
  // heading, else last attributed file", never "nearest preceding heading" — so a note with
  //   ### a/one.json   <one adjacent block>
  //   ### b/two.mjs    <prose>   <three blocks>
  // attributed all four to a/one.json and reported three patterns missing from a file they
  // were never associated with. A child project sat blocked on exactly that shape.
  //
  // Fixed by indexing ACCEPTED headings by offset and looking up the nearest preceding one.
  // ACCEPTED, not merely recognised: `resolveTargetPath` must admit it. An ungated scan would
  // break tests/unit/reviewer-search-target-attribution.test.mjs, which requires a block to
  // resolve ACROSS a rejected prose heading like `### 2. Wire the ledger row` to the last real
  // file — the 0.50.3 gate this must not regress.
  //
  // LAYERED ABOVE the sticky fallback, never replacing it: a Pattern 1 `## Edit <path>` heading
  // is rejected by resolveTargetPath and reaches a later block only through `currentFile`.
  const acceptedHeadings = [];
  {
    const headingRegex = /^#{1,6}[ \t]+([^\n]+)$/gm;
    let h;
    while ((h = headingRegex.exec(storyContentStripped)) !== null) {
      const accepted = resolveTargetPath(h[1].trim(), projectRoot, declaredTargets);
      if (accepted) acceptedHeadings.push({ offset: h.index, file: accepted });
    }
  }
  const nearestPrecedingHeading = (offset) => {
    let found = null;
    for (const h of acceptedHeadings) {
  ...
  while ((match = atAtRegex.exec(storyContentStripped)) !== null) {
    ...
    // The gate applies to the MERGED value: `:242` above joins the File:/Target: arm and
    // the heading arm into one candidate, and both fail the same way. A candidate is a
    // target only if it has the shape of a path AND resolves on disk or is declared.
    const verifiedFile = resolveTargetPath(resolvedFileRaw, projectRoot, declaredTargets);
    // Never propagate an unverified candidate. Assigning currentFile from a rejected
    // value is what turned ONE bad heading into "3 of 4 SEARCH patterns not found":
    // the poison carried forward to every later unattributed block.
    if (verifiedFile) currentFile = verifiedFile;
    // Precedence: an adjacent heading wins; else the nearest preceding ACCEPTED heading; else
    // the sticky file. The middle tier is the fix — without it a prose-separated heading is
    // invisible and its blocks silently inherit the previous file.
    const resolvedFile = verifiedFile || nearestPrecedingHeading(match.index) || currentFile || null;
    // Kept for diagnosis: a rejected candidate must be reportable as rejected rather
    // than reported as if it were the file.
    const rejectedCandidate = !verifiedFile && resolvedFileRaw ? resolvedFileRaw : null;
...
export function validateExplicitEdits(edits, projectRoot, targetFiles = []) {
  ...
  for (const [blockIndex, edit] of edits.entries()) {
    ...
    if (!targetFile && targetFiles.length > 1 && edit.search) {
      ...
      // TWO OR MORE is not "no candidate", and the author's next action differs: with zero
      // they must supply a heading; with several they must disambiguate a pattern that is a
      // substring of each. `matches.length === 1` failed identically for both, so a block
      // whose SEARCH text appeared in four declared targets reported as if it matched none.
      else if (matches.length > 1) ambiguousMatches = matches;
    }

    if (!targetFile) {
      // LOCATOR. The failure is that no path resolved, so there is no path to report — and
      // reporting a guessed one would be a fabricated status. What a reader needs instead is
      // enough to find the BLOCK among N: its index, and the first line of its SEARCH text.
      // Both are carried, because neither alone is sufficient — an index is not
      // human-locatable in a long note, and a fragment does not disambiguate two blocks that
      // search for the same text.
      const searchFragment = String(edit.search || "")
        .split("\n")
        .find((l) => l.trim()) || "";
      const locator = searchFragment.trim().slice(0, 80);
      ...
      result.valid = false;
      result.issues.push({
        type: ambiguousMatches ? "target_ambiguous" : "missing_file",
        edit: edit.description,
        blockIndex,
        searchFragment: locator,
        ...(ambiguousMatches ? { candidates: ambiguousMatches } : {}),
        message: ambiguousMatches
          ? `SEARCH block #${blockIndex} matches ${ambiguousMatches.length} declared targets `
            + `(${ambiguousMatches.join(", ")}) and cannot be attributed to one. `
            + `Add a "### <path>" heading above it. First SEARCH line: ${locator}`
          : `SEARCH block #${blockIndex} could not be attributed to any file. `
            + `Add a "### <path>" heading above it. First SEARCH line: ${locator}`
      });
    ...
    if (edit.search) {
      ...
      if (occurrences === 0) {
        ...
        result.valid = false;
        // TIER ATTRIBUTION. Report what each tier OBSERVED, not just the outcome — an edit
        // that fails exact while matching normalised is a whitespace defect, and reporting it
        // as a bare pattern_not_found points away from the cause.
        const presence = patternPresenceInContent(content, searchPattern);
        result.issues.push({
          ...
          message: presence.whitespaceMismatch
            ? "Search pattern matches only after normalizing whitespace; execution requires exact bytes"
            : "Search pattern does not exist in file"
        });
 ...
 *
 * Delegates to the SHARED counter in validation/search-replace.mjs. This function used to be an
 * independent implementation, which is half of the defect recorded in
 * backlog.fix.search-pattern-presence-not-reconciled-across-validators — two presence matchers
 * that agreed until they didn't. Exported so the reconciliation is assertable rather than a
 * claim about an import statement.
 */
export function countOccurrences(content, search) {
  return countPatternOccurrences(content, search);
}
...
export async function runReviewerMode({
  ...
  const explicitEdits = extractExplicitEdits(storyContent, projectRoot, targetFiles);
  ...
  if (validCount < totalCount) {
    return {
      ...
      error: describeEditFailures(validation.issues, totalCount - validCount, totalCount),
      causes: summariseEditFailures(validation.issues),
      ambiguityPolicy: AMBIGUITY_POLICY,
