...
function extractSection(markdown, headingRegex) {
  ...
  const collected = [];
  let capture = false;
  // backlog.fix.target-parse-fence-blind-false-targets: this extractor had no fence
  // state, so every line of a fenced code block was handed to the bullet regex in
  // parseTargetsFromMarkdown. A JSDoc continuation line — " * @param {string} handle"
  // — is asterisk-led, so it matched, and the remainder became a "target path". A
  // child project reported targetCount 53 against 10 real targets, with 43 blocking
  // missing_create_directive issues naming prose fragments.
  //
  // The PO story format GUARANTEES the trigger: governor-po.md mandates a fenced
  // block of real content for every op:create target, so the better-documented a
  // story is, the more likely it was to block.
  //
  // Tracked here rather than in the bullet regex because the raw lines only exist
  // here — this is the earliest point at which a fenced line is distinguishable
  // from a structural one.
  let inFence = false;
  for (const line of lines) {
    // An opening fence may carry an info string (```javascript), which the PO format
    // emits; a matcher keyed to a bare triple backtick would miss every real one.
    if (/^\s*(```|~~~)/.test(line)) {
      if (capture) inFence = !inFence;
      continue;
    }
    if (inFence) {
      // Deliberately still collected-adjacent: a heading INSIDE a fence must not
      // terminate the section either, or fenced markdown would truncate it.
      continue;
    }
