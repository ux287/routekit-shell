...
// Single-source key-presence authority (INV-5 no-drift): the recognized-key list AND the
// hasLlmCredential/isKeyless decision live in packages/hooks/system/credential-presence.mjs, so the
// SERVER and the HOOK layer make ONE identical keyless decision — never re-derived here. Imported
// as local bindings (resolveSources uses RECOGNIZED_CREDENTIAL_KEYS as a default) and re-exported.
// (BRAVE_SEARCH_API_KEY / GITHUB_TOKEN gate optional features and are intentionally out of scope.)
import { RECOGNIZED_CREDENTIAL_KEYS, hasLlmCredential, isKeyless } from "../../../hooks/system/credential-presence.mjs";
export { RECOGNIZED_CREDENTIAL_KEYS, hasLlmCredential, isKeyless };
...
/**
 * Invoke-time credential guard for the Anthropic SDK paths.
 *
 * Shared single source of truth reused at EVERY `new Anthropic()` construction site
 * (agents/runner.mjs, agents/visual.mjs, server/qa-agent.mjs). Because the boot preflight is now
 * non-fatal (the server boots key-free so RAG + other key-free tools stay reachable), a missing
 * credential must surface as a clear, actionable message at call time — this guard throws it just
 * before the SDK is constructed, instead of letting an opaque SDK error escape later.
 *
 * Value-free: the thrown message names the KEY only, never the credential value. Defaults to the
 * live `process.env` (the runtime source at every call site); accepts an injected env for tests.
 */
export function assertAnthropicCredential(env = process.env) {
  if (!env || !env.ANTHROPIC_API_KEY) {
    throw new Error(
      "Missing required credential ANTHROPIC_API_KEY: this tool needs an Anthropic API key. " +
        "Set ANTHROPIC_API_KEY in your environment or project .env and retry."
    );
  }
}
