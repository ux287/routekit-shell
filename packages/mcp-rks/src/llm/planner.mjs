...
export function normalizePlannerSteps(parsedSteps = []) {
  ...
  const hasExecutableWithContent = normalized.some(
    (s) =>
  ...
  return { steps: normalized, hasExecutableWithContent, diffRejected };
}
...
function buildTestGenerationPrompt({ testPath, testExemplar, requirements, sourceChanges, existingContent }) {
  const parts = [
...
const TEST_GEN_SYSTEM_PROMPT =
  "You are a test code generator. Output ONLY raw JavaScript/TypeScript code. No JSON, no markdown fences, no prose outside the code. Every test must contain real assertions.";

async function callLlmForRawCode(env, prompt, llmContext = {}) {
...
async function enrichTestFileContent({ actions, testExemplar, requirements, env, llmContext = {} }) {
  ...
  for (let i = 0; i < enriched.length; i++) {
    ...
    try {
      const rawCode = await callLlmForRawCode(env, testPrompt, llmContext);
      if (rawCode && !hasEmptyTestBodies(rawCode)) {
        enriched[i] = { ...step, content: rawCode };
        enrichedCount++;
        console.error(
          `[planner] enrichTestFileContent: enriched ${step.path} (${rawCode.length} chars, has assertions)`
        );
      } else {
        console.error(
          `[planner] enrichTestFileContent: LLM output for ${step.path} still lacks assertions, keeping original`
        );
      }
    } catch (err) {
...
export async function runLlmPlanner({
  ...
  const runSinglePassPlanner = async ({
    ...
    const second = await callOnce(secondPrompt);
