 ...
 *
 * `truncated` answers "is this the whole NOTE?" and is therefore true for every
 * selection by construction — a selection is narrower than the note by definition.
 * `selectionComplete` answers the different question "did what I ASKED FOR come back
 * whole?", and is the field a caller must read before rewriting anything from a slice.
 * Conflating them is how a caller rebuilds an array from a partial read: a truncated
 * flag that is unconditionally true tells it nothing.
 *
 * @returns {{truncated: boolean, content?: string, manifest?: object, totalBytes: number, returnedBytes: number, bounded: boolean, selectionBytes: number, selectionComplete: boolean, fieldsNotFound: string[]}}
 */
/**
...
export function sliceNote(raw, options = {}) {
  ...
  if (!hasSelector && totalBytes <= maxBytes) {
    // Nothing was narrowed, so the "selection" is the whole note and it came back whole.
    // Stated rather than omitted: an absent field is falsy, and a caller testing
    // falsiness would read a complete read as a cut one.
    return {
      truncated: false,
      content: raw,
      totalBytes,
  ...
  if (!hasSelector) {
    ...
    return {
      ...
      returnedBytes: 0,
      bounded,
      // The caller asked for the whole note and got none of it. `returnedBytes` 0
      // against a non-zero `selectionBytes` is exactly the incomplete case.
      selectionBytes: totalBytes,
  ...
  const { frontmatter, body } = splitNote(raw);
  const parts = [];
  // Derived from the frontmatter TEXT via the same helper selectFields matches on, so
  // the two cannot drift: a name reported found contributes its block to the content,
  // and a name reported missing contributes nothing.
  const fieldsNotFound = fields ? fields.filter((n) => !frontmatterKeys(frontmatter).includes(n)) : [];
  ...
  // The size of what was ASKED FOR, captured before the cap can overwrite it below.
  // Without this the only surviving number is the post-cut size, and a cut selection
  // is then indistinguishable from a complete one.
  const selectionBytes = measuredBytes;
  ...
  // DERIVED from two measurements, not from selector presence, not from `bounded`, and
  // not from which branch ran. An empty-but-uncut selection (limit 0, or fields naming
  // only absent keys) is COMPLETE at 0 === 0 — found-ness and completeness are
  // independent questions, and a non-empty fieldsNotFound must not force this false.
  const selectionComplete = returnedBytes === selectionBytes;

  return { truncated, content, totalBytes, returnedBytes, bounded, selectionBytes, selectionComplete, fieldsNotFound };
}
