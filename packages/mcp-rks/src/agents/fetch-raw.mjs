...
async function validateTarget(u, allowedHosts, resolveDns) {
  if (u.protocol !== "https:") return { ok: false, reason: "scheme_denied", message: `Only https:// is allowed (got ${u.protocol}).` };
  if (!hostAllowed(u.hostname, allowedHosts)) return { ok: false, reason: "host_not_allowlisted", message: `Host '${u.hostname}' is not in fetchRaw.allowedHosts (default-deny).` };
  let ips;
  try {
 ...
 * @param {string} url
 * @param {object} [opts] projectRoot, allowedHosts, fetch, resolveDns, maxBytes, timeoutMs,
 *   cacheDir, recordProvenance, now
...
export async function fetchRaw(url, opts = {}) {
  ...
  const firstCheck = await validateTarget(u, allowedHosts, resolveDns);
  ...
  try {
    ...
    for (;;) {
      ...
      if ([301, 302, 303, 307, 308].includes(status)) {
        ...
        // Re-validate every hop against scheme + allowlist + SSRF.
        const hopCheck = await validateTarget(next, allowedHosts, resolveDns);
        if (!hopCheck.ok) {
          clearTimeout(timer);
