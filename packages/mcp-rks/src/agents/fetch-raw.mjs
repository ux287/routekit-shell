...
import { lastCharBoundaryAtOrBefore } from "../shared/note-slice.mjs";
...
async function validateTarget(u, allowedHosts, resolveDns, mode = "allowlist") {
  ...
  if (mode !== "open" && !hostAllowed(u.hostname, allowedHosts))
    return {
      ...
      // Carried to the caller so the denial object itself names WHICH host was
      // refused and how big the allowlist was — the message text below is
      // preserved verbatim, it is self-documenting and users act on it.
      extra: {
        host: u.hostname,
        mode,
        allowedHostCount: Array.isArray(allowedHosts) ? allowedHosts.length : 0,
      },
      message:
...
export async function fetchRaw(url, opts = {}) {
  ...
  // TELEMETRY — the whole point of this function's observability.
  //
  // A default-deny that announces itself is a policy a user can act on. A
  // default-deny that returns a thinner answer is indistinguishable from "the
  // internet did not have much on this", and it silently degraded every external
  // research this project ever did until 2026-08-13. `deny()` below is the single
  // choke point every refusal passes through, so the emit lives INSIDE it — that
  // way a new denial reason cannot be added without being loud, by construction.
  //
  // The real collector is built ONLY when the caller explicitly passed projectRoot.
  // Defaulting it would attach the process-wide telemetry singleton (rooted at
  // process.cwd()) and schedule flush timers inside forked vitest workers for the
  // 27 existing tests that call fetchRaw() with no projectRoot.
  const collector =
    opts.collector !== undefined
      ? opts.collector
      : opts.projectRoot !== undefined
        ? (() => {
            try { return ensureTelemetryStorage(opts.projectRoot); }
            catch { return { emit: () => {} }; }
          })()
        : { emit: () => {} };

  // Telemetry is best-effort and must NEVER fail a fetch.
  const emit = (type, payload) => {
    try { collector.emit(type, projectId, payload); } catch { /* never break the fetch */ }
  };

  const deny = (reason, message, extra = {}) => {
    let host = null;
    try { host = new URL(url).hostname; } catch { /* invalid_url — no host to report */ }
    emit("agent.fetch-raw.denied", {
      reason,
      host,
      url,
      mode,
    ...
    return { ok: false, reason, message, url, ...extra };
  };
  ...
  emit("agent.fetch-raw.started", { url, host: u.hostname });
  ...
  // Forward validateTarget's extras — without this the host/mode/allowedHostCount
  // it computed are discarded and the denial event cannot name the refused host.
  if (!firstCheck.ok) return deny(firstCheck.reason, firstCheck.message, firstCheck.extra || {});
    ...
    for (;;) {
      ...
      if ([301, 302, 303, 307, 308].includes(status)) {
        ...
        if (!hopCheck.ok) {
          clearTimeout(timer);
          return deny(
            hopCheck.reason === "host_not_allowlisted" ? "redirect_host_not_allowlisted" : hopCheck.reason,
            `Redirect blocked: ${hopCheck.message}`,
            // The refused host here is the REDIRECT TARGET, not the original url —
            // without forwarding these the event would name the wrong host.
            hopCheck.extra || {},
          );
        }
        target = next.href;
  ...
  try {
    ...
    if (reader) {
      ...
      if (Buffer.byteLength(text, "utf8") > maxBytes) {
        // Cut on a character boundary. A bare subarray splits multi-byte
        // sequences and the decoder turns the orphaned bytes into U+FFFD, which
        // both corrupts the body and makes it exceed the cap it was cutting to.
        // Same defect, same fix, ONE definition — shared with the note byte cap.
        const buf = Buffer.from(text, "utf8");
        body = buf.subarray(0, lastCharBoundaryAtOrBefore(buf, maxBytes)).toString("utf8");
        truncated = true;
      } else {
        body = text;
      }
  ...
  emit("agent.fetch-raw.complete", { host: u.hostname, bytes, truncated, sha256 });
