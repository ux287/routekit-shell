...
export const CycleCompleteOutputSchema = z.object({
  ...
  data: z.object({
    ...
    // `gitClean` was REMOVED, not renamed. It was an optional field the model
    // filled in from its own narration: nothing bound it to the check_git_state
    // tool's result, so it reported a clean tree on a dirty one. The tool still
    // runs and still returns a real `clean` computed from `git status
    // --porcelain`; callers must read that rather than a self-reported summary.
    // Re-adding this field without wiring it to the tool result would restore
    // the defect.
  }).optional(),
});
      ...
      // --- embed_rag ---
      {
        ...
        execute: async () => {
          try {
            ...
            // Re-embed in-process via @routekit/rag. This call site used to `await import`
            // a server-local rag module that has not existed since the RAG extraction, so the embed
            // silently never ran — and `embedded: true` was returned regardless.
            //
            // runRagEmbed does NOT throw on failure: it returns { ok: false, error, ... }. So the
            // return value must be OBSERVED. Reporting embedded: true without looking at it is the
            // original defect, not merely a style problem.
