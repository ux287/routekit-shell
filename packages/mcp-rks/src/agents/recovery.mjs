...
import { runRagCompact, runRagEmbed } from '@routekit/rag';
...
export const RecoveryOutputSchema = z.object({
  ...
  /**
   * MECHANISM 3 — machine-detectable refusal.
   *
   * When the agent is asked for something it has no capability to perform, it must say
   * so HERE, as structure the caller can branch on — never by narrating a plausible
   * result. Its presence is the signal; no string matching required.
   */
  capabilityRefusal: z.object({
    requested: z.string().describe('What was asked for that this agent cannot perform'),
    reason: z.string().describe('Why it cannot be performed'),
  }).optional(),

  /**
   * Tool attribution. A claim about system state must be backed by a tool that actually
   * observed it — this is what makes the anti-fabrication rule enforceable rather than
   * merely requested.
   */
  toolsUsed: z.array(z.string()).optional()
    .describe('Names of the tools actually invoked to reach these conclusions'),
})
  .superRefine((val, ctx) => {
    // MECHANISM 2 — PARTIAL MITIGATION, deliberately not the whole fix.
    //
    // Rejects output that is shaped like a command transcript. This is a heuristic: a
    // fabrication phrased without these markers still passes, which is exactly why the
    // attribution rule below is the contract-grade control and this is a backstop.
    // Scoped to RecoveryOutputSchema ON PURPOSE — a filter in the shared runner funnel
    // would false-positive on research/telemetry/git answers that legitimately quote logs.
    const TRANSCRIPT_MARKERS = [
      /\bEXIT\s*STATUS\s*:/i,
      /\bexit\s*code\s*:\s*\d/i,
      /^\s*\$\s+\S/m,
      /^\s*>\s*(npm|node|git|npx)\s/m,
    ];
    ...
    (function walk(node, depth) {
      ...
      if (typeof node === 'object') { for (const n of Object.values(node)) walk(n, depth + 1); }
    })(val, 0);

    for (const s of strings) {
      for (const re of TRANSCRIPT_MARKERS) {
        if (re.test(s)) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message:
              'Recovery output contains a simulated command transcript. This agent cannot execute ' +
              'commands; report what tools observed, or return capabilityRefusal. Never reconstruct output.',
            path: ['summary'],
          });
          return;
        }
    ...
    // Attribution: diagnosis claims require at least one tool to have been invoked.
    // A refusal is exempt — refusing correctly requires no tool call.
    if (val.data?.diagnosis && !val.capabilityRefusal) {
      if (!Array.isArray(val.toolsUsed) || val.toolsUsed.length === 0) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'Recovery output asserts a diagnosis without tool attribution. Populate toolsUsed with ' +
            'the tools actually invoked, or return capabilityRefusal instead of an unbacked claim.',
          path: ['toolsUsed'],
        });
      }
    }
...
## Rules
- NEVER simulate, reconstruct, or hand-render command output, exit codes, terminal
  transcripts, or file listings. You cannot execute shell commands. If you are asked for
  the result of one, populate \`capabilityRefusal\` ({ requested, reason }) and stop —
  a refusal is a correct answer; an invented result is not. Your output is schema-checked
  for transcript-shaped content and will be REJECTED, so narrating one fails the call.
- Every claim about system state must come from a tool you actually invoked, and those
  tool names must be listed in \`toolsUsed\`. Unattributed diagnoses are rejected.
...
function buildTools({ projectId, projectRoot }) {
  ...
  return [
    ...
    // --- fix_rag ---
    {
      ...
      execute: async (input) => {
        try {
          // In-process via @routekit/rag. These two sites used to spawn
          // a server-local rag module that has not existed since the extraction —
          // so fix_rag, the tool whose job is repairing a broken RAG index, was itself broken and
          // reported its own dangling wiring as 'RAG embed failed' with the hint "try rks_rag_embed
          // manually". Someone debugging a bad index was sent in a circle.
          //
          // Neither function takes a projectId: projectRoot is the sole addressing parameter, so the
          // old '--project' argv flag is dropped rather than translated. Neither returns a child
          // process, so the result.stdout/result.stderr reads are dropped with it.
          if (input.action === 'embed') {
            const result = await runRagEmbed(projectRoot);
            if (result?.ok === false) {
              return {
                ok: false,
                error: result.error || 'RAG embed failed',
                hint: 'Try calling rks_rag_embed manually',
              };
            }
          ...
          if (input.action === 'compact') {
            ...
            if (result?.ok === false) {
              return {
                ok: false,
                error: result.error || 'RAG compact failed',
                hint: 'Try calling rks_rag_compact manually',
              };
            }
