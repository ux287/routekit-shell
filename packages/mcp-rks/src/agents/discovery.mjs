...
import { validateAgentDefinition, AGENT_DEFINITION_KEYS } from '../shared/agent-schema.mjs';
...
export function discoverProjectAgents(projectRoot) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // The SUCCESS path speaks. Distinct from the invalid-agent warning above in both wording and
      // consequence: that one says the agent was skipped, this one says the agent registered and
      // part of what the author wrote did not survive. Silence here is what let a `prompt` key
      // vanish while the tool reported success.
      if (validation.unknownKeys.length > 0) {
        console.warn(
          `[agent-discovery] ${file}: registered "${validation.data.name}", but DROPPED ` +
            `unrecognised key(s): ${validation.unknownKeys.join(', ')}. ` +
            `They are not part of the agent definition schema and have no effect. ` +
            `Recognised keys: ${AGENT_DEFINITION_KEYS.join(', ')}.`,
        );
      }
