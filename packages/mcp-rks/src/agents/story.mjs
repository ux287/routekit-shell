...
async function queryRag(projectId, projectRoot, query, k = 5) {
  try {
    // Static import from @routekit/rag. The former dynamic import of a server-local rag module
    // at a module that has not existed since the extraction, so it threw on every call — and the
    // "RAG module not available" soft-fail guard that used to sit here was UNREACHABLE, because
    // the import threw before it could be evaluated. The guard is deleted rather than preserved:
    // with a static import the module cannot be absent at runtime.
    const result = await runRagQuery(projectRoot, { q: query, k });
