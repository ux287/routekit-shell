 ...
 * (that prose summary is now SECONDARY — see below).
 *
 * OUTPUT CONTRACT: this agent may QUOTE what a git tool actually returned on this
 * invocation, and may ASSERT conclusions drawn from those quotes, but the two go in
 * different fields. findings[] are quotes bound to a ledger index and verified by
 * substring containment; conclusions[] are inferences whose basis indices are checked
 * for resolution ONLY. evidence[] is built in code from the wrapped tool executes and
 * is not writable by the model. summary is the model's reading and carries a
 * degradation banner when the audit says the reading outran the evidence.
 * See agents/git-evidence.mjs and notes/design.evidence-bound-reporting-invariant.md.
...
export const GitOutputSchema = z.object({
  ok: z.boolean(),
  summary: z.string().describe('Human-readable summary of what happened. This is your READING of the evidence, not evidence itself. Every load-bearing statement here must also appear as a findings[] quote or a conclusions[] statement.'),
  data: z.record(z.unknown()).optional().describe('Structured data from the operation'),
  // QUOTED material. Each entry binds a verbatim substring of what a git tool
  // actually returned to the ledger index of the call that returned it. Verified
  // by code in auditGitClaims; a quote that matches nothing is annotated, not
  // dropped. Declared here (not merely in the transform) because z.object()
  // strips undeclared keys — without this line the model's claims never arrive.
  findings: z.array(FindingSchema).default([]).describe('Verbatim quotes from recorded git output. Each MUST carry the evidenceIndex of the call it came from.'),
  // ASSERTED material. Inferences the agent drew. Code checks only that the
  // basis indices resolve to real recorded calls — never whether the evidence
  // entails the statement.
  conclusions: z.array(ConclusionSchema).default([]).describe('Inferences drawn from the findings. Each MUST cite a basis array of evidenceIndex values.'),
});
...
HARD LIMITS:
- Maximum 6 tool calls per request. This is a budget you are asked to stay within, not a limit the
  system enforces. If you run out of budget before you can answer, say so plainly in summary.

SEPARATE WHAT GIT SAID FROM WHAT YOU CONCLUDED. Your summary prose is not evidence, and the caller
cannot tell a sound verdict from an invented one unless you split them:
- findings: verbatim quotes of text a git tool actually returned on THIS invocation. Every entry
  needs an evidenceIndex naming which tool call it came from, and the quote must be copied exactly
  and be substantial (a fragment of a dozen characters or more, never an empty string).
- conclusions: what you infer from those findings. Every entry needs a basis array of evidenceIndex
  values. If you cannot cite one, still write the conclusion and leave basis empty rather than
  inventing a citation.
- summary: your reading, in prose. State nothing here that is not backed by a finding or a
  conclusion. If you did not observe something, say you did not observe it - absence of output is
  not evidence of absence.

These instructions help you fill the contract, but they are SECONDARY. The evidence ledger is built
from the tool calls you actually made and is not writable by you, and your claims are checked against
it in code after you answer. A claim that cites nothing is reported as citing nothing.
...
export function createGitAgent({ projectId, request, projectRoot }) {
  ...
  return {
    name: 'git',
    model: cfg.model,
    // A project-supplied prompt override REPLACES GIT_SYSTEM_PROMPT wholesale,
    // including the findings/conclusions instruction above. That is intended and
    // fails closed rather than quiet: an overridden agent emits no claims, so every
    // response degrades with reason 'no_claims_declared' and carries the banner. The
    // contract announces that it is not being used instead of silently returning
    // unbound prose. A project that overrides the prompt should carry the claim
    // instruction across.
    ...
    outputSchema: buildGitOutputSchema({ baseSchema: GitOutputSchema, ledger }),
    rawInput: { projectId, request },
    maxTurns: cfg.maxTurns,
    timeoutMs: cfg.timeoutMs,
    projectId,
    projectRoot,
    tools: instrumentToolsWithLedger([
      // --- git_state ---
      {
        name: 'git_state',
      ...
      {
        name: 'git_stash',
        description: 'Stash operations: pop, list, apply, drop. Creating a stash is NOT available here — use the rks_stash MCP tool, which registers an auto-pop with the Governor session.',
        inputSchema: z.object({
          action: z.enum(['pop', 'list', 'apply', 'drop']).describe('Stash action. Creating a stash is not available from this agent.'),
          message: z.string().optional().describe('Stash message (unused — creating a stash is not available here)'),
          stashIndex: z.number().optional().describe('Stash index (for pop/apply/drop, default 0)'),
        }),
        execute: async (input) => {
          // The enum above is ADVISORY ONLY. runner.mjs invokes tool.execute(toolUse.input)
          // with no per-tool zod validation, and runGitStash defaults action to 'save' — so a
          // model that omits `action`, or emits 'save' regardless, would otherwise still
          // create a stash. This guard is the actual containment; the enum is documentation.
          //
          // Why creating is refused at all: nothing here can restore it. Agent tools receive
          // no session token, so setPendingStash cannot be registered and endSession's auto-pop
          // never fires. A stash created here is invisible to every recovery path, and the
          // agent's own summary field is model-authored prose that has claimed the files
          // "remain in that state" while they sat in an abandoned stash for eight days.
          const action = input.action ?? 'save';
          if (action === 'save' || action === 'push') {
            return {
              ok: false,
              error: 'git_stash cannot create a stash. A stash created by this agent has no restore path: agent tools receive no session token, so no auto-pop can be registered against the Governor session.',
              hint: 'Commit the changes with rks_git_commit, or call the rks_stash MCP tool with a live Governor session — that path registers an auto-pop that fires when the session ends.',
            };
          }
          try {
