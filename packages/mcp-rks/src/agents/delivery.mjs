...
export const DeliveryInputSchema = z.object({
  ...
  storyIds: z.array(z.string()).optional()
    .describe('Specific story IDs to deliver. If omitted, autoDiscover must be set explicitly.'),
  dryRun: z.boolean().optional().describe('If true, validate only — do not ship'),
  /**
   * backlog.fix.agent-run-strict-input-and-delivery-guard — ARCH-ruled guard.
   *
   * An omitted `storyIds` used to mean "discover every ready story and ship it." A
   * malformed call that lost `storyIds` to silent key-stripping therefore escalated from
   * "do nothing" to "ship everything ready in the project", and ran for 1800s doing it.
   *
   * Auto-discovery is now opt-in. `storyIds` deliberately stays OPTIONAL — the read-only
   * "what is shippable?" preview must remain reachable in one call, and making it required
   * would invert a correctly-named existing test. `dryRun` deliberately stays a MODE, not
   * an authorization: forcing it would invite a reflexive `dryRun: false` and restore the
   * hazard while looking deliberate. The extra token belongs on the DANGEROUS path only.
   */
  autoDiscover: z.boolean().optional()
    .describe('Explicitly authorize acting on auto-discovered ready stories when storyIds is omitted. Defaults to false.'),
});

/**
 * Hard cap on how many auto-discovered stories one invocation may act on.
 * Bounds the blast radius even when auto-discovery IS authorized.
 */
export const AUTO_DISCOVER_MAX = 5;
...
1. If story IDs were provided, use exactly those — do NOT call list_ready_stories to add more.
   If NO story IDs were provided:
     - autoDiscover is set: call list_ready_stories, then act on AT MOST the capped number
       it returns. Never widen beyond that.
     - autoDiscover is NOT set: STOP. Do not ship anything. You may call list_ready_stories
       for a read-only preview and report what is shippable, but you must not plan,
       implement, ship, or complete cycles. Say that autoDiscover is required to proceed.
 ...
 */
export function createDeliveryAgent({ projectId, title, storyIds, dryRun, autoDiscover, projectRoot }) {
  ...
  return {
    ...
    prompt,
    userMessage: buildUserMessage({ projectId, title: releaseTitle, storyIds, dryRun, autoDiscover }),
    inputSchema: DeliveryInputSchema,
    outputSchema: DeliveryOutputSchema,
    rawInput: { projectId, title: releaseTitle, storyIds, dryRun, autoDiscover },
    projectId,
    projectRoot,
    tools: buildTools({ projectId, title: releaseTitle, storyIds, projectRoot, autoDiscover }),
  };
}

function buildUserMessage({ projectId, title, storyIds, dryRun, autoDiscover }) {
  const parts = [`Deliver release "${title}" for project ${projectId}.`];
  if (storyIds?.length) parts.push(`Stories: ${storyIds.join(', ')}`);
  else if (autoDiscover === true) {
    parts.push(`No story IDs given. autoDiscover IS authorized — you may act on at most ${AUTO_DISCOVER_MAX} discovered stories.`);
  } else {
    parts.push(
      'No story IDs given and autoDiscover is NOT set. Do NOT ship anything. ' +
      'You may list what is ready as a read-only preview, then stop and report that ' +
      'autoDiscover is required to proceed.',
    );
  }
...
function buildTools({ projectId, title, storyIds, projectRoot, autoDiscover }) {
    ...
    // --- list_ready_stories ---
    {
      ...
      execute: async (input) => {
        try {
          ...
          // Hard cap the blast radius. Previously this returned EVERY matching story
          // with no bound, so one under-specified call could act on the whole backlog.
          // The full count is still reported so the caller sees what was withheld.
          const capped = stories.slice(0, AUTO_DISCOVER_MAX);
          return {
            count: capped.length,
            totalFound: stories.length,
            capped: stories.length > AUTO_DISCOVER_MAX,
            cap: AUTO_DISCOVER_MAX,
            autoDiscoverAuthorized: autoDiscover === true,
            stories: capped,
            ...(autoDiscover === true
              ? {}
              : {
                  notice:
                    'autoDiscover is not set — this listing is a READ-ONLY preview. ' +
                    'Do not plan, implement, ship, or complete cycles for these stories.',
                }),
          };
        } catch (err) {
