 ...
 */
function finalizeResult({ name, rawText, outputSchema, telemetryId, emitTelemetry, startTime, turns, tokens }) {
  const durationMs = Date.now() - startTime;

  if (!rawText) {
    emitTelemetry('failed', { error: 'empty_response', turns, durationMs, tokens });
    return { ok: false, error: 'Agent returned empty response', telemetryId };
  }
  ...
  if (parsed === undefined) {
    if (outputSchema) {
      ...
      return { ok: false, error: 'Agent output is not valid JSON', rawText, telemetryId };
    }
    // No schema — return raw text as answer
    emitTelemetry('complete', { turns, durationMs, outputSummary: rawText.slice(0, 200), tokens });
    return { ok: true, answer: rawText, telemetryId };
  }
  ...
  if (outputSchema) {
    try {
      ...
      emitTelemetry('complete', { turns, durationMs, validated: true, outputSummary: rawText.slice(0, 200), tokens });
      return { ...validated, telemetryId };
    } catch (err) {
  ...
  emitTelemetry('complete', { turns, durationMs, outputSummary: rawText.slice(0, 200), tokens });
  return { ok: true, ...parsed, telemetryId };
}
