...
import { redactValueSecretsOnly, redactStringSecretsOnly } from '@routekit/telemetry/redact';
import { assertAnthropicCredential } from '../llm/credential-preflight.mjs';

// backlog.fix.hook-fallthrough-on-research-agent-outage
// Classify a research failure's error string. Returns { type, category } where type is
// 'infrastructure' (the API/network/billing is down — a genuine outage), 'capability' (the agent
// is UP but produced a bad/unparseable/over-length output — must NOT be treated as an outage), or
// 'unknown' (ambiguous — fail closed, never an outage). Capability faults are checked FIRST so a
// message like "exceeded max turns" can't be mis-caught by the timeout branch.
export function classifyAgentError(error) {
  if (typeof error !== 'string' || !error) return { type: 'unknown', category: 'unknown' };
  const e = error.toLowerCase();
  if (e.includes('not valid json') || e.includes('output validation failed') ||
      e.includes('empty response') || e.includes('exceeded max turns')) {
    return { type: 'capability', category: 'capability_fault' };
  }
  if (/\b5\d\d\b/.test(error) || e.includes('internal server error') || e.includes('bad gateway') ||
      e.includes('service unavailable') || e.includes('overloaded')) {
    return { type: 'infrastructure', category: 'server_5xx' };
  }
  if (e.includes('credit balance') || e.includes('credit-balance') || e.includes('billing') ||
      e.includes('quota') || e.includes('insufficient') || e.includes('429')) {
    return { type: 'infrastructure', category: 'billing' };
  }
  if (/\b401\b/.test(error) || /\b403\b/.test(error) || e.includes('unauthorized') ||
      e.includes('forbidden') || e.includes('authentication') || e.includes('invalid api key') ||
      e.includes('invalid x-api-key') || e.includes('permission denied')) {
    return { type: 'infrastructure', category: 'auth' };
  }
  if (e.includes('econnrefused') || e.includes('econnreset') || e.includes('enotfound') ||
      e.includes('network') || e.includes('socket hang') || e.includes('fetch failed') ||
      e.includes('connection error') || e.includes('connection refused')) {
    return { type: 'infrastructure', category: 'network' };
  }
  if (e.includes('timeout') || e.includes('timed out') || e.includes('aborterror') || e.includes('aborted')) {
    return { type: 'infrastructure', category: 'timeout' };
  }
  return { type: 'unknown', category: 'unknown' };
}

function researchOutageBreadcrumbPath(projectRoot) {
  return path.join(projectRoot, '.rks', 'telemetry', 'research-agent-outage.json');
}

/** Write the outage breadcrumb the read hooks consult. Best-effort; never breaks the agent. */
function writeResearchOutageBreadcrumb(projectRoot, category) {
  try {
    const file = researchOutageBreadcrumbPath(projectRoot);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, JSON.stringify({ timestamp: new Date().toISOString(), category }), 'utf8');
  } catch { /* best-effort */ }
}

/** Clear the outage breadcrumb (recovery closes the fallthrough). Best-effort. */
function clearResearchOutageBreadcrumb(projectRoot) {
  try { fs.rmSync(researchOutageBreadcrumbPath(projectRoot), { force: true }); } catch { /* best-effort */ }
}

/**
 * Record/clear the outage breadcrumb after a research invocation. A successful result (ok:true —
 * including a legitimate not-found `{ok:true, advisory:true, answer:''}`) means the agent is up →
 * clear. An infra-classified failure → write. A capability fault or ambiguous error → leave the
 * breadcrumb untouched (no write, no clear). Only the research agent participates.
 */
export function recordResearchOutage(config, result, deps = {}) {
  const write = deps.write || writeResearchOutageBreadcrumb;
  const clear = deps.clear || clearResearchOutageBreadcrumb;
  try {
    const projectRoot = config && config.projectRoot;
    const name = config && config.name;
    const isResearch = typeof name === 'string' && name.includes('research');
    if (!projectRoot || !isResearch || !result) return;
    if (result.ok) { clear(projectRoot); return; }
    if (result.ok === false && classifyAgentError(result.error).type === 'infrastructure') {
      write(projectRoot, classifyAgentError(result.error).category);
    }
    // capability / unknown → leave breadcrumb as-is
  } catch { /* best-effort: breadcrumb must never break the agent */ }
}
...
export async function runAgent(config) {
  ...
  // backlog.fix.hook-fallthrough-on-research-agent-outage: every terminal result the caller sees
  // passes through finish(), which writes/clears the research-outage breadcrumb the read hooks
  // consult. Applied at ALL return sites so an escalated result updates the breadcrumb too.
  const finish = (r) => { recordResearchOutage(config, r); return r; };
  ...
  if (!result.ok && fallbackModel && model !== fallbackModel && !_isEscalation) {
    ...
    return finish(stripSelfEscalate(escalatedResult));
  }
  ...
  if (result.ok && result._selfEscalate && fallbackModel && model !== fallbackModel && !_isEscalation) {
    ...
    return finish(stripSelfEscalate(escalated));
  }

  return finish(stripSelfEscalate(result));
}
...
async function _executeAgent(config) {
  ...
  try {
    ...
    if (_testClient) {
      ...
      // constructed on a keyless boot, instead of an opaque @anthropic-ai/sdk construction error.
...
export function finalizeResult({ name, rawText, outputSchema, telemetryId, emitTelemetry, startTime, turns, tokens }) {
  ...
  if (parsed === undefined) {
    if (outputSchema) {
      // MECHANISM 1 (backlog.fix.recovery-agent-fabricated-command-output) — the
      // prose-passthrough hole, and the SUFFICIENT fix for the observed incident.
      //
      // This branch used to return `rawText` to the caller. When an agent cannot perform
      // what it was asked to do, it may respond with prose instead of schema-valid JSON —
      // and that prose was handed straight back. The recovery agent exploited exactly this:
      // asked for the result of a command it had no capability to run, it hand-rendered a
      // fake terminal transcript ("EXIT STATUS: 1") naming a drift file that exists in
      // neither hook tree, and the caller read it as findings. The real command exits 0.
      //
      // The text is NOT surfaced to the caller. It still reaches telemetry, scrubbed and
      // truncated, so the failure stays debuggable without being readable as a result.
      //
      // The error string is load-bearing VERBATIM: tests/unit/research-agent-outage-fallthrough.test.mjs
      // matches it literally to assert classifyAgentError(...).type === 'capability', and
      // runner.mjs:173 consumes it. Only the payload SHAPE changes here.
      ...
      return redactValueSecretsOnly({
        ...
        // Machine-detectable: the caller branches on this instead of string-matching,
        // and there is no prose field to mistake for an answer.
        validated: false,
        unparsedLength: rawText.length,
        telemetryId,
      });
    }
  ...
  if (outputSchema) {
    try {
      ...
      // Sources-sanitization observability (backlog.fix.research-agent-sources-sanitized-emit):
      // if the schema recorded a sources[] drop during THIS parse, surface it as exactly one
      // agent.research.sources_sanitized event. Runs synchronously here (no await between parse
      // and consume) so parallel runs cannot clobber each other's record; consume-once clears it.
      // Optional-chained hook keeps the generic runner decoupled from research.mjs; wrapped
      // best-effort so a telemetry/hook fault never throws and never mutates `validated`.
      try {
        const _sanMeta = outputSchema?._consumeSanitizationMeta?.();
        if (_sanMeta && _sanMeta.dropped > 0) {
          emitTelemetry('sources_sanitized', { dropped: _sanMeta.dropped, kept: _sanMeta.kept });
        }
      } catch { /* best-effort: never break the validated result */ }
