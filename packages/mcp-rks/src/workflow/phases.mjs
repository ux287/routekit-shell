 ...
 *
 * Does the story's current PHASE permit this tool?
 *
 * This is the probe `classifyChainRefusal` needs. Without it that classifier is inert by
 * construction: its `phaseKnown` guard requires both a probe and a string phase, and with no
 * probe `phaseBlocks` is always false — so `blockedBy` can only ever report `'state'` and the
 * `'both'` / `wedged` case, which is the whole reason the classifier exists, is unreachable.
 *
 * Derived from OPERATION_TRANSITIONS rather than a hand-written map, so it cannot drift from
 * the phase machine the rest of the system obeys. Only tools that are genuinely phase-gated
 * are consulted; every other tool is phase-agnostic and must return true, or a chain refusal
 * would be blamed on the phase when the phase had nothing to do with it.
 *
 * @param {string} phase - the story's current frontmatter phase
 * @param {string} toolName
 * @returns {boolean} true when the phase permits the tool (or the tool is not phase-gated)
 */
export function phaseAllows(phase, toolName) {
  if (typeof phase !== "string" || phase === "") return true;

  // rks_exec is gated on an exact phase by exec.mjs; the rest read from the machine.
  if (toolName === "rks_exec") return phase === PHASE_GATE_EXEC;

  const op = TOOL_PHASE_OPERATIONS[toolName];
  if (!op) return true; // not phase-gated

  const from = OPERATION_TRANSITIONS[op]?.from;
  if (!Array.isArray(from)) return true; // unknown operation — do not invent a refusal
  return from.includes(phase);
}

/**
 * Tools whose legality depends on the story phase, mapped to the phase-machine operation that
 * governs them. Deliberately small: a tool listed here that is NOT actually phase-gated would
 * produce false `phase`-blamed refusals, which is worse than reporting `state` alone.
 */
export const TOOL_PHASE_OPERATIONS = {
  rks_plan: "plan",
};
