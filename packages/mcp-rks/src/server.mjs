...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  const tools = [
    ...
    },
    {
      ...
      description: "ABORT an off-rail session WITHOUT shipping. Discards ALL working-tree changes since the session started (hard reset to the session's start commit + clean of new untracked files) and restores the guardrails hooks — NO commit, branch, PR, merge, or push. Use this to throw away off-rail work instead of rks_guardrails_on (which always commits/ships). Gitignored paths (node_modules, .env, RAG index) are preserved.",
      inputSchema: {
        type: "object",
        properties: {
          projectId: { type: "string", description: "Project identifier from registry" },
    ...
    },
    {
      ...
      description: "Read a Dendron note's content (frontmatter + body). Callable directly with a governor token. Large notes: pass sections, fields, offset/limit or maxBytes for a bounded read; an unselected read over the byte cap returns a navigable manifest (frontmatter keys and headings) with truncated: true instead of overflowing.",
      inputSchema: {
        type: "object",
        properties: {
          filename: { type: "string" },
...
`;
    ...
    if (tool === "rks_guardrails_abort") {
      ...
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      const res = await guardrailsAbort(projectRoot, {}, input.projectId);
      return { content: [{ type: "text", text: JSON.stringify(res, null, 2) }] };
    }
    ...
    if (tool === ARCH_VERDICT_TOOL) {
      ...
      // Retain the WHOLE parse result, not just .data. The coalesced write below
      // uses formatWithFrontmatter, which writes only what it is handed — unlike
      // updateField/updateFieldDirect, which re-serialise the whole note. Keeping
      // only .data here would leave no body in scope at the write site and destroy
      // the note.
      const parsed = parseFrontmatter(readNoteRaw(notePath));
      ...
      // reviewable frontmatter fields. Nothing arch-owned, nothing time-varying, so
      // recording a verdict cannot invalidate the subject it was recorded against.
      ...
      try {
        writeNoteRaw(notePath, formatWithFrontmatter(nextFm, parsed?.content || ""), skipCommit ? {} : { skipEmbed: true });
      } catch (err) {
        return { content: [{ type: "text", text: JSON.stringify({
          ok: false,
          error: "write_failed",
