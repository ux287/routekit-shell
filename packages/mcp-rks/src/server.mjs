...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  const tools = [
    ...
    {
      name: "rks_fetch_raw",
      description: "Fetch a COMPLETE raw document from an ALLOWLISTED https URL (curl mode) — for reading a full external doc/spec when RAG snippets are not enough. Default-deny host allowlist (.rks/project.json fetchRaw.allowedHosts), HTTPS-only, SSRF-guarded, size-capped + timed out, and recorded in the write-ledger. Not a crawler; no auth; no JS render.",
      inputSchema: {
        type: "object",
        properties: {
          projectId: { type: "string", description: "Project identifier from registry" },
          url: { type: "string", description: "The https URL to fetch. Host must be in fetchRaw.allowedHosts." },
