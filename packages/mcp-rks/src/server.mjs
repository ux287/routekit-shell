...
const refineApplySchema = z.object({
  ...
  refinements: z.array(z.object({
    // 13 values, matching the advertised enum. z.enum REJECTS rather than strips,
    // so omitting fix_duplicate_frontmatter made the refine engine capable of
    // suggesting a refinement type its own tool's parser threw on.
    type: z.enum(["add_target_files", "add_code_snippet", "add_test_exemplar", "clarify_ac", "decompose", "fix_target_files", "add_test_requirements", "upgrade_target_files_format", "add_search_pattern", "create_file_directive", "acknowledge_multi_file", "acknowledge_destructive_rewrite", "fix_duplicate_frontmatter"]),
    data: z.any().optional(),
  })),
});
...
`;
    ...
    if (tool === "rks_refine_apply") {
      ...
      if (_governorToken && validateToken(_governorToken) && res.status === "refine_noop") {
        const resultTransition = advanceStateOnResult(_governorToken, 'refine_apply.noop');
        if (resultTransition?.transitioned) {
          res._governorState = { from: resultTransition.previousState, to: resultTransition.newState };
          try {
            getTelemetryCollector().emit('governor.state.transition', input.projectId, {
              tool, resultKey: 'refine_apply.noop',
              from: resultTransition.previousState,
              to: resultTransition.newState,
              trigger: 'refine_noop',
            });
