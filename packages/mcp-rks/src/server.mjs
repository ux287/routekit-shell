...
// Imported straight from the shared module: dendron.mjs re-exports only the
// format/parse pair, and the arch-verdict write needs the byte-stable split/join
// pair plus the bare YAML serializer.
import { splitNoteFrontmatter, joinNoteFrontmatter, serializeFrontmatterYaml } from "./shared/frontmatter.mjs";
...
import { runRagEmbed, runRagInit, runRagQuery, runRagCompact, ensureRagIndex, runExhaustiveSearch, resolveFidelityCeiling, setRagHostHooks } from "@routekit/rag";
import { addRagSourcedPath } from "./shared/session-state.mjs";
import { createCapabilityToken } from "@routekit/rag/capability-token";
import { AGENT_ROLES } from "@routekit/rag/capability-profiles";
import {
  runAnalyzeTool,
  runPlanTool,
...
import { reviewPlan, resolvedPlanIdentity, describePlanContents, isStatePreservingCall } from "./server/plan-quality.mjs";
import {
  runExecTool,
  runApplyTool,
  runExecAbortTool,
...
import { guardrailsOff, guardrailsOn, guardrailsAbort, getSessionHistory } from "./server/guardrails-audit.mjs";
...
import { advancePhase, repairPhaseToIntegrated } from "./workflow/auto-phase.mjs";
import { ensureExecStartPhase } from "./server/exec-start-durability.mjs";
import { commitAndEmbedNote } from "./shared/commit-and-embed-note.mjs";
import { runPreflight, hasPreflightChecks, LOADED_RKS_VERSION, readDiskRksVersion, checkGitReadiness, ragKeylessPreflightMessage } from "./server/preflight.mjs";
import { getTelemetryCollector } from "@routekit/telemetry/collector";
...
import { getAgent, getAgentByToolName, generateAgentToolDefinitions, listAgents, initProjectAgents, parseAgentInput } from "./agents/registry.mjs";
...
import { TOOL_NAME as ARCH_VERDICT_TOOL, TOOL_DESCRIPTION as ARCH_VERDICT_DESC, INPUT_SCHEMA as ARCH_VERDICT_SCHEMA, computeArchVerdict, subjectDigest, ARCH_MAX_ROUNDS } from "./workflow/arch-verdict.mjs";
import { classifyMarkerFailure, buildPlanReviewFailureResponse, decideToolOutcome, toolPayloadFromResponse, TOOL_FAILED } from "./server/failure-classification.mjs";
import { requireToken, isProtectedTool, unauthorizedResponse, validateToken, checkAllowedTool, touchSession, advanceState, revertStateOnFailure, advanceStateOnResult, getSession, getActiveChild, updateChildState, advanceToNextChild, setProjectRoot, detectOrphanedGuardrails, setPendingStash, clearPendingStash, createSession, flushPendingStashPops } from "./shared/governor-token.mjs";
import { sliceNote } from "./shared/note-slice.mjs";
 ...
 */
// Exported so the auto-route allowlist can be asserted by importing it rather than by
// regexing this file's source — same precedent as TOOL_ARG_SCHEMAS below, which is exported
// for tests/integration/mcp-schema-drift-guard.spec.mjs. Additive; no behaviour change.
export const TOOL_TO_AGENT_MAP = {
  ...
  // rks_stash is deliberately NOT mapped. Auto-routing it sent an untokened caller to the LLM
  // git agent, which creates stashes nothing restores — the agent cannot register a pending
  // stash because agent tools never receive a token (runner.mjs invokes tool.execute(input)
  // with arguments only), so endSession's auto-pop never fires for one. Flattening `action`
  // and `includeUntracked` into prose for the model to re-parse made it worse. Untokened
  // callers now get unauthorizedResponse('rks_stash') instead: an
  // honest refusal; a tokened caller with a LIVE Governor session reaches the inline handler via
  // STATE_BYPASS_TOOLS (governor-state.mjs:847), which is consulted inside checkStateAllowed —
  // i.e. AFTER checkAllowedTool's session lookup. Deliberately NOT COMMON_TOOLS, which returns at
  // governor-token.mjs:557 before that lookup and would make setPendingStash a silent no-op.
  ...
  // rks_plan, rks_plan_review, rks_plan_ready are unprotected (direct execution by thin Governor)
      ...
      // ── Research tools: DELIBERATELY NOT ROUTED ──
      //
      // backlog.fix.autoroute-substitutes-llm-for-deterministic-retrieval
      //
      // `rks_rag_query`, `rks_kg_query` and `rks_exhaustive_search` used to be mapped to the
      // research agent here. An unmapped tool falls through to a real refusal (see the
      // `if (autoRouted)` check at the call site), which is the correct behaviour and the
      // reason this is a deletion rather than a rewrite.
      //
      // The original rationale was that an unauthorized call should "redirect to the evidence
      // layer (RAG) just as raw Grep is redirected". That analogy holds for LOCATING and breaks
      // for RETRIEVAL. Redirecting Grep to RAG substitutes one way of finding a file for
      // another. Substituting an LLM for `rks_exhaustive_search` substitutes invented text for
      // verbatim text — the research agent's output contract carries no line column at all
      // (`agents/research.mjs`, `sources: [{ file, snippet? }]`), so every file:line it returns
      // on this path is fabricated by construction. Its `buildInput` also dropped `path`,
      // `maxResults` and `countOnly`, turning a bounded scoped search into an unbounded
      // semantic question.
      //
      // Reported from the field on v0.43.0: a symbol reported absent that exists, a return
      // statement placed 90 lines off, a file reported nonexistent, and an invented CLI
      // subcommand — each returned as `ok: true` with only an appended `_autoRouted` field to
      // distinguish it. A governor takes "no such symbol" as proven absence and authors the
      // wrong anchor into a story. A refusal is recoverable; a confident wrong answer is not.
      //
      // Note `rks_rag_query` is in UNPROTECTED_TOOLS, so its entry was already unreachable on
      // this path. It is removed anyway: leaving dead misleading routing arms a trap the day
      // the tool becomes protected.
...
async function autoRouteUnauthorized(tool, cleanArgs) {
  ...
  try {
    ...
    // A PROJECT THAT DID NOT RESOLVE IS NOT AN AUTHORIZATION REFUSAL.
    //
    // loadContext throws four distinct resolution failures, this blanket catch
    // swallowed all of them, and the caller fell through to unauthorizedResponse
    // — so a missing registry entry, a missing project.json and a missing KG file
    // were all reported as "unauthorized". Telling them apart required moving
    // projects/index.jsonl aside and re-running, by hand, every time.
    //
    // SUBSTRING, not equality: project.mjs re-wraps this in an McpError, which
    // PREFIXES the message, so the producer's literals survive only as
    // substrings. An equality or startsWith match would silently regress every
    // resolution failure straight back to unauthorized.
    const resolution = classifyResolutionFailure(err);
    if (resolution) {
      // GUARDED. `recordIntervention` never throws, but resolving the root can —
      // plausibly for the SAME reason the resolution failed in the first place,
      // which is exactly when this branch runs. Unguarded, the receipt would
      // escape this catch and convert a graceful refusal into an unhandled
      // rejection: instrumentation crashing the path it exists to explain.
      let intervention = null;
      try {
        intervention = recordIntervention(resolveProjectRoot(), {
        ...
        // Report the failure as a receipt rather than losing it silently — the
        // operator still learns the refusal was a resolution failure.
        intervention = {
          kind: 'project_unresolved',
          cause: resolution,
          projectId,
          recorded: false,
          recordPath: null,
          writeError: receiptErr?.message || String(receiptErr),
        };
      }
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            // Deliberately NO repoRoot and NO registry path: loadProjectContext
            // resolves the registry inside loadProjects and never binds either as
            // a local, so this catch cannot observe them. Recomputing one would
            // report a path the producer never confirmed reading.
            projectId,
...
/**
 * The four resolution failures loadProjectContext throws, matched by SUBSTRING.
 *
 * Returns a stable cause token, or null for anything that is not one of them —
 * so an unrecognised error still falls through to the authorization refusal and
 * the classifier cannot over-claim.
 */
export function classifyResolutionFailure(err) {
  const message = err?.message || String(err || '');
  if (message.includes('Project not found:')) return 'project_not_in_registry';
  if (message.includes('missing root in registry')) return 'registry_entry_missing_root';
  if (message.includes('Missing .rks/project.json or routekit/project.json')) return 'project_json_missing';
  if (message.includes('KG file not found for')) return 'kg_file_missing';
  return null;
}
...
async function commitDendronWriteResult({ tool, innerResult, skipCommit, extraStagePaths }) {
  ...
  try {
    ...
    const commitResult = await commitAndEmbedNote({
      ...
      commitMessage,
      // Forwarded so the SCOPED commit covers a rename's old and new path. These were
      // staged above but invisible downstream, where both the pre-check and the commit
      // saw only the note path.
      extraStagePaths,
    });
    ...
    // Read back from git, not echoed from intent — a caller can tell from the envelope
    // alone whether the commit was actually scoped to this tool's write set.
    if (commitResult.committedPaths) merged.committedPaths = commitResult.committedPaths;
...
import { runRefineTool, runRefineApplyTool, REFINEMENT_TYPES } from "./server/refine.mjs";
...
const execSchema = z.object({
  ...
  expectedAnchor: z.string().optional(),
});
...
const cycleCompleteSchema = z.object({
  projectId: z.string(),
  // backlog.fix.cycle-complete-ungated-hard-reset: opt-in for the destructive
  // `git reset --hard origin/<working>`. OPTIONAL by design — absent means refuse, mirroring
  // rks_branch_repair's `confirm`. Must stay out of required[]; adding it there would make every
  // caller state a destructive intent they usually do not have.
  // NOTE: the advertised inputSchema for rks_cycle_complete must carry the identical optionality —
  // tests/integration/mcp-schema-drift-guard.spec.mjs diffs the two bidirectionally.
...
const rksInitSchema = z.object({
  ...
  // Advertised since inception but missing here, so z.object stripped it and the
  // read below always saw undefined — every caller asking for 2-branch silently
  // received 3-branch. Adding it CHANGES BEHAVIOUR for those callers, on purpose.
  branchModel: z.enum(["2-branch", "3-branch"]).optional(),
});
const ragInitSchema = z.object({ projectId: z.string() });
// `files` is advertised, and the tool description promises it for incremental
// updates; without it here the array was stripped before reaching the handler.
...
const refineApplySchema = z.object({
  ...
  refinements: z.array(z.object({
    // backlog.fix.refine-noop-escalation-false-positive: derived from REFINEMENT_TYPES rather
    // than hand-written. z.enum REJECTS rather than strips, so every emitted-but-unlisted type
    // made the refine engine capable of suggesting a refinement type its own tool's parser threw
    // on. Hand-listing 13 values left 8 emitters unaccepted; deriving from the emitter set is
    // what keeps the two from drifting apart again.
    //
    // NOTE: this must stay a ZodObject carrying a `type` ZodEnum —
    // tests/integration/mcp-schema-drift-guard.spec.mjs reaches structurally into
    // `.shape.refinements._def.type.shape.type._def.values`. `.passthrough()` preserves that
    // path; z.record()/z.any() would break it.
    type: z.enum(REFINEMENT_TYPES),
    data: z.any().optional(),
  // .passthrough() because `rks_refine` puts its payload at the TOP LEVEL of each suggestion
  // (`file`, `files`, `hint`, `reason`, `priority`), not under `data`. The default strip deleted
  // exactly those keys, so handing a suggestion straight back delivered `{ type }` alone and all
  // eight `data?.file` handlers in refine.mjs read undefined — the refinement was dropped before
  // any handler could see it, and the resulting empty `applied` was reported as a genuine no-op.
  }).passthrough()),
});
...
const governorInitSchema = z.object({
  ...
  // REQUIRED, not cosmetic: this is a plain z.object(), so parse() STRIPS unknown keys and a
  // reset declared only in INPUT_SCHEMA would never reach handleGovernorInit. The two
  // declarations must also stay in step for the mcp-schema-drift-guard comparison.
  reset: z.boolean().optional(),
});

// Kept in step with INPUT_SCHEMA in workflow/arch-verdict.mjs — the drift guard
// (tests/integration/mcp-schema-drift-guard.spec.mjs) compares key presence and
// required-ness between the advertised schema and this one.
const archVerdictSchema = z.object({
  projectId: z.string(),
  storyId: z.string(),
  findings: z.array(
    z.object({
      // Typed as a number deliberately: findingKey slugifies, so a non-numeric
      // item could not corrupt the key format — but typing it here makes the
      // key's shape a parse-time guarantee rather than a downstream rescue.
      item: z.number(),
      file: z.string(),
...
const guardrailsOnSchema = z.object({
  projectId: z.string(),
  // Per-invocation escape from an `offRail: block` enforcement halt. The reason
  // is REQUIRED when overriding and is recorded in shipSteps and telemetry —
  // an override that leaves no trace would be a silent bypass, which is the
  // defect the gate exists to remove.
  enforcementOverride: z
    .object({
      enabled: z.literal(true),
      reason: z.string().min(1, "an enforcement override must state a reason"),
    })
    .optional(),
});
const guardrailsAbortSchema = z.object({
  projectId: z.string(),
  discardPreExisting: z.boolean().optional(),
});
const guardrailsStatusSchema = z.object({
  projectId: z.string(),
  limit: z.number().int().positive().optional(),
});
const phaseRepairSchema = z.object({
  projectId: z.string(),
  storyId: z.string(),
});
...
const updateFieldSchema = z.object({ filename: z.string(), field: z.string(), value: z.union([z.string(), z.array(z.unknown())]), acknowledgeRemoval: z.boolean().optional() });
const markImplementedSchema = z.object({ filename: z.string(), commitId: z.string().optional() });

/**
 * The missing link between a tool's NAME and the zod schema that actually parses
 * its arguments.
 *
 * Every tool here carries two independent hand-maintained descriptions of one
 * contract: the zod schema below, which PARSES arguments, and a hand-written
 * JSON Schema `inputSchema` literal in the ListTools handler, which tells clients
 * what to SEND. Nothing linked them, so they drifted silently — and the failure
 * is asymmetric, which is how it survived ~80 tools. A new tool missing its
 * advertised half is uncallable and gets caught immediately; a field added to an
 * existing tool's zod schema is merely unreachable, forever and quietly.
 *
 * These are REFERENCES to the same const bindings the CallTool branches parse
 * with — never copies and never re-declarations — so this map cannot describe a
 * schema different from the one actually enforced. The one remaining rot vector
 * is forgetting to register a NEW tool, which the drift guard turns into a test
 * failure rather than a silent pass (see tests/integration/mcp-schema-drift-guard.spec.mjs).
 *
 * Deliberately excluded: tools whose zod schema exists but whose tool is not
 * advertised at all (rks_telemetry_analysis, rks_ready), since every key here
 * must appear in live listTools() output.
 */
export const TOOL_ARG_SCHEMAS = Object.freeze({
  rks_init: rksInitSchema,
  rks_project_init: projectInitSchema,
  rks_project_get: projectGetSchema,
  rks_kg_query: kgQuerySchema,
  rks_analyze: analyzeSchema,
  rks_plan: planSchema,
  rks_plan_review: planReviewSchema,
  rks_plan_ready: planReadySchema,
  rks_validate_story: validateStorySchema,
  rks_exec: execSchema,
  rks_apply: applySchema,
  rks_refine: refineSchema,
  rks_refine_apply: refineApplySchema,
  rks_templates_list: templatesListSchema,
  rks_stash: stashSchema,
  rks_restore: restoreSchema,
  rks_cycle_complete: cycleCompleteSchema,
  rks_promote: promoteSchema,
  rks_ship: shipSchema,
  rks_reset: resetSchema,
  rks_revert: revertSchema,
  rks_tag: tagSchema,
  rks_cherry_pick: cherryPickSchema,
  rks_publish: publishSchema,
  rks_publish_profiles: publishProfilesSchema,
  rks_rag_init: ragInitSchema,
  rks_rag_embed: ragEmbedSchema,
  rks_rag_query: ragQuerySchema,
  rks_rag_compact: ragCompactSchema,
  rks_git_state: gitStateSchema,
  rks_git_branch: gitBranchSchema,
  rks_git_commit: gitCommitSchema,
  rks_git_push: gitPushSchema,
  rks_git_merge: gitMergeSchema,
  rks_checkout: checkoutSchema,
  rks_branch_repair: branchRepairSchema,
  rks_staging_pr: stagingPrSchema,
  rks_staging_merge: stagingMergeSchema,
  rks_telemetry_query: telemetryQuerySchema,
  rks_telemetry_report: telemetryReportSchema,
  rks_telemetry_export: telemetryExportSchema,
  rks_token_cost_report: tokenCostReportSchema,
  rks_fetch_raw: fetchRawSchema,
  rks_onboarder: onboarderSchema,
  rks_interview: interviewSchema,
  rks_guardrails_off: guardrailsOffSchema,
  rks_guardrails_on: guardrailsOnSchema,
  rks_guardrails_abort: guardrailsAbortSchema,
  rks_guardrails_status: guardrailsStatusSchema,
  rks_phase_repair: phaseRepairSchema,
  rks_guardrails_simulate: guardrailSimSchema,
  [GOVERNOR_INIT_TOOL]: governorInitSchema,
  [ARCH_VERDICT_TOOL]: archVerdictSchema,
  dendron_create_note: createNoteSchema,
  dendron_edit_note: editNoteSchema,
  dendron_update_field: updateFieldSchema,
  dendron_fix_frontmatter: fixFrontmatterSchema,
  dendron_validate_schema: validateSchemaSchema,
  dendron_mark_implemented: markImplementedSchema,
});
  ...
  const tools = [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          expectedAnchor: { type: "string", description: "Git anchor (@sha or @sha+dirty, as returned by rks_exhaustive_search) asserting which working tree was verified. Refuses with status anchor_mismatch on a different commit, or when a clean tree was asserted and the tree is dirty. A +dirty expectation proceeds as anchorMatch dirty-weak — two dirty trees at one HEAD are not the same tree, so it is a weaker guarantee than exact; a +dirty expectation against a clean tree is also dirty-weak, since the expectation itself was stated loosely. The leading @ is optional. If the working-tree anchor cannot be computed at all, the call refuses rather than proceeding unverified." },
        },
        required: ["projectId"],
      },
    },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          projectId: { type: "string" },
          pattern: { type: "string", description: "Literal string to search for. Matching is a CASE-SENSITIVE substring test, so a zero result may mean the pattern differs in case from the source rather than that the content is absent." },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          enforcementOverride: {
            type: "object",
            description: "Deliberately release past a blocking CI verdict. Requires a reason — an override that leaves no trace is indistinguishable from the fail-open this gate exists to prevent.",
            properties: {
              enabled: { type: "boolean", enum: [true], description: "Must be true" },
              reason: { type: "string", description: "Why the CI gate is being overridden (non-empty; recorded in the response and in telemetry)" },
            },
    ...
    },
    {
      ...
      description: "Complete feature→working branch cycle. Syncs with origin (unless local-only), deletes feature branch. Uses branch config. Refuses if the working branch has unpushed local commits unless discardLocalCommits is true.",
      inputSchema: {
        type: "object",
        properties: {
          projectId: { type: "string", description: "Project identifier from registry" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          since: { type: "string", description: "Relative lookback window (e.g. '7d', '24h')" },
          lastNCycles: { type: "number", description: "Restrict to the last N completed cycles" },
        },
        required: ["projectId"],
      },
    },
        ...
        properties: {
          ...
          refinements: {
            ...
            items: {
              type: "object",
              properties: {
                // backlog.fix.refine-noop-escalation-false-positive: derived from the same
                // REFINEMENT_TYPES constant as the zod enum above, so the advertised schema
                // and the parser cannot drift apart.
                type: { type: "string", enum: [...REFINEMENT_TYPES] },
                data: { type: "object", description: "Refinement-specific data" },
              },
              // Suggestions carry their payload at the top level, so the advertised schema must
              // not claim to be closed. Mirrors .passthrough() on the zod side.
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          skipStage: { type: "boolean", description: "Skip the current stage and advance to the next one" },
          bounce: { type: "boolean", description: "Re-run the current stage instead of advancing" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          enforcementOverride: {
            type: "object",
            description: "Proceed past an enforcement-gate halt. The reason is REQUIRED and is recorded in shipSteps and telemetry — an override that leaves no trace would be a silent bypass.",
            properties: {
              enabled: { type: "boolean", enum: [true], description: "Must be true" },
              reason: { type: "string", description: "Why the halt is being overridden (non-empty)" },
            },
            required: ["enabled", "reason"],
          },
        },
    ...
    {
      name: "rks_guardrails_abort",
      description: "ABORT an off-rail session WITHOUT shipping: hard reset to the session's start commit + clean of new untracked files, hooks restored, NO commit, branch, PR, merge, or push. REFUSES rather than discarding when the working tree was ALREADY DIRTY when the session opened — the reset targets a COMMIT, so it would also destroy uncommitted work that predated the session and was never this session's. On refusal it returns ok:false with the pre-existing paths, changes nothing, and leaves the session active; pass discardPreExisting: true to destroy them deliberately. Use this to throw away off-rail work instead of rks_guardrails_on (which always commits/ships). Gitignored paths (node_modules, .env, RAG index) are preserved.",
      inputSchema: {
        type: "object",
        properties: {
          projectId: { type: "string", description: "Project identifier from registry" },
          discardPreExisting: { type: "boolean", description: "Explicit opt-in to ALSO discard uncommitted work that already existed when the session opened. Absent or false, such work causes a refusal and nothing is discarded." },
        },
        required: ["projectId"],
      },
    },
    ...
    {
      name: "rks_phase_repair",
      description:
        "Advance a story to 'integrated' after an off-rail ship whose phase advance was suppressed by the review gate. EVIDENCE-BOUND: refuses unless a commit carrying the story's 'Story:' trailer is reachable from the integration branch, and refuses on an unreadable git history rather than treating that as absence. Reports an already-advanced story as a no-op, never as a repair.",
      inputSchema: {
        type: "object",
        properties: {
          projectId: { type: "string", description: "Project identifier from registry" },
          storyId: { type: "string", description: "Story note filename, e.g. backlog.fix.my-story" },
        },
    ...
    },
    {
      ...
      description: "Read a Dendron note's content (frontmatter + body). Callable directly with a governor token. Large notes: pass sections, fields, offset/limit or maxBytes for a bounded read; an unselected read over the byte cap returns a navigable manifest (frontmatter keys and headings) with truncated: true instead of overflowing. TWO DIFFERENT COMPLETENESS FIELDS: truncated means only that the response is not the whole note, so it is true for every selection by construction and answers nothing about your selection; selectionComplete is the field that answers whether what you asked for came back whole. Read selectionComplete — never truncated — before rewriting a field from a slice, because a whole-array write built from a partial read silently destroys the entries you did not see. fieldsNotFound lists any requested field names the note does not carry.",
      inputSchema: {
        type: "object",
        properties: {
          filename: { type: "string" },
          sections: { type: "array", items: { type: "string" }, description: "Body headings to return, each running to the next heading" },
          fields: { type: "array", items: { type: "string" }, description: "Top-level frontmatter keys to return, with their indented blocks" },
          offset: { type: "number", description: "0-based first line, when slicing by line" },
          limit: { type: "number", description: "Line count, when slicing by line" },
          maxBytes: { type: "number", description: "Byte cap on returned content; requests a bounded read even when the note fits" },
        },
        required: ["filename"],
      },
    },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          acknowledgeRemoval: {
            type: "boolean",
            description:
              "Permit a write that REMOVES entries from an array field (targetFiles, dependsOn, testFiles, testRequirements). Such a write is refused by default, because the usual cause is a caller rewriting the whole array from a partial read. Pass true only when the removal is intended; the response reports beforeCount, afterCount and removed either way.",
          },
        },
    ...
    {
      name: ARCH_VERDICT_TOOL,
      description: ARCH_VERDICT_DESC,
      inputSchema: ARCH_VERDICT_SCHEMA,
    },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  // backlog.fix.governor-phase-state-desync-and-recovery: the entry transition this call
  // committed, held so the catch below can hand it back if the tool failed.
  let _entryTransition = null;
  let _entryTransitionToken = null;
  ...
  try {
    // The tool body is wrapped so its RETURN VALUE is observable before the finally
    // emits. Previously `_auditOk` flipped false only in the catch, so a handler that
    // returned an ok:false payload WITHOUT throwing was recorded as mcp.tool.complete
    // and the failure went uncounted. Wrapping is used rather than touching 92 return
    // sites; every `return` below still returns the same value, now via the IIFE.
    // Outer `let`s (_entryTransitionToken, _entryTransition) stay mutable through the
    // closure, so the catch and finally observe them exactly as before.
    ...
    if (activeToken && validateToken(activeToken)) {
      ...
      //
      // backlog.fix.planned-state-plan-contents-disclosure: a DRY RUN must not move the chain.
      // Entry transitions are keyed on tool name and fire before the tool body, so they cannot
      // see `dryRun` — which meant inspecting a plan with rks_exec dryRun demoted `planned` to
      // `executing` (4 tools, no search, no research) while doing no work at all. Strictly
      // worse than the state you started in, for a call whose whole purpose is to look.
      const stateTransition = isStatePreservingCall(tool, cleanArgs)
        ? null
        : advanceState(activeToken, tool);
      // backlog.fix.governor-phase-state-desync-and-recovery: remembered so a FAILED call can
      // give the transition back. Entry transitions gate in-flight calls correctly, but a call
      // that errored did no work and must not leave the chain moved — that is how `planned`
      // silently became `refining` (where rks_exec is forbidden) on a failed research call.
      _entryTransition = stateTransition;
      _entryTransitionToken = activeToken;
    ...
    if (tool === "rks_plan") {
      ...
      // Capture the worker's stderr. It was `'ignore'`, so every `[planner]` and
      // `[plan-worker]` diagnostic was discarded by the OS — including the one line
      // that would have explained a silent phase-write failure. That turned a
      // deterministic bug into a multi-session investigation.
      //
      // The run folder does not exist yet at spawn time, so the log is keyed by
      // startedAt under a stable directory. Failure to open it must NEVER stop a
      // plan: fall back to 'ignore' and carry on.
      const workerLogPath = path.join(ctx.record.root, '.rks', 'plan-logs', `${startedAt}.log`);
      let workerLogFd = null;
      try {
        fs.mkdirSync(path.dirname(workerLogPath), { recursive: true });
        workerLogFd = fs.openSync(workerLogPath, 'a');
      } catch (err) {
        console.error(`[rks.plan] worker log unavailable (${err?.message}); continuing without capture`);
        workerLogFd = null;
      }

      const child = spawn('node', [workerScript, paramsFile], {
        detached: true,
        stdio: ['ignore', 'ignore', workerLogFd ?? 'ignore'],
        env: { ...process.env },
        cwd: ctx.record.root,
      });
      // The child holds its own dup of the fd; the parent's copy is dead weight.
      if (workerLogFd !== null) { try { fs.closeSync(workerLogFd); } catch { /* best-effort */ } }
      ...
      writePendingPlanMarker(ctx.record.root, {
        ...
        startedAt, pid: child.pid, workerLogPath: workerLogFd !== null ? workerLogPath : null,
      });
    ...
    if (tool === "rks_plan_review") {
        ...
        try {
          ...
          if (marker && marker.projectId === input.projectId) {
            ...
            if (marker.done === true && marker.ok === true) {
              // PARENT-SIDE DURABILITY NET. The planner runs in a DETACHED worker whose
              // stderr we only started capturing today; if its arch-approved → executing
              // write failed there, nothing surfaced it — rks_exec then refused the story
              // forever, and rks_plan_ready's stale-executing self-heal undid any hand-set
              // value. Land the transition here, in the server's own known-good context,
              // and VERIFY it on disk rather than trusting a return value.
              // Idempotent: a worker that already landed it makes this a no-op.
              if (marker.problemId) {
                const phaseRes = await ensureExecStartPhase({
                  projectRoot: ctx.record.root,
                  problemId: marker.problemId,
                  projectId: input.projectId,
                  allowNoopWhenExecuting: true,
                });
                if (!phaseRes.ok) {
                  ...
                  return { content: [{ type: "text", text: JSON.stringify({
                    ...
                    problemId: marker.problemId,
                    phaseWrite: {
                      attempted: phaseRes.attempted, from: phaseRes.from ?? null,
                      to: phaseRes.to, observed: phaseRes.observed ?? null, error: phaseRes.error,
                    },
                    workerPhaseWrite: marker.phaseWrite ?? null,
                    workerLogPath: marker.workerLogPath ?? null,
                    message: `Plan completed and is on disk, but the story phase could not be advanced to 'executing': ${phaseRes.error}. rks_exec will refuse this story until that is resolved.`,
                  }, null, 2) }] };
                }
              }
              ...
              // SSOT: the classifier and its operator message live in
              // server/failure-classification.mjs. This site previously re-derived the class
              // inline AND had no `structural` message branch, so a structural marker — which
              // DOES carry a stamped class — fell through to the worker-crashed message and
              // told the operator to re-run a plan that deterministically cannot succeed.
              // The payload (incl. uncoveredTargets / droppedSteps / workerLogPath and a
              // reason-matched message) is built by a pure helper so it is testable.
              const failureResponse = buildPlanReviewFailureResponse(marker, elapsed, classifyMarkerFailure(marker));
      ...
      // backlog.fix.planned-state-plan-contents-disclosure: say WHICH plan this is and WHAT it
      // will do. Strictly additive — no existing field is changed — and attached regardless of
      // review.ok, because a caller debugging a failing review needs the identity just as much.
      review.resolved = resolvedPlanIdentity({
        planPath, runDir, slug,
        problemId: input.problemId || null,
        identifiedBy: input.label ? 'label' : (input.problemId ? 'problemId' : 'latest'),
      });
      review.plan = describePlanContents(plan);
    ...
    if (tool === "rks_preflight") {
      ...
      // Check 4: API key. When the RAG index is present but no key is set, retrieval is still
      // fully usable (keyless local embedder + LanceDB) — only answer synthesis and governed
      // workflows need the key. Say so via the shared keyless-messaging seam, rather than the bare
      // "set your key" hint implying RAG is dead. (Decision is pure/unit-drivable in preflight.mjs.)
      const hasApiKey = !!process.env.ANTHROPIC_API_KEY;
      const keyless = ragKeylessPreflightMessage({ ragInitialized: !!ragExists, hasApiKey });
      checks.push({
        name: "api_key",
        passed: hasApiKey,
        detail: keyless.message,
        hint: hasApiKey ? null : (keyless.hint || "Set ANTHROPIC_API_KEY in .env")
      });
      ...
      // backlog.fix.clean-machine-honesty: report the version we are RUNNING (module-load), beside the
      // version on DISK. This line used to call readRksVersion(), which re-read package.json — so it
      // announced the disk version next to a checks array produced by whatever code Node had loaded
      // at startup. After a `git checkout <newtag>` without a server restart, those are different
      // builds, and the field was confidently naming the wrong one.
      return { content: [{ type: "text", text: JSON.stringify({ ok: allPassed, rksVersion: LOADED_RKS_VERSION, diskRksVersion: readDiskRksVersion(), checks, workflowInfo }, null, 2) }] };
    }
    ...
    if (tool === "rks_exec") {
      ...
      //
      // backlog.fix.planned-state-plan-contents-disclosure: a DRY RUN must not move the chain
      // on this path either. A dry run returns ok:true, so without this guard resultKey is
      // 'exec.ok' and the session advances exactly as if the plan had been applied — leaving
      // the entry-transition guard above a half-fix that silently did nothing.
      if (_governorToken && validateToken(_governorToken) && !isStatePreservingCall(tool, cleanArgs)) {
    ...
    if (tool === "rks_cycle_complete") {
      ...
      const res = await runCycleComplete({
        projectRoot: context.record.root,
        projectId: input.projectId,
        // backlog.fix.cycle-complete-ungated-hard-reset: pass the parsed opt-in through.
        // `context.record.root` is the developer's real repository root — this handler needs no
        // misconfiguration to reach the destructive reset, which is why the gate exists.
        discardLocalCommits: input.discardLocalCommits === true
      });
...
`;
    ...
    if (tool === "rks_rag_embed") {
      ...
      const result = await runRagEmbed(projectRoot, { glob: input.glob, files: input.files });
      const payload = {
        ok: result?.ok ?? false,
        projectId: input.projectId,
        projectRoot,
        glob: input.glob || null,
        // ECHOED so a caller can confirm the request arrived intact. Its absence is what
        // made `indexed: 4050` unreadable — there was nothing in the response tying the
        // number to the one file that had been asked for.
        files: input.files ?? null,
        // UNCHANGED MEANING. Consumers read this as the whole-corpus row count, and
        // tools.mjs treats indexed === 0 as its zero-work predicate, so it keeps both.
        indexed: result?.indexed,
        // The same total under a name that says what it counts, so no field named for a
        // run result holds a corpus figure.
        corpusRowTotal: result?.corpusRowTotal,
        // PER-RUN work, which the embed layer already computed and the projection discarded.
        processedNotes: result?.processedNotes,
        embeddedNotes: result?.embeddedNotes,
        skippedNotes: result?.skippedNotes,
        skippedCodeFiles: result?.skippedCodeFiles,
        // PER-FILE outcomes, each written by the code path that handled that file. This is
        // what lets a caller answer "was MY file embedded?" without a second query.
    ...
    if (tool === "rks_rag_query") {
      ...
      const payload = {
        ...
        fidelityApplied: capabilityToken
          ? `L${capabilityToken.maxFidelity}`
          : (resolveFidelityCeiling(projectRoot) === "full" ? "L3 (owned corpus)" : "L2 (default)"),
        hint: result?.error,
      };
      return {
        content: [
    ...
    if (tool === "rks_refine_apply") {
      ...
      // backlog.fix.build-governor-self-heal: EMIT THE ESCALATION.
      //
      // `advanceStateOnResult` was reachable ONLY on `res.decomposed`, so a refine that changed
      // nothing never emitted a result key at all — and the state machine's `refine_apply.noop`
      // entry would have been unreachable dead code no matter where we put it. The signal has to
      // actually be sent, or the Governor stays in `refining` and re-plans forever.
      // backlog.fix.refine-inapplicable-status: a DISTINCT key for the new status.
      // Emitting 'refine_apply.noop' for it would increment the consecutive-no-op
      // streak; emitting nothing at all would leave the streak neither incremented
      // NOR cleared, so a later genuine no-op would still count as the second. The
      // reset branch in governor-token.mjs fires on any key that is not
      // 'refine_apply.noop', which is what makes a distinct key sufficient here.
      if (_governorToken && validateToken(_governorToken)
          && (res.status === "refine_noop" || res.status === "refine_inapplicable")) {
        const resultKeyForStatus = res.status === "refine_inapplicable"
          ? 'refine_apply.inapplicable'
          : 'refine_apply.noop';
        const resultTransition = advanceStateOnResult(_governorToken, resultKeyForStatus);
        // backlog.fix.refine-noop-escalation-false-positive: this used to be gated on
        // `resultTransition?.transitioned`, which suppressed BOTH the response field and the
        // telemetry emit whenever state did not change. A first no-op now deliberately SELF-LOOPS
        // (refining -> refining), so `transitioned` is false on exactly the case a caller most
        // needs to see — the counter ticked, the run continues, and nothing was reported. Report
        // on any non-null result instead; `transitioned` is still surfaced so a caller can tell a
        // self-loop from a real move. Scoped to this block only: the other advanceStateOnResult
        // call sites in this file keep their original guard.
        if (resultTransition) {
          res._governorState = {
            from: resultTransition.previousState,
            to: resultTransition.newState,
            transitioned: resultTransition.transitioned,
            // The key that actually drove the table lookup. On a repeat this is
            // 'refine_apply.noop_repeated', so the response cannot claim the wrong key.
            resultKey: resultTransition.effectiveResultKey,
          };
          try {
            getTelemetryCollector().emit('governor.state.transition', input.projectId, {
              tool, resultKey: resultTransition.effectiveResultKey,
              from: resultTransition.previousState,
              to: resultTransition.newState,
              transitioned: resultTransition.transitioned,
              trigger: res.status,
    ...
    if (tool === "rks_release") {
      const input = z.object({
        projectId: z.string(),
        version: z.enum(["patch", "minor", "major"]).optional(),
        changelog: z.string().optional(),
        // enabled is z.literal(true) so the override cannot default on; reason
        // is min(1) so a traceless override is rejected rather than accepted.
        // Same shape as guardrailsOnSchema's enforcementOverride.
        enforcementOverride: z.object({
          enabled: z.literal(true),
          reason: z.string().trim().min(1, "an enforcement override must state a reason"),
        }).optional(),
      }).parse(cleanArgs);
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      const res = await runRelease({ projectRoot, version: input.version || "patch", changelog: input.changelog, projectId: input.projectId, projectRecord: context.record, projectJson: context.projectJson, enforcementOverride: input.enforcementOverride || null });
      return { content: [{ type: "text", text: JSON.stringify(res, null, 2) }] };
    }
    if (tool === "rks_telemetry_query") {
      ...
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      const { queryTelemetry } = await import("@routekit/telemetry/query");
    ...
    if (tool === "rks_telemetry_report") {
      ...
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      const { generateReport } = await import("@routekit/telemetry/reports");
    ...
    if (tool === "rks_telemetry_export") {
      ...
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      const { exportTelemetry } = await import("@routekit/telemetry/export");
    ...
    if (tool === "rks_guardrails_on") {
      ...
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      const res = await guardrailsOn(
        projectRoot,
        input.enforcementOverride ? { enforcementOverride: input.enforcementOverride } : {},
        input.projectId,
      );
      return { content: [{ type: "text", text: JSON.stringify(res, null, 2) }] };
    }

    if (tool === "rks_guardrails_abort") {
      const input = guardrailsAbortSchema.parse(cleanArgs);
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      const res = await guardrailsAbort(projectRoot, { discardPreExisting: input.discardPreExisting === true }, input.projectId);
      return { content: [{ type: "text", text: JSON.stringify(res, null, 2) }] };
    }
    ...
    if (tool === "rks_phase_repair") {
      const input = phaseRepairSchema.parse(cleanArgs);
      const context = await loadContext(input.projectId);
      const projectRoot = context.record.root;
      // The integration branch is where a shipped story's commit must be reachable FROM. Taken
      // from branch topology rather than assumed, so a 2-branch project and a 3-branch project
      // are both answered against their own integration branch.
      const branchConfig = getBranchConfig(context.record, context.projectJson);
      const res = await repairPhaseToIntegrated(projectRoot, input.storyId, input.projectId, {
        integrationBranch: branchConfig.integration,
      });
      return { content: [{ type: "text", text: JSON.stringify(res, null, 2) }] };
    }
    ...
    if (tool === "dendron_read_note") {
      ...
      try {
        ...
        const raw = readNoteRaw(notePath);
        const sliced = sliceNote(raw, args || {});
        // BYTE-IDENTICAL ON THE UNBOUNDED PATH. An under-cap read with no
        // selector returns exactly { ok, filename, content } as it always has —
        // this tool is the only route a Governor has to its own story note, and
        // every existing caller parses that shape. The size report is attached
        // only when the caller asked for a bounded read; on the unbounded path
        // `truncated` is always false and returnedBytes always equals totalBytes,
        // so reporting them there would be a value fixed by which branch ran
        // rather than by an observation.
        if (!sliced.bounded && !sliced.truncated) {
        ...
        const payload = {
          ...
          returnedBytes: sliced.returnedBytes,
          // `truncated` is true for EVERY selection by construction, so it cannot tell a
          // caller whether what it asked for came back whole. These three can. A caller
          // that rebuilds an array from a slice must read selectionComplete, not truncated.
          selectionBytes: sliced.selectionBytes,
          selectionComplete: sliced.selectionComplete,
          fieldsNotFound: sliced.fieldsNotFound,
        };
        // Over cap with nothing named: a navigable manifest and NO content key,
        // so the first call always succeeds and names what can be asked for next.
        if (sliced.manifest) payload.manifest = sliced.manifest;
        else payload.content = sliced.content;
    ...
    if (tool === "dendron_update_field") {
      ...
      if (!fs.existsSync(notePath)) throw new McpError(ErrorCode.InvalidParams, `Note not found`);
      const skipCommit = cleanArgs.skipCommit === true;
      // acknowledgeRemoval is threaded from the PARSED input, not from cleanArgs — declaring
      // it on both the zod schema and the advertised entry is what makes it discoverable,
      // unlike skipCommit above, which is declared on neither.
      const writeOptions = skipCommit ? {} : { skipEmbed: true };
      if (input.acknowledgeRemoval === true) writeOptions.acknowledgeRemoval = true;
    ...
    // --- ARCH verdict (computed, never asserted) ---
    if (tool === ARCH_VERDICT_TOOL) {
      const input = archVerdictSchema.parse(cleanArgs);
      const { notesDir } = getDendronContext();
      assertNotesDir(notesDir);
      const notePath = notePathFromFilename(notesDir, input.storyId);
      if (!fs.existsSync(notePath)) throw new McpError(ErrorCode.InvalidParams, `Note not found`);

      // Retain the WHOLE parse result AND the raw bytes, not just .data. The coalesced
      // write below writes only what it is handed — unlike updateField/updateFieldDirect,
      // which re-serialise the whole note. Keeping only .data here would leave no body in
      // scope at the write site and destroy the note. The raw text is retained because
      // the write rejoins the body VERBATIM; see the write site below.
      const rawNote = readNoteRaw(notePath);
      const parsed = parseFrontmatter(rawNote);
      const before = parsed?.data || {};

      // Refuse, do not coerce. computeArchVerdict's Number.isFinite fallback is
      // correct for a total pure function; the defect was that the caller handed it
      // a value needing the fallback — a corrupt arch_round became NaN, fell back to
      // 0, and silently restarted the round counter, re-freezing the ledger and
      // discarding the recorded round. Absent / null / "" still mean round 0.
      const rawRound = before.arch_round;
      if (rawRound !== undefined && rawRound !== null && rawRound !== "") {
        const isScalar = typeof rawRound === "string" || typeof rawRound === "number";
        if (!isScalar || !Number.isFinite(Number(rawRound))) {
          return { content: [{ type: "text", text: JSON.stringify({
            ok: false,
            error: "invalid_arch_round",
            field: "arch_round",
            value: rawRound,
            storyId: input.storyId,
            detail: "arch_round must be a finite numeric scalar. Refusing rather than coercing to 0, which would silently restart the round counter and re-freeze the ledger.",
          }, null, 2) }] };
        }
      }

      // The subject is digested from the note ALREADY READ above — body plus the two
      // reviewable frontmatter fields. Nothing arch-owned and nothing time-varying
      // reaches it: the frontmatter side by construction (subjectDigest is handed three
      // parameters and the arch fields are not among them), and the BODY side by
      // subjectDigest subtracting the ARCH-owned `## ARCH Guidance` section, which
      // governor-arch.md step 3(b) requires ARCH to rewrite on every pass.
      //
      // The body side was NOT sound before backlog.fix.arch-guidance-write-self-rebases-ledger:
      // ARCH's own mandated narrative write changed the digested body, so the next call
      // rebased, `round` was permanently 1 on the mandated path, and the deferred channel
      // and round cap were unreachable. Two things hold that closed, and BOTH are load
      // bearing — the digest-side subtraction above, and the byte-stable rejoin at the
      // write site below. Either one alone leaves the subject unstable across a pass.
      const currentSubject = subjectDigest({
        body: parsed?.content || "",
        targetFiles: before.targetFiles,
        testRequirements: before.testRequirements,
      });

      const computed = computeArchVerdict({
        priorLedger: Array.isArray(before.arch_ledger) ? before.arch_ledger : [],
        priorRound: Number(before.arch_round || 0),
        submitted: input.findings,
        recordedSubject: typeof before.arch_subject === "string" ? before.arch_subject : undefined,
        currentSubject,
        priorTotalRounds: before.arch_total_rounds,
        priorRoundFindings: before.arch_round_findings,
      });

      const skipCommit = cleanArgs.skipCommit === true;
      const noteId = before.id || canonicalIdFromFilename(input.storyId);
      const nextFm = {
        ...before,
        id: noteId,
        // Both replaced primitives bumped this (dendron.mjs:395, :484). Omitting it
        // would silently stop the note advancing its mtime field.
        updated: Date.now(),
        arch_ledger: computed.ledger,
        arch_deferred: computed.deferred,
        arch_round: String(computed.round),
        arch_findings_count: String(computed.blocking.length),
        arch_verdict: computed.verdict,
        arch_subject: computed.subject ?? currentSubject,
        arch_total_rounds: String(computed.totalRounds),
        arch_round_findings: computed.roundFindings.map(String),
      };
      // Phase advances when and ONLY when the computation approved.
      if (computed.verdict === "approved") nextFm.phase = "arch-approved";

      // ONE write or none. The six sequential updateField calls this replaces could
      // fail mid-sequence and leave the note holding a state the monotone ledger
      // forbids — ledger written, verdict not — with no rollback. writeNoteRaw swaps
      // atomically, so a throw here leaves the note byte-identical.
      //
      // BYTE-STABLE BODY. formatWithFrontmatter strips authored leading whitespace
      // (shared/frontmatter.mjs `replace(/^\s+/, "")`), so routing the body through it
      // emits bytes that are not the bytes digested a few lines above — making the
      // recorded arch_subject a digest of a document that no longer exists on disk, and
      // rebasing the NEXT call even when ARCH wrote no guidance at all. Observed: a body
      // authored with two leading newlines came back with one. So the frontmatter is
      // re-serialised and the body is rejoined VERBATIM from the raw text read above.
      // Only when the note carries no frontmatter at all is there no raw body to
      // preserve, and the format path is the correct fallback.
      const split = splitNoteFrontmatter(rawNote);
      const nextRaw =
        split.frontmatter === null
          ? formatWithFrontmatter(nextFm, parsed?.content || "")
          : joinNoteFrontmatter(serializeFrontmatterYaml(nextFm), split.body);
      try {
        writeNoteRaw(notePath, nextRaw, skipCommit ? {} : { skipEmbed: true });
      } catch (err) {
        return { content: [{ type: "text", text: JSON.stringify({
          ok: false,
          error: "write_failed",
      ...
      // Evidence-bound reporting: every value below is READ BACK off the note
      // after writing, never echoed from what we intended to write. See
      // notes/design.evidence-bound-reporting-invariant.md.
      const after = parseFrontmatter(readNoteRaw(notePath))?.data || {};
      const asArray = (v) => (Array.isArray(v) ? v.map(String) : []);
      const observed = {
        arch_verdict: after.arch_verdict,
        arch_round: String(after.arch_round ?? ""),
        arch_findings_count: String(after.arch_findings_count ?? ""),
        arch_ledger: asArray(after.arch_ledger),
        arch_deferred: asArray(after.arch_deferred),
        arch_total_rounds: String(after.arch_total_rounds ?? ""),
        arch_round_findings: asArray(after.arch_round_findings),
        phase: after.phase,
      };
      const mismatches = [];
      if (observed.arch_verdict !== computed.verdict) mismatches.push("arch_verdict");
      if (observed.arch_round !== String(computed.round)) mismatches.push("arch_round");
      if (observed.arch_findings_count !== String(computed.blocking.length)) mismatches.push("arch_findings_count");
      if (JSON.stringify(observed.arch_ledger) !== JSON.stringify(computed.ledger.map(String))) mismatches.push("arch_ledger");
      if (JSON.stringify(observed.arch_deferred) !== JSON.stringify(computed.deferred.map(String))) mismatches.push("arch_deferred");
      // The advisory fields are read back and compared like every other written field. A
      // field exempted from this list is silently accepted at whatever value landed on
      // disk, which is the failure mode this whole read-back block exists to prevent.
      if (observed.arch_total_rounds !== String(computed.totalRounds)) mismatches.push("arch_total_rounds");
      if (JSON.stringify(observed.arch_round_findings) !== JSON.stringify(computed.roundFindings.map(String))) mismatches.push("arch_round_findings");
      if (computed.verdict === "approved" && observed.phase !== "arch-approved") mismatches.push("phase");

      if (mismatches.length > 0) {
        return { content: [{ type: "text", text: JSON.stringify({
          ok: false,
          error: "read_back_mismatch",
          mismatches,
          observed,
          storyId: input.storyId,
        }, null, 2) }] };
      ...
      // path and id satisfy commitDendronWriteResult's documented innerResult
      // contract (server.mjs:504). They are inputs to the commit, not status
      // claims, so they carry no evidence-bound reporting hazard; every status
      // field below is still sourced from observed.*.
      const inner = {
        ok: true,
        path: path.relative(notesDir, notePath),
        id: noteId,
        storyId: input.storyId,
        verdict: observed.arch_verdict,
        round: computed.round,
        capped: computed.capped,
        rebased: computed.rebased,
        maxRounds: ARCH_MAX_ROUNDS,
        // Surfaced beside maxRounds precisely because this is where the wrong inference is
        // made: `round: 1, maxRounds: 3` reads as "one review so far" when the story may
        // have had several. totalRounds is the number that answers that question.
        totalRounds: Number(observed.arch_total_rounds),
        roundFindings: observed.arch_round_findings.map(Number),
        blocking: observed.arch_ledger,
        deferred: observed.arch_deferred,
        phase: observed.phase,
        findingsCount: Number(observed.arch_findings_count),
      };
      const wrapped = await commitDendronWriteResult({ tool: ARCH_VERDICT_TOOL, innerResult: inner, skipCommit });
      return { content: [{ type: "text", text: JSON.stringify(wrapped, null, 2) }] };
    }
    // --- Governor session tools ---
    ...
    if (tool === "rks_agent_run") {
      ...
      // Reject unknown input keys BEFORE the factory runs — a silently-dropped key
      // turns a malformed call into a different, destructive one.
      const parsedInput = parseAgentInput(agentName, agentInput);
      if (!parsedInput.ok) {
        return { content: [{ type: "text", text: JSON.stringify(parsedInput.error, null, 2) }] };
      }
      const context = await loadContext(parsedInput.data.projectId);
      const config = agentFactory({ ...parsedInput.data, projectRoot: context.record.root });
      const result = await runAgent(config);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
    ...
    if (agentEntry) {
      // Same gate as the generic path above — strictness must not be enforceable on
      // only one of the two dispatch routes.
      const parsedConv = parseAgentInput(agentEntry.name, cleanArgs);
      if (!parsedConv.ok) {
        return { content: [{ type: "text", text: JSON.stringify(parsedConv.error, null, 2) }] };
      }
      const context = await loadContext(parsedConv.data.projectId);
      const config = agentEntry.factory({ ...parsedConv.data, projectRoot: context.record.root });
      const result = await runAgent(config);
      // backlog.fix.governor-phase-state-desync-and-recovery
      //
      // An agent that fails returns a PAYLOAD rather than throwing — "Agent exceeded max
      // turns (7)" arrives here as data, so the outer catch never sees it. This is the exact
      // path that produced the observed wedge: the entry transition had already demoted
      // `planned` -> `refining`, and nothing gave it back. Reverting here makes the two
      // failure shapes — thrown and returned — behave identically for the chain.
      // `failureCategory` is deliberately NOT a rollback trigger. It is declared on an
      // `ok: true` result in agents/research.mjs and carries diagnostic values such as
      // `partial_answer`, so treating its presence as failure reverts the chain on a call
      // that succeeded. `ok === false` and `status === 'failed'` are the failure signals.
      if (result?.ok === false || result?.status === 'failed') {
        try {
          if (_entryTransitionToken) revertStateOnFailure(_entryTransitionToken, _entryTransition);
    ...
    // A payload-level failure is a failure. Decided by the same exported helper the unit
    // tier drives, so the rule lives in one place rather than being restated here.
    if (decideToolOutcome({ threw: false, result: toolPayloadFromResponse(_response) }) === TOOL_FAILED) {
      _auditOk = false;
    }
    return _response;
  } catch (error) {
    ...
    // backlog.fix.governor-phase-state-desync-and-recovery: give back the entry transition.
    // A throw means the tool body did not complete, so it has no claim on the chain position.
    try {
      if (_entryTransitionToken) revertStateOnFailure(_entryTransitionToken, _entryTransition);
    ...
    // THE SINGLE AWAIT POINT for stash auto-pops fired by endSession.
    //
    // endSession is synchronous and cannot await its own pop — advanceStateOnResult
    // alone has 18 call sites here, and endSession has 64 references across 14 test
    // files, so making it async would turn every one of those into a new floating
    // promise. It therefore RETAINS the promise and this awaits it once.
    //
    // `finally` is chosen over wrapping the dispatch body deliberately: an await
    // here suspends the async handler, so the MCP response is not delivered until
    // every in-flight pop has settled. That covers all inline returns AND the throw
    // path from ONE insertion. Wrapping instead would mean extracting a ~2,250-line
    // handler body with dozens of inline returns — a far larger refactor than the
    // fix it carries.
    //
    // KNOWN AND ACCEPTED: `finally` runs after the return value is fixed, so a
    // failed pop cannot be surfaced on THIS response. Reporting it is deferred to
    // its own story; the outcome objects carry `projectId` so that consumer has
    // what it needs. Never return or throw from here — either would discard the
    // real result or swallow an in-flight exception.
    try {
      await flushPendingStashPops();
    } catch (_e) { /* a pop outcome is already recorded; never mask the tool result */ }
  }
});
...
export async function startServer() {
  ...
  // Wire the RAG module's host hooks (session-state provenance + telemetry collector) before
  // accepting connections. RAG ops only fire on MCP tool calls after server.connect() below, so this
  // precedes any production RAG op. src/rag/** deliberately has NO static import of these (dependency
  // inversion so RAG can be lifted into a standalone @routekit/rag package without a host cycle).
  setRagHostHooks({ addRagSourcedPath, getTelemetryCollector });
