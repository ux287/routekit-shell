...
try {
  ...
  // Non-fatal: the MCP server boots KEY-FREE. RAG (rks_rag_*) and other key-free tools register
  // and are reachable with no credential; the credential requirement is enforced at invoke time on
  // the LLM-backed paths (assertAnthropicCredential), so a missing key becomes a clear per-tool
  // error rather than a whole-server block. Warn (key name / provider only — never a value) so a
  // forgotten key is still fast, visible startup feedback.
  console.error(`[preflight] WARNING: ${err.message}`);
  console.error(
    '[preflight] Continuing key-free boot: RAG and other key-free tools are available; ' +
      'LLM-backed tools will error with this message until a credential is provided.'
  );
}
