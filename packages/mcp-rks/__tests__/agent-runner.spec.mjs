...
describe("Agent Config Loader", () => {
  ...
  it("falls back to global defaults for unknown agent", () => {
    ...
    try {
      ...
      assert.ok(cfg.model.includes("sonnet"), `expected sonnet global default, got ${cfg.model}`);
...
describe("Story Agent Factory", () => {
  ...
  it("uses sonnet model by default", () => {
    const config = createStoryAgent({
      projectId: "test",
      storyId: "backlog.test",
      projectRoot: process.cwd(),
    });
    assert.ok(config.model.includes("sonnet"), `expected sonnet model, got ${config.model}`);
  });
  ...
  it("config loads story agent defaults", () => {
    ...
    try {
      ...
      assert.ok(cfg.model.includes("sonnet"), `expected sonnet default, got ${cfg.model}`);
      assert.strictEqual(cfg.maxTurns, 7);
      assert.strictEqual(cfg.timeoutMs, 90000);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
...
describe("Agent Runner", () => {
  ...
  it("forwards accumulated messages to escalated call on max_turns_exceeded", async () => {
    ...
    assert.ok(typeof escalatedMessages[0].content === "string" && escalatedMessages[0].content.includes("Original question"), "first message should be original user message");
