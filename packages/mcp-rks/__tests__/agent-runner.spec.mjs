/**
 * DEAD TEST ROOT - NOTHING IN THIS DIRECTORY EXECUTES.
 *
 * packages/mcp-rks/__tests__/ is matched by NONE of the project vitest configs:
 *   vitest.config.unit.mjs - includes tests/unit and tests root only
 *   vitest.config.mock.mjs - includes tests/integration only
 *   vitest.config.e2e.mjs  - includes tests/e2e only
 *   vitest.config.mjs      - the fallback tier, includes tests only
 * All four scope their include globs to the tests/ root, and CI invokes exactly those tiers
 * (.github/workflows/ci.yml:215, :263, :283). These assertions cannot fail, because they
 * cannot run - verified by running vitest against this file with the unit config, which
 * returns "No test files found, exiting with code 1".
 *
 * Do NOT cite anything here as proof that a behaviour holds. The GitOutputSchema
 * back-compat proof lives in tests/unit/git-evidence.test.mjs, which the unit tier runs.
 */
/**
...
// ──────────────────────────────────────────
// 2b. generateAgentToolDefinitions — defensive tool-list builder
//     backlog.feat.listtools-null-inputschema-guard
//
//     A child project's custom agent registers with inputSchema:null (discovery.mjs).
//     Previously zodToJsonSchema(null) threw "expected a Zod schema", the throw
//     propagated out of ListTools, and the client saw ZERO tools
//     ("connected · tools fetch failed", MCP -32603). These tests pin the two-layer
//     defense: (1) null → generic permissive schema, (2) any other malformed agent is
//     skipped + logged, never aborting the whole enumeration.
//
//     The optional `registry` arg lets us exercise the loop with crafted null/non-Zod
//     schemas without mutating the module registry.
// ──────────────────────────────────────────

describe("generateAgentToolDefinitions — null/invalid inputSchema hardening", () => {
  it("emits a generic permissive schema for an agent with inputSchema:null (does not call zodToJsonSchema)", () => {
    const registry = {
      "proj-agent": {
        inputSchema: null, // exactly how discovery.mjs registers project agents
        description: "A project agent using generic input",
        toolName: "rks_agent_proj_agent",
        source: "project",
      },
    };
    const tools = generateAgentToolDefinitions(registry);
    const tool = tools.find(t => t.name === "rks_agent_proj_agent");
    assert.ok(tool, "the null-schema project agent tool should be present");
  ...
  it("still converts a real Zod inputSchema via zodToJsonSchema unchanged", () => {
    const registry = {
      "zsample": {
        inputSchema: z.object({ projectId: z.string(), count: z.number() }),
        description: "a core-like agent with a real Zod schema",
        toolName: "rks_agent_zsample",
        source: "core",
      },
    };
    const tools = generateAgentToolDefinitions(registry);
    const tool = tools.find(t => t.name === "rks_agent_zsample");
    assert.ok(tool);
    assert.deepStrictEqual(tool.inputSchema, {
      type: "object",
      properties: { projectId: { type: "string" }, count: { type: "number" } },
      required: ["projectId", "count"],
    });
  });

  it("does not throw when one agent has an invalid/non-Zod schema — bad agent skipped, others still enumerate", () => {
    const registry = {
      "good-null": { inputSchema: null, description: "d", toolName: "rks_agent_good_null", source: "project" },
      // truthy but non-Zod (no _def) → zodToJsonSchema throws inside the loop
      "bad": { inputSchema: {}, description: "d", toolName: "rks_agent_bad", source: "core" },
      "good-zod": { inputSchema: z.object({ x: z.string() }), description: "d", toolName: "rks_agent_good_zod", source: "core" },
    };
    let tools;
    assert.doesNotThrow(() => { tools = generateAgentToolDefinitions(registry); });
    const names = tools.map(t => t.name);
    assert.ok(names.includes("rks_agent_run"), "generic runner tool is always present");
    assert.ok(names.includes("rks_agent_good_null"), "null-schema agent still enumerates");
    assert.ok(names.includes("rks_agent_good_zod"), "valid-Zod agent still enumerates");
    assert.ok(!names.includes("rks_agent_bad"), "the malformed agent is skipped");
  });

  it("logs the skipped malformed agent to stderr while every other tool still appears", () => {
    const registry = {
      "bad": { inputSchema: {}, description: "d", toolName: "rks_agent_bad", source: "core" },
      "ok": { inputSchema: null, description: "d", toolName: "rks_agent_ok", source: "project" },
    };
    ...
    let tools;
    try {
      tools = generateAgentToolDefinitions(registry);
    } finally {
      console.error = original;
    }
    assert.ok(logged.some(l => l.includes("'bad'")), "should log the skipped agent name to stderr");
    assert.ok(tools.map(t => t.name).includes("rks_agent_ok"), "the other tool still enumerates");
  });

  it("default (module registry) call still enumerates core tools and does not throw", () => {
    let tools;
    assert.doesNotThrow(() => { tools = generateAgentToolDefinitions(); });
    const names = tools.map(t => t.name);
    assert.ok(names.includes("rks_agent_validate_story"));
    assert.ok(names.includes("rks_agent_research"));
  });
});
