...
export const LOADED_RKS_VERSION = (() => {
  // Walk up to the monorepo root — the package.json that declares `workspaces` (or the routekit-shell
  // name) — so the stamped shell version is found regardless of THIS package's depth in the tree.
  // (A fixed-depth "../../../../../package.json" broke when the subsystem moved into @routekit/telemetry.)
  try {
    let dir = __telemetryDirname;
    for (let i = 0; i < 12; i++) {
      try {
        const pkg = JSON.parse(fs.readFileSync(path.join(dir, "package.json"), "utf8"));
        if (pkg.workspaces || (pkg.name && pkg.name.startsWith("routekit-shell"))) {
