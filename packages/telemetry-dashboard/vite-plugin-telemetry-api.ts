    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/telemetry/failures', async (req, res) => {
        try {
          ...
          // Aggregation extracted to the exported pure fn aggregateFailures (mirrors
          // aggregateTrustCounters) so it is unit-testable with synthetic events, and so
          // structural_create_unauthorable surfaces as its own topReasons entry.
          const result = aggregateFailures(allEvents);
      ...
      server.middlewares.use('/api/telemetry/token-costs', async (req, res) => {
        try {
          ...
          const project = getProjectFilter(url);
          const allEvents = await readAllEvents(getTelemetryDir(project));
          // Aggregation extracted to the exported pure fn aggregateTokenCosts (mirrors
          // aggregateTrustCounters) so it is unit-testable with synthetic events, and so the
          // aggregate cacheRatio / cacheCreate / byModel model-mix are computed inline here WITHOUT
          // importing the MCP-server cost report module (the not.toContain invariant).
          const result = aggregateTokenCosts(allEvents);
...
/**
 * Aggregate the /api/telemetry/failures view from a raw event array. Exported (mirrors
 * aggregateTrustCounters) so it is unit-testable with synthetic events.
 *
 * Structural planner give-ups — plan.failed{reason:'structural_create_unauthorable'} OR
 * plan.retry.exhausted{failureClass:'structural'} — are surfaced as their OWN
 * 'structural_create_unauthorable' topReasons entry, distinct from generic plan.failed reasons.
 * plan.retry.exhausted is included even though it is not a '.failed' event.
 * (backlog.feat.telemetry-report-escalation-structural-rollup)
 */
export function aggregateFailures(events: any[]) {
  ...
  for (const event of events || []) {
    ...
    if (!isFailed && !isStructural) continue;
    ...
    const reason = isStructural
      ? 'structural_create_unauthorable'
      : (payload.error || payload.reason || payload.message || eventType);
  ...
  return { total: failures.length, byLocation, topReasons, lastFailure: failures[0] || null };
}

/**
 * Aggregate the /api/telemetry/token-costs view from a raw event array. Exported (mirrors
 * aggregateTrustCounters) so it is unit-testable with synthetic events. Computes the aggregate
 * cache-hit rate (input-side denominator INCLUDING cacheCreate), the cache write/read/uncached
 * split, cacheCreate, and a byModel (haiku-vs-sonnet) mix INLINE — it does NOT import the
 * MCP-server cost report module. Preserves the existing per-story `stories[]` + 14-point `dailySeries`.
 * (story: cache-and-model-rollup)
 */
export function aggregateTokenCosts(allEvents: any[]) {
  const tokenEvents = (allEvents || []).filter((ev) => ev.payload?.tokens != null);

  const storyMap = new Map<string, { storyId: string; rawCost: number; wastedCost: number; cacheReadTotal: number; inputTotal: number }>();
  let aggInput = 0, aggCacheRead = 0, aggCacheCreate = 0;
  const byModel: Record<string, { calls: number; tokens: number; share: number }> = {};
  ...
  for (const ev of tokenEvents) {
    ...
    const tokCacheCreate: number = t.cacheCreate || 0;
    const cost = tokIn + tokOut;
    const isWasted = (ev.type || '').includes('failed');
    if (!storyMap.has(storyId)) storyMap.set(storyId, { storyId, rawCost: 0, wastedCost: 0, cacheReadTotal: 0, inputTotal: 0 });
    ...
    aggInput += tokIn;
    aggCacheRead += tokCacheRead;
    aggCacheCreate += tokCacheCreate;
    const modelKey = ev.payload?.model || 'unknown';
    if (!byModel[modelKey]) byModel[modelKey] = { calls: 0, tokens: 0, share: 0 };
    byModel[modelKey].calls += 1;
    byModel[modelKey].tokens += tokIn + tokOut;
  }
  ...
  // Aggregate cache-hit rate: input-side denominator INCLUDES cacheCreate (cache-warming).
  const inputSideTotal = aggInput + aggCacheRead + aggCacheCreate;
  const cacheRatio = inputSideTotal > 0 ? aggCacheRead / inputSideTotal : 0;
  const cacheBreakdown = { write: aggCacheCreate, read: aggCacheRead, uncached: aggInput };
  const totalModelTokens = Object.values(byModel).reduce((s, m) => s + m.tokens, 0);
  for (const m of Object.values(byModel)) m.share = totalModelTokens > 0 ? m.tokens / totalModelTokens : 0;

  return { stories, dailySeries, cacheRatio, cacheCreate: aggCacheCreate, cacheBreakdown, byModel };
}
