...
> **If you pin to a tag, know that `git checkout <tag>` leaves you on a detached HEAD.** That is fine for *running* rks, but you cannot develop from it — commits you make there belong to no branch and are easy to lose, and `npm run setup` will not move you onto `staging` while the working tree is dirty. `rks_preflight` now reports a detached HEAD as unhealthy and tells you how to land on a branch: `git checkout -B staging --track origin/staging`.
