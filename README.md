...
routekit project init --id my-app --stack app.web.react.spa --path ~/Projects/my-app
```
...
Every unit of work in rks leaves the same four artifacts behind. Here is a real one.

**The story.** A clean clone of the public repo reported a missing skill and told you to run a
command that could not possibly work — the skill had never been in that tree. The defect and its
exit criteria live in `backlog.fix.preflight-core-skills-not-distributed`:

```text
Acceptance
  - on a published tree, the core_skills check PASSES and its hint contains no
    "git checkout" instruction
  - on a full checkout with the companion package present, an absent skill still FAILS
    (the check must not go blind)

targetFiles
  - packages/mcp-rks/src/server/preflight.mjs   edit
  - scripts/setup.mjs                           edit
```

The second acceptance criterion is the interesting one. It exists because the easy fix — delete the
entry — would have made the check pass by making it stop checking.

**The plan.** Before anything is written, the plan names every file and the operation applied to it:

```text
edit    .routekit/skills-manifest.json                        declare the skill as not-published
edit    packages/mcp-rks/src/shared/skills-manifest.mjs       add the classification helper
edit    packages/mcp-rks/src/server/preflight.mjs             consume it; drop the dead advice
edit    scripts/setup.mjs                                     same defect, earlier in the run
create  tests/integration/published-tree-core-skills.test.mjs
edit    tests/unit/preflight-core-skills.test.mjs
edit    tests/unit/setup-script.test.mjs
```

Review gates run against that plan rather than the diff. One of them caught that fixing
`preflight.mjs` alone would leave the same false report coming from `scripts/setup.mjs`, which runs
earlier in the same first-run sequence — two consumers, one bug. The fourth line above exists
because of that finding, and it was added before any code was written.

**The change.** Commit `8f1751b4` — 7 files changed, 430 insertions, 20 deletions. Three of the
seven are tests, including one that builds the real published archive and runs the check against the
extracted tree, so "it works on a clean clone" is verified rather than assumed.

**The cost.** Every governed run emits token telemetry, and `/telemetry` reports it per story:

```text
rawCost        the tokens actually spent
efficientCost  what the same work would have cost with no rework
wasteRatio     the gap between them
cacheRatio     how much was served from cache
healthBand     green / yellow / red
```

`wasteRatio` is the one to watch: it is how you notice an agent looping before the bill does.
Run `/telemetry` in your own project for real figures — the fields above are the report's shape,
...
> **If you pin to a tag, know that `git checkout <tag>` leaves you on a detached HEAD.** That is fine for *running* rks, but you cannot develop from it — commits you make there belong to no branch and are easy to lose, and `npm run setup` will not move you onto `staging` while the working tree is dirty. `rks_preflight` reports a detached HEAD as unhealthy and tells you how to land on a branch.
>
> The command depends on which clone you have. If you cloned the **public mirror**, there is no `origin/staging` to track — the mirror publishes only `main` and tags — so use `git checkout -B staging` (adding `--track origin/staging` there fails with *"'origin/staging' is not a commit"*). Only a clone that actually has `origin/staging` should track it.
>
> **After a `git checkout` of a new tag, restart the MCP server.** Node loaded rks's code at startup and a checkout does not change that, so the server keeps running the old build. Preflight's `server_freshness` check catches this and tells you — it used to report the new version while running the old code, which sent one UAT round chasing a bug that had already been fixed.
...
**The shell is not one of its own children.** `sync`/`upgrade`/`doctor` operate on *child projects*; pointing them at the shell itself is refused, because syncing a directory from itself deletes the very files it is about to copy. Update the shell with git, not with `routekit project`.
...
rks follows [Semantic Versioning](https://semver.org/). While pre-1.0, the intent axis — not surface size — decides the bump:

- **PATCH** (`0.24.0 → 0.24.1`) — **fixing broken behavior**: a bug fix that makes the code do what it was already meant to do.
- **MINOR** (`0.24.0 → 0.25.0`) — **new or changed intended behavior**: a feature, or a deliberate change to how something is supposed to work.
- **MAJOR** — reserved for `1.0` and, thereafter, genuinely breaking changes.

The sharpening test when a change feels ambiguous: **changing or adding intended behavior is a minor; fixing broken behavior is a patch.** (Pre-1.0, that's the only objectively-decidable line — "breaking = major" doesn't yet apply.)

All three workspace `package.json` versions — root, `packages/mcp-rks`, and `packages/cli` — are kept in **lockstep**: `rks_release` bumps them together so they never drift. There is a single source of truth for "what version is this."
...
rks is free software under the GNU AGPL-3.0-or-later, and is fully featured for solo developers and small teams. A separate commercial licence is available for teams that cannot accept the AGPL's network-copyleft terms.
...
Outside contributors: see [CONTRIBUTING.md](CONTRIBUTING.md) for the fork → branch → pull request path. **A signed [Contributor License Agreement](CLA.md) is required before any contribution is accepted** — rks is dual-licensed (AGPL-3.0-or-later plus a commercial licence), so contributions must be relicensable by the maintainer. The internal dogfood loop above is documented in [`CLAUDE.md`](CLAUDE.md) and in the deep-dive blog series; it is not the contribution path for outside contributors.
...
rks is **dual-licensed**: AGPL-3.0-or-later for everyone, plus a separate commercial licence for teams that cannot accept the AGPL's network-copyleft terms. Both cover the same software.

Copyright and third-party attribution live in [NOTICE](NOTICE), not in `LICENSE` — `LICENSE` is the unmodified FSF text and is never edited. Contributions are accepted under the [Contributor License Agreement](CLA.md); the outside-contributor path is [CONTRIBUTING.md](CONTRIBUTING.md).
