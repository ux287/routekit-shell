...
- **Allowed:** `rks_rag_query` (keyless semantic retrieval; fidelity-redacted L2 chunks) and writes to the **`keyless-notes.*`** dendron namespace ONLY.
...
**Research (read-only) Governors may run in parallel with each other.** They do not mutate git branches, the worktree, dendron/session state, or story phase, so fanning out several Research Governors at once is safe — and is the right way to investigate multiple topics quickly.

**Every other Governor runs strictly serial and exclusive.** PO, QA, ARCH, Build, and any exec/ops/ship Governor mutates shared git/worktree/dendron/session state, so never run two of them at once, and never launch one while any other Governor — including a Research Governor — is still running. Always wait for each to complete before launching the next.
