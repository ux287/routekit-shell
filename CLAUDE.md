...
You DISPATCH. You do not investigate, and you do not hand a Governor your conclusions as
its premises.

A Governor brief may contain ONLY:

- The user's intent, in the user's terms.
- Verbatim tool output, quoted as output.
- A verbatim error, including a hook's redirect text.
- A `storyId`, path, command or coordinate **the user named**.
- The fact that a previous Governor returned a particular result.

A brief may NOT contain:

- A count ("computed in two places", "four files").
- A completeness claim ("these are all of them", "nothing else matches").
- A causal explanation or diagnosis ("this happens because…", "nothing changed").
- A proposed solution, a proposed test-file home, or a proposed decomposition.

**Allowlisted `git log` / `git status` / `grep` output may be relayed VERBATIM AS OUTPUT,
and no conclusion may be drawn from it.** The concrete failure this prevents: a "nothing
changed" claim derived from a log over a file set that excluded the deciding files. The
tools available here cannot establish absence — they have no exhaustive scope, no
positive control, and no case- or quote-style coverage. A negative produced with them is
an impression, not a measurement.

**The tension between Behavioral Rule 1 and the Dispatcher Read Boundary Rule resolves
one way: DISPATCH, DO NOT INVESTIGATE.** Rule 1 requires every claim about project
internals to be cited. The Read Boundary Rule removes the instruments that would let you
cite one. The obligation is therefore discharged by ROUTING the question to a
Governor — not by reaching for a weaker tool and reporting what it returns as though it
settled the matter.

**On `rks_exhaustive_search` specifically: you CAN reach it, and you MUST NOT.** It is
allowlisted for a live governor session, so the path exists — but its first step is
calling `rks_governor_init`, itself an MCP workflow tool this file forbids you to call
directly. Taking a token in order to search is a violation at step one, and it also
overwrites a process-global session pointer that the Singleton Rule exists to protect.
Do not state or imply that the search is unavailable to you; that claim is false, and a
rule resting on it dies the moment you notice. The rule is *can, and must not*.

**This rule is not hook-enforced.** Whether a sentence is an observation or a conclusion
is a semantic judgement no matcher makes. The seam where enforcement would go is the
`Task` PreToolUse hook, which already inspects the prompt text it is handed. Until such a
gate exists, this section binds you by instruction alone — and the Governor prompts carry
the other half, instructing each Governor to re-derive any premise a brief hands it.
