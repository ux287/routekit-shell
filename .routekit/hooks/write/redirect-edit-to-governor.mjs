...
import {
  ...
  isKeyless, isKeylessNotesTarget,
...
async function main() {
  ...
  // Keyless mode (INV-4 bounded write): the ONE keyless write target is the keyless-notes.*
  // namespace. Gated by the SHARED realpath-anchored isKeylessNotesTarget — nothing else opens.
  // Inserted AFTER the active-scope early-exit and BEFORE the memory/catch-all redirect so the
  // ordering (active-scope < keyless-notes < memory < governor) pinned by memory-write-redirect
  // is preserved, and key-present behavior is byte-identical (isKeyless() short-circuits false).
  if (filePath && isKeyless() && isKeylessNotesTarget(filePath)) process.exit(0);
