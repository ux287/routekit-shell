...
import { classifyReadIntent } from '../lib/read-classification.mjs';
import { recordRead, getRecentReads } from '../lib/session-state.mjs';
import { isResearchAgentOutage, emitOutageFallthrough } from '../system/hook-output.mjs';
...
async function main() {
  ...
  //
  // session_id comes from the PreToolUse stdin envelope and is what scopes the
  // harness-session-output allowance to THIS session. Passing it through is not
  // optional: without it that rule fails closed and an agent still cannot read
  // its own overflowed tool output.
  ...
  // Research-Agent outage fallthrough (fail-closed) — READ intents ONLY. Edit/Write/NotebookEdit
  // and bash edit/write ops NEVER fall through: writes stay blocked even during an outage. A fresh
  // infra breadcrumb permits an otherwise-unprovenance'd READ for diagnosis. Sits after the
  // provenance decision, immediately before the block.
  const isReadIntent = toolName === 'Read' || toolName === 'Glob' || toolName === 'Grep'
    || (bashFileOp && bashFileOp.operation === 'read');
  if (isReadIntent) {
    const outage = isResearchAgentOutage(PROJECT_DIR);
    if (outage) {
      emitOutageFallthrough({ blockedTool: effectiveTool, targetPath, category: outage.category, projectDir: PROJECT_DIR });
      process.exit(0);
    }
  }
