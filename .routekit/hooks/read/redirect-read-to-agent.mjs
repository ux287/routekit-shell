 ...
 * provenance. The complete allow set is enumerated below.
 *
 ...
 *
 * Allowed reads (the complete set — nothing else is granted here):
 * - Files in the active off-rail session's allowedFiles (step 0, checked first)
 * - runtime_paths from read-policy.yaml (infrastructure/config allowlist)
 ...
 * - Files this session itself wrote, within the session write-ledger TTL
 *   (WRITE_LEDGER_TTL_MS, 10 min) — read-what-you-wrote
 * - This session's own harness output (tasks / scratchpad / tool-results)
 *
 * Everything else routes to the Research Governor.
 * Branch membership is NOT a read grant: a file merely touched on the current
 * branch carries no provenance here and is denied like any other unprovenanced
 * project file.
 *
 * Output mechanism:
...
import { hasValidProvenance } from "../lib/session-state.mjs";
import {
...
async function main() {
  ...
  // 3.5 Research-Agent outage fallthrough (LAST resort, fail-closed). Positioned AFTER the off-rail
  //     scope check (step 0) and provenance (step 3): if the Research Agent is down (a FRESH infra
  //     breadcrumb), permit a BOUNDED direct READ for diagnosis instead of redirecting into the
  //     broken component. This hook only handles Read, so no write can reach here. Audit-logged.
  //     Absent/stale/malformed breadcrumb → falls through to the normal redirect below.
  const outage = isResearchAgentOutage(PROJECT_DIR);
  if (outage) {
    emitOutageFallthrough({ blockedTool: "Read", targetPath: relativePath, category: outage.category, projectDir: PROJECT_DIR });
    process.exit(0);
  }
  ...
  // Keyless mode is MESSAGING-ONLY here, and ONLY on this generic provenance-failure redirect —
  // the off-rail deny handoff (step 0 above) is deliberately left UNCHANGED (INV-2). The read
  // decision is unchanged; with no LLM credential the Research Agent is unavailable, so point at
  // the keyless retrieval primitive.
  const instructions = isKeyless()
    ? [
        "Keyless mode (no LLM credential): the Research Agent is unavailable.",
        "Use rks_rag_query for keyless semantic retrieval over the knowledge base instead of a raw Read.",
      ]
    : [
