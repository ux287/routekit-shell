...
1. **Project setup**: `routekit project init --id my-project --stack app.web.react.spa --path /path/to/project`
...
4. **QA review**: `/qa` adds test requirements — the story advances from `draft` to `ready`
5. **ARCH review**: `/arch` reviews the design — the story advances from `ready` to `arch-approved`
6. **Plan**: `rks_plan` with the story ID
7. **Execute**: `rks_exec` to apply the plan
8. **Ship**: `rks_ship` or manual PR workflow
...
Use `rks_agent_research` for intelligent multi-source search:
...
```
draft → ready → arch-approved → planned → executed → committed → integrated → released
```

- **draft**: Initial creation, still being refined
- **ready**: QA has added test requirements; validated and ready for architectural review
- **arch-approved**: ARCH review passed — the story may now be built. Build is gated on this
- **planned**: Has an implementation plan
- **executed**: Plan applied, PR created
- **committed**: Changes committed to the working branch, not yet merged
