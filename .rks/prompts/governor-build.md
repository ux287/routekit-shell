...
6. mcp__rks__rks_exec({ projectId: '__PROJECT_ID__', _governorToken: TOKEN })
   ...
   → On pre-apply baseline refusal: if the exec result carries stage: 'pre_apply_baseline', the tests were already failing BEFORE any change was applied — nothing was applied and refining the story cannot fix it. STOP immediately. Do NOT call rks_refine, rks_refine_apply, rks_plan, or rks_exec again. Return { status: 'failed', testsFailed: true, stage: 'pre_apply_baseline', testsFailedLog, summary: '<tests were failing before any change was applied>' }.
6a. **Refine-retry loop** (max 2 Governor-level attempts — separate from exec's internal retry count):
   When rks_exec fails with testsFailed: true and the result does NOT carry stage: 'pre_apply_baseline', do NOT stop immediately. Instead:
...
- Pre-apply baseline refusal (the exec result carries stage: 'pre_apply_baseline') → STOP. Do NOT call rks_refine or re-exec — the tests were failing before any change was applied. Return { status: 'failed', testsFailed: true, stage: 'pre_apply_baseline', testsFailedLog, summary }.
- Test failure with retries remaining (and not a pre-apply baseline refusal) → Enter the refine-retry loop (step 6a). Use rks_refine with trigger 'test_failed', apply suggestions, re-plan, and re-exec.
