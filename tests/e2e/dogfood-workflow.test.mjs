...
// Deliberately still under repoRoot: this reads the REAL project registry, and is not a fixture
// root. backlog.fix.test-fixture-repo-containment moved the fixture roots only.
...
describe("Dogfood E2E Workflow", () => {
  ...
  function setupTestProject(testName) {
    ...
    // backlog.fix.test-fixture-repo-containment: rooted OUTSIDE the repository. This used to be
    // path.join(repoRoot, …), and a fixture inside the repo whose git init failed silently became
    // a handle on the developer's own repository via git's walk-up.
    projectRoot = path.join(TEMP_BASE, `e2e-dogfood-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    ...
    // Initialize git repo.
    //
    // backlog.fix.test-fixture-repo-containment: this block used to be wrapped in a catch that
    // downgraded ANY failure to console.warn and continued. That is the worst shape available —
    // after a failed `git init` the remaining commands operate on the AMBIENT repository:
    // `git config` writes the real .git/config, `git add .` stages real files. A fixture that
    // cannot initialise must abort the test.
    //
    // `git init -b main` replaces the old bare `git init` plus a separate `checkout -b main`.
    // That checkout was redundant once the branch is named at init, and it fails wherever git's
    // init.defaultBranch is already `main` (the branch exists but is unborn) — a failure the
    // deleted catch was silently absorbing.
    // NOTE: these deliberately carry no `timeout:`. This file is the single entry in
    // subprocess-timeout-convention.test.mjs's ALLOWLIST, and that scanner also fails when an
    // allowlist entry becomes STALE — so adding timeouts here reddens it until the entry is
    // removed, and that file is outside this story's scope. Adding the timeouts and dropping the
    // exemption together is worth doing; it belongs in its own change.
