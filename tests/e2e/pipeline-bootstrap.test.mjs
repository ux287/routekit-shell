...
function setupChild(tag) {
  ...
  // backlog.fix.test-fixture-repo-containment: rooted outside the repository working tree.
  const projectRoot = path.join(TEMP_BASE, `e2e-pipeline-${stamp}`);
  ...
  // The ship review gate fails CLOSED: rks_story_ship halts when the reviewer
  // LLM did not run, and that includes the no-credential case. This fixture runs
  // under RKS_SKIP_LLM=1 with no ANTHROPIC_API_KEY, so without an explicit policy
  // loadReviewPolicy would default to enabled:true and the ship below would
  // legitimately halt. Record the opt-out the way a real project has to — an
  // explicit, committed decision in the policy file, never inferred from a
  // missing environment variable.
  fs.writeFileSync(
    path.join(projectRoot, ".rks", "review-policy.yaml"),
    [
      "# E2E fixture: no reviewer credential is available under RKS_SKIP_LLM=1.",
      "# The review gate fails closed by default, so opt out explicitly here.",
      "enabled: false",
      "",
    ].join("\n"),
  );
