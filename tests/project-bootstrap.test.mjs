...
describe("project init/attach bootstrap convergence", () => {
  ...
  it("substitutes projectId in copied skill files", () => {
    ...
    copyDirRecursive(path.join(repoRoot, ".rks", "prompts"), path.join(shellRoot, ".rks", "prompts"));
    copyDirRecursive(path.join(repoRoot, ".claude", "skills"), path.join(shellRoot, ".claude", "skills"));
    // FIXTURE COMPLETION (ARCH round 2, Finding A). The mirror-clone identity recovery reads
    // {shellRoot}/.rks/project.json. Without it this case would fail on a mirror clone for a reason
    // unrelated to the defect, making the RED uninterpretable — and un-gating alone would then have
    // left mirror CI permanently red. Copied, then asserted as an explicit precondition.
    const shellIdFile = path.join(shellRoot, ".rks", "project.json");
    fs.copyFileSync(path.join(repoRoot, ".rks", "project.json"), shellIdFile);
    expect(fs.existsSync(shellIdFile), `fixture precondition: ${shellIdFile}`).toBe(true);
