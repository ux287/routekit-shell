...
describe("issue-intake plugin bundle", () => {
  ...
  // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. plugins/** is on no include
  // pattern in publish-profiles.yaml, so the bundle this whole block dogfoods is genuinely
  // absent from a published tree and all seven cases threw ENOENT there. CONDITION-SCOPED on
  // the bundle's presence: wherever it ships, every case runs and still fails on a malformed
  // manifest or a dropped skill. The suite above this describe exercises the plugin SURFACE
  // against synthetic fixtures and is unaffected — that coverage keeps shipping.
  const bundleShips = existsSync(path.join(bundleRoot, MANIFEST_FILENAME));
  ...
  it.skipIf(!bundleShips)("the real bundle manifest passes validateManifest fail-closed", () => {
    const r = validateManifest(readManifest());
    expect(r.ok, JSON.stringify(r.errors)).toBe(true);
    expect(r.errors).toEqual([]);
  });

  it.skipIf(!bundleShips)("declares ONLY the skills surface, resolving to the bundle's skills dir", () => {
  ...
  it.skipIf(!bundleShips)("the loader installs the issue-intake skill into <project>/.claude/skills/issue-intake/ byte-for-byte", () => {
  ...
  it.skipIf(!bundleShips)("SKILL.md declares the FULL flow (no step silently dropped)", () => {
    const md = skillMd();
    for (const marker of [
  ...
  it.skipIf(!bundleShips)("SKILL.md documents the provider-adapter contract (detect/fetch/comment/close + NormalizedIssue), GitHub first", () => {
    const md = skillMd();
  ...
  it.skipIf(!bundleShips)("SKILL.md instructs graceful degradation (MCP-unavailable + write-governance + no-auto-embed)", () => {
    const md = skillMd();
  ...
  it.skipIf(!bundleShips)("SKILL.md has valid frontmatter (name: issue-intake + description)", () => {
    const md = skillMd();
