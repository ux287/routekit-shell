...
describe("buildPrompt — live content and story-intent labeling", () => {
  ...
  // backlog.fix.planner-single-source-of-truth-token-divergence
  // Prevention (not a runtime detector): the code-gen prompt instructs the planner to define a
  // value that must be shared across generated files ONCE and import it, so it can't re-derive
  // divergent literals per file (the palette.ts/tokens.css 16-mismatch bug).
  it("code-gen prompt carries a SINGLE SOURCE OF TRUTH rule for shared values", async () => {
    const mod = await import("../../packages/mcp-rks/src/llm/planner.mjs");
    const prompt = mod.buildPrompt({ requirements: "test" });

    expect(prompt).toContain("SINGLE SOURCE OF TRUTH");
    expect(prompt).toMatch(/define it once|define it ONCE/i);
    expect(prompt).toMatch(/import/i);
    expect(prompt).toMatch(/never re-derive|never re-declare/i);
    expect(prompt).toMatch(/byte-identical/i);
  });

  // The prevention rule lives in code-gen (buildPrompt), NOT the test-gen prompt, and must not have
  // disturbed the v0.23.0 test-generation hygiene rules (assertion-outside-loop, toBeCloseTo).
  it("regression: v0.23.0 test-gen hygiene rules are untouched", async () => {
    const mod = await import("../../packages/mcp-rks/src/llm/planner.mjs");
    const testPrompt = mod.buildTestGenerationPrompt({
      testPath: "src/x.test.ts",
      testExemplar: "it('x', () => { expect(1).toBe(1); });",
      requirements: "test",
      sourceChanges: "",
      existingContent: "",
    });
    expect(testPrompt).toContain("toBeCloseTo");
    expect(testPrompt).toMatch(/outside the loop/i);
  });
});
