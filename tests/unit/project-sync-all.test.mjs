...
// ══════════════════════════════════════════════════════════════════════════════════
// backlog.fix.shell-self-sync-skill-wipe-health-gate — `sync --all` must not eat the shell
// ══════════════════════════════════════════════════════════════════════════════════
//
// setup.mjs registers the SHELL in its own registry, and this loop walks every record. Without a
// skip, the shell is synced FROM ITSELF: syncProject rm's each of its skills to make room for the
// copy, the copy then has nothing to copy, and the whole thing exits 0. Silently. Every skill gone —
// which is exactly what a clean-machine UAT round found, with rks reporting itself healthy.
describe('handleProjectCommand — sync --all skips the shell\'s own registry record', () => {
  let SHELL_ROOT, kid, processExit, syncProject, loadProjects;

  beforeEach(() => {
    SHELL_ROOT = mkdtempSync(path.join(os.tmpdir(), 'sync-all-shell-'));
    kid = makeFakeChild('kid');
    processExit = vi.fn();
    syncProject = vi.fn(() => ['a.md']);
    loadProjects = vi.fn(() => [
      { id: 'routekit-shell-core', root: SHELL_ROOT }, // ← the shell, in its own registry
      { id: 'child-a', root: kid },
    ]);
  });
  afterEach(() => {
    for (const d of [SHELL_ROOT, kid]) if (d) rmSync(d, { recursive: true, force: true });
  });

  it('SKIPS the shell record and still syncs the real child', async () => {
    await handleProjectCommand(
      { sub: 'sync', kv: { all: true }, SHELL_ROOT },
      { syncProject, loadProjects, processExit },
    );

    // POSITIVE CONTROL: the loop genuinely ran and reached the fixer for a real child. Without this,
    // "the shell wasn't synced" would also be true of a command that bailed before the loop.
    expect(syncProject).toHaveBeenCalledWith(expect.objectContaining({ projectRoot: kid, projectId: 'child-a' }));
    // THE CLAIM.
    expect(syncProject).not.toHaveBeenCalledWith(expect.objectContaining({ projectRoot: SHELL_ROOT }));
    expect(syncProject).toHaveBeenCalledTimes(1);
    // A skip is not a failure: the command still succeeds, and the shell is not in the denominator.
    expect(processExit).toHaveBeenCalledWith(0);
  });
});
