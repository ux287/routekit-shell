...
describe("RAG import redirect — consumers route through the barrel", () => {
  ...
  it("STRUCTURAL INVARIANT (backlog.feat.rag-host-hooks-inversion): no file under src/rag/** statically imports outside src/rag/ — the substrate is liftable as @routekit/rag with no host cycle", () => {
    // A STATIC import/export-from whose specifier resolves outside src/rag/ would, after the physical
    // @routekit/rag split, become a package importing back into its host (mcp-rks) → cross-package
    // cycle. Bare specifiers (npm deps, node: builtins) are fine; only relative escapes are offenders.
    const RAG = resolve(SRC, "rag");
    const fromRe = /^\s*(?:import|export)\b[^;]*?\bfrom\s*["']([^"']+)["']/gm;
    const offenders = [];
    for (const file of walkMjs(RAG)) {
      const src = readFileSync(file, "utf8");
      let m;
      fromRe.lastIndex = 0;
      while ((m = fromRe.exec(src)) !== null) {
        const spec = m[1];
        if (!spec.startsWith(".")) continue; // bare npm / node builtin — fine
        const resolved = resolve(dirname(file), spec);
        if (resolved !== RAG && !resolved.startsWith(RAG + "/")) {
          offenders.push(`${file.slice(SRC.length + 1)} → ${spec}`);
        }
      }
    }
    expect(offenders, "src/rag/** must not statically import outside src/rag/").toEqual([]);
  });
