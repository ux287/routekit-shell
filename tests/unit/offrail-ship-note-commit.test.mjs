...
describe('commitAndPushNote — off-rail story-note persistence (bug #7)', () => {
  ...
  it('NO-REORDER INVARIANT: the ship-note step is wired AFTER advance_phase in guardrailsOn', () => {
    ...
    //
    // STRENGTHENED by backlog.fix.offrail-gate-block-findings-are-inert, which adds a SECOND
    // `step: "advance_phase"` literal for the suppressed branch. `indexOf` takes only the FIRST
    // occurrence, so the original single-index comparison would have stayed green while
    // silently no longer covering the branch that was added — assert EVERY occurrence precedes
    // `step: "ship-note"` instead. Both branches must sit above the note commit, because the
    // note commit persists a phase bump that only one of them performs.
    ...
    expect(shipNoteIdx).toBeGreaterThan(-1);

    const advanceIdxs = [];
    for (let i = src.indexOf('step: "advance_phase"'); i !== -1; i = src.indexOf('step: "advance_phase"', i + 1)) {
      advanceIdxs.push(i);
    }
    expect(advanceIdxs.length).toBeGreaterThan(0);
    for (const idx of advanceIdxs) {
      expect(shipNoteIdx).toBeGreaterThan(idx);
    }

    // And there is exactly ONE ship-note step in the whole module: the suppressed branch must
    // inherit its non-execution from the existing `phaseResult.ok` gate, never from a second one.
    expect((src.match(/step: "ship-note"/g) || []).length).toBe(1);
  });
});
