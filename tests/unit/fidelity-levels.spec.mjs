...
describe('applyFidelity', () => {
  ...
  describe('L3 - Full text', () => {
    it('returns complete content, with literal secrets still scrubbed', () => {
      // backlog.feat.rag-fidelity-ownership-scoped: L3 returns full CONTENT fidelity, but literal
      // secret values are never echoed — even to a caller authorized over an owned corpus. This is
      // still the positive L3 witness (full surrounding content returned), now also proving the scrub.
      const result = applyFidelity(sampleResult, FIDELITY_LEVELS.L3_FULL);
      expect(result.fidelity).toBe('L3');
      // Full surrounding content survives (sampleResult.text contains `password: hunter2`).
      expect(result.text).toContain('This is the full content with');
      expect(result.text).toContain('and more text here that goes on');
      // ...but the literal secret value is redacted at full fidelity.
