...
describe("a refinement that changes nothing ESCALATES", () => {
  it("returns refine_noop, does NOT say ok, and does NOT tell you to re-plan", async () => {
    ...
    //
    // This previously required `nextTool: "rks_exec_abort"`, i.e. "give up now". That was correct
    // only for a genuinely unconvergeable story, and this tool cannot tell that case from a false
    // no-op: the streak counter that distinguishes them lives on the Governor session, not here.
    // Directing an abort on the FIRST no-op is what killed healthy builds. The tool now points at
    // the actionable remedy — read `skipped`, fix the named cause, apply again — and the abort is
    // reached via `refine_apply.noop_repeated` after a consecutive repeat (pinned in
    // governor-state-refine-noop.test.mjs).
    expect(res.escalation?.nextTool).toBe("rks_refine_apply");
    expect(res.escalation?.guidance).toMatch(/second consecutive no-op|rks_exec_abort/i);
