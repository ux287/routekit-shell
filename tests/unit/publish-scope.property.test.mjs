 ...
 * publish-rks-public-profile.test.mjs.
 *
 * REWORK (backlog.feat.suppressible-public-publish): the profile under test is now a FIXTURE
 * rather than a read of the live .routekit/publish-profiles.yaml. These are PROPERTIES of the
 * include/exclude engine — "a private file never ships however many extra include globs you add",
 * "the exclude post-filter always wins" — and they hold for any allowlist-shaped profile. Reading
 * the live config only coupled them to whether the operator currently has a public mirror armed;
 * the properties themselves are unchanged and unweakened.
 */

// Allowlist-shaped fixture profile: the same privacy-by-omission structure as rks-public
// (explicit per-package includes, no bare packages/**, an exclude post-filter for cruft and
// the marketing/content surface).
const rksPublic = yaml.load(`
identity:
