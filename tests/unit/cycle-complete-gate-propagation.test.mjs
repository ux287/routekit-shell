...
describe("runCycleComplete's signature carries the opt-in, defaulting to refuse", () => {
  it("declares discardLocalCommits AND discardUncommitted with false defaults", () => {
    ...
    // backlog.fix.cycle-complete-discards-uncommitted-changes added the second opt-in; a regex,
    // not a fixed string, so parameter order and spacing may move without a false red.
    const sig = GIT_SHIP.match(/export async function runCycleComplete\(\{([^}]*)\}\) \{/);
    expect(sig, "runCycleComplete signature not found").toBeTruthy();
    expect(sig[1]).toMatch(/\bprojectRoot\b/);
    expect(sig[1]).toMatch(/\bprojectId\b/);
    expect(sig[1]).toMatch(/\bdiscardLocalCommits = false\b/);
    expect(sig[1]).toMatch(/\bdiscardUncommitted = false\b/);
  });

  it("a dirty-tree check precedes the reset in source order", () => {
    // backlog.fix.cycle-complete-discards-uncommitted-changes: the reset also destroys uncommitted
    // tracked changes, so its own consent must be checked before it — strictly `!== true`.
    const dirtyGateIdx = GIT_SHIP.indexOf("if (dirtyBefore.length > 0 && discardUncommitted !== true) {");
    const resetIdx = GIT_SHIP.indexOf("runGit(projectRoot, ['reset', '--hard', `origin/${working}`]);");
    const observeIdx = GIT_SHIP.indexOf("dirtyBefore = trackedDirtyPaths(projectRoot);");
    expect(dirtyGateIdx, "dirty-tree gate not found").toBeGreaterThan(-1);
    expect(observeIdx, "pre-sync status observation not found").toBeGreaterThan(-1);
    expect(observeIdx).toBeLessThan(dirtyGateIdx);
    expect(dirtyGateIdx).toBeLessThan(resetIdx);
  });
...
describe("all four production callers thread the opt-in", () => {
  ...
  it("caller 4's cycle_complete ship step carries preservedPaths on ok and dirtyPaths on refusal", () => {
    // backlog.fix.cycle-complete-discards-uncommitted-changes: the step must report what was
    // preserved (success) or which paths blocked the sync (refusal). The success half is driven
    // end-to-end in tests/integration/off-rail-scoped-staging.test.mjs; this pins both halves.
    const idx = GUARDRAILS.indexOf(
      "const cycleResult = await runCycleComplete({ projectRoot, discardLocalCommits: false });",
    );
    expect(idx).toBeGreaterThan(-1);
    const step = GUARDRAILS.slice(idx, idx + 2000);
    expect(step).toContain("...(cycleResult.ok ? { preservedPaths: cycleResult.preservedPaths ?? [] } : {}),");
    expect(step).toContain("dirtyPaths: cycleResult.dirtyPaths ?? [],");
  });

  it("NO production caller passes discardUncommitted, let alone hardcodes it true", () => {
    // backlog.fix.cycle-complete-discards-uncommitted-changes: discardUncommitted is a
    // function-level parameter ONLY. Every production caller preserves uncommitted work by
    // default; none may opt in on the user's behalf.
    for (const [name, src] of [
      ["git-ship.mjs", GIT_SHIP],
      ["server.mjs", SERVER],
      ["story-ship.mjs", STORY_SHIP],
      ...
      expect(callArgs.length, `${name}: no call site found`).toBeGreaterThan(0);
      for (const args of callArgs) {
        expect(args, `${name} call site must not hardcode discardUncommitted: true`)
          .not.toMatch(/discardUncommitted\s*:\s*true/);
        expect(args, `${name} call site must not pass discardUncommitted at all`)
          .not.toMatch(/discardUncommitted/);
      }
    }
  });
...
describe("the MCP schema exposes the opt-in on BOTH sides", () => {
  ...
  it("discardUncommitted is NOT exposed on the MCP surface at all", () => {
    // backlog.fix.cycle-complete-discards-uncommitted-changes (ARCH round 1, Item 5 option (b)):
    // an MCP switch that discards uncommitted work would be a one-flag route back to the defect.
    // From the MCP surface the user commits or stashes.
    expect(SERVER).not.toContain("discardUncommitted");
  });
