 ...
 *   - projectId is pinned to "routekit-shell" — the PUBLIC MIRROR's own identity, and the mirror is
 *     where the silence actually happened. NOTE: this id no longer selects any special code path.
 *     sync.mjs once skipped the projectId-substitution readdir when projectId === "routekit-shell";
 *     THAT EXEMPTION IS DELETED (the substitution sentinel is now __RKS_SOURCE_PROJECT__, which is
 *     not a projectId, so no target can ever equal it and exempting the mirror would merely ship it
 *     an unsubstituted sentinel). The substitution readdir now runs for ANY truthy projectId.
 *     THE OPERATIVE SELF-PROTECTION IS THE SAME-DIRECTORY REFUSALS in sync.mjs — the entry guard
 *     and the point-of-destruction guard, which compare device+inode and throw SelfSyncRefusedError
 *     before the rm. That is what these assertions pin, and it is independent of the projectId.
