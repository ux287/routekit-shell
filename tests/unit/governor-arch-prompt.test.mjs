...
describe("governor-arch.md — structural requirements", () => {
  ...
  it("writes a scalar verdict to frontmatter, never a nested object", () => {
    // arch_guidance used to be written as a nested object passed to
    // dendron_update_field, whose value contract is string-or-array only. On the
    // agent route that got JSON.stringify'd into a natural-language request and
    // re-emitted by an LLM, landing multi-thousand-word verdicts in YAML
    // frontmatter as compounding escaped JSON — one note reached 50KB.
    ...
    // No dendron_update_field call may pass an object literal as its value.
