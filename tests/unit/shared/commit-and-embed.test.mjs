...
function setupExecSync(files = 'notes/foo.md\nnotes/bar.md') {
  // TWO execFileSync calls now, not one. The commit goes through execFileSync
  // (`git commit --cleanup=verbatim -F -`), and so does the changed-file read-back, which
  // moved from `execSync('git diff --name-only HEAD~1..HEAD')` to
  // `execFileSync('git', ['diff-tree', …, '--root', commitId])` — `HEAD~1` does not resolve
  // on a root commit, and `--root` is what makes a parentless commit readable.
  // execFileSync was chosen over spawnSync precisely so it still THROWS on non-zero; a
  // spawnSync port without a status check would return an empty list in silence.
  // Only rev-parse still uses execSync.
  execFileSync
    .mockReturnValueOnce(undefined)          // git commit
    .mockReturnValueOnce(`${files}\n`);      // git diff-tree --root <sha>
  execSync.mockReturnValueOnce(`${FAKE_SHA}\n`); // git rev-parse HEAD
}
