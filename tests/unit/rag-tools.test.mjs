...
import { setRagHostHooks } from '@routekit/rag/tools';

const ragToolsSrc = fs.readFileSync(
  path.resolve('packages/rag/src/tools.mjs'),
  'utf8'
);
...
// Stage 3 (backlog.feat.rag-package-absorb-scripts-stage3): the CLI config seam and the absorbed
// embed/query/init pipeline are now sibling modules under packages/rag/src/. tools.mjs reaches
// @routekit/cli's config via the shared dynamic loader and loads the pipeline via sibling dynamic
// import — NOT the old repoRoot-relative loadScript("scripts/rag/*.mjs") indirection.
describe('Stage 3 config seam + sibling pipeline imports', () => {
  it('imports getRagPathsFor from the shared rag-config-loader (no static @routekit/cli config import)', () => {
    expect(ragToolsSrc).toMatch(/import\s*\{\s*getRagPathsFor\s*\}\s*from\s*['"]\.\/rag-config-loader\.mjs['"]/);
    expect(ragToolsSrc).not.toMatch(/from\s*['"][^'"]*packages\/cli\/src\/rag\/config\.mjs['"]/);
  });

  it('loads the absorbed pipeline via sibling dynamic import, not loadScript("scripts/rag/*")', () => {
    expect(ragToolsSrc).toContain('await import("./init.mjs")');
    expect(ragToolsSrc).toContain('await import("./embed.mjs")');
    expect(ragToolsSrc).toContain('await import("./query.mjs")');
    expect(ragToolsSrc).not.toContain('loadScript("scripts/rag');
  });
});
