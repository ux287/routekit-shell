...
// backlog.fix.upgrade-all-counts-non-upgrades-as-upgraded — the headline counts MUTATIONS.
//
// `Upgraded N/M children` used to be a did-not-fail count: already-current, refused-downgrade,
// gated-major and dry-run children all satisfy `report.ok` and all used to increment it. These
// fixtures model production faithfully — upgrade.mjs sets `stampAdvanced` true only BELOW every
// early return, so a no-op report always carries `stampAdvanced: false`.

/** A report for a child that upgradeProject did NOT mutate. */
function noopReport(over = {}) {
  return okReport({ stampAdvanced: false, restartRequired: false, ...over });
}

describe("handleProjectCommand — upgrade --all headline counts only mutated children", () => {
  let logs;
  let spy;
  ...
  const headline = () => logs.find((l) => l.startsWith("Upgraded ")) || "";

  function makeChildren(n) {
    return Array.from({ length: n }, (_, i) => ({
      id: `child-${i}`,
      root: mkdtempSync(path.join(os.tmpdir(), `count-kid-${i}-`)),
    }));
  }

  function cleanup(children) {
    for (const c of children) rmSync(c.root, { recursive: true, force: true });
  }

  it("already-current batch reports 0 upgraded, explains why, and still exits 0", async () => {
    const children = makeChildren(3);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(() =>
          noopReport({ boundary: "none", from: "0.51.0", to: "0.51.0", warnings: ["Already at 0.51.0 — nothing to do."] }),
        ),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true }, SHELL_ROOT }, deps);
      expect(deps.upgradeProject).toHaveBeenCalledTimes(3);
      expect(headline()).toContain("Upgraded 0/3 children");
      expect(headline()).toMatch(/already current/i);
      expect(deps.processExit).toHaveBeenCalledWith(0);
    } finally {
      cleanup(children);
    }
  ...
  it("refused-downgrade batch reports 0 upgraded and still exits 0", async () => {
    const children = makeChildren(2);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(() =>
          noopReport({ boundary: "downgrade", from: "0.52.0", to: "0.51.0", warnings: ["child is newer than the shell — refusing to downgrade"] }),
        ),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true }, SHELL_ROOT }, deps);
      expect(deps.upgradeProject).toHaveBeenCalledTimes(2);
      expect(headline()).toContain("Upgraded 0/2 children");
      expect(headline()).toMatch(/downgrade/i);
      expect(deps.processExit).toHaveBeenCalledWith(0);
    } finally {
      cleanup(children);
    }
  ...
  it("gated-major batch reports 0 upgraded and still exits 0", async () => {
    const children = makeChildren(2);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(() =>
          noopReport({ boundary: "major", gated: true, warnings: ["major upgrade not yet supported"] }),
        ),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true }, SHELL_ROOT }, deps);
      expect(deps.upgradeProject).toHaveBeenCalledTimes(2);
      expect(headline()).toContain("Upgraded 0/2 children");
      expect(headline()).toMatch(/gated/i);
      expect(deps.processExit).toHaveBeenCalledWith(0);
    } finally {
      cleanup(children);
    }
  ...
  it("dry-run batch reports 0 upgraded, names the run a preview, and still exits 0", async () => {
    const children = makeChildren(2);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(() => noopReport({ boundary: "minor", dryRun: true })),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true, "dry-run": true }, SHELL_ROOT }, deps);
      expect(deps.upgradeProject).toHaveBeenCalledTimes(2);
      expect(headline()).toContain("Upgraded 0/2 children");
      expect(headline()).toMatch(/preview/i);
      expect(headline()).toMatch(/dry-run/i);
      expect(deps.processExit).toHaveBeenCalledWith(0);
    } finally {
      cleanup(children);
    }
  ...
  it("mixed batch discriminates: only the mutated child is counted", async () => {
    const children = makeChildren(3);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(({ projectId }) => {
          if (projectId === "child-0") return okReport({ boundary: "patch", stampAdvanced: true });
          if (projectId === "child-1") return noopReport({ boundary: "none", warnings: ["Already at 0.20.19 — nothing to do."] });
          return noopReport({ boundary: "major", gated: true, warnings: ["major upgrade not yet supported"] });
        }),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true }, SHELL_ROOT }, deps);
      expect(headline()).toContain("Upgraded 1/3 children");
      expect(headline()).toMatch(/already current/i);
      expect(headline()).toMatch(/gated/i);
      expect(deps.processExit).toHaveBeenCalledWith(0);
    } finally {
  ...
  it("a failing child does not contribute to the count and still exits 1", async () => {
    const children = makeChildren(2);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(({ projectId }) =>
          projectId === "child-0"
            ? okReport({ boundary: "patch", stampAdvanced: true })
            : okReport({ ok: false, boundary: "invalid", stampAdvanced: false, warnings: ["unreadable version stamp"] }),
        ),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true }, SHELL_ROOT }, deps);
      expect(headline()).toContain("Upgraded 1/2 children");
      expect(headline()).toContain("1 failed");
      expect(deps.processExit).toHaveBeenCalledWith(1);
    } finally {
      cleanup(children);
    }
  ...
  it("a thrown upgradeProject does not contribute to the count and still exits 1", async () => {
    const children = makeChildren(2);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(({ projectId }) => {
          if (projectId === "child-0") return noopReport({ boundary: "none", warnings: ["Already at 0.20.19 — nothing to do."] });
          throw new Error("boom");
        }),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true }, SHELL_ROOT }, deps);
  ...
  it("the explaining summary adds no extra line carrying the per-child shell root", async () => {
    const children = makeChildren(2);
    try {
      const deps = makeDeps({
        loadProjects: vi.fn(() => children),
        upgradeProject: vi.fn(() =>
          noopReport({ boundary: "none", shellRoot: "/resolved/shell", warnings: ["Already at 0.20.19 — nothing to do."] }),
        ),
      });
      await handleProjectCommand({ sub: "upgrade", kv: { all: true }, SHELL_ROOT }, deps);
      expect(logs.filter((l) => l.includes("/resolved/shell"))).toHaveLength(2);
      expect(headline()).not.toContain("/resolved/shell");
    } finally {
      cleanup(children);
    }
  });
...
describe("handleProjectCommand — upgrade printers render the resolved shell root", () => {
  ...
  it("single-project no-op path still prints the shell root", async () => {
    ...
    const deps = makeDeps({
      upgradeProject: vi.fn(() =>
        okReport({ boundary: "none", stampAdvanced: false, restartRequired: false, shellRoot: "/resolved/shell", warnings: ["Already at 0.20.19 — nothing to do."] }),
      ),
    });
    await handleProjectCommand({ sub: "upgrade", kv: { id: "fixture-child" }, SHELL_ROOT }, deps);
    expect(logs.join("\n")).toContain("/resolved/shell");
