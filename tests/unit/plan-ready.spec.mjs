...
`);
    ...
    it("non-existent target with no op:create and no body directive produces stale_target_path", async () => {
      projectRoot = makeTempProject(`---
id: backlog.test-story
title: Test Story
phase: ready
...
`);
      ...
      // INVERTED by backlog.fix.plan-ready-targetfiles-path-offset-revalidation — see the
      // sibling case in plan-ready-frontmatter-create.test.mjs. A frontmatter op:edit target
      // that does not resolve is a stale reference; the old suggestion would have created an
      // empty file rather than surfacing the bad path.
