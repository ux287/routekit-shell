...
describe('disk_fetch_context — real handler', () => {
  ...
  it('leaves the story note body unchanged and returns content out of band', async () => {
    ...
    // the rest. The .trim() is applied HERE, visibly, because these assertions hold
    // a content invariant and not byte-identity: re-serialising the note shifts a
    // leading newline, so an untrimmed comparison asserts something false.
    expect(bodyOf(after).trim()).toBe(bodyOf(before).trim());
  ...
  it('the note body does NOT grow across repeated disk-fetch cycles', async () => {
    ...
    // a content invariant and not byte-identity: re-serialising the note shifts a
    // leading newline, so an untrimmed comparison asserts something false.
    const initial = bodyOf(fs.readFileSync(notePath, 'utf8')).trim();
    ...
    for (let i = 0; i < 3; i++) {
      ...
      expect(bodyOf(fs.readFileSync(notePath, 'utf8')).trim()).toBe(initial);
    }
  });
});
