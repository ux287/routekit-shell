...
describe("Pre-flight validation", () => {
  ...
  // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. This is the only case here
  // that passes a storyId, and preflight resolves it against notes/. notes/backlog.* is on no
  // include pattern, so in a published tree preflight correctly reports "story not found" and
  // the happy path cannot hold. The sibling cases below — missing projectId, invalid playbook,
  // nonexistent story — all assert FAILURE and are unaffected, so the negative coverage this
  // suite exists for keeps shipping intact.
  const lifecycleStoryAvailable = fs.existsSync(
    path.join(NOTES_DIR, "backlog.governor.thinking-log.md"),
  );

  it.skipIf(!lifecycleStoryAvailable)("passes with valid lifecycle params", () => {
