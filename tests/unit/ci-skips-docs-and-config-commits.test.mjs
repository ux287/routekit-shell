...
describe('AC1 — on.push.paths-ignore includes all A2 docs/config entries', () => {
  for (const entry of A2_NEW_ENTRIES) {
    ...
    describe('AC-Ideas — on.push.paths-ignore includes ideas/design/notes entries', () => {
      const NEW_ENTRIES_IDEAS = [
        'notes/ideas.**',
        'notes/design.**',
        'notes/notes.**',
      ];
      for (const entry of NEW_ENTRIES_IDEAS) {
        it(`on.push.paths-ignore contains '${entry}'`, () => {
          expect(CI_CONFIG.on.push['paths-ignore']).toContain(entry);
        });
      }
    ...
    describe('AC-Ideas — on.pull_request.paths-ignore mirrors push for ideas/design/notes', () => {
      const NEW_ENTRIES_IDEAS = [
        'notes/ideas.**',
        'notes/design.**',
        'notes/notes.**',
...
describe('AC4 — path-filter negation sanity', () => {
  ...
  it('commit touching notes/how-to.update-fixtures.md → CI skips', () => {
    expect(allFilesIgnored(['notes/how-to.update-fixtures.md'])).toBe(true);
      ...
      it('SEMANTIC WITNESS (ideas): commit touching ONLY notes/ideas.triage.md → CI skips', () => {
        expect(allFilesIgnored(['notes/ideas.triage.md'])).toBe(true);
      });
      
      it('SEMANTIC WITNESS (design): commit touching ONLY notes/design.some-topic.md → CI skips', () => {
        expect(allFilesIgnored(['notes/design.some-topic.md'])).toBe(true);
      });
      
      it('SEMANTIC WITNESS (notes): commit touching ONLY notes/notes.some-topic.md → CI skips', () => {
        expect(allFilesIgnored(['notes/notes.some-topic.md'])).toBe(true);
      });
      
      it('SEMANTIC NEGATION GUARD: commit touching notes/ideas.triage.md + packages/mcp-rks/src/foo.mjs → CI runs', () => {
        expect(allFilesIgnored(['notes/ideas.triage.md', 'packages/mcp-rks/src/foo.mjs'])).toBe(false);
      });
    });

    describe('AC-Ideas — semantic witness for pull_request block', () => {
      // Replicate the same witness tests on the pull_request paths-ignore array
      const PR_IGNORE_PATTERNS = CI_CONFIG.on.pull_request['paths-ignore'];
      
      function matchesIgnorePR(filePath) {
        return PR_IGNORE_PATTERNS.some((pattern) => {
          const re = new RegExp(
            '^' +
              pattern
                .replace(/[.+?^${}()|[\]\\]/g, '\\$&')
      ...
      function allFilesIgnoredPR(files) {
        return files.length > 0 && files.every(matchesIgnorePR);
      }
      
      it('SEMANTIC WITNESS (ideas, PR): commit touching ONLY notes/ideas.triage.md → CI skips', () => {
        expect(allFilesIgnoredPR(['notes/ideas.triage.md'])).toBe(true);
      });
      
      it('SEMANTIC WITNESS (design, PR): commit touching ONLY notes/design.some-topic.md → CI skips', () => {
        expect(allFilesIgnoredPR(['notes/design.some-topic.md'])).toBe(true);
      });
      
      it('SEMANTIC WITNESS (notes, PR): commit touching ONLY notes/notes.some-topic.md → CI skips', () => {
        expect(allFilesIgnoredPR(['notes/notes.some-topic.md'])).toBe(true);
      });
    });
    
    describe('BLANKET GUARD — paths-ignore does not use wildcard notes/**', () => {
      it('on.push.paths-ignore does not contain blanket notes/** entry', () => {
        expect(CI_CONFIG.on.push['paths-ignore']).not.toContain('notes/**');
      });
      
      it('on.pull_request.paths-ignore does not contain blanket notes/** entry', () => {
        expect(CI_CONFIG.on.pull_request['paths-ignore']).not.toContain('notes/**');
      });
    });
...
describe('Story A1 regression guard (do not break A1)', () => {
  it("git-release.mjs still contains the literal '[skip ci]' substring from A1", () => {
    ...
    describe('AC-Ideas — push and pull_request paths-ignore symmetry', () => {
      it('on.push.paths-ignore and on.pull_request.paths-ignore are element-for-element identical', () => {
        expect(CI_CONFIG.on.push['paths-ignore']).toEqual(CI_CONFIG.on.pull_request['paths-ignore']);
      });
    });
