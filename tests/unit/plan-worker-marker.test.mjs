...
import { classifyMarkerFailure } from "../../packages/mcp-rks/src/server/failure-classification.mjs";
...
describe("failureClass crosses the marker boundary", () => {
  it("a failing result keeps its stamped failureClass", () => {
    const m = buildMarkerUpdate({ ok: false, failureClass: "output_invalid", reason: "x" });
    expect(m.failureClass).toBe("output_invalid");
  });

  it("a successful result never carries failureClass", () => {
    const m = buildMarkerUpdate({ ok: true, failureClass: "output_invalid" });
    expect(m.failureClass).toBeUndefined();
  });

  it("null or undefined failureClass adds no key", () => {
    expect(buildMarkerUpdate({ ok: false, failureClass: null })).not.toHaveProperty("failureClass");
    expect(buildMarkerUpdate({ ok: false, failureClass: undefined })).not.toHaveProperty("failureClass");
  });

  it("the stamp survives the marker and wins over re-derivation", () => {
    const marker = buildMarkerUpdate({
      ok: false, status: "refinement_required", reason: "stale_edits", failureClass: "structural",
    });
    expect(classifyMarkerFailure(marker).failureClass).toBe("structural");
  });
});
