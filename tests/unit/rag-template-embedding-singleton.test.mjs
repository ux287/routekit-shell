...
const SHELL_QUERY = "packages/rag/src/query.mjs";
...
describe("RAG embedding-pipeline consolidation — shell scripts/rag", () => {
  it("packages/rag/src/query.mjs delegates to the shared singleton", () => {
    const src = read(SHELL_QUERY);
    expect(src).toContain("getSharedEmbeddingPipeline");
    expect(src).toMatch(/@routekit\/rag\/embedding-pipeline/);
  });

  it("packages/rag/src/query.mjs holds no verbatim model load", () => {
    expect(hasVerbatimLoad(read(SHELL_QUERY))).toBe(false);
  });

  it("no verbatim model load remains in the relocated shell rag consumers", () => {
    // The relocated consumers delegate to the shared singleton; only that singleton
    // (embedding-pipeline.mjs) legitimately owns the verbatim Xenova load, so scan the
    // five moved modules specifically rather than all of packages/rag/src.
    const consumers = ["embed.mjs", "query.mjs", "init.mjs", "utils.mjs", "benchmark-chunker.mjs"];
    const offenders = consumers.filter((f) => {
      const p = resolve(ROOT, "packages/rag/src", f);
      return existsSync(p) && hasVerbatimLoad(readFileSync(p, "utf8"));
    });
