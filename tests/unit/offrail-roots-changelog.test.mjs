...
describe('offRail.roots — CHANGELOG.md entry', () => {
  ...
  it('has exactly 30 string entries and every entry is a non-empty string', () => {
    expect(realRoots).toHaveLength(30);
  ...
  it('leaves offRail.enabled true and the surrounding config intact', () => {
    ...
    // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. `.rks/project.json` is one
    // of the seven classes normalizeExportIdentity rewrites, so the id is the upstream name here
    // and the public name on the mirror. Both are CORRECT in their own topology; what would be
    // wrong is a third value. Accept exactly the two the profile declares — this still fails on
    // any other id, and it cannot be satisfied by comparing the value to itself.
    const profileSrc = fs.readFileSync(
      path.join(REPO_ROOT, '.routekit', 'publish-profiles.yaml'), 'utf8',
    );
    expect(realProjectJson.id).toBeTruthy();
    expect(
      profileSrc,
      `project id "${realProjectJson.id}" is neither identity value declared in the publish profile`,
    ).toMatch(new RegExp(`^\\s*(from|to):\\s*${realProjectJson.id}\\s*$`, 'm'));
