...
import { resolveHookPath } from '../helpers/hook-path.mjs';

const HOOK = resolveHookPath('write/redirect-github-tools-to-governor.mjs');
...
function runHook(toolName, toolInput = {}, env = {}) {
  ...
  // Discriminate a module-load failure from a logic failure. When an off-rail
  // session relocates write/ under hooks.bak/, this hook's relative import of
  // ../system/hook-output.mjs used to fail with ERR_MODULE_NOT_FOUND and exit 1
  // before running a line — indistinguishable, by exit code alone, from a real
  // behavioural regression. (backlog.fix.off-rail-hook-loadability)
