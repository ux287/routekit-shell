...
//
// A skill can be DELIBERATELY excluded from the publish set: publish-profiles.yaml GROUP 3
// excludes `.claude/skills/whitepaper/**` because its executable (`packages/whitepaper/**`) is
// excluded too, so the mirror must not ship a skill whose binary is absent. The manifest itself
// DOES ship, so in a published tree such a skill is declared but not on disk.
//
// That is the exclusion working, not manifest drift. A phantom is therefore forgiven ONLY when
// the profile explicitly names that skill's directory — every other phantom is still a hard
// failure, in BOTH topologies. Keying on "is it missing" instead would have turned a real drift
// detector into one that accepts any absence.
const PROFILE_SRC = (() => {
  const p = path.join(ROOT, ".routekit", "publish-profiles.yaml");
  return fs.existsSync(p) ? fs.readFileSync(p, "utf8") : "";
})();
const deliberatelyUnshipped = (skill) => PROFILE_SRC.includes(`.claude/skills/${skill}/`);
...
describe("skills-manifest.json is in lockstep with the filesystem", () => {
  ...
  it("the unshipped-skill carve-out is narrow and real (non-vacuity control)", () => {
    // The profile must actually be readable, and the carve-out must NOT swallow an arbitrary
    // name — otherwise the phantom check below would accept anything.
    expect(PROFILE_SRC.length).toBeGreaterThan(0);
    expect(deliberatelyUnshipped("definitely-not-an-excluded-skill")).toBe(false);
  });
  ...
  it("declares EXACTLY the skills that exist on disk — no more, no less", () => {
    ...
    const phantom = m.skills.filter((s) => !onDisk.includes(s) && !deliberatelyUnshipped(s));
    const declared = m.skills.filter((s) => !deliberatelyUnshipped(s) || onDisk.includes(s)).sort();
  ...
  it("every declared skill is actually present (findMissingSkills is clean on a healthy tree)", () => {
    const m = loadSkillsManifest(ROOT);
    const missing = findMissingSkills(ROOT, m.skills).filter((s) => !deliberatelyUnshipped(s));
