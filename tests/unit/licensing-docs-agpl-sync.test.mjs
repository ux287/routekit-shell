...
describe('licensing docs are synced to the AGPL-3.0 dual-licence posture', () => {
  ...
  describe('no functional CONTRIBUTOR-LICENSE.md reference survives', () => {
    ...
    it('leaves exactly one surviving reference, and it is comment-only', () => {
      // CHANGELOG.md is a historical record, not a functional reference: every release note
      // that documents the CONTRIBUTOR-LICENSE.md -> CLA.md rename adds an occurrence, and that
      // is correct changelog behaviour. Excluded on the same footing as notes/. The sweep's
      // subject is live references, and every OTHER scope keeps full teeth below.
      const HISTORICAL = new Set([SELF, 'CHANGELOG.md']);
      const files = tracked().filter((f) => !f.startsWith('notes/') && !HISTORICAL.has(f));
