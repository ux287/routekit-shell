...
describe("runner prompt caching — cache_control on the stable prefix", () => {
  ...
  it("conversation prefix IS cache-controlled: the last message's last content block carries cache_control", async () => {
    // Reversed from the prior "message history is NOT cache-controlled" assertion: conversation-
    // prefix caching (backlog.fix.conversation-prefix-caching) now marks the growing transcript.
    const calls = [];
    await runAgent({ ...CONFIG(), _testClient: makeMock(calls) });
    const req = calls[0];
    const lastMsg = req.messages[req.messages.length - 1];
    // The last message is converted to block form and its last content block carries the marker.
    expect(Array.isArray(lastMsg.content)).toBe(true);
    const lastBlock = lastMsg.content[lastMsg.content.length - 1];
    expect(lastBlock.cache_control).toEqual({ type: "ephemeral" });
    // The marker lives on the CONTENT BLOCK (Anthropic contract), not the message object — a
    // guard against re-introducing the old `expect(m.cache_control).toBeUndefined()` loop, which
    // stayed green while asserting the opposite of the shipped behavior.
