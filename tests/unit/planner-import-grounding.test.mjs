...
describe('reviewPlan — import grounding (undeclared package detection)', () => {
  ...
  describe('WORKSPACES: the declared set resolves from the importing file, not the repo root', () => {
    // A workspace member declares its siblings in its OWN manifest. The root
    // declares `workspaces` and zero members, so a root-only lookup reported every
    // intra-workspace import as undeclared and blocked the story outright.
    function writeWorkspacePkg(relDir, deps = {}, devDeps = {}) {
      const dir = path.join(root, relDir);
      fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(
        path.join(dir, 'package.json'),
        JSON.stringify({ name: `fixture-${relDir.replace(/\//g, '-')}`, version: '1.0.0', dependencies: deps, devDependencies: devDeps }, null, 2),
      );
    }

    it('does NOT flag a workspace sibling declared in the importing package manifest', async () => {
      writePkg({}, { vitest: '^2.1.9' });                      // root declares no @growth/* at all
      writeWorkspacePkg('packages/cli', { '@growth/archive': '*' });
      const plan = { steps: [createStep('packages/cli/src/index.mjs', "import { writeRun } from '@growth/archive';")] };

      const r = await reviewPlan({ projectRoot: root, plan });
      expect(findImportIssue(r), 'a sibling declared in packages/cli/package.json must not be flagged').toBeFalsy();
    });

    it('STILL flags a package declared nowhere, in that same workspace file', async () => {
      // Anti-vacuity for the case above: the walk-up must not simply disable the check.
      writePkg({}, { vitest: '^2.1.9' });
      writeWorkspacePkg('packages/cli', { '@growth/archive': '*' });
      const plan = { steps: [createStep('packages/cli/src/index.mjs', "import pad from 'left-pad';")] };

      const r = await reviewPlan({ projectRoot: root, plan });
      const issue = findImportIssue(r);
      expect(issue, 'an undeclared package must still be caught inside a workspace member').toBeTruthy();
      expect(issue.package).toBe('left-pad');
    });

    it('a root-hoisted devDependency stays visible to a workspace file (union, not replacement)', async () => {
      writePkg({}, { vitest: '^2.1.9' });
      writeWorkspacePkg('packages/cli', { '@growth/archive': '*' });
      const plan = { steps: [createStep('packages/cli/src/index.mjs', "import { describe } from 'vitest';")] };

      const r = await reviewPlan({ projectRoot: root, plan });
      expect(findImportIssue(r)).toBeFalsy();
    });

    it('a SIBLING manifest does not leak — resolution walks UP, never sideways', async () => {
      // The decisive scoping case. A naive "collect every package.json in the repo"
      // implementation passes the three above and fails this one.
      writePkg({}, { vitest: '^2.1.9' });
      writeWorkspacePkg('packages/cli', { '@growth/archive': '*' });
      writeWorkspacePkg('packages/archive', { '@growth/schema': '*' });
      const plan = { steps: [createStep('packages/cli/src/index.mjs', "import { Item } from '@growth/schema';")] };

      const r = await reviewPlan({ projectRoot: root, plan });
      const issue = findImportIssue(r);
      expect(issue, '@growth/schema is declared only by a SIBLING and must still be flagged').toBeTruthy();
      expect(issue.package).toBe('@growth/schema');
    });
