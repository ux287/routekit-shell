...
describe('Subprocess timeout rule for new spawn sites (ARCH ask)', () => {
  it('every spawnSync call ADDED for the diagnostic feature has an explicit timeout option', () => {
    ...
    // Scoped by FUNCTION BOUNDARY, not a fixed-width window. The previous form
    // anchored on the literal `const ciCheck = spawnSync` and sliced 800 chars;
    // when the gate moved into evaluateCiGate and took an injectable `spawn`,
    // that indexOf returned -1 and the slice silently degraded. Asserting every
    // spawn site inside the helper carries a timeout survives restructuring.
    const gateStart = GIT_RELEASE_SRC.indexOf('export function evaluateCiGate');
    expect(gateStart, 'evaluateCiGate not found — anchor has drifted').toBeGreaterThan(-1);
    const afterGate = GIT_RELEASE_SRC.indexOf('\nexport ', gateStart + 1);
    const gateSrc = GIT_RELEASE_SRC.slice(gateStart, afterGate > -1 ? afterGate : undefined);

    const spawnCalls = (gateSrc.match(/\bspawn\s*\(/g) || []).length;
    const timeouts = (gateSrc.match(/timeout:\s*\d/g) || []).length;
    expect(spawnCalls, 'the gate must spawn at least the gh query').toBeGreaterThan(0);
    expect(timeouts, 'every spawn site in the gate needs an explicit timeout').toBeGreaterThanOrEqual(spawnCalls);
  });
});
