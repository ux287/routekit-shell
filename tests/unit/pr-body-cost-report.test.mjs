...
vi.mock("@routekit/telemetry/cost-report", () => ({
  generateCostReport: vi.fn(),
}));

import { spawnSync } from "child_process";
import { generateCostReport } from "@routekit/telemetry/cost-report";
...
describe("story-ship.mjs — cost report guard (source verification)", () => {
  ...
  it("story-ship.mjs imports generateCostReport from @routekit/telemetry/cost-report", () => {
    const source = fs.readFileSync(
      path.resolve("packages/mcp-rks/src/server/story-ship.mjs"), "utf8"
    );
    expect(source).toContain("@routekit/telemetry/cost-report");
    expect(source).toContain("generateCostReport");
  });

  it("story-ship.mjs carries the cost report as a ship STEP, not a PR-body block", () => {
    // backlog.fix.ship-delivery-implemented-twice.
    //
    // This assertion used to read `toContain("costBlock")` / `toContain("runGitPR")`,
    // pinning the PR-body vehicle. That vehicle is gone: this path opens no PR for any
    // topology, so a cost block had nowhere to be rendered. The report is now a
    // `cost_report` entry on the `steps` array the caller already reads.
    //
    // Worth recording WHY this had to be rewritten rather than left alone: after the
    // change it still PASSED — on a source COMMENT that mentioned the old identifiers
    // while describing their removal. A `toContain` over source text cannot tell code
    // from prose, so it reported the binding intact when the binding was gone.
    const source = fs.readFileSync(
      path.resolve("packages/mcp-rks/src/server/story-ship.mjs"), "utf8"
    );
    expect(source).toContain("step: 'cost_report'");
    // The PR-body identifier is gone from the file entirely — code AND comments, so this
    // assertion cannot itself be satisfied by prose.
    expect(source).not.toContain("costBlock");
  });
});
