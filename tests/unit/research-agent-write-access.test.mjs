...
//
// This door hardcoded `ok: true` and discarded the library return, so a library refusal
// arrived at the caller as a success. It now spreads the library result FIRST and
// re-applies filename and field after — pinned in BOTH directions, because a spread
// changes a payload by adding keys AND by dropping them, and only the adding direction
// was ever measured. The library result carries neither filename nor field.

describe('createResearchAgent — dendron_update_field forwards a library refusal', () => {
  let projectRoot;
  beforeEach(() => {
    projectRoot = makeTempDir('research-array-guard');
    setupProject(projectRoot);
  });

  async function seedNote(agent, filename) {
    const createTool = agent.tools.find(t => t.name === 'dendron_create_note');
    await createTool.execute({ filename, title: 'T', body: 'content' });
  }

  it('a shrinking write returns ok false rather than the old hardcoded ok true', async () => {
    const agent = createResearchAgent({ projectId: 'test', query: 'test', projectRoot });
    await seedNote(agent, 'notes.array-guard');
    const tool = agent.tools.find(t => t.name === 'dendron_update_field');

    const grow = await tool.execute({ filename: 'notes.array-guard', field: 'testFiles', value: ['a', 'b', 'c'] });
    expect(grow.ok).toBe(true);

    const shrink = await tool.execute({ filename: 'notes.array-guard', field: 'testFiles', value: ['a'] });
    expect(shrink.ok).toBe(false);
    expect(shrink.error).toBe('array_entries_removed');
    expect(shrink.removed).toEqual(['b', 'c']);
  });

  it('POSITIVE — a successful write still returns filename and field alongside the library ok', async () => {
    // Keys DROPPED is the direction that bites. A bare `return result` would drop both,
    // because the library result carries only ok, path and id. The absence of a
    // whole-object assertion elsewhere in this file is NOT evidence for this.
    const agent = createResearchAgent({ projectId: 'test', query: 'test', projectRoot });
    await seedNote(agent, 'notes.array-keys');
    const tool = agent.tools.find(t => t.name === 'dendron_update_field');

    const res = await tool.execute({ filename: 'notes.array-keys', field: 'title', value: 'Updated' });
    expect(res.ok).toBe(true);
    expect(res.filename).toBe('notes.array-keys');
    expect(res.field).toBe('title');
  });

  it('the refusal payload survives the door intact', async () => {
    const agent = createResearchAgent({ projectId: 'test', query: 'test', projectRoot });
    await seedNote(agent, 'notes.array-payload');
    const tool = agent.tools.find(t => t.name === 'dendron_update_field');
    await tool.execute({ filename: 'notes.array-payload', field: 'testFiles', value: ['a', 'b'] });

    const res = await tool.execute({ filename: 'notes.array-payload', field: 'testFiles', value: [] });
