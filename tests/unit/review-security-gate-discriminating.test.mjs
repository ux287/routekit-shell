 ...
 *
 * RETURN SHAPE: computeFinalVerdict returns
 * `{ verdict, downgradedFrom, downgradeReason }` since
 * backlog.fix.review-verdict-downgrade-legibility, so the assertions below read
 * `.verdict`. The discrimination behaviour this file guards is unchanged — the
 * 64-case parity matrix in tests/unit/ship-review-fail-closed.test.mjs proves
 * that against a transcription of the pre-change function.
 */
...
describe('security gate — direction 2: the gate must still fire', () => {
  ...
  it('that credential produces a blocking VERDICT even under verdictMode warn', () => {
    ...
    // Not a downgrade — a hard block that was never softened.
    expect(verdict.downgradedFrom).toBeNull();
  });
...
describe('computeFinalVerdict — the two verdict-path defects', () => {
  ...
  it('FIX (a): a block-severity security_issue still forces a block', () => {
    ...
    // An UPGRADE by hasPatternBlockers. Upgrades are never recorded as downgrades.
    expect(verdict.verdict).toBe('block');
    expect(verdict.downgradedFrom).toBeNull();
  });
  ...
  it('FIX (b): the hard-block set is read from policy, differentially', () => {
    ...
    expect(withCategory.verdict).toBe('block');
    ...
    expect(withoutCategory.verdict).toBe('warn');
    expect(withoutCategory.verdict).not.toBe(withCategory.verdict);
    // And the softened one now SAYS it was softened, while the hard block does not.
    expect(withCategory.downgradedFrom).toBeNull();
    expect(withoutCategory.downgradedFrom).toBe('block');
    expect(withoutCategory.downgradeReason).toContain('blockCategories');
  });
