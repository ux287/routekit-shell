...
describe("project registry helpers", () => {
  ...
  // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. seedProjectTemplateNotes
  // (packages/cli/src/project/index.js:333) globs notes/project-templates.content-cms.*.md,
  // which is on NO include pattern in publish-profiles.yaml. In a published tree the glob
  // matches nothing, so nothing can be seeded and `created.length > 0` cannot hold.
  // Condition-scoped on those template notes being present — wherever they are, this runs
  // unchanged and still fails if seeding or substitution regresses.
  const contentCmsTemplatesAvailable = (() => {
    try {
      return fs
        .readdirSync(path.join(process.cwd(), "notes"))
        .some((f) => f.startsWith("project-templates.content-cms.") && f.endsWith(".md"));
    } catch {
      return false;
    }
  })();

  it.skipIf(!contentCmsTemplatesAvailable)("seeds content-cms template notes with substitutions", async () => {
