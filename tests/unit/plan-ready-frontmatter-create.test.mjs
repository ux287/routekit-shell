...
`);
  ...
  it("non-existent target with op:edit and no body directive produces stale_target_path, not missing_create_directive", async () => {
    projectRoot = makeTempProject(`---
id: backlog.test-story
title: Test Story
phase: ready
...
`);
    ...
    // INVERTED by backlog.fix.plan-ready-targetfiles-path-offset-revalidation. This case is
    // a frontmatter op:edit target whose path does not resolve — a STALE REFERENCE, not a
    // missing create directive. The old diagnosis' suggestion ("add // CREATE FILE:") would
    // have turned a typo'd or moved path into a newly created empty file.
    //
    // Deliberately inverted rather than deleted: deleting it would silently drop the
    // no-collateral guard below, which is what catches a new push nested in the wrong branch.
    ...
    expect(stale[0].file).toBe("src/new-file.mjs");
    // The suggestion must NOT send the author toward creating the file.
    ...
    // Retained no-collateral witness. This target has NO directive, so it never enters the
    // hasDirective arm and the create-block check must stay silent. If the new push were
    // nested in the wrong branch, this goes red.
...
// ─── The field-facing warrant ────────────────────────────────────────────────
// backlog.fix.target-parse-fence-blind-false-targets.
//
// The unit tests in parse-targets.test.mjs pin the parser. This block is the only
// assertion that observes the REPORTED symptom at the code that emits it: a child
// project saw targetCount 53 against 10 real targets and 43 blocking
// missing_create_directive issues whose messages named JSDoc prose fragments, each
// carrying the suggestion `Add "// CREATE FILE: @param {string} handle" to story
// body`. That absurd remediation is the correct output of the old code on corrupt
// input, which is why the proof has to be taken here rather than at the parser.
//
// FIXTURE ARITHMETIC. targetCount is allTargets.length, the UNION of frontmatter
// targetFiles and body-parsed targets. The frontmatter below declares exactly the
// same two paths as the genuine body bullets, so the union size IS the genuine
// bullet count. Both paths exist on disk, or they would raise
// missing_create_directive themselves and poison the assertion.
describe("plan-ready: a fenced block in ## Target Files is content, not targets", () => {
  let projectRoot;
  ...
  // Written as concatenation so this file's own source cannot be mis-parsed by a
  // run of the parser over it.
  const STAR = " " + "*" + " ";
  ...
  const STORY = [
    ...
    "## Target Files",
    "",
    "- `src/alpha.mjs` — EDIT — genuine target one",
    "- `src/beta.mjs` — EDIT — genuine target two",
    "- `src/gamma.mjs` — EDIT — genuine target three",
    "",
    "```javascript",
    "/**",
    STAR + "Owned-account analytics storage and queries.",
    STAR + "@param {string} handle",
    STAR + "@returns {Promise<void>}",
    " */",
    "- this dash-led line is inside the fence too",
    "```",
    "",
    "## Next",
    "",
  ...
  it("raises no missing_create_directive naming a prose fragment, and targetCount is the genuine bullet count", async () => {
    projectRoot = makeTempProject(STORY);
    fs.mkdirSync(path.join(projectRoot, "src"), { recursive: true });
    fs.writeFileSync(path.join(projectRoot, "src/alpha.mjs"), "export const a = 1;\n");
    fs.writeFileSync(path.join(projectRoot, "src/beta.mjs"), "export const b = 2;\n");
    fs.writeFileSync(path.join(projectRoot, "src/gamma.mjs"), "export const g = 3;\n");

    const res = await runPlanReadyTool({
      projectRoot,
      problemId: "backlog.test-story",
      projectId: "p",
    });

    const createIssues = (res.issues || []).filter((i) => i.check === "missing_create_directive");
    const named = createIssues.map((i) => i.file || i.message).join(" | ");

    // The reported symptom: issues naming prose. Asserted by absence AND by the
    // count, so a fix that merely renamed the check would not pass.
    expect(createIssues, `prose fragments raised as files to create: ${named}`).toHaveLength(0);
    expect(named).not.toContain("@param");
    expect(named).not.toContain("Owned-account");

    // THE INFLATED COUNT. targetCount is emitted only on the multi-file check, which
    // fires at >= 3 targets — so the fixture carries THREE genuine bullets and the
    // check fires both before and after the fix. That makes the count a real
    // comparison rather than an absence: the old parser yielded 3 genuine + 3 prose
    // lines = 6, the new one yields 3. A 2-target fixture would emit no targetCount
    // at all and the assertion would witness nothing.
    const counted = [...(res.issues || []), ...(res.warnings || [])]
      .find((e) => e.check === "multi_file_blocked" || e.check === "multi_file_story");
    expect(counted, "the multi-file check must fire so targetCount is observable").toBeTruthy();
    expect(counted.targetCount, "prose fragments inflated the target count").toBe(3);
  });
});
