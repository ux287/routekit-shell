...
describe('package-lock.json is a valid v3 lockfile', () => {
  ...
  it('discovers the workspaces from the root package.json globs', () => {
    ...
    // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. packages/whitepaper/** is
    // EXCLUDED from the publish set, so the workspace glob genuinely resolves without it on a
    // mirror clone. Assert the named workspace only where the package actually exists; the
    // length floor above still proves the glob resolved something, so this cannot go vacuous.
    if (existsSync(resolve(ROOT, 'packages', 'whitepaper'))) {
