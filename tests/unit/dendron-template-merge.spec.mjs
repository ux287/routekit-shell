...
Body content here.`;
  ...
  describe('integration: backlog template application', () => {
    ...
    // is on no include pattern in publish-profiles.yaml, so both cases below threw ENOENT on
    // the mirror. CONDITION-SCOPED: wherever the template IS present they run unchanged and
    // still fail on a malformed template.
    const templateAvailable = existsSync(join(PROJECT_ROOT, 'notes', 'templates.backlog.md'));

    it.skipIf(!templateAvailable)('reads actual templates.backlog.md and verifies structure', () => {
    ...
    it.skipIf(!templateAvailable)('simulates full backlog note creation with content', () => {
