...
describe("ARCH_RESERVED_FIELDS — the set itself", () => {
  it("holds exactly the arch-owned fields", () => {
    // Exact-set, so any addition must be a deliberate decision made here. The two
    // cumulative fields were added by backlog.fix.arch-no-cumulative-round-bound-across-rebases;
    // they are ADVISORY but still reserved, because the guard is exact membership rather
    // than an arch_ prefix rule and an unlisted arch_* field is forgeable.
