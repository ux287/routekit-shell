...
describe.skip('runRelease gh release create', () => {
  ...
  async function loadRunRelease(spyFn) {
    ...
    // This suite is describe.skip'd (see the SKIPPED note above) precisely because the
    // vi.doMock + vi.resetModules + dynamic-import pattern does NOT intercept — restructuring
    // it to satisfy unit-tier-purity rule (b) would fire a real `gh release create` against
    // GitHub's API. The marker below must stay on the line IMMEDIATELY above the import: the
    // detector's opt-out lookup is [lines[i], lines[i-1], lines[j], lines[j-1]].
    // timeout-opt-out: describe.skip'd suite — the re-import never executes; restructuring it would hit the real gh API.
