...
const VERDICTS = {
  ...
  'tests/unit/review-ac-coverage-telemetry.test.mjs':
    'READS the stub deliberately — the acceptance criteria of backlog.feat.ship-review-ac-evidence-and-suppression-telemetry are about what the full review.complete emit records: that its acCoverage is the bound, redacted and normalized value the phase-advance gate reads, and that no credential survives into it. It drives the real runReview from packages/mcp-rks/src/server/review.mjs with a stubbed reviewer reply and reads the emitted payloads, filtered by event type and problemId, so reading the stub IS the requirement. It resolves through ensureTelemetryStorage because that is the seat review.mjs calls.',
...
describe('global @routekit/telemetry mock — stub surface lock', () => {
  ...
  it('the ensureTelemetryStorage seat exec.mjs calls is mockStorage and exposes startCorrelation', () => {
    // exec.mjs imports ensureTelemetryStorage from the barrel and calls it, so under the
    // global mock the collector it receives is mockStorage from tests/setup.mjs. This file
    // resolves the barrel THROUGH that mock with no escape, which is why the check lives
    // here and not in exec-lifecycle-emission-cardinality.test.mjs, whose barrel unmock
    // hands it the REAL collector. Use typeof and the in operator, never Object.keys --
    // the real collector carries startTimer and startCorrelation on its prototype.
    const storage = barrelMod.ensureTelemetryStorage();
    expect(storage).not.toBe(barrelMod.getTelemetryCollector());
    expect('startTimer' in storage).toBe(false);
    expect(typeof storage.startCorrelation).toBe('function');
  });
});
