...
// `services/sqliteService.ts` is not a real file in this repo, and extractExplicitEdits now
// refuses to adopt a candidate that neither resolves on disk nor is declared. Declaring it
// here keeps these fixtures exercising attribution rather than accidentally testing the gate's
// rejection path. Asserted file VALUES below are unchanged.
const DECLARED_TARGETS = [TARGET_FILE];
...
describe('extractExplicitEdits — @@SEARCH/@@REPLACE/@@END', () => {
  it('parses a single @@SEARCH/@@REPLACE/@@END block with File: line', () => {
    ...
    const edits = extractExplicitEdits(body, null, DECLARED_TARGETS);
