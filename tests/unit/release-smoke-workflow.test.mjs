 ...
 *   - node packages/rag/src/embed.mjs literal pin
...
describe('AC7 — documentation present', () => {
  // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. Only ten explicit
  // notes/how-to.* files are on the publish include list, and this is not one of them, so both
  // cases threw or failed on the mirror. The gate keys on the SHIPPED-NESS of this specific
  // note, and both cases still run — and still fail — wherever it is present.
  const howToShips = fs.existsSync(path.join(REPO_ROOT, 'notes/how-to.release-smoke-verification.md'));

  it.skipIf(!howToShips)('notes/how-to.release-smoke-verification.md exists', () => {
    const docPath = path.join(REPO_ROOT, 'notes/how-to.release-smoke-verification.md');
    expect(fs.existsSync(docPath)).toBe(true);
  });

  it.skipIf(!howToShips)('how-to references the workflow file path and the fixture vault path', () => {
    const docPath = path.join(REPO_ROOT, 'notes/how-to.release-smoke-verification.md');
...
describe('Production embed reuse — workflow invokes packages/rag/src/embed.mjs (not a duplicate)', () => {
  it('workflow YAML contains the literal `node packages/rag/src/embed.mjs`', () => {
    expect(SMOKE_YML_SRC).toContain('node packages/rag/src/embed.mjs');
  });
});
