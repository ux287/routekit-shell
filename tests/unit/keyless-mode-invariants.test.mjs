 ...
 *  INV-3 no read scope-escape     — keyless rag is tokenless & SCOPE-bounded (a subset of governed
 *                                   retrieval). Fidelity is now OWNERSHIP-scoped, not L2-capped: over a
 *                                   locally-owned corpus retrieval is authorized to L3 full text (secrets
 *                                   still scrubbed) per backlog.feat.rag-fidelity-ownership-scoped; the
 *                                   redaction ceiling is preserved for the non-owned (RKS-Pro) case.
 *                                   Fidelity behavior is asserted in tests/unit/rag-fidelity-ownership.test.mjs.
