...
describe('building-a-product-knowledge-graph deck — cover neutrality (framing safeguard)', () => {
  ...
  it('carries the model-economics beat, placed after the Act II reveal', () => {
    ...
    // The onramp slice alone can't catch an early-placed slide, because this copy
    // contains no "rks"/"AI" token — so witness the placement positionally.
    expect(src.indexOf('the fallback, not the default.')).toBeGreaterThan(
      src.indexOf("id: 'what-we-have'")
    );
  });
