...
describe("arch SKILL.md — body", () => {
  ...
  // arch/SKILL.md is the CANONICAL TWO-TOKEN EXEMPLAR: __RKS_SOURCE_PROJECT__ is the
  // SUBSTITUTED sentinel, __PROJECT_ID__ the SURVIVING placeholder. Pinning the trailing clause
  // verbatim is the cheapest guard against the ONE-TOKEN COLLAPSE that aborted a prior build.
  it("bootstrap instructions substitute __RKS_SOURCE_PROJECT__ while __PROJECT_ID__ survives", () => {
    expect(body).toContain("__PROJECT_ID__");
    expect(body).toContain("__RKS_SOURCE_PROJECT__");
    // the bare projectId literal is gone — it was one string doing two jobs
    expect(body).not.toContain("routekit-shell");
    // TWO DIFFERENT TOKENS, NEVER ONE
    expect(body).toContain("Replace __PROJECT_ID__ with __RKS_SOURCE_PROJECT__");
  });
