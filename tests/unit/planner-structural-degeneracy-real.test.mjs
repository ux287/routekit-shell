...
describe('planner.mjs — note-only op:create routes to structural BEFORE the has_note_steps retry', () => {
  ...
  it('uncoveredCreatesAfterLlm = frontmatterCreateFiles − automatedStepPaths − llmCreatePaths', () => {
    ...
    // backlog.fix.create-target-coverage-predicate-diverges replaced both exact Set.has
    // subtractions with the shared boundary-aware predicate, so plan-ready and planner can
    // no longer disagree about whether a create target is covered. The subtraction is still
    // frontmatterCreateFiles minus automatedStepPaths minus llmCreatePaths.
    expect(src).toMatch(/automatedStepPaths[\s\S]{0,120}createTargetPathsMatch/);
    expect(src).toMatch(/llmCreatePaths[\s\S]{0,120}createTargetPathsMatch/);
  });
  ...
  it('names the concrete fix — a fenced code block — while keeping every pinned phrase', () => {
    const s = buildStructuralFailure(['src/new-thing.mjs']);

    // The reword is ADDITIVE. tests/unit/planner-note-step-degeneracy.test.mjs pins these
    // four phrases and is deliberately left unedited as an independent witness; asserting
    // them here too means a replace-instead-of-append reddens in both places.
    expect(s.message).toMatch(/no usable content/i);
    expect(s.message).toMatch(/hand-author/i);
    expect(s.message).toMatch(/escalate to a stronger model/i);
    expect(s.message).toMatch(/will not help/i);

    // The added guidance: say what actually fixes it.
    expect(s.message).toMatch(/fenced code block/i);
    expect(s.message).toContain('// CREATE FILE:');

    // Still must NOT suggest refine — this failure class is refinable:false.
    expect(s.message).not.toMatch(/run rks_refine/i);
    expect(s.refinable).toBe(false);
  });
});
