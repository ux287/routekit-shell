...
describe('sync-hooks.mjs behavior — against a temp fixture repo', () => {
  ...
  // backlog.feat.hook-orphan-audit-guard — the blind spot this story closes: a deployed-only
  // ORPHAN in .routekit/hooks/ (not the template) was previously invisible to --check.
  it('--check mode fails (exit 1) on a deployed-only ORPHAN in .routekit/hooks/', () => {
    expect(runScript([]).status).toBe(0); // sync: canonical == template == .routekit/hooks
    const orphan = path.join(tmpRepo, '.routekit', 'hooks', 'write', 'ghost.mjs');
    fs.mkdirSync(path.dirname(orphan), { recursive: true });
    fs.writeFileSync(orphan, '// deployed-only orphan, no packages/hooks source\n');
    const result = runScript(['--check']);
    expect(result.status).toBe(1);
    expect(result.stderr || '').toContain('DRIFT:');
    expect(result.stderr || '').toMatch(/orphan/);
    expect(result.stderr || '').toContain('ghost.mjs');
  });
});
