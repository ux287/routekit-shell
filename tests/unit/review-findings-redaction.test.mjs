...
import {
  buildReviewStepEntry,
  normalizeReviewAcCoverage,
...
} from "../../packages/mcp-rks/src/shared/ac-coverage.mjs";
...
describe("rawResponse carries the same redaction invariant as findings", () => {
  // backlog.fix.review-malformed-response-indistinguishable added a rawResponse
  // field so an operator can see what a malformed reviewer actually SAID. That
  // text is model-authored and quotes the diff, so it can echo a credential —
  // the same hazard a finding's `line` carries, on a new field.
  //
  // The scrub happens at the PRODUCER (review.mjs callReviewer), not at
  // redactReview: redactReview is a shallow spread that promises only findings
  // work, and runOffRailEnforcementGate builds a review-shaped literal by hand
  // that bypasses it entirely. These assert the producer's tool works and that
  // its output composes cleanly through both step builders.
  const policy = loadReviewPolicy("/nonexistent-root-so-defaults-apply");

  it("FIXTURE PRECONDITION — the default policy really carries securityPatterns", () => {
    // Anti-vacuity: with an empty pattern list the scrub is an identity function
    // and every assertion below would pass without redacting anything.
    expect(Array.isArray(policy.securityPatterns)).toBe(true);
    expect(policy.securityPatterns.length).toBeGreaterThan(0);
  });

  it("scrubs a credential-shaped literal out of reviewer prose", () => {
    const dirty = `I reviewed the diff. It adds password = "${CANARY}" which is unsafe.`;
    expect(leaked(dirty)).toBe(true); // the canary is genuinely present first
    expect(leaked(scrubSecretLiterals(dirty, policy))).toBe(false);
  });

  it("TRUNCATION IS NOT REDACTION — scrubbing precedes the 500-char cap", () => {
    // The order the producer uses. Slicing FIRST can cut a credential
    // mid-literal, leaving a fragment the pattern no longer matches, which then
    // survives the scrub. Here the secret sits inside the first 500 chars, so
    // only a real scrub removes it.
    const dirty = `password = "${CANARY}" ` + "x".repeat(2000);
    const producerOrder = scrubSecretLiterals(dirty, policy).slice(0, 500);
    expect(producerOrder.length).toBeLessThanOrEqual(500);
    expect(leaked(producerOrder)).toBe(false);
  });

  it("a scrubbed rawResponse reaches neither shipStep carrying the canary", () => {
    const clean = scrubSecretLiterals(
      `The diff adds password = "${CANARY}".`,
      policy,
    );
    const offRail = buildOffRailReviewStep({
      llmFailed: true,
      cause: "malformed_response",
      rawResponse: clean,
    });
    const onRail = buildReviewStepEntry({
      llmFailed: true,
      cause: "malformed_response",
      rawResponse: clean,
    });
    expect(offRail.rawResponse).toBeDefined();
    expect(onRail.rawResponse).toBeDefined();
    expect(leaked(offRail)).toBe(false);
    expect(leaked(onRail)).toBe(false);
  });
});

describe("the rks_story_ship review payload is normalized, not merely redacted", () => {
  const AC_LITERAL = `password = "${CANARY}"`;
  const review = (acCoverage) => ({
    ok: true,
    verdict: "block",
    summary: "a summary",
    findings: [],
  ...
  const usable = () => ({
    assessed: true,
    covered: ["AC1 is covered"],
    notCovered: ["AC2 is not"],
    uncertain: [],
  });

  // The three shapes an entry-wise string scrub cannot reach.
  const SHAPES = {
    "a string-valued notCovered": { assessed: true, covered: [], notCovered: AC_LITERAL, uncertain: [] },
    "an extra model-authored key": { assessed: true, covered: [], notCovered: [], uncertain: [], notes: AC_LITERAL },
    "a non-coercible entry": {
      assessed: true,
      covered: [],
      notCovered: [Object.assign(JSON.parse('{"toString":1}'), { detail: AC_LITERAL })],
      uncertain: [],
    },
  };

  for (const [label, shape] of Object.entries(SHAPES)) {
    it(`${label} — redaction alone leaks it, the helper does not`, () => {
      const input = review(shape);
      // PRECONDITION: this is exactly what rks_story_ship returned before the fix.
      expect(leaked(redactReview(input))).toBe(true);

      expect(leaked(normalizeReviewAcCoverage(redactReview(input)))).toBe(false);
    });
  }

  it("a non-coercible entry inside the cap yields acCoverage null", () => {
    const out = normalizeReviewAcCoverage(redactReview(review(SHAPES["a non-coercible entry"])));
    expect(out.acCoverage).toBeNull();
  });

  it("a usable acCoverage matches what the off-rail gate reads", () => {
    const input = review(usable());
    const out = normalizeReviewAcCoverage(redactReview(input));

    expect(out.acCoverage).toEqual(buildOffRailReviewStep(redactReview(input)).acCoverage);
    expect(Object.keys(out.acCoverage).sort()).toEqual([
      "assessable",
      "assessed",
      "covered",
      "notCovered",
  ...
  it("bounds an over-cap list by the exported caps", () => {
    const notCovered = Array.from({ length: MAX_AC_COVERAGE_ENTRIES + 5 }, (_, i) =>
      i === 0 ? "x".repeat(MAX_AC_COVERAGE_ENTRY_CHARS + 50) : `AC${i}`,
    );
    const out = normalizeReviewAcCoverage(
      redactReview(review({ assessed: true, covered: [], notCovered, uncertain: [] })),
    );

    expect(out.acCoverage.notCovered).toHaveLength(MAX_AC_COVERAGE_ENTRIES);
    expect(out.acCoverage.notCovered[0]).toHaveLength(MAX_AC_COVERAGE_ENTRY_CHARS);
  });

  it("an absent acCoverage key stays ABSENT, and an explicit null stays null", () => {
    const withoutKey = { ok: true, verdict: "pass", findings: [] };
    expect("acCoverage" in normalizeReviewAcCoverage(withoutKey)).toBe(false);

    const withNull = { ok: true, verdict: "pass", findings: [], acCoverage: null };
    const out = normalizeReviewAcCoverage(withNull);
    expect("acCoverage" in out).toBe(true);
    expect(out.acCoverage).toBeNull();
  });

  it("changes no other field, and never mutates its input", () => {
    for (const acCoverage of [usable(), SHAPES["an extra model-authored key"]]) {
      const input = review(acCoverage);
      const before = JSON.stringify(input);
      const out = normalizeReviewAcCoverage(redactReview(input));

      for (const key of ["ok", "verdict", "summary", "findings"]) {
        expect(out[key]).toEqual(input[key]);
      }
      expect(JSON.stringify(input)).toBe(before);
      expect(leaked(input.acCoverage)).toBe(leaked(acCoverage));
    }
  });
});
...
describe("source wiring", () => {
  it("redacts at the runReview boundary on the on-rail path", async () => {
    ...
    // ...and NORMALIZES at that same boundary. Static import, because review.mjs is
    // reached through a dynamic import that two integration tests mock with three names.
    expect(src).toMatch(/import \{ normalizeAcCoverage \} from ['"]\.\.\/shared\/ac-coverage\.mjs['"]/);
    expect(src).toMatch(/normalizeReviewAcCoverage\(redactReview\(await runReview\(/);
    // The dynamic import still destructures exactly the three mocked names.
    expect(src).toMatch(/const \{ runReview, loadReviewPolicy, redactReview \} = await import\(['"]\.\/review\.mjs['"]\)/);
    // The helper is declared OUTSIDE the runStoryShipTool body region.
    expect(src.indexOf("export function normalizeReviewAcCoverage"))
      .toBeLessThan(src.indexOf("export async function runStoryShipTool"));
  });

  it("review.mjs no longer says the story-ship payloads are unnormalized", async () => {
    const fs = await import("node:fs");
    const src = fs.readFileSync("packages/mcp-rks/src/server/review.mjs", "utf8");
    const fnIdx = src.indexOf("function redactAcCoverage(");
    const comment = src.slice(src.lastIndexOf("/**", fnIdx), fnIdx);

    expect(comment.length).toBeGreaterThan(100);
    expect(src).not.toMatch(/are NOT normalized/);
  });
  ...
  it("keeps the degraded review.complete emit on one line per field", async () => {
    ...
    // ABSENCE. The degraded emit carries no acCoverage key by construction —
    // buildUnavailableReview produces none — so only the full emit, written
    // after the reviewer ran, may record coverage evidence.
    expect(window).not.toContain("acCoverage");
  });
});
