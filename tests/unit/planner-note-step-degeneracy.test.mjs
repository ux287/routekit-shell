...
// Mirror of the source coverage rule (planner.mjs): a REAL create_file step (non-empty content)
// covers its op:create target; an empty/missing one leaves it uncovered → loud structural fail.
...
describe("PART A — authorable create passes through, un-authorable create fails loud", () => {
  ...
  it("coverage rule requires NON-EMPTY content (an empty-content create_file does not cover)", () => {
    // Static guard: llmCreatePaths only counts create_file steps whose content is non-empty.
    expect(plannerSrc).toMatch(/action === "create_file" && a\?\.content && String\(a\.content\)\.trim\(\)/);
  });
...
describe("structural failure signal shape and phrasing", () => {
  ...
  it("plan.retry.exhausted return is ADDITIVE — preserves pinned fields, adds discriminator (static-source)", () => {
    ...
    expect(plannerSrc).toContain("failureClass: structural.failureClass");
    expect(plannerSrc).toContain("uncoveredCreateTargets: structural.uncoveredCreateTargets");
    // Telemetry payload carries the discriminator too
