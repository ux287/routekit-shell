...
// ─── Stage 4 demo advance fix (backlog.fix.onboarder-demo-stage-stuck) ────────

describe("Stage 4 demo advance — no longer stuck (F2)", () => {
  it("demo input populates nextAction (dispatch po) — not empty/stuck", async () => {
    const result = await runOnboarder({
      projectRoot: tmpDir,
      stage: "first_story",
      responses: { userInput: "demo" },
    });
    expect(result.pendingDispatch).toMatchObject({ type: "po", isDemo: true });
    expect(result.nextAction).toBeTruthy();
    expect(result.nextAction.type).toBe("dispatch");
    expect(result.nextAction.governor).toBe("po");
  });

  it("input display DIFFERS from the bare Stage 4 prompt (no identical re-render)", async () => {
    const prompt = await runOnboarder({ projectRoot: tmpDir, stage: "first_story" });
    const demo = await runOnboarder({
      projectRoot: tmpDir,
      stage: "first_story",
      responses: { userInput: "demo" },
    });
    expect(demo.display).not.toBe(prompt.display);
    expect(demo.display).toContain("dispatching the Product Owner Governor");
    expect(demo.display).not.toContain("What problem do you want to solve");
  });

  it("free-form input also gets an acknowledgement display + dispatch nextAction", async () => {
    const result = await runOnboarder({
      projectRoot: tmpDir,
      stage: "first_story",
      responses: { userInput: "Build a calculator" },
    });
    expect(result.display).toContain("dispatching the Product Owner Governor");
    expect(result.nextAction.type).toBe("dispatch");
    expect(result.pendingDispatch.isDemo).toBe(false);
  });
});

describe("Stage 4 demo/init reconciliation + framework boundary (F3)", () => {
  const src = fs.readFileSync(
    path.resolve("packages/mcp-rks/src/server/onboarder.mjs"),
    "utf8"
  );

  it("demo copy no longer assumes an init step the tour never runs", () => {
    expect(src).not.toContain("If you initialized a fresh project");
    expect(src).toContain("Do this in your own project, not the rks framework repo");
  });

  it("demo target does not point at framework internals", () => {
    expect(DEMO_PROBLEM).not.toContain("packages/mcp-rks");
    expect(src).not.toMatch(/DEMO_PROBLEM_STATEMENT[\s\S]{0,160}packages\/mcp-rks/);
  });
});
