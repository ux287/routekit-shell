...
describe("R1.3-followup: rks_release migration to advancePhase('release')", () => {
  ...
  describe("AC6 — canon GAP-3 annotation updated", () => {
    // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. notes/canon.* is not in the
    // publish set, so this threw ENOENT on the mirror. Condition-scoped: still runs, and still
    // fails, wherever the canon note is present.
    it.skipIf(!fs.existsSync(path.join(REPO_ROOT, "notes/canon.phase-state-machine.md")))(
      "canon.phase-state-machine.md GAP-3 §8 entry is marked CLOSED with closure path", () => {
