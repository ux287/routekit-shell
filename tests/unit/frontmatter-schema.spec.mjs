...
describe('frontmatter schema validation', () => {
  describe('backlog files', () => {
    ...
    // include pattern in publish-profiles.yaml, so the published tree carries no backlog notes
    // and this non-vacuity floor cannot hold there. Gated on the NAMESPACE being present at
    // all — which preserves the discrimination that matters: where any backlog note exists,
    // the filtered set must still be non-empty, so a broken filter still reddens upstream.
    const anyBacklogNote = getMarkdownFiles(NOTES_DIR).some((f) => f.includes('backlog.'));

    it.skipIf(!anyBacklogNote)('should have backlog files to validate', () => {
