...
`
  ...
  describe("guardrailsOff with problemId", () => {
    ...
    it("rejects with story_note_not_on_branch when the story note is absent from the worktree", async () => {
      // Previously asserted "story_not_ready". A note that is not in this
      // worktree is a BRANCH problem, not a phase problem — reporting it as an
      // unready phase sent the Dispatcher to re-run PO → QA → ARCH on a story
      // that is frequently already arch-approved on another branch.
      const result = await guardrailsModule.guardrailsOff(
        projectDir,
      ...
      expect(result.notePath).toContain("backlog.feat.nonexistent.md");
