...
describe("rag MCP surface consolidation — shared source of truth", () => {
  ...
  it("the shared rag_init/rag_embed handlers delegate to the RAW scripts, NOT the governed wrappers", () => {
    ...
    expect(raw).toMatch(/from ["']@routekit\/rag\/init["']/);
    expect(raw).toMatch(/from ["']@routekit\/rag\/embed["']/);
  ...
  it("no server imports the raw init/embed scripts directly anymore (handlers hoisted to shared)", () => {
    for (const f of [STDIO, HTTP]) {
      const src = read(f);
      expect(src, `${f} must not import raw init.mjs directly`).not.toMatch(/from ["']@routekit\/rag\/init["']/);
      expect(src, `${f} must not import raw embed.mjs directly`).not.toMatch(/from ["']@routekit\/rag\/embed["']/);
    }
  });
});

describe("rag MCP surface consolidation — behavior preserved per transport", () => {
  it("stdio rag_query now uses the direct ragQuery path (vestigial routing dropped), not retrieveWithRouting or runRagQuery", () => {
  ...
  it("http rag_query STILL uses the direct ragQuery path, unchanged", () => {
    const src = read(HTTP);
    expect(src).toMatch(/import \{ query as ragQuery \} from ["']@routekit\/rag\/query["']/);
