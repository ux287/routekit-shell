...
describe("governor-token — stash tracking", () => {
  ...
  it("registered cleanup function is called on session end in terminal state", async () => {
    ...
    // COMPENSATION REMOVED (backlog.fix.endsession-stash-autopop-unawaited). This
    // was `await new Promise(resolve => setTimeout(resolve, 0))` — a scheduler
    // yield standing in for the missing await, which is what made the assertion
    // pass against the floating-promise defect. Await the retained promise instead.
  ...
  it("cleanup NOT called if clearPendingStash was called before session ends", async () => {
    ...
    // Same compensation removed here. Note this case asserts a NEGATIVE, so the
    // yield was hiding less — but leaving it would have kept the pattern alive.
    await flushPendingStashPops();
