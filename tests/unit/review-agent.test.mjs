...
`);
  ...
  it('keeps the default category split exactly as it was — no category moved', () => {
    // backlog.fix.review-verdict-downgrade-legibility is a no-behaviour-change
    // story. The tempting "fix" for an ac_coverage block being softened to warn
    // is to move ac_coverage into blockCategories; the story forbids it, because
    // that would fail the off-rail escape hatch closed. This is the guard.
    const policy = loadReviewPolicy(tempDir);
    expect(policy.blockCategories).toEqual(['enforcement_modification', 'security_issue']);
    expect(policy.warnCategories).toContain('ac_coverage');
    expect(policy.warnCategories).not.toContain('enforcement_modification');
    expect(policy.warnCategories).not.toContain('security_issue');
    expect(policy.blockCategories).not.toContain('ac_coverage');
    expect(policy.verdictMode).toBe('warn');
  });
});
