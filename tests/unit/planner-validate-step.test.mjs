...
describe('plan emission — collects and guards on invalid steps', () => {
  ...
  // backlog.fix.planner-note-step-degeneracy — the exhausted return is reconciled ADDITIVELY:
  // the pinned fields above stay, and a machine-readable structural discriminator plus the
  // uncovered op:create target names are added when the degenerate plan cannot be refined.
  it('exhausted return adds a structural discriminator + uncovered-create-target names', () => {
    ...
    // Discriminator sourced from buildStructuralFailure, gated on uncovered op:create targets.
    expect(plannerSrc).toContain('buildStructuralFailure(exhaustedUncoveredCreates)');
    expect(plannerSrc).toMatch(/failureClass:\s*["']structural["']/);
  });
});
