 ...
 * - op:create excluded from the @@SEARCH coverage loop (computeEarlyExitCoverage), but the early-exit
...
describe("early-exit coverage check logic", () => {
  // backlog.fix.identity-hunk-passes-as-change-spec: this suite used to define a LOCAL
  // re-implementation of the runPlanTool coverage loop. It could not observe changes to
  // production at all — and it had already drifted: production `break`s and RETURNS the
  // steps accumulated so far, while the mirror early-returned `steps: []`. It now drives
  // the real exported function, so a change to the loop is visible here.
  const checkAllCovered = computeEarlyExitCoverage;
...
`;
  ...
  it("op:create targets not in editableTargetPaths → excluded from check", () => {
    ...
    // The @@SEARCH coverage loop (computeEarlyExitCoverage) only iterates editableTargetPaths, so it ignores
  ...
  it("empty editableTargetPaths → early-exit does not fire (no steps)", () => {
    ...
    // With empty array, computeEarlyExitCoverage is never called
