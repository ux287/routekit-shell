...
describe('story-ship.mjs wiring (full-source scan, no fixed windows)', () => {
  ...
  it('halts before BOTH landing sites', () => {
    // backlog.fix.ship-delivery-implemented-twice, FINDING C.
    //
    // This used to anchor on 'Step 2: Merge the PR to working branch' — the PR merge that
    // landed the work. That step is gone: no PR is opened on this path for any topology.
    // The gate is NOT re-pointed at a surviving string; it is re-stated as the property
    // that actually matters, against BOTH sites that now land work.
    //
    // Why both: the 3-branch arm lands via localMerge, the 2-branch arm via
    // deliverFeatureBranch. If the gate sits below either, it does not merely run late —
    // it is ELIMINATED, because local-merge.mjs checks out the target and never checks
    // back, so `git diff <target>...HEAD` is empty and review.mjs short-circuits an empty
    // diff to a PASS without ever calling the reviewer.
    //
    // Anchors chosen for uniqueness: bare `deliverFeatureBranch` also matches its import,
    // so an indexOf on it would resolve above the gate wherever the gate sits and this
    // assertion would be false against a CORRECT implementation.
    const haltIdx = SHIP_SRC.indexOf("reason: 'reviewer_unavailable'");
    const localMergeIdx = SHIP_SRC.indexOf('localMerge(projectRoot');
    const deliverIdx = SHIP_SRC.indexOf('deliverFeatureBranch({');

    expect(haltIdx).toBeGreaterThan(-1);
    expect(localMergeIdx).toBeGreaterThan(-1);
    expect(deliverIdx).toBeGreaterThan(-1);

    expect(localMergeIdx).toBeGreaterThan(haltIdx);
    expect(deliverIdx).toBeGreaterThan(haltIdx);
  });
