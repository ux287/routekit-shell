...
describe('marketing-site scaffold — composition + hero', () => {
  ...
  it('declares Open Graph + Twitter Card link-preview metadata', () => {
    const html = read('index.html');
  ...
  it('uses ABSOLUTE image URLs for the preview (relative paths silently fail in scrapers)', () => {
    const html = read('index.html');
    // The actual failure mode: a bare "og-image.png" would pass a naive contains()
    // check while producing an imageless preview. Pin the absolute-URL requirement.
    const og = html.match(/property="og:image"\s+content="([^"]+)"/);
    const tw = html.match(/name="twitter:image"\s+content="([^"]+)"/);
    expect(og?.[1]).toMatch(/^https:\/\//);
    expect(tw?.[1]).toMatch(/^https:\/\//);
  });

  it('carries the approved preview copy verbatim', () => {
    const html = read('index.html');
    expect(html).toContain('Routekit Shell — the project operating system');
    expect(html).toContain(
      'Coordinate your AI models, knowledge, tools, and workflows using infrastructure you already own.'
    );
    // The page <title>/description keep the older framing on purpose (reconciliation
    // is a separate positioning pass); they must not be silently aligned here.
    expect(html).toContain('<title>Routekit Shell — skills and governance for AI-assisted development</title>');
  });
});
