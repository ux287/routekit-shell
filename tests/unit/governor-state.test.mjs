...
const NON_OPEN_FLOWS = ["story", "qa", "ship", "ops"];
...
describe("governor-state.mjs — dendron-tool allowlist symmetry (hook chain misconfig fix)", () => {
  ...
  describe("AC2 / AC4 — OPEN_STATES symmetry (incl. test-file-scanning)", () => {
    ...
    it("every open-flow state that allows dendron_update_field also allows rks_arch_verdict", () => {
      // Derived set equality, NOT a hand-listed set of state names: the obligation is
      // "wherever a frontmatter write is reachable, the governed verdict write is too",
      // so a state added later inherits it without anyone remembering to edit this test.
      const violations = [];
      for (const [stateName, stateDef] of Object.entries(states)) {
        if (!stateDef.allowed?.has("dendron_update_field")) continue;
        if (!stateDef.allowed.has("rks_arch_verdict")) {
          violations.push(`OPEN_STATES['${stateName}'].allowed lacks rks_arch_verdict`);
        }
      }
      expect(violations).toEqual([]);
    });

    // Violation detection extracted as a pure function ON PURPOSE. Inlined in the
    // loop it could only ever be proven to PASS — never proven to work. Extracted,
    // the negative controls below drive the identical logic with a synthetic table
    // and prove it reports what it is supposed to report.
    function scanForArchVerdict(flow, states) {
      const violations = [];
      const missingAllowed = [];
      let inspected = 0;
      for (const [stateName, stateDef] of Object.entries(states)) {
        inspected += 1;
        // `allowed instanceof Set`, NOT the old `allowed?.has(...) ?? false`: that
        // optional chain let a state with no `allowed` set satisfy the assertion from
        // absence of evidence rather than from evidence of absence.
        if (!(stateDef.allowed instanceof Set)) {
          missingAllowed.push(`${flow} flow state '${stateName}' has no allowed Set`);
          continue;
        }
        if (stateDef.allowed.has("rks_arch_verdict")) {
          violations.push(`${flow} flow state '${stateName}' must not allow rks_arch_verdict`);
        }
      }
      return { violations, missingAllowed, inspected };
    }

    it("does NOT widen any non-open flow with rks_arch_verdict", () => {
      // governor-open-flow-research-chain.test.mjs holds an exact sorted allowed-set
      // equality over NON_OPEN_FLOW_BASELINE. Widening a non-open flow reddens it, so
      // the containment is asserted here too rather than discovered there.
      const tables = NON_OPEN_FLOWS.map((flow) => getStates(flow));

      // Vacuity guard 1 — an empty table skips the loop body and passes silently.
      NON_OPEN_FLOWS.forEach((flow, i) => {
        expect(
          Object.keys(tables[i]).length,
          `${flow} flow resolved to an empty state table`,
        ).toBeGreaterThan(0);
      });

      // Vacuity guard 2 — getStates falls THROUGH to STORY_STATES for any unrecognised
      // flow name, so a typo or a renamed flow does not empty the loop: it silently
      // inspects STORY_STATES twice while reporting coverage of four flows. Identity
      // comparison catches that; a length check cannot.
      expect(
        new Set(tables).size,
        "the four flows must resolve to four DISTINCT state tables — a repeat means one aliased to STORY_STATES",
      ).toBe(NON_OPEN_FLOWS.length);

      let inspected = 0;
      const violations = [];
      const missingAllowed = [];
      NON_OPEN_FLOWS.forEach((flow, i) => {
        const scan = scanForArchVerdict(flow, tables[i]);
        inspected += scan.inspected;
        violations.push(...scan.violations);
        missingAllowed.push(...scan.missingAllowed);
      });

      // Vacuity guard 3 — prove something was actually looked at.
      expect(inspected, "no states were inspected across any non-open flow").toBeGreaterThan(0);
      expect(missingAllowed).toEqual([]);
      expect(violations).toEqual([]);
    });

    // Fixtures are locally constructed: the real tables are module-level singletons
    // shared with every other getStates call site in this file, so mutating one to
    // build a control would leak.
    it("NEGATIVE CONTROL — the scan reports a violation when a state DOES allow rks_arch_verdict", () => {
      const synthetic = {
        clean: { allowed: new Set(["dendron_update_field"]) },
        offending: { allowed: new Set(["dendron_update_field", "rks_arch_verdict"]) },
      };
      const scan = scanForArchVerdict("story", synthetic);
      expect(scan.inspected).toBe(2);
      expect(scan.violations).toHaveLength(1);
      expect(scan.violations[0]).toContain("offending");
    });

    it("NEGATIVE CONTROL — the scan reports a state whose allowed is absent or not a Set", () => {
      const synthetic = {
        undefinedAllowed: { allowed: undefined },
        arrayAllowed: { allowed: ["rks_arch_verdict"] },
      };
      const scan = scanForArchVerdict("qa", synthetic);
      // Both are reported as broken rather than one silently passing on `?? false`
      // and the other passing because an Array has no .has().
      expect(scan.missingAllowed).toHaveLength(2);
...
describe("governor-state.mjs — rks_fetch_raw open-flow parity (backlog.fix.fetch-raw-uninvocable-research-gov)", () => {
  // The five OPEN_STATES where rks_agent_external_research is a member. rks_fetch_raw is its
  // research-toolbox sibling and must be reachable in exactly the same open-flow states.
  const OPEN_RESEARCH_STATES = ["init", "researching", "concern-separating", "test-file-scanning", "writing"];

  it("rks_fetch_raw is allowed in ALL FIVE open research/scan states (parity witness)", () => {
    for (const state of OPEN_RESEARCH_STATES) {
      expect(checkStateAllowed("open", state, "rks_fetch_raw").allowed, `open/${state}`).toBe(true);
    }
  });

  it("state-level parity: rks_fetch_raw is allowed exactly where external_research is (open flow)", () => {
    for (const [stateName, stateDef] of Object.entries(getStates("open"))) {
      expect(
        stateDef.allowed?.has("rks_fetch_raw") ?? false,
        `open/${stateName}: fetch_raw vs external_research membership must match`,
      ).toBe(stateDef.allowed?.has("rks_agent_external_research") ?? false);
    }
  });

  it("is NOT added to story/ops/qa/ship flow states (open-flow only, by design)", () => {
    for (const flow of ["story", "ops", "qa", "ship"]) {
      for (const state of Object.keys(getStates(flow))) {
        expect(checkStateAllowed(flow, state, "rks_fetch_raw").allowed, `${flow}/${state}`).toBe(false);
      }
    }
  });

  it("open non-research states (review, failed) do NOT allow it — matches external_research", () => {
    for (const state of ["review", "failed"]) {
      expect(checkStateAllowed("open", state, "rks_fetch_raw").allowed, `open/${state}`).toBe(false);
    }
  });
});
