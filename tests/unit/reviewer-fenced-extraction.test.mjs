...
// Every fixture path in this file is synthetic — none exists on disk. extractExplicitEdits now
// refuses a candidate that neither resolves under projectRoot nor appears in the declared target
// set, so the synthetic paths are declared here. This file's job is to pin FENCE STRIPPING, not
// the attribution gate; without this the fixtures would silently start testing the gate instead.
...
`;
  ...
  it("bare block resolves file from heading", () => {
    const edits = extractExplicitEdits(bareStory, null, DECLARED_TARGETS);
  ...
  it("fenced block resolves file correctly when heading precedes fence", () => {
    const edits = extractExplicitEdits(fencedStory, null, DECLARED_TARGETS);
    expect(edits.length).toBeGreaterThan(0);
    const edit = edits.find(e => e.source === "at_marker_block");
    expect(edit).toBeDefined();
    expect(edit.file).toBe("src/foo.mjs");
  ...
  it("fenced and bare blocks produce identical edit objects for same content", () => {
    const bareEdits = extractExplicitEdits(bareStory, null, DECLARED_TARGETS);
    const fencedEdits = extractExplicitEdits(fencedStory, null, DECLARED_TARGETS);
  ...
  it("fenced block with File: prefix resolves file", () => {
    ...
    const edits = extractExplicitEdits(story, null, DECLARED_TARGETS);
    const edit = edits.find(e => e.source === "at_marker_block");
    expect(edit).toBeDefined();
...
\`\`\`
`;
    const edits = extractExplicitEdits(story, null, DECLARED_TARGETS).filter(e => e.source === "at_marker_block");
