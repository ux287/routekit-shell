...
describe("detectCreateFileDirective — frontmatter create markers", () => {
  it("recognises op:'create', not only create:true", async () => {
    // TELEMETRY ACCURACY ONLY. Its sole consumer is the advisory planner.create_file_gate
    // emit, which hardcodes gateTriggered:false — this gates nothing, and no assertion here
    // should imply otherwise. Before this fix, stories using the DOCUMENTED `op: create`
    // convention were undercounted in that advisory.
    const { detectCreateFileDirective } = await import(
      "../../packages/mcp-rks/src/server/planner-llm.mjs"
    );

    expect(detectCreateFileDirective("", [{ path: "a.mjs", op: "create" }])).toBe(true);
    expect(detectCreateFileDirective("", [{ path: "a.mjs", create: true }])).toBe(true);
    expect(detectCreateFileDirective("", [{ path: "a.mjs", op: "edit" }])).toBe(false);

    // The typeof guard must survive: frontmatterTargets is a plain string array on some
    // call paths, and `t?.op` on a string must not throw.
    expect(() => detectCreateFileDirective("", ["src/new.ts"])).not.toThrow();
    expect(detectCreateFileDirective("", ["src/new.ts"])).toBe(false);
  });
});
