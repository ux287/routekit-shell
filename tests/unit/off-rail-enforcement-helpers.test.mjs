...
describe('buildOffRailReviewStep — a review that did not run is never a pass', () => {
  ...
  it("carries the recognized cause 'malformed_response' through to the shipStep", () => {
    // backlog.fix.review-malformed-response-indistinguishable. The sibling of
    // the normalization test above: the cause vocabulary widened by one value,
    // and this is the half that must SURVIVE while 'weird' above still does not.
    // Together they pin an allowlist rather than blanket passthrough.
    const step = buildOffRailReviewStep({ llmFailed: true, cause: 'malformed_response' });
...
describe('buildOffRailReviewStep — the downgrade record reaches the payload', () => {
  // backlog.fix.review-verdict-downgrade-legibility. Before this, a policy-softened
  // block and a genuine warn were the same byte in the rks_guardrails_on response,
  // while the findings sitting beside them still read severity: 'block'.

  it('carries downgradedFrom and downgradeReason when the review recorded a downgrade', () => {
    const step = buildOffRailReviewStep({
      ok: true,
      verdict: 'warn',
      ...
      downgradeReason:
        ...
        '[ac_coverage] are not listed in policy blockCategories [security_issue]',
  ...
  it("carries the verdictMode 'skip' force-to-pass record", () => {
    const step = buildOffRailReviewStep({
      ok: true,
      verdict: 'pass',
      downgradedFrom: 'block',
      downgradeReason: "verdictMode 'skip' forced pass, overriding verdict 'block'",
    });
    expect(step.verdict).toBe('pass');
    expect(step.downgradedFrom).toBe('block');
    expect(step.downgradeReason).toContain('skip');
  });

  it('OMITS both keys when no downgrade occurred — absence is the signal', () => {
    for (const result of [
      { ok: true, verdict: 'pass', findings: [] },
      { ok: true, verdict: 'warn', downgradedFrom: null, downgradeReason: null, findings: [] },
      { ok: true, verdict: 'block', downgradedFrom: null, downgradeReason: null },
    ]) {
  ...
  it('adds NO downgrade keys to the skipped early return, whatever the input carries', () => {
    // The twin of the strict toEqual pin above, and of
    // tests/unit/review-findings-redaction.test.mjs. A skipped review has no
    // verdict, so it can have no downgrade either — an extra key here reddens
    // both pins, which is exactly what they are for.
    const step = buildOffRailReviewStep({
      skipped: true,
      reason: 'policy_disabled',
      downgradedFrom: 'block',
      downgradeReason: 'should not appear',
    });
    expect(step).toEqual({ step: 'review', skipped: true, reason: 'policy_disabled' });
  });

  it('adds no downgrade keys on the unavailable-reviewer path', () => {
    const step = buildOffRailReviewStep({
      ok: true,
      verdict: 'unavailable',
      reviewerUnavailable: true,
    ...
    expect(step.ok).toBe(false);
    expect(step.cause).toBe('not_configured');
    expect(step.downgradedFrom).toBeUndefined();
    expect(step.downgradeReason).toBeUndefined();
  });

  it('a downgraded warn is still not a block, so the halt discriminator is unmoved', () => {
    // haltReason is 'review_block' when and only when the step verdict is
    // 'block'. Recording the downgrade must not change which side of that line
    // the step falls on.
    const downgraded = buildOffRailReviewStep({
      ok: true,
      verdict: 'warn',
      downgradedFrom: 'block',
      downgradeReason: 'softened',
    });
    expect(downgraded.verdict).not.toBe('block');
    const hardBlock = buildOffRailReviewStep({ ok: true, verdict: 'block', findings: [] });
    expect(hardBlock.verdict).toBe('block');
    expect(hardBlock.downgradedFrom).toBeUndefined();
  });
});
...
describe('buildScopeReconcileStep — the unevaluated state says so instead of reporting clean', () => {
  const allowed = ['packages/mcp-rks/src/example.mjs'];

  it('OMITS ok entirely — absent, not false and not true — in both flavours', () => {
    // Load-bearing. resolveOffRailHalt reads `scopeStep?.ok === false`, so an
    // ok:false unevaluated step would fire the scope_violation halt under block
    // posture at both gates — including gate alpha on every empty-index ship —
    // shipping the deliberately-deferred halt-posture decision by accident.
    for (const reason of ['no_commit', 'manifest_unreadable']) {
      const step = buildScopeReconcileStep({ allowedFiles: allowed, unevaluated: { reason } });
  ...
  it('carries the underlying git cause on a manifest_unreadable step', () => {
    const step = buildScopeReconcileStep({
      allowedFiles: allowed,
      unevaluated: { reason: 'manifest_unreadable', error: 'git diff-tree exited 128' },
    });
    expect(step.error).toContain('128');
  });

  it('omits error on a no_commit step — there is no failure to describe', () => {
    const step = buildScopeReconcileStep({
      allowedFiles: allowed,
      unevaluated: { reason: 'no_commit' },
    });
    expect('error' in step).toBe(false);
  });

  it('is observably DIFFERENT from a genuinely clean commit, by field value', () => {
    // Not merely "has no violations" — a reader must be able to tell the two
    // apart without inferring from an absence.
    const clean = buildScopeReconcileStep({ changedFiles: [], allowedFiles: allowed });
    const unevaluated = buildScopeReconcileStep({
      allowedFiles: allowed,
      unevaluated: { reason: 'no_commit' },
    });
    expect(clean.ok).toBe(true);
    expect(clean.evaluated).toBeUndefined();
    expect(unevaluated.evaluated).toBe(false);
    expect(clean).not.toEqual(unevaluated);
  });

  it('is NOT inferred from an empty manifest — an observed empty commit is still clean', () => {
    // A commit that genuinely touched nothing takes the evaluated path. Unevaluated
    // is a state the caller declares, never one deduced from silence.
    const step = buildScopeReconcileStep({ changedFiles: [], allowedFiles: allowed });
    expect(step.ok).toBe(true);
    expect(step.violations).toEqual([]);
    expect('evaluated' in step).toBe(false);
  });

  it('keeps the EVALUATED shape at exactly four keys', () => {
    // tests/integration/off-rail-scoped-staging.test.mjs:300 is a strict key-set
    // pin. evaluated/reason/error exist ONLY in the unevaluated shape.
    const step = buildScopeReconcileStep({ changedFiles: ['x.mjs'], allowedFiles: allowed });
    expect(Object.keys(step).sort()).toEqual(['inScopeCount', 'ok', 'step', 'violations']);
  });

  it('still returns the no_scope skip shape when there is no scope at all', () => {
    const step = buildScopeReconcileStep({ allowedFiles: null, unevaluated: { reason: 'no_commit' } });
    expect(step).toEqual({ step: 'scope_reconcile', skipped: true, reason: 'no_scope' });
  });
});
...
describe('wiring (full-source scans, no fixed windows)', () => {
  ...
  it('stages an explicit pathspec when scoped, keeping the sweep as the scopeless fallback', () => {
    // REPLACES an earlier invariant that required the sweep to be argument-free
    // on every path. That assertion defended a source-text PROXY for "no silent
    // work loss", and its stated premise was wrong: it claimed the unstaged
    // remainder sat on an off-rail branch the later delete/merge steps destroy.
    // They do not — `git checkout` carries uncommitted changes across a switch
    // (and refuses rather than discards when it cannot), and `git branch -D` does
    // not touch the worktree at all.
    //
    // The guarantee now asserted directly, and behaviourally, in
    // tests/integration/off-rail-scoped-staging.test.mjs: an out-of-scope path is
    // never staged, never committed, and is STILL READABLE IN THE WORKTREE with
    // its content intact after the merge-and-delete. Files the session does not
    // own stay where they are instead of being swept into its commit — which is
    // what commit f8ff97b1 did.
    const autoShip = AUDIT_SRC.slice(AUDIT_SRC.indexOf('skipAutoShip'));

    // Scoped: real worktree paths, resolved through the matcher, as an explicit
    // `git add --` pathspec. Never a raw allowedFiles entry — a glob handed to
    // git as a literal pathspec stages nothing.
    expect(autoShip).toMatch(/spawnSync\("git", \["add", "--", \.\.\.shipScope\.stagePaths\]/);

    // Scopeless: the historical sweep survives, exactly once, and only as the
    // fallback — a session with no allowedFiles must still be able to ship.
    expect(autoShip.match(/execSync\("git add -A"/g) || []).toHaveLength(1);

    const scopedIdx = autoShip.indexOf('if (shipScope.scoped) {');
    const sweepIdx = autoShip.indexOf('execSync("git add -A"');
    expect(scopedIdx).toBeGreaterThan(-1);
    expect(sweepIdx).toBeGreaterThan(scopedIdx);
  });

  it('derives the ship scope from the snapshot, never a fresh read at staging time', () => {
    // removeScopeFile() has already deleted .rks/active-scope.json by the time
    // staging runs, so a second readActiveScope() there returns null and collapses
    // every scoped session into the sweep — a failure that is invisible, because
    // the ship still succeeds and still looks correct.
    const GUARDRAILS_ON = AUDIT_SRC.slice(AUDIT_SRC.indexOf('export async function guardrailsOn'));
    expect(GUARDRAILS_ON).toMatch(/buildShipScope\(\{[\s\S]*?allowedFiles: scopeSnapshot\?\.allowedFiles \?\? null/);
    // Exactly one real readActiveScope call inside guardrailsOn: the snapshot
    // taken before removeScopeFile. A second one anywhere below it would read a
    // file that no longer exists.
    expect(GUARDRAILS_ON.match(/readActiveScope\(projectRoot\)/g) || []).toHaveLength(1);
  });
