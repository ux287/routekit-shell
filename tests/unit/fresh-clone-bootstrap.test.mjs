...
describe("fresh-clone bootstrap — portable .mcp.json.example", () => {
  ...
  it("resolves root from cwd (NO ${workspaceFolder}) and keeps the project id", () => {
    ...
    // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. `.mcp.json.example` is one
    // of the SEVEN file classes normalizeExportIdentity rewrites, so a hardcoded upstream
    // literal here agreed upstream and disagreed on the mirror. `.rks/project.json` is rewritten
    // by the SAME pass, so comparing the two is rewrite-invariant — they move together — while
    // still failing if the two ever disagree, which is the property this line is really about.
    const projectJsonId = JSON.parse(read(".rks/project.json")).id;
    expect(projectJsonId).toBeTruthy();
    expect(cfg.mcpServers["rks-gov"].env.ROUTEKIT_PROJECT_ID).toBe(projectJsonId);
  });
