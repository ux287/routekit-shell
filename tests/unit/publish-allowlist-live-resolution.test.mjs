...
//
// The "must exist for this to mean anything" controls below assert that each named offender is
// PRESENT in the tree, so that "it is not in the resolved publish set" is a real exclusion
// rather than a vacuous truth about a file that was never there.
//
// That control is correct UPSTREAM and impossible in a PUBLISHED tree: there, every offender
// has already been excluded, so its absence IS the exclusion working. Asserting its presence
// would fail for exactly the reason the exclusion succeeded.
//
// So the controls are gated on the SOURCE TREE being complete, detected by one offender that
// upstream always carries. Upstream nothing weakens — every control still fires, and a stale
// profile entry naming a file that no longer exists still reddens.
const SOURCE_TREE_COMPLETE = fs.existsSync(
  path.join(REPO_ROOT, "tests/unit/marketing-site-scaffold.test.mjs"),
);
...
describe("THE LEGAL GATE — no presentation, deck or marketing content resolves", () => {
  ...
  it("the client-engagement fixture carriers are gone", () => {
    ...
    for (const bad of [
      ...
      // Same SOURCE_TREE_COMPLETE gate as above: upstream the offender exists and the control
      // is meaningful; in a published tree it has already been excluded and its absence IS the
      // exclusion working.
      if (SOURCE_TREE_COMPLETE) {
