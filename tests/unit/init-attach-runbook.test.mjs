...
describe("CLAUDE.md — INIT/ATTACH RUNBOOK", () => {
  ...
  describe("template selection", () => {
    ...
    it("names the stack ids with when-to-use guidance", () => {
      const r = runbook();
      expect(r).toContain("app.web.react.spa");
      expect(r).toContain("base");
      expect(r).toMatch(/web app|browser|localhost|npm run dev/i);   // app.web.react.spa = web app
      expect(r).toMatch(/react|vite/i);                              // app.web.react.spa = React/Vite SPA
    });
  });
  ...
  it("has the calculator worked example on app.web.react.spa", () => {
    const r = runbook();
