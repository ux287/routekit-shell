...
describe('runStoryShipTool — every failure return routes through the helper', () => {
  ...
  it('leaves no ok:false literal outside the helper except the non-returning ci_check step', () => {
    ...
    //
    // backlog.fix.ship-delivery-implemented-twice added a `delete_branch` step whose
    // outcome can be false. It does NOT appear here, deliberately: its `ok` is DERIVED
    // from the shared module's observed record (`ok: deleteRecord.ok === true`) rather
    // than written as a literal. That is the stricter form, so this invariant is
  ...
  it('still contains every failure exit', () => {
    // backlog.fix.ship-delivery-implemented-twice removed three exits along with the
    // fragments that raised them — the feature-branch push, the PR open and the PR merge
    // are all gone from this path, which opens no PR for any topology. `push_working`
    // replaces them: it is the shared module's push of the WORKING branch.
    for (const marker of [
      'not from a protected branch',
      'preflight_dirty_tree',
      'Failed at local_merge',
      'Failed at push_working',
      'Code review blocked merge',
    ]) {
      expect(TOOL_BODY).toContain(marker);
    }
    // The retired exits must be GONE, not merely unasserted — leaving a failure message
    // for a step that no longer runs is intent-sourced reporting.
    for (const retired of [
      'Failed to push branch to remote',
      'Failed at working_pr',
      'Failed at working_merge',
    ]) {
      expect(TOOL_BODY).not.toContain(retired);
    }
  });
