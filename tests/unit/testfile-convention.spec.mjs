...
Body`;
  ...
  describe("template", () => {
    // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. notes/templates.backlog.md
    // is on no include pattern in publish-profiles.yaml, so both cases threw ENOENT on the
    // mirror. Condition-scoped: wherever the template IS present they run and still fail on a
    // template that has lost its testFile field or its Testing Requirements section.
    const templateAvailable = existsSync(join(NOTES_DIR, "templates.backlog.md"));

    it.skipIf(!templateAvailable)("backlog template includes testFile in frontmatter", () => {
    ...
    it.skipIf(!templateAvailable)("backlog template includes Testing Requirements section", () => {
      const templatePath = join(NOTES_DIR, "templates.backlog.md");
      const content = readFileSync(templatePath, "utf8");
  ...
  describe("backfill verification", () => {
    // backlog.fix.mirror-ci-green-unship-doc-integrity-tests — AC5. backlog.z_implemented.* is
    // on no include pattern, so the published tree carries none of these notes and the floor of
    // 10 cannot hold there. Gated on the NAMESPACE being present at all, which keeps the
    // distinction that matters: "the mirror has no shipped-story notes" versus "the backfill
    // regressed", and only the second still fails.
    const anyImplementedNote =
      existsSync(NOTES_DIR) &&
      readdirSync(NOTES_DIR).some((f) => f.startsWith("backlog.z_implemented.") && f.endsWith(".md"));

    it.skipIf(!anyImplementedNote)("at least 10 z_implemented stories have testFile field", () => {
      const fs = require("fs");
