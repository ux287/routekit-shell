...
function buildShellRoot(dir) {
  // Seeded with a COMPLETE TWO-TOKEN LAUNCH DIRECTIVE so placeholder survival is assertable
  // here too: __RKS_SOURCE_PROJECT__ is SUBSTITUTED, __PROJECT_ID__ SURVIVES.
  write(
...
describe("vendor-skills.sh registry-driven targets", () => {
  ...
  it("substitutes projectId in copied skill files", () => {
    ...
    expect(skillContent).toContain("for projectId project-one."); // target id present, verbatim
    expect(skillContent).not.toContain("__RKS_SOURCE_PROJECT__"); // no surviving sentinel
    expect(skillContent).toContain("__PROJECT_ID__"); // placeholder SURVIVES
    expect(skillContent).toContain("Replace __PROJECT_ID__ with project-one"); // two distinct tokens
  });
