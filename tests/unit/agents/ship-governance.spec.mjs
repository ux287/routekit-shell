...
describe("Ship Agent governance", () => {
  ...
  describe("ShipInputSchema", () => {
    ...
    it("rejects unknown properties in strict mode", () => {
      // backlog.fix.agent-run-strict-input-and-delivery-guard — BODY FLIPPED, NAME KEPT.
      //
      // This test's name always stated the correct intent; its body used to assert the
      // opposite (that unknown keys are silently stripped), encoding the very defect that
      // let a `command` key vanish out of a delivery call and trigger an unbounded ship.
      //
      // Strictness is enforced at the DISPATCH layer (registry.parseAgentInput), not by
      // mutating the shared schema — one choke point covers both dispatch routes and any
      // future agent, where making each schema strict individually could be missed. So the
      // assertion is on strict-mode parsing, which is what dispatch actually applies.
      const result = ShipInputSchema.strict().safeParse({
        projectId: "test",
        baseBranch: "main",
      });
      expect(result.success).toBe(false);
      expect(JSON.stringify(result.error.issues)).toContain("baseBranch");
    });
  });
