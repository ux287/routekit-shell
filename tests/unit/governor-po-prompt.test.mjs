...
describe("governor-po.md — Decomposition Rules section", () => {
  ...
  it("tells the PO that every op:create target needs a FENCED CODE BLOCK, not just a directive", () => {
    expect(src).toMatch(/FENCED CODE BLOCK/i);
    expect(src).toContain("create_target_no_authorable_block");
  });

  it("states that rule OUTSIDE any code fence, so it reads as instruction and not as example", () => {
    // The trap this guards: governor-po.md already carried a CREATE-FILE instruction, but it
    // sits inside a ```markdown fence — rendered as sample text rather than as a rule. A plain
    // `toContain` passes while the instruction stays buried, so it must be checked positionally.
    //
    // Parity-splitting on "```" is NOT good enough here: this file opens a fence with an info
    // string and contains a second ```markdown line INSIDE it. Under CommonMark a closing fence
    // carries no info string, so that inner line is content, not a toggle — parity miscounts and
    // reports correctly-placed text as fenced. Track fence state the way a renderer does.
    const outside = [];
    let inFence = false;
    for (const line of src.split("\n")) {
      const isFenceLine = line.trimStart().startsWith("```");
      if (!inFence && isFenceLine) { inFence = true; continue; }
      if (inFence && line.trim() === "```") { inFence = false; continue; }
      if (!inFence) outside.push(line);
    }
    const outsideFence = outside.join("\n");

    expect(outsideFence).toMatch(/FENCED CODE BLOCK/i);
    expect(outsideFence).toContain("create_target_no_authorable_block");
  });
});
