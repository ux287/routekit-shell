...
describe("plan-staging suggestion — size is tractability, not a sibling split (backlog.feat.reconcile-story-sizing-po-arch-planner)", () => {
  ...
  it("the plan-staging hint directs to a multi-step plan and reserves siblings for an independent-concern break", () => {
    // Bounded by the emit block's own terminator, NOT by a fixed byte count. The
    // previous `slice(idx, idx + 400)` broke the moment a field was added ahead of
    // the hint — backlog.fix.refine-plan-staging-advisory-channel added
    // `applicable` / `applicableReason` and pushed the hint past the window,
    // reddening a test whose subject had not changed at all.
    const idx = src.indexOf('type: "plan_staging"');
    expect(idx).toBeGreaterThan(-1);
    const end = src.indexOf("});", idx);
    expect(end, "emit block terminator not found after the plan_staging type").toBeGreaterThan(idx);
    const suggestion = src.slice(idx, end);
