...
describe("decompose hint — childSlug field", () => {
  const src = readSource("packages/mcp-rks/src/server/refine.mjs");

  it("decompose suggestion hint mentions childSlug", () => {
    const hintMatch = src.match(/hint:\s*["']([^"']*childSlug[^"']*)/);
    expect(hintMatch).not.toBeNull();
    expect(hintMatch[1]).toContain("childSlug");
  });

  it("decompose suggestion hint mentions data.children", () => {
    const decompSuggestion = src.slice(
      src.indexOf('type: "decompose"'),
      src.indexOf('type: "decompose"') + 300
    );
    expect(decompSuggestion).toMatch(/data\.children|childSlug/);
  });
});
