...
describe('guardrailsOn 3-branch auto-ship', () => {
  ...
  it('skips push/PR/merge/cycle_complete and does local merge into working branch when working !== integration', async () => {
    ...
    // Scoped to the three local-only skips. The off-rail enforcement gate also
    // appends a skipped `review` step (reason policy_disabled in this fixture),
    // which is a different concern and carries its own reason.
    for (const s of skippedSteps) {
      if (!['working_pr', 'working_merge', 'cycle_complete'].includes(s.step)) continue;
  ...
  it('falls back to existing 2-branch dispatch when project.json has no branches field (regression preservation)', () => {
    // Static-analysis assertion: source still dispatches on isThreeBranch, but the
    // predicate and the delivery mechanics now come from the SHARED module rather than
    // being written out here. backlog.fix.ship-delivery-implemented-twice: this file and
    // story-ship.mjs each carried their own copy of both, and took OPPOSITE branches on
    // the same false predicate.
    // (Behavioral 2-branch coverage lives in tests/unit/guardrails-audit.spec.mjs.)

    // The predicate is imported, not recomputed. The inline form must be GONE — that
    // duplication is the defect this story removes.
    expect(guardrailsSrc).toMatch(/import\s*\{[^}]*\bisThreeBranchTopology\b[^}]*\}\s*from\s*["']\.\/git\/branch-delivery\.mjs["']/);
    expect(guardrailsSrc).toMatch(/const\s+isThreeBranch\s*=\s*isThreeBranchTopology\s*\(\s*branchConfig\s*\)/);
    expect(guardrailsSrc).not.toMatch(/const\s+isThreeBranch\s*=\s*branchConfig\.working\s*!==\s*branchConfig\.integration/);

    // Both arms delegate delivery to the shared implementation.
    expect(guardrailsSrc).toMatch(/if\s*\(\s*isThreeBranch\s*\)\s*\{[\s\S]*deliverFeatureBranch\s*\(/);
    expect(guardrailsSrc).toMatch(/}\s*else\s*\{[\s\S]*deliverFeatureBranch\s*\([\s\S]*push-staging/);

    // The 2-branch delivery MECHANICS no longer live inline in this file: the delete and
    // the push are the shared module's, and the rail only MAPS their records into its own
    // step names.
    //
    // Deliberately NOT asserted as file-wide absence of `branch -D` or `push origin` —
    // that over-reaches. This file legitimately retains both elsewhere: a helper push, a
    // recovery-path force-delete, and the unpushed-commits push that runs BEFORE the
    // delivery block. A file-wide negative would redden on code this story never touches.
    expect(guardrailsSrc).toMatch(/delivery\.steps\.find\(\(s\) => s\.kind === "delete_branch"\)/);
    expect(guardrailsSrc).toMatch(/delivery\.steps\.find\(\(s\) => s\.kind === "push_working"\)/);
  });
  ...
  it('3-branch unpushed-commits path stays local instead of pushing to remote', () => {
    ...
    //
    // The returned EXPRESSION is deliberately NOT pinned. This witness has now
    // broken twice on exactly that mistake: 8acaa672 fixed a stale regex here,
    // and b869adfb reddened it again by wrapping every exit in
    // finalizeShipOutcome(...) — a pure refactor with no behaviour change.
    // Assert that an early return EXISTS; never assert its text.
    //
    // The return witness is scoped to the isThreeBranch guard, NOT to the wider
    // aheadBlock. That wider window also contains the gate_alpha halt return, so
    // a bare /\breturn\b/ over it would keep passing even if this entire guard
    // were deleted — green while proving nothing.
    ...
    const idxThree = guardrailsSrc.indexOf('if (isThreeBranch)', idxAhead);
    expect(idxThree).toBeGreaterThan(-1);
    // Load-bearing: if the guard were deleted, indexOf would run on to the later
    // isThreeBranch occurrence past the push and produce a reversed (empty)
    // slice, which would fail for the wrong reason. Fail loudly here instead.
    expect(idxThree).toBeLessThan(idxPush);
    const threeBranchGuard = guardrailsSrc.slice(idxThree, idxPush);
    expect(threeBranchGuard).toMatch(/localOnly\s*=\s*true/);
    expect(threeBranchGuard).toMatch(/\breturn\b/);
  });
});
