...
// Per-case fixture-root allocation, hoisted to module scope so the uniqueness
// witness below exercises the SAME allocator the suite uses, not a re-implementation.
function allocateProjectRoot(base) {
  // mkdtempSync is atomic and cannot collide. A Date.now()-derived name plus
  // mkdirSync({ recursive: true }) silently SHARED a root between two cases
  // entering beforeEach in the same millisecond, so a case could assert against
  // the previous case's files. recursive:true made that collision invisible.
  return fs.mkdtempSync(path.join(base, "project-"));
}

describe("interview module", () => {
  let tempDir;
  let projectRoot;
  const allocatedRoots = [];
  ...
  beforeEach(() => {
    ...
    projectRoot = allocateProjectRoot(tempDir);
    allocatedRoots.push(projectRoot);
  });
  ...
  describe("runInterview", () => {
    ...
    it("creates project.overview.md on completion", async () => {
      const notePathBefore = path.join(projectRoot, "notes", "project.overview.md");
      expect(fs.existsSync(notePathBefore)).toBe(false);
  ...
  describe("fixture-root allocation", () => {
    it("allocates a distinct root on every call even when the clock does not advance", () => {
      const realNow = Date.now;
      try {
        Date.now = () => 1700000000000;
        const roots = [];
        for (let i = 0; i < 50; i++) {
          roots.push(allocateProjectRoot(tempDir));
        }
    ...
    it("gave every case its own fixture root, all inside the suite temp dir", () => {
      expect(allocatedRoots.length).toBeGreaterThanOrEqual(2);
      expect(new Set(allocatedRoots).size).toBe(allocatedRoots.length);
      const outside = allocatedRoots.filter(r => !r.startsWith(tempDir + path.sep));
