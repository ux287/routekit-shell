...
// ── backlog.fix.dendron-write-commit-unscoped-pathspec ───────────────────────
//
// Reported from child project ux287-bpm: seven unrelated staged files were swept into
// a note-creation commit, which ended up carrying 8 files and 1,467 insertions. The
// stage and the idempotence pre-check were already path-scoped; only the commit was not.
//
// "The file the tool wrote" is sometimes MORE than one path — a rename declares an extra
// staged path — so the scope is the tool's declared write set, not literally one file.

describe('a dendron commit is scoped to its own write set', () => {
  let repo;
  ...
  function stagedPaths() {
    return spawnSync('git', ['diff', '--cached', '--name-only'], { cwd: repo, encoding: 'utf8' })
      .stdout.trim().split('\n').filter(Boolean);
  }

  function headPaths() {
    return spawnSync('git', ['diff-tree', '--no-commit-id', '--name-only', '-r', 'HEAD'], { cwd: repo, encoding: 'utf8' })
      .stdout.trim().split('\n').filter(Boolean);
  }

  function seedUnrelatedStagedWork() {
    fs.writeFileSync(path.join(repo, 'unrelated-a.txt'), 'a\n');
    fs.writeFileSync(path.join(repo, 'unrelated-b.txt'), 'b\n');
    spawnSync('git', ['add', 'unrelated-a.txt', 'unrelated-b.txt'], { cwd: repo });
  }

  it('THE DEFECT — unrelated staged files are NOT absorbed into the note commit', async () => {
    seedUnrelatedStagedWork();
    const rel = path.join('notes', 'scoped.md');
    fs.writeFileSync(path.join(repo, rel), '---\nid: "scoped"\n---\n\nbody\n');

    const res = await commitAndEmbedNote({
      projectRoot: repo, notePath: rel, commitMessage: 'docs(notes): create scoped',
    });

    expect(res.commitOk).toBe(true);
    expect(headPaths()).toEqual([rel]);
    // Still staged, still uncommitted — the caller's own work survives untouched.
    expect(stagedPaths().sort()).toEqual(['unrelated-a.txt', 'unrelated-b.txt']);
  });

  it('reports the paths the commit ACTUALLY contained, read back from git', async () => {
    seedUnrelatedStagedWork();
    const rel = path.join('notes', 'reported.md');
    fs.writeFileSync(path.join(repo, rel), '---\nid: "reported"\n---\n\nbody\n');

    const res = await commitAndEmbedNote({
      projectRoot: repo, notePath: rel, commitMessage: 'docs(notes): create reported',
    });

    // A caller can tell from the return value alone whether the scoping worked.
    expect(res.committedPaths).toEqual([rel]);
    expect(res.committedPaths).not.toContain('unrelated-a.txt');
  });

  it('covers a rename — both declared paths land in one scoped commit', async () => {
    const oldRel = path.join('notes', 'old-name.md');
    fs.writeFileSync(path.join(repo, oldRel), '---\nid: "old-name"\n---\n\nbody\n');
    spawnSync('git', ['add', oldRel], { cwd: repo });
    spawnSync('git', ['commit', '-m', 'seed'], { cwd: repo });

    seedUnrelatedStagedWork();
    fs.rmSync(path.join(repo, oldRel));
    const newRel = path.join('notes', 'new-name.md');
    fs.writeFileSync(path.join(repo, newRel), '---\nid: "new-name"\n---\n\nbody\n');
    spawnSync('git', ['add', oldRel], { cwd: repo });

    const res = await commitAndEmbedNote({
      projectRoot: repo,
      notePath: newRel,
      commitMessage: 'docs(notes): implement new-name',
      extraStagePaths: [oldRel],
    });

    expect(res.commitOk).toBe(true);
    expect(headPaths().sort()).toEqual([newRel, oldRel].sort());
    expect(stagedPaths().sort()).toEqual(['unrelated-a.txt', 'unrelated-b.txt']);
  });

  it('the RAG re-embed covers only the scoped write set', async () => {
    // The embed itself is suppressed under vitest (commit-and-embed.mjs guards on
    // process.env.VITEST, because runRagEmbed loads a real transformer model). So this
    // asserts the value the embed is FED FROM rather than the call: commitAndEmbed
    // derives `changedFiles` from the commit it just made and passes that same array
    // both to runRagEmbed and back as committedPaths. Scoping the commit therefore
    // scopes the embed as a consequence, and this is the observable half of that.
    seedUnrelatedStagedWork();
    const rel = path.join('notes', 'embedded.md');
    fs.writeFileSync(path.join(repo, rel), '---\nid: "embedded"\n---\n\nbody\n');

    const res = await commitAndEmbedNote({
      projectRoot: repo, notePath: rel, commitMessage: 'docs(notes): create embedded',
    });

    expect(res.committedPaths).toEqual([rel]);
    expect(res.committedPaths).not.toContain('unrelated-a.txt');
    expect(res.committedPaths).not.toContain('unrelated-b.txt');
  });

  it('the skipEmbed branch is scoped too, and reports its committed paths', async () => {
    seedUnrelatedStagedWork();
    const rel = path.join('notes', 'raw.md');
    fs.writeFileSync(path.join(repo, rel), '---\nid: "raw"\n---\n\nbody\n');

    const res = await commitAndEmbedNote({
      projectRoot: repo, notePath: rel, commitMessage: 'docs(notes): create raw', skipEmbed: true,
    });

    expect(res.commitOk).toBe(true);
    expect(res.committedPaths).toEqual([rel]);
    expect(stagedPaths().sort()).toEqual(['unrelated-a.txt', 'unrelated-b.txt']);
    expect(ragEmbedCalls).toHaveLength(0);
  });

  it('the legacy memory form is scoped as well', async () => {
    seedUnrelatedStagedWork();
    const res = await commitAndEmbedNote({ projectRoot: repo, slug: 'scoped-memory', content: 'remembered' });

    expect(res.ok).toBe(true);
    expect(res.committedPaths).toEqual([path.join('notes', 'memories.scoped-memory.md')]);
    expect(stagedPaths().sort()).toEqual(['unrelated-a.txt', 'unrelated-b.txt']);
  });

  it('the note COMMIT CONTENT is the note, not merely the note path', async () => {
    // A trailing pathspec makes this a partial commit, which takes the path FROM THE
    // INDEX. Asserting only the file list would pass against a commit that recorded the
    // right name with the wrong bytes.
    seedUnrelatedStagedWork();
    const rel = path.join('notes', 'content.md');
    fs.writeFileSync(path.join(repo, rel), '---\nid: "content"\n---\n\nEXPECTED BODY\n');

    await commitAndEmbedNote({
      projectRoot: repo, notePath: rel, commitMessage: 'docs(notes): create content',
    });

    const committed = spawnSync('git', ['show', `HEAD:${rel}`], { cwd: repo, encoding: 'utf8' }).stdout;
    expect(committed).toContain('EXPECTED BODY');
  });
});
