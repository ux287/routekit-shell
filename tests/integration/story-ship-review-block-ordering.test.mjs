...
describe("the acCoverage on the blocked-ship response is allowlisted and bounded", () => {
  beforeEach(() => {
    mockRoot = makeTempRepo();
    // failOpen false with ok true, so the halt this reaches is the VERDICT-BLOCK one and
    // not the reviewer-did-not-run one. The exact error string below is what proves which
    // of the two fired.
    reviewPolicyFixture = { enabled: true, failOpen: false };
    reviewResultFixture = {
      ok: true,
      ...
      summary: "blocking finding present",
      findings: [{ severity: "block", message: "no" }],
      // Two shapes redactReview cannot reach: this file's redactReview mock is the
      // IDENTITY function, so what this case proves is the allowlist and the bound,
      // never the scrub.
      acCoverage: {
        assessed: true,
        covered: ["AC1 is covered"],
        notCovered: "AC2 is not covered",
        uncertain: [],
        notes: "a key the model invented",
      },
    };
  });
  ...
  it("drops the invented key and the string-valued list before the caller sees them", async () => {
    const result = await runStoryShipTool({
      projectId: "test-project",
      problemId: "backlog.feat.test",
    });
    ...
    expect(result.ok).toBe(false);
    expect(result.error).toBe("Code review blocked merge");

    expect(Object.keys(result.review.acCoverage).sort()).toEqual([
      "assessable",
      "assessed",
      "covered",
      "notCovered",
    ...
    expect(result.review.acCoverage.notes).toBeUndefined();
    expect(result.review.acCoverage.notCovered).toEqual([]);
  });
});
