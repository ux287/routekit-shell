 ...
 * backlog.fix.test-fixture-repo-containment: a fixture that cannot initialise must abort the
 * test, never continue against whatever repository happens to enclose it.
 */
function checkedGit(cwd, args) {
  const res = spawnSync("git", args, { cwd, encoding: "utf8", timeout: 30_000 });
  if (res.status !== 0) {
    throw new Error(
      `fixture git ${args.join(" ")} failed in ${cwd} (status ${res.status}): ` +
...
describe("cycle-complete telemetry and stale branch cleanup", { timeout: 30_000 }, () => {
  ...
  function initGitRepoWithOrigin(dir) {
    // backlog.fix.test-fixture-repo-containment: BOTH inits are status-checked.
    //
    // Neither was, and each failure mode is silent in its own way. A failed WORKING init leaves
    // every subsequent git call to bind, via git's walk-up, to whatever repository encloses the
    // fixture — which is how this suite hard-reset the developer's repo. A failed BARE init is
    // quieter still: the push below fails, the working repo has no origin, and every
    // `origin/staging` divergence assertion in this file becomes vacuously true while the tests
    // stay green.
    //
    // `dir` is now under os.tmpdir() (makeTempDir), and `bareDir` is its sibling under the same
    // base so the globalTeardown sweep still reclaims it.
    const bareDir = dir + "-origin";
    fs.mkdirSync(bareDir, { recursive: true });
    checkedGit(bareDir, ["init", "--bare", "-b", "staging"]);

    // Init the working repo
    checkedGit(dir, ["init", "-b", "staging"]);
