...
describe("routekit project attach --stack", () => {
  it("applies template kg/protected-files and seeds notes without overwrite", () => {
    ...
    const templateSrc = path.join(repoRoot, "templates", "app.web.react.spa");
    const templateDest = path.join(shellRoot, "templates", "app.web.react.spa");
    ...
    expect(fs.readFileSync(kgPath, "utf8")).toMatch(/framework:\s*vite-react/);

    const projectJsonPath = path.join(projectRoot, "routekit", "project.json");
    const projectJson = JSON.parse(fs.readFileSync(projectJsonPath, "utf8"));
    expect(projectJson.stack).toBe("app.web.react.spa");
    ...
    expect(record.stack).toBe("app.web.react.spa");
    ...
    const reattach = spawnSync(
      process.execPath,
      [cliPath, "project", "attach", "--id", "snacks", "--path", projectRoot, "--stack", "app.web.react.spa"],
