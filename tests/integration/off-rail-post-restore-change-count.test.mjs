...
describe("off-rail change count reflects the post-restore, committable set", () => {
  ...
  it("creates no off-rail commit when the session changed nothing, even with unpushed commits present", async () => {
    ...
    // NOTE, deliberately not asserted here: guardrailsOn may still push the
    // pre-existing unpushed commit via the separate no-changes aheadCount path
    // (guardrails-audit.mjs ~:1828), which cannot tell a session commit from one
    // that predates the session. That is a distinct pre-existing defect, out of
    // scope for this story, and pinning it either way here would be wrong.
  });
