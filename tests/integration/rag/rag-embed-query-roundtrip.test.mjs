...
import { verifyReadContract } from '@routekit/rag/embed';
...
describe('rag embed→query roundtrip: status-column contract (integration)', () => {
  ...
  // backlog.fix.rag-query-arrow-tags-serialization — real-driver guard.
  // Pins the decode against the ACTUALLY INSTALLED @lancedb/lancedb rather than a hand-rolled Arrow
  // stand-in. Deliberately asserts only the decoded OUTPUT: the driver is declared as a floating
  // `^0.21.3` caret, so asserting its internal representation would make this test a version canary
  // instead of a contract check — and the whole point of the fix is tolerating that drift.
  it('CORE: tags read back through the required projection decode to a plain string[]', async () => {
    const rows = normalizeRagRows([codeRow('c-tags'), noteRow('n-tags')]);
    const db = await connect(path.join(tmpRoot, 'tags'));
    const table = await db.createTable('embeddings', rows);

    const out = await table.query().select([...RAG_REQUIRED_COLUMNS]).limit(10).toArray();
    const byId = Object.fromEntries(out.map((r) => [r.id, r]));

    const codeTags = toPlainList(byId['c-tags'].tags);
    expect(Array.isArray(codeTags)).toBe(true);
    expect(codeTags).toEqual(['code']);
    expect(codeTags.every((t) => typeof t === 'string')).toBe(true);

    expect(toPlainList(byId['n-tags'].tags)).toEqual(['design-system']);
  }, 60_000);
