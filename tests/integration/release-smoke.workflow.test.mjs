 ...
 * Asserts that the production packages/rag/src/embed.mjs, run against the fixture
 ...
 *   node packages/rag/src/embed.mjs
 *
 *   ROUTEKIT_PROJECT_ROOT="$(pwd)/tests/integration/fixtures/release-smoke-vault" \
...
// `node packages/rag/src/embed.mjs` first, so the LanceDB index is absent and the
  ...
  if (!fs.existsSync(lanceDbPath)) {
    dbOpenError = new Error(
      `LanceDB index not found at ${lanceDbPath}. ` +
        `Did you run \`node packages/rag/src/embed.mjs\` against the fixture vault first? ` +
