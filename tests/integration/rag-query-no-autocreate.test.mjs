...
describe("rag create paths untouched — guard is query-path-only (AC4)", () => {
  it("the no-auto-create guard lives ONLY in the query path", () => {
    expect(read("scripts/rag/query.mjs")).toMatch(/RAG store not found at/);
    expect(read("scripts/rag/embed.mjs")).not.toMatch(/RAG store not found at/);
    expect(read("scripts/rag/init.mjs")).not.toMatch(/RAG store not found at/);
  });
  ...
  it("the embed path still creates the embeddings table", () => {
    ...
    expect(read("scripts/rag/embed.mjs")).toMatch(/createTable/);
  });
});
