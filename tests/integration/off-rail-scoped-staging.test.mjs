...
describe("work preservation — nothing is lost across the merge and the branch delete", () => {
  ...
  it("keeps an uncommitted edit to a TRACKED out-of-scope file through the auto-ship cycle_complete", async () => {
    // backlog.fix.cycle-complete-discards-uncommitted-changes. The IRREPLACEABLE case below covers
    // an UNTRACKED file, which `reset --hard` never touches — it passed while the defect was live.
    // A tracked file is the case the reset destroyed: the ship left it unstaged by design
    // (unstagedOutOfScope), and cycle_complete's `reset --hard origin/staging` then reverted it.
    const TRACKED = "package-lock.json";
    write(repo, TRACKED, '{ "version": "0.56.0" }\n');
    git(repo.dir, ["add", TRACKED]);
    expect(git(repo.dir, ["commit", "-q", "-m", "chore: tracked baseline"]).status).toBe(0);
    expect(git(repo.dir, ["push", "-q", "origin", "staging"]).status).toBe(0);

    expect((await guardrailsOff(repo.dir, "t", "all", STORY_ID, "test-project")).ok).toBe(true);
    write(repo, IN_SCOPE, "export const v = 2;\n");
    write(repo, TRACKED, '{ "version": "0.58.5" }\n');

    const res = await guardrailsOn(repo.dir, {}, "test-project");
    expectShipped(res);
    expect(res.unstagedOutOfScope).toContain(TRACKED);

    // The content survives on disk, and git still sees it as a modification.
    expect(read(repo, TRACKED)).toBe('{ "version": "0.58.5" }\n');
    expect(porcelain(repo, TRACKED)).toMatch(/^ M package-lock\.json$/m);

    const cycle = (res.shipSteps || []).find((s) => s.step === "cycle_complete");
    expect(cycle, "cycle_complete ship step missing").toBeTruthy();
    expect(cycle.ok).toBe(true);
    expect(cycle.preservedPaths).toContain(TRACKED);
  });
