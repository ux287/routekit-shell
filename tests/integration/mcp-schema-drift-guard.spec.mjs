...
describe("MCP tool contract — no zod/inputSchema drift", () => {
  ...
  it("rks_arch_verdict rejects a finding with no numeric `item`, and accepts item 11", () => {
    // Establishes WHICH failure mode an unnumbered checklist entry produces. The story
    // this guards assumed a cross-story finding "could not enter the frozen ledger"; the
    // actual mechanism is rejection at the call boundary, because server.mjs .parse()s the
    // args before computeArchVerdict ever runs. So findingKey's `slug()` totality — which
    // would otherwise yield a degenerate `item-nan-<file>` key — is unreachable through the
    // tool. The operational consequence is worse than a dropped finding: ONE unnumbered
    // finding throws the WHOLE verdict call, so a governor either drops it or mis-attributes
    // it to another item's number. Item 11 exists so it has a number to carry.
    const schema = TOOL_ARG_SCHEMAS["rks_arch_verdict"];
    expect(schema, "rks_arch_verdict has no zod schema").toBeTruthy();

    const base = { projectId: "p", storyId: "backlog.fix.example" };
    const at = (result, segs) =>
      (result.error?.issues ?? []).some((i) => JSON.stringify(i.path) === JSON.stringify(segs));

    // NEGATIVE half — the discriminating one. A well-formed finding parses under BOTH the
    // reject and the admit-with-nan hypotheses, so the positive half alone proves nothing.
    const missing = schema.safeParse({ ...base, findings: [{ file: "a.md" }] });
    expect(missing.success, "a finding with no `item` must not parse").toBe(false);
    expect(at(missing, ["findings", 0, "item"]), "rejection must be attributed to `item`").toBe(true);

    // A non-numeric item is rejected too — `item` is z.number(), not a coerced string.
    const wrongType = schema.safeParse({ ...base, findings: [{ item: "11", file: "a.md" }] });
    expect(wrongType.success, "`item` must be a number, not a numeric string").toBe(false);

    // POSITIVE half — 11 is admissible with no schema change. There is no enum and no
    // .max() on `item`, which is why promoting the cross-story check to a numbered item
    // required no edit to server.mjs or arch-verdict.mjs.
    const numbered = schema.safeParse({ ...base, findings: [{ item: 11, file: "a.md" }] });
    expect(numbered.success, "a numeric item 11 must parse").toBe(true);
  });
