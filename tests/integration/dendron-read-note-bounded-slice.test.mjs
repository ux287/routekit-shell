...
describe("selectors reach the handler and are observed, not assumed", () => {
  ...
  it("maxBytes ALONE requests bounded mode, even when the note fits under it", async () => {
    ...
    expect(res.content).toBe(SMALL_NOTE);
    // backlog.fix.dendron-read-note-selection-completeness-signal — STRICT equality,
    // not toBeTruthy: this exit is reached with no selector, and an implementation that
    // attaches the field only on the selection branch leaves it undefined here, which a
    // truthiness check would silently pass.
    expect(res.selectionComplete).toBe(true);
  });
...
//
// A whole-array write built from a partial read destroyed 22 of 40 entries with every
// success flag reading true. The read side's contribution was that nothing in the
// payload distinguished a complete narrow selection from a cut one — `truncated` is
// true for every selection by construction.

describe("selection completeness crosses the MCP wire", () => {
  it("a complete fields selection reports selectionComplete true alongside truncated true", async () => {
    const res = await callTool({ filename: "backlog.fix.fixture", fields: ["id"] });
    expect(res.selectionComplete).toBe(true);
    expect(res.truncated).toBe(true);
    expect(res.selectionBytes).toBe(res.returnedBytes);
    expect(res.fieldsNotFound).toEqual([]);
  });

  it("a CUT selection reports selectionComplete false with selectionBytes above returnedBytes", async () => {
    const res = await callTool({ filename: "backlog.fix.fixture", sections: ["Solution"], maxBytes: 12 });
    expect(res.selectionComplete).toBe(false);
    expect(res.selectionBytes).toBeGreaterThan(res.returnedBytes);
  });

  it("the over-cap manifest path reports selectionComplete false with returnedBytes 0", async () => {
    const res = await callTool({ filename: "backlog.fix.fixture", maxBytes: 10 });
    expect(res.manifest).toBeTruthy();
    expect(res.content).toBeUndefined();
    expect(res.returnedBytes).toBe(0);
    expect(res.selectionBytes).toBe(res.totalBytes);
    expect(res.selectionComplete).toBe(false);
    expect(res.fieldsNotFound).toEqual([]);
  });

  it("names requested fields the note does not carry", async () => {
    const res = await callTool({ filename: "backlog.fix.fixture", fields: ["id", "notAField"] });
    expect(res.fieldsNotFound).toEqual(["notAField"]);
    // Found-ness is independent of completeness — nothing was cut.
    expect(res.selectionComplete).toBe(true);
  });

  it("does NOT leak the new fields onto the byte-identical unbounded path", async () => {
    const res = await callTool({ filename: "backlog.fix.small" });
    expect(Object.keys(res).sort()).toEqual(["content", "filename", "ok"]);
  });

  it("the advertised description distinguishes the two completeness fields", async () => {
    const tool = await listReadNoteSchema();
    expect(tool.description).toContain("selectionComplete");
    expect(tool.description).toMatch(/truncated means only that the response is not the whole note/);
    expect(tool.description).toMatch(/true for every selection by construction/);
  });

  it("introduces NO new input parameter", async () => {
    const tool = await listReadNoteSchema();
    const props = Object.keys(tool.inputSchema.properties);
    for (const added of ["selectionComplete", "selectionBytes", "fieldsNotFound"]) {
      expect(props, `${added} must not become an input`).not.toContain(added);
    }
  });
});
