...
describe("planner Targets (globs/dirs)", () => {
  it("expands directory targets to existing files and allows create under the directory", () => {
    ...
    // backlog.fix.planner-note-step-false-rejection: an out-of-target step is now REJECTED
    // (_invalid, with a reason) rather than disguised as a "note". The old note-downgrade dropped
    // the step's `path`, so a step that also lacked a title became an unnameable note — and a
    // single such note discarded the entire plan while every diagnostic rendered it as "".
    // Rejection is still rejection; it just keeps its identity now.
    const createRejected = validateStep(
      { action: "create_file", path: "src/_includes/components/new-comp.njk", content: "ok" },
      resolved,
      projectRoot
    );
    expect(createRejected._invalid).toBe(true);
    expect(createRejected._invalidReason).toMatch(/not in editable targets/i);
    expect(createRejected.path).toBe("src/_includes/components/new-comp.njk"); // still nameable
  });
  ...
  it("supports explicit file targets", () => {
    ...
    // Rejected, not disguised as a note — see the note above.
    const editRejected = validateStep(
      { action: "edit_file", path: "src/b.js", content: "export const b = 1;\n" },
      resolved,
      projectRoot
    );
    expect(editRejected._invalid).toBe(true);
    expect(editRejected._invalidReason).toMatch(/not in editable targets/i);
    expect(editRejected.path).toBe("src/b.js"); // still nameable
  });
});
