...
// `vi` only: the afterEach sweep this file used to register is gone (see the note at the
// foot of the file), and with it the fs and path imports it needed.
...
vi.mock('@routekit/telemetry', () => {
  const mockStorage = {
    ...
    flush: vi.fn(),
    clearBuffer: vi.fn(),
    // Mirrors a method a shipped consumer calls: exec.mjs takes its correlationId from
    // ensureTelemetryStorage().startCorrelation(). Returns a stable string.
    startCorrelation: vi.fn(() => 'test-correlation-id'),
  };
  return {
...
// ---------------------------------------------------------------------------
// NOTHING HERE MAY TOUCH .rks/telemetry. It is the LIVE sink this project writes:
// packages/telemetry/src/storage.mjs appends events-<date>.jsonl there for the running
// MCP server, and the hooks under .routekit/hooks/system append to it independently.
// `path.resolve` binds to the worker cwd, which is the repository root whenever the
// suite is launched there, and .gitignore excludes the directory — so a sweep here
// destroyed real, unrecoverable operational data on every test run.
//
// This file used to end with an afterEach that readdir'd that directory and unlinked
// every .jsonl in it. No test depended on it: isolation comes from the two vi.mock
// blocks above, and every opt-out roots its storage in a temp directory. NO cleanup that
// deletes or shortens a file at this path can be made safe, because live writers append to
// the same files concurrently and nothing on disk attributes a byte to a test rather than
// to the server. Tests that need a telemetry sink must root one in a temp directory.
// (backlog.fix.test-suite-wipes-project-telemetry-sink)
// ---------------------------------------------------------------------------
