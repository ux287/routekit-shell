...
description: |
  ...
  $ARGUMENTS (e.g. backlog.agents.skills.my-story). Every story passes the ARCH gate first:
  draft runs QA then ARCH then builds; ready runs ARCH then builds; arch-approved builds directly.
...
1. Check the story phase from $ARGUMENTS (the storyId). No branch reaches Build without ARCH approval:
   - If phase is `draft`: QA (step 2), then ARCH (step 3), then Build (step 4).
   - If phase is `ready`: ARCH (step 3), then Build (step 4).
   - If phase is `arch-approved`: Build (step 4) directly.
...
3. ARCH Governor (if draft or ready) — launch and wait for completion:
  ...
  prompt:
   ...
   - `needs-revision`: STOP. Surface every finding with its file/line detail and wait for user direction. Do NOT launch Build.
