...
async function getEmbeddingPipeline() {
  if (!embeddingPipeline) {
    console.log('🤖 Loading embedding model (Xenova/all-MiniLM-L6-v2)...', { stream: process.stderr });
    const startTime = Date.now();
    embeddingPipeline = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2');
    const duration = Date.now() - startTime;
    
    // Log the model loading as a tool call
    await logToolCall('embedding_model_load', { model: 'Xenova/all-MiniLM-L6-v2' }, duration, true);
    
    console.log('✅ Embedding model loaded', { stream: process.stderr });
  }
