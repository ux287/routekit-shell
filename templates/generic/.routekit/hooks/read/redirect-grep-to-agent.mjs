...
import { isHarnessSessionOutputPath } from "../lib/read-classification.mjs";
...
async function main() {
  ...
  // Harness session output — searching output THIS session produced is not
  // codebase exploration. This hook never consults classifyReadIntent, so the
  // carve-out there does not reach Grep; it needs its own check or an agent
  // still cannot grep its own overflowed tool output.
  if (isHarnessSessionOutputPath(searchPath, hookData.session_id)) process.exit(0);

  // Research-Agent outage fallthrough (fail-closed) — AFTER the active-scope check so scope still
  // wins. A fresh infra breadcrumb permits a bounded direct search rather than redirecting into the
  // down agent. Absent/stale/malformed → normal redirect below.
  const outage = isResearchAgentOutage(PROJECT_DIR);
  if (outage) {
    emitOutageFallthrough({ blockedTool: "Grep", targetPath: searchPath, category: outage.category, projectDir: PROJECT_DIR });
    process.exit(0);
  }
  ...
  // Keyless mode is MESSAGING-ONLY (INV-2): Grep still redirects (decision unchanged); with no LLM
  // credential the Research Agent is unavailable, so point at the keyless retrieval primitive.
  const instructions = isKeyless()
    ? [
        "Keyless mode (no LLM credential): the Research Agent is unavailable.",
        "Use rks_rag_query for keyless semantic retrieval over the knowledge base instead of raw Grep.",
      ]
    : [
