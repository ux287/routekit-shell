...
async function embedNotes(
  ...
  try {
    ...
    if (!refreshedTableNames.includes("embeddings")) {
      ...
      // Ensure at least one record has proper tag/array structure for schema inference
      if (!allEmbeddings[0].tags || allEmbeddings[0].tags.length === 0) {
        allEmbeddings[0].tags = ["design-system"];
      }
      if (!allEmbeddings[0].heading_path || allEmbeddings[0].heading_path.length === 0) {
        allEmbeddings[0].heading_path = ["root"];
      }
      ...
      // Check for schema mismatch - detect new fields not in existing table
      const existingSchema = await table.schema();
      const existingFields = new Set(existingSchema.fields.map(f => f.name));
      const newFields = Object.keys(allEmbeddings[0] || {}).filter(k => !existingFields.has(k));

      if (newFields.length > 0) {
        console.log(`⚠️  Schema mismatch detected. New fields: ${newFields.join(', ')}`);
        ...
        if (allEmbeddings.length > 0 && (!allEmbeddings[0].heading_path || allEmbeddings[0].heading_path.length === 0)) {
          allEmbeddings[0].heading_path = ["root"];
        }
        table = await db.createTable("embeddings", allEmbeddings);
        chunksAdded = allEmbeddings.length;
    ...
    // Persist manifest after successful DB write
    saveManifest(currentContext.projectRoot, { hashes: newHashes });
