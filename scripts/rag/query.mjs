...
async function getEmbeddingPipeline() {
  if (!embeddingPipeline) {
    if (!isSilent()) console.error('🤖 Loading embedding model (Xenova/all-MiniLM-L6-v2)...');
    embeddingPipeline = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2');
    if (!isSilent()) console.error('✅ Embedding model loaded');
  }
...
async function queryEmbeddings(query, limit = DEFAULT_LIMIT, dbPath = DB_PATH, projectSlug = DEFAULT_PROJECT_SLUG, intent = 'neutral') {
  try {
    ...
    const results = await table
      .search(queryVector)
      .select(['id', 'slug', 'title', 'path', 'text', 'chunkId', 'tags', 'status', 'updatedAt', 'content_type'])
      .limit(limit)
      .toArray();
