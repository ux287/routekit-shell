...
import { CONTENT_TYPE_BOOST, inferQueryIntent, isImplementationQuery, getNamespaceBoost } from '@routekit/rag/query-intent';
import { missingRequiredColumns, selectableProjection, tableFieldNames } from '@routekit/rag/rag-columns';
...
async function queryEmbeddings(query, limit = DEFAULT_LIMIT, dbPath = DB_PATH, projectSlug = DEFAULT_PROJECT_SLUG, intent = 'neutral') {
  try {
    ...
    // (scripts/rag/init.mjs, scripts/rag/embed.mjs) are untouched and must still create on first run.
