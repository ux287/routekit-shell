...
// Load routing config (same as MCP server)
let routingConfig;
...
try {
  ...
  console.log("✅ Loaded routing configuration");
} catch (error) {
  ...
  for (const test of integrationTests) {
    ...
    try {
      ...
      if (shouldOrch) {
        ...
        const { passages, TRACE } = await retrieveWithRouting(test.query, routingConfig, null);
...
async function performanceTest() {
  ...
  const { passages: simplePassages } = await retrieveWithRouting(simpleQuery, routingConfig, null);
  ...
  const orchestrationResults = await executeOrchestration(analysis.orchestrationPlan, routingConfig, null);
