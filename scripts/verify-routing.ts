...
async function main() {
  ...
  const guard = yaml.load(fs.readFileSync(path.join(".routekit","policy.guardrails.yaml"), "utf8"));
  ...
  for (const q of queries) {
    const { passages, trace, TRACE } = await retrieveWithRouting(q as string, cfg, guard);
    console.log("\nQ:", q);
