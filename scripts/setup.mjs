...
/**
 * Default runner — real subprocess. Throws on non-zero exit.
 *
 * `timeout` is per-call (default 600000ms / 10min — the protective cap for the fast
 * steps). Pass `timeout: 0` to disable the cap for genuinely long steps (the cold-clone
 * RAG embed): a fixed cap there fired mid-embed and — because spawnSync SIGTERMs only its
 * direct child, not the process group — orphaned the grandchild embedder (it reparented to
 * PID 1 and kept running, holding the TTY) while setup reported a false ETIMEDOUT failure.
 * Untiming the embed makes setup block until it truly completes, so nothing orphans.
 */
export function defaultRunner(cmd, args, { cwd, timeout = 600000 }) {
  const r = spawnSync(cmd, args, { cwd, stdio: "inherit", timeout });
...
/**
 * backlog.fix.setup-preserve-detached-head-pin
 *
 * THE single detached-HEAD detection in this file. Both the land-on-staging guard in runSetup() and
 * the health gate in checkCloneHealth() route through it, so the two can never disagree about what
 * "detached" means.
 *
 * `rev-parse --abbrev-ref HEAD` EXITS 0 when HEAD is detached and returns the literal string
 * "HEAD" — which is precisely why every truthiness- and status-based check in this codebase read a
 * detached clone as healthy. Compare the trimmed VALUE; never the exit status.
 */
export function isDetachedHead(gitCapture, root) {
  const branch = (gitCapture(["rev-parse", "--abbrev-ref", "HEAD"], { cwd: root })?.stdout || "").trim();
  return branch === "HEAD";
}

/** The ref a detached clone is sitting on, for operator-facing messages. Never returns empty. */
function describeDetachedRef(gitCapture, root) {
  const described = (gitCapture(["describe", "--tags", "--always"], { cwd: root })?.stdout || "").trim();
  return described || "an unnamed commit";
}
...
export async function runSetup({
  ...
  // No timeout: a cold first clone downloads the ONNX model, creates the LanceDB table, and
  // embeds the whole repo from scratch, which can exceed the 10-min cap the fast steps keep.
  // A cap here fired mid-embed and orphaned the embedder (see defaultRunner). Block until done.
  runner("routekit", ["rag", "embed", projectId], { cwd: root, timeout: 0 });
  ...
  // backlog.fix.setup-preserve-detached-head-pin: this guard is FIRST, ahead of the origin/staging
  // and dirty checks, because a detached HEAD is a DELIBERATE state — the README's own "pin to a tag
  // for stability" advice puts you in it — and the advice we owe the operator is the same whether or
  // not origin/staging exists. Running it after the !hasRemoteStaging arm would swallow the pin
  // notice on a mirror clone, which is exactly the machine most likely to be tag-pinned.
  const detached = isDetachedHead(gitCapture, root);
  if (detached) {
    warn(
      `HEAD is detached (at ${describeDetachedRef(gitCapture, root)}) — setup deliberately did NOT switch\n` +
        "     you to the staging branch. Pinning to a tag detaches HEAD, and switching would silently\n" +
        "     discard that pin. Nothing here is broken; you are on the ref you asked for.\n" +
        "     To move onto the working branch yourself:\n" +
        `       ${landOnStagingCommand({ hasRemoteStaging: Boolean(hasRemoteStaging) })}`,
    );
  } else if (!hasRemoteStaging) {
  ...
  const health = await checkCloneHealth(root, {
    ...
    log,
    warn,
    hasRemoteStaging: Boolean(hasRemoteStaging),
    // The guard above already reported the pin. The health gate must still DETECT and REPORT it in
    // its return value (that is what proves the detection was hoisted, not moved) — it just should
    // not say the same thing to the operator twice.
    detachedAlreadyReported: detached,
  });
...
export async function checkCloneHealth(root, { gitCapture, runner, log, warn, hasRemoteStaging, detachedAlreadyReported = false }) {
  const result = { detachedHead: false, missingSkills: [], notDistributedSkills: [], healed: false };

  // Detection routes through isDetachedHead() — the single implementation shared with runSetup()'s
  // land-on-staging guard, so the guard and the health gate can never disagree.
  if (isDetachedHead(gitCapture, root)) {
    result.detachedHead = true;
    if (!detachedAlreadyReported) {
      warn(
        `HEAD is DETACHED (at ${describeDetachedRef(gitCapture, root)}).\n` +
  ...
  let loadSkillsManifest, findMissingSkills, partitionMissingSkills;
  try {
    ({ loadSkillsManifest, findMissingSkills, partitionMissingSkills } = await import(
  ...
  // Skills excluded from THIS distribution are not missing — they were never here to restore.
  // A published archive drops .claude/skills/whitepaper together with packages/whitepaper, so a
  // clean mirror clone must not be told to run a `git checkout` that silently does nothing.
  // The discriminator is the companion path, so a real skill wipe still reports as missing.
  const { genuinelyMissing, notDistributed } = partitionMissingSkills(
    root,
    manifest.skills,
    manifest.notPublished,
  );
  result.missingSkills = genuinelyMissing;
  result.notDistributedSkills = notDistributed;
  if (genuinelyMissing.length === 0) {
    const distributed = manifest.skills.length - notDistributed.length;
    log(
      `✓ Core skills present (${distributed}/${distributed}).` +
        (notDistributed.length
          ? ` Not part of this distribution: ${notDistributed.join(", ")} — expected, nothing to restore.`
          : ""),
    );
    return result;
  }
  ...
  warn(
    `${genuinelyMissing.length} core skill(s) are MISSING from .claude/skills: ${genuinelyMissing.join(", ")}\n` +
  ...
  try {
    ...
    const stillMissing = partitionMissingSkills(
      root,
      manifest.skills,
      manifest.notPublished,
    ).genuinelyMissing;
