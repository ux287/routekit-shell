...
export async function runSetup({
  ...
  if (!hasRemoteStaging) {
    ...
    warn(
      "Uncommitted changes present — setup did NOT switch you to the staging branch.\n" +
        ...
        "       git stash && git checkout -B staging --track origin/staging",
    );
  } else {
  ...
  const health = await checkCloneHealth(root, { gitCapture, runner, log, warn });
 ...
 */
export async function checkCloneHealth(root, { gitCapture, runner, log, warn }) {
