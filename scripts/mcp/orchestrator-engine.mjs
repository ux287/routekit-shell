 ...
 * @param {Object} guardrailConfig - Guardrail configuration for self-querying
 * @returns {Object} Analysis result with type, complexity, and orchestration plan
 */
export async function analyzeQuery(query, context = "", routingConfig = null, guardrailConfig = null) {
  ...
  if (routingConfig && guardrailConfig) {
    try {
      console.log(`🧠 Self-querying learning patterns for ${queryType}`);
      const learningAnalysis = await queryLearningPatterns(queryType, routingConfig, guardrailConfig);
 ...
 */
export async function shouldOrchestrate(query, context = "", routingConfig = null, guardrailConfig = null) {
  const analysis = await analyzeQuery(query, context, routingConfig, guardrailConfig);
 ...
 */
export async function executeOrchestration(plan, routingConfig, guardrailConfig) {
  const results = {
    steps: [],
    allPassages: [],
    traces: [],
  ...
  for (const step of plan.steps) {
    const stepResult = await executeStep(step, routingConfig, guardrailConfig);
 ...
 */
async function executeStep(step, routingConfig, guardrailConfig) {
  const result = {
    type: step.type,
  ...
  try {
    switch (step.type) {
      ...
      case "comprehensive_search":
        const retrieved = await retrieveWithRouting(step.query, routingConfig, guardrailConfig);
      ...
      case "parallel_search":
        const parallelResults = await Promise.all(
          step.queries.map(q => retrieveWithRouting(q, routingConfig, guardrailConfig))
        );
 ...
 */
export async function handleLowConfidenceResponse(query, confidence, context, routingConfig, guardrailConfig) {
  ...
  if (confidence < CONFIDENCE_THRESHOLDS.LOW) {
    ...
    const fallbackResult = await tryFallbackSearch(query, context, routingConfig, guardrailConfig);
 ...
 */
async function tryFallbackSearch(query, context, routingConfig, guardrailConfig) {
  ...
  for (const { name, fn } of fallbackStrategies) {
    try {
      ...
      const result = await fn(query, context, routingConfig, guardrailConfig);
 ...
 */
async function broaderSemanticSearch(query, _context, routingConfig, guardrailConfig) {
  ...
  if (broadQuery !== query) {
    ...
    const result = await retrieveWithRouting(broadQuery, routingConfig, guardrailConfig);
 ...
 */
async function keywordBasedSearch(query, context, routingConfig, guardrailConfig) {
  ...
  if (keywords.length > 0) {
    ...
    const result = await retrieveWithRouting(keywordQuery, routingConfig, guardrailConfig);
 ...
 */
async function domainSpecificSearch(query, context, routingConfig, guardrailConfig) {
  const domain = extractDomain(query, context);
  const domainQuery = `${domain} ${query}`;
  
  console.log(`🎯 Domain search: "${domainQuery}"`);
  const result = await retrieveWithRouting(domainQuery, routingConfig, guardrailConfig);
 ...
 */
export async function synthesizeResponseWithValidation(results, format, originalQuery, routingConfig, guardrailConfig) {
