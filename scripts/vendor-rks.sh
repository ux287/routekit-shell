...
# Source-root override, mirroring vendor-skills.sh. Without it SOURCE_DIR is
# unconditionally the live worktree, so a test that needs to add a skill to the SOURCE
# fixture (CONVERGENCE MUTATION 2) could only do so by writing into the real repo.
# Unset -> "$SCRIPT_DIR/..", byte-identical to the previous behaviour.
SHELL_ROOT="${ROUTEKIT_SHELL_ROOT:-"$SCRIPT_DIR/.."}"
SOURCE_DIR="$SHELL_ROOT/.claude/skills"
PROMPTS_DIR="$SHELL_ROOT/.rks/prompts"
HOOKS_DIR="$SHELL_ROOT/.routekit/hooks"
SETTINGS_FILE="$SHELL_ROOT/.claude/settings.json"
# The substitution SENTINEL — not a projectId. Regex-metacharacter-free, so it is
# safe unquoted inside the sed s/// below. The surviving placeholder token is a
# DIFFERENT token and is never substituted here.
SOURCE_PROJECT_ID="__RKS_SOURCE_PROJECT__"
...
for target in "${TARGETS[@]}"; do
  ...
  for skill_dir in "$SOURCE_DIR"/*/; do
    ...
    # No equality guard: the sentinel is not a projectId, so no target can ever equal it.
