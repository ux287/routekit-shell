...
# safe unquoted inside the perl s/// below. The surviving placeholder token is a
...
for target in "${TARGETS[@]}"; do
  ...
  for skill_dir in "$SOURCE_DIR"/*/; do
    ...
    # Substitute the sentinel with the target projectId in all copied SKILL.md files.
    # No equality guard: the sentinel is not a projectId, so no target can ever equal it,
    # and a comparison here would imply a shell exemption that cannot exist.
